...
from sp_utilities import get_im, write_header, model_circle, read_text_row, set_params2D, write_text_row, \
    set_params_proj, compose_transform3, model_blank, get_params_proj
...
def apsh(refimgs, imgs2align, outangles=None, refanglesdoc=None, outaligndoc=None, outerradius=-1,
    ...
    for imgindex in range(numimg):
        ...
        if mirrorflag == 1:
            tempeulers = list(
                compose_transform3(besteulers[0], besteulers[1], besteulers[2], besteulers[3], besteulers[4], 0, 1,
                                   0, 180, 0, 0, 0, 0, 1))
            combinedparams = list(
                compose_transform3(tempeulers[0], tempeulers[1], tempeulers[2], tempeulers[3], tempeulers[4], 0, 1,
                                   0, 0, -angt, 0, 0, 0, 1))
        else:
            combinedparams = list(
                compose_transform3(besteulers[0], besteulers[1], besteulers[2], besteulers[3], besteulers[4], 0, 1,
...
def mode_meridien(reconfile, classavgstack, classdocs, partangles, selectdoc, maxshift, outerrad,
    ...
    for classidx, classdoc in enumerate(classdoclist):
        ...
        refprjreal = prgl(reconprep, [avg_phi_final, avg_theta_final, 0, 0, 0], interpolation_method=1,
                          return_real=True)
        ...
        # Direct3 (angles seemed to be quantized)
            ...
            [[ang_align2d, sxs, sys, mirrorflag, peak]] = align2d_direct3([classavg], refprjreal, maxshift, maxshift,
                                                                          ou=outerrad)

        # APSH-like alignment (default)
        else:
            [[ang_align2d, sxs, sys, mirrorflag, scale]] = apsh(refprjreal, classavg,
                                                                outerradius=outerrad, maxshift=maxshift,
                                                                ringstep=ringstep)
