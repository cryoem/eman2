...
def combine_isac_params(isac_dir, classavgstack, chains_params_file, old_combined_parts, classdoc, combined_params_file,
        ...
        for class_num in range(num_classes):
            ...
            for idx, im in enumerate(members):
                tmp_par = combine_params2(
                    ...
                    all_params[im][0], old_div(all_params[im][1], isac_shrink_ratio),
                    old_div(all_params[im][2], isac_shrink_ratio), all_params[im][3])

                # Combine with class-average parameters
                P = combine_params2(tmp_par[0], tmp_par[1], tmp_par[2], tmp_par[3],
                                    chains_params_list[class_num][2], chains_params_list[class_num][3],
                                    chains_params_list[class_num][4], chains_params_list[class_num][5])
                ...
                try:
                    ...
                    print("Couldn't find particle: class_num %s, old_part_num %s, new_part_num %s" % (
...
def filter_shrink(input_bdb_name, output_stack_path, output_bdb_name, alignYN=False, filtrad=None, shrink=None,
                  verbose=False):
    """
