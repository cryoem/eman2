...
def do_generation(
    ...
    # print("Buffer shape is ", buffer.shape)
    # print("reshape parameters", nimastack, target_nx)

    buffer = buffer.reshape(nimastack, target_nx, target_nx)
    # bigbuffer = EMAN2.from_numpy(buffer)
    # bigbuffer = sp_utilities.numpy2em_python(buffer)
    ...
    # bigbuffer.write_image("py3buffer.hdf")
    mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
    ...
    # ptr = ctypes.cast(base_ptr + i * size_of_one_image * ctypes.c_int, ctypes.POINTER(size_of_one_image * ctypes.c_int))
    # pointer_location = base_ptr + i * size_of_one_image * disp_unit
    msk = sp_utilities.model_blank(target_nx, target_nx, 1, 1)
    for i in range(nimastack):
