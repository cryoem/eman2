...
def iter_isac_pap(alldata, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, \
    ...
    if os.path.exists(ali_params_dir):
        ERROR(
            'Output directory %s for alignment parameters exists, please either change its name or delete it and restart the program' % ali_params_dir,
            "iter_isac", 1, myid)
    mpi_barrier(MPI_COMM_WORLD)
    ...
    if myid == main_node:
        ...
        for i, im in enumerate(data):
            ...
            all_ali_params[i] = [alpha, sx, sy, mirror]
        print("****************************************************************************************************")
        print("*         Generation finished                 " + strftime("%a, %d %b %Y %H:%M:%S",
                                                                          localtime()) + "                            *")
        print("****************************************************************************************************")
...
def isac_MPI_pap(stack, refim, d, maskfile=None, ir=1, ou=-1, rs=1, xrng=0, yrng=0, step=1,
    ...
    while ((main_iter < max_iter) and (terminate == 0)):
        Iter += 1
        if my_abs_id == main_node: print("Iteration within isac_MPI Iter =>", Iter, "	main_iter = ", main_iter,
                                         "	len data = ", image_end - image_start, "   ",
                                         strftime("%a, %d %b %Y %H:%M:%S", localtime()))
        mashi = cnx - ou - 2
        for j in range(numref):
            refi[j].process_inplace("normalize.mask",
                                    {"mask": mask, "no_sigma": 1})  # normalize reference images to N(0,1)
            ###if myid == main_node:
        ...
        for im in range(image_start, image_end):
            ...
            # normalize
            alldata[im].process_inplace("normalize.mask",
                                        {"mask": mask, "no_sigma": 0})  # subtract average under the mask
            ...
            temp = Util.multiref_polar_ali_2d_peaklist(alldata[im], refi, txrng, tyrng, step, mode, numr, cnx + sxi,
                                                       cny + syi)
            for iref in range(numref):
                [alphan, sxn, syn, mn] = \
                    combine_params2(0.0, -sxi, -syi, 0, temp[iref * 5 + 1], temp[iref * 5 + 2], temp[iref * 5 + 3],
                                    int(temp[iref * 5 + 4]))
        ...
        if (Blockdata["subgroup_myid"] > -1):
            ...
            if (Blockdata["no_of_groups"] > 1):
                ...
                for j in range(numref):
                    ...
                    dbuf = mpi_reduce(dbuf, nima, MPI_FLOAT, MPI_SUM, main_node,
                                      Blockdata["subgroup_comm"])  # RETURNS numpy array
        ...
        if myid == main_node:
            ...
            id_list_long = Util.assign_groups(str(d.__array_interface__['data'][0]), numref,
                                              nima)  # string with memory address is passed as parameters
            # del d
        ...
        if myid == main_node:
            ...
            dummy = within_group_refinement(refi, mask, True, first_ring, last_ring, rstep, [xrng], [yrng], [step], dst,
                                            maxit, FH, FF)
            ref_ali_params = []
            for j in range(numref):
        ...
        for im in range(image_start, image_end):
            matchref = belongsto[im]
            alpha, sx, sy, mirror, scale = get_params2D(alldata[im])
            alphan, sxn, syn, mirrorn = combine_params2(alpha, sx, sy, mirror, ref_ali_params[matchref * 4],
        ...
        if do_within_group == 1:
            ...
            if check_stability:
                ...
                if myid == main_node:
                    ...
                    print(">>>  Assignment agreement with previous iteration  %5.1f" % (agreement * 100), "   ",
                          strftime("%a, %d %b %Y %H:%M:%S", localtime()))
...
def do_generation(main_iter, generation_iter, target_nx, target_xr, target_yr, target_radius, options):
    ...
    if (Blockdata["subgroup_myid"] > -1):
        ...
        if (Blockdata["no_of_groups"] > 1):
            plist = bcast_list_to_all(plist, Blockdata["subgroup_myid"], source_node=0,
    ...
    for i in range(nimastack):
        ...
        img_buffer = np.frombuffer(np.core.multiarray.int_asbuffer(pointer_location, size_of_one_image * disp_unit),
                                   dtype='f4')
    ...
    if (Blockdata["myid"] == 0):
        print("*************************************************************************************")
        print("     Main iteration: %3d,  Generation: %3d. " % (main_iter, generation_iter) + "   " + strftime(
            "%a, %d %b %Y %H:%M:%S", localtime()))

    ave, all_params = iter_isac_pap(alldata, options.ir, target_radius, options.rs, target_xr, target_yr, options.ts,
                                    options.maxit, False, 1.0, \
                                    options.dst, options.FL, options.FH, options.FF, options.init_iter, dummy_main_iter,
                                    options.iter_reali, match_first, \
                                    max_round, match_second, options.stab_ali, options.thld_err, indep_run, thld_grp, \
                                    options.img_per_grp, generation_iter, False, random_seed=options.rand_seed,
                                    new=False)  # options.new)
        ...
        for i, q in enumerate(ave):
            ...
            q.write_image(
                os.path.join(Blockdata["masterdir"], "main%03d" % main_iter, "generation%03d" % generation_iter,
                             "original_class_averages_%03d_%03d.hdf" % (main_iter, generation_iter)), i)
            q.set_attr("members", members)
            q.write_image(
                os.path.join(Blockdata["masterdir"], "main%03d" % main_iter, "generation%03d" % generation_iter,
                             "class_averages_%03d_%03d.hdf" % (main_iter, generation_iter)), i)
            if (len(members) > options.minimum_grp_size):
                good += members
                q.write_image(
                    os.path.join(Blockdata["masterdir"], "main%03d" % main_iter, "generation%03d" % generation_iter,
                                 "good_class_averages_%03d_%03d.hdf" % (main_iter, generation_iter)), j)
        ...
        if (len(bad) > 0):
            ...
            if ((int(len(bad) * 1.2) < 2 * options.img_per_grp) or ((len(good) == 0) and (generation_iter == 1))):
                ...
                if ((len(leftout) < 2 * options.img_per_grp) or ((len(good) == 0) and (generation_iter == 1))):
                    ...
                    cmd = "{} {}".format("touch", os.path.join(Blockdata["masterdir"], "main%03d" % main_iter,
                                                               "generation%03d" % generation_iter, "finished"))
                    junk = cmdexecute(cmd)
                    cmd = "{} {}".format("touch",
                    ...
                    junk = cmdexecute(cmd)
                    print("*         There are no more images to form averages, program finishes     " + strftime(
                        "%a, %d %b %Y %H:%M:%S", localtime()) + "     *")
                else:
    ...
    keepdoing_generation = bcast_number_to_all(keepdoing_generation, source_node=Blockdata["main_node"],
...
def main(args):
    ...
    usage = (
                progname + " stack_file  [output_directory] --radius=particle_radius --img_per_grp=img_per_grp --CTF <The remaining parameters are optional --ir=ir --rs=rs --xr=xr --yr=yr --ts=ts --maxit=maxit --dst=dst --FL=FL --FH=FH --FF=FF --init_iter=init_iter" +
                " --iter_reali=iter_reali --stab_ali=stab_ali --thld_err=thld_err" +
                " --rand_seed=rand_seed>")

    parser = OptionParser(usage, version=SPARXVERSION)
    parser.add_option("--radius", type="int",
                      help="particle radius: there is no default, a sensible number has to be provided, units - pixels (default required int)")
    parser.add_option("--target_radius", type="int", default=29,
    ...
    parser.add_option("--target_nx", type="int", default=76,
    ...
    parser.add_option("--img_per_grp", type="int", default=200,
    ...
    parser.add_option("--CTF", action="store_true", default=False,
    ...
    parser.add_option("--center_method", type="int", default=-1,
    ...
    parser.add_option("--dst", type="float", default=90.0,
                      help="discrete angle used in within group alignment (default 90.0)")
    parser.add_option("--FL", type="float", default=0.2,
                      help="lowest stopband: frequency used in the tangent filter (default 0.2)")
    parser.add_option("--FH", type="float", default=0.45,
    ...
    parser.add_option("--init_iter", type="int", default=7,
    ...
    parser.add_option("--iter_reali", type="int", default=1,
    ...
    parser.add_option("--stab_ali", type="int", default=5,
    ...
    parser.add_option("--rand_seed", type="int",
    ...
    parser.add_option("--restart", type="int", default='-1',
    ...
    parser.add_option("--skip_prealignment", action="store_true", default=False,
    ...
    if (myid == main_node):
        print("****************************************************************************************************")
        print("*                                                                                                  *")
        print("* ISAC (Iterative Stable Alignment and Clustering)   " + strftime("%a, %d %b %Y %H:%M:%S",
                                                                                 localtime()) + "                     *")
    ...
    if not checkitem(os.path.join(init2dir, "Finished_initial_2d_alignment.txt")):
        ...
        if options.skip_prealignment:
            ...
            if (txrm < 0):            ERROR(
                "ERROR!!   Radius of the structure larger than the window data size permits   %d" % (radi), "sxisac", 1,
                myid)
            if (txrm / nxrsteps > 0):
                tss = ""
                txr = ""
        ...
        if (myid == main_node):
            if options.skip_prealignment:
                print("=========================================")
                print("There is no alignment step, '%s' params are set to zero for later use." % os.path.join(init2dir,
                                                                                                              "initial2Dparams.txt"))
                print("=========================================")
        ...
        for im in range(nima):
            ...
            if options.CTF:
                ...
                aligned_images[im] = fft(
                    filt_table(filt_ctf(fft(aligned_images[im]), aligned_images[im].get_attr("ctf"), binary=True), rpw))
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        if (not os.path.exists(os.path.join(Blockdata["masterdir"], "main001", "generation000"))):
            ...
            write_text_row([[0.0, 0.0, 0.0, -1] for i in range(Blockdata["total_nima"])],
            ...
            if (options.restart == 0):
                ...
                while (keepdoing_main):
                    ...
                    if (os.path.exists(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter))):
                        if (
                        not os.path.exists(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter, "finished"))):
    ...
    if (error == 1):
        ERROR("isac2",
              "The restart value is greater than -1, but the ISAC directory does not exist for continuation. Please choose a restart value of -1 to start a fresh run.")
    elif (error == 2):
        ERROR("isac2",
              "Cannot restart from unfinished main iteration  %d. To automatically detect the latest ISAC directory for continuation please choose a restart value of 0." % main_iter)
        ...
        if (checkitem(os.path.join(Blockdata["masterdir"], "finished"))):
            ...
            if (not checkitem(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter))):
                ...
                if (Blockdata["myid"] == 0):
                    ...
                    if (main_iter > 1):
                        ...
                        cmd = "{} {} {} {}".format("cp -Rp", \
                                                   os.path.join(Blockdata["masterdir"], "main%03d" % (main_iter - 1),
                                                                "processed_images.txt"), \
                                                   os.path.join(Blockdata["masterdir"], "main%03d" % (main_iter - 1),
                                                                "class_averages.hdf"), \
                                                   os.path.join(Blockdata["masterdir"]))
                        junk = cmdexecute(cmd)
                        junk = os.path.join(Blockdata["masterdir"], "main%03d" % (main_iter - 1),
            ...
            if (not checkitem(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter, "finished"))):
                ...
                while (keepdoing_generation):
                    generation_iter += 1
                    if checkitem(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter,
                                              "generation%03d" % generation_iter)):
                        if checkitem(os.path.join(Blockdata["masterdir"], "main%03d" % main_iter,
                                                  "generation%03d" % generation_iter, "finished")):
                            okdo = False
                    ...
                    if okdo:
                        ...
                        # exit()
                        # DO THIS GENERATION
                        keepdoing_main, keepdoing_generation = do_generation(main_iter, generation_iter, target_nx,
                                                                             target_xr, target_yr, target_radius,
                                                                             options)
    ...
    if (Blockdata["myid"] == 0):
        if (os.path.exists(os.path.join(Blockdata["masterdir"], "class_averages.hdf"))):
            cmd = "{} {} {} {} {} {} {} {} {} {}".format("sxchains.py",
                                                         ...
                                                         "--circular", "--radius=%d" % target_radius,
                                                         "--xr=%d" % (target_xr + 1), "--yr=%d" % (target_yr + 1),
                                                         "--align", ">/dev/null")
            junk = cmdexecute(cmd)
