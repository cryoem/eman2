...
from sp_applications import header, pca, prepare_2d_forPCA
from datetime import datetime
from sp_logger import Logger, BaseLogger_Files, BaseLogger_Print
from sp_utilities import get_params2D, read_text_row, get_im, write_text_row, montage_scale
...
""" % (__file__, 
	   DOCFILEDIR, CLASSMAPFILE, DOCFILEDIR, CLASSDOCPREFIX, CLASSORIGPREFIX, STACKFILEDIR, ALIGNSTACKPREFIX, STACKFILEDIR, FILTSTACKPREFIX, 
...
def separate_class(classavgstack, instack, options, outdir='.', verbose=False):
	...
	if options.align_isac_dir:
		...
		combine_isac_params(options.align_isac_dir, classavgstack, chains_params, classed_imgs, classdoc_template, combined_params_file, log, verbose)
	...
	for class_num in xrange(num_classes):
		...
		if not options.align_isac_dir:
			# Write class stack
			cmd = "e2bdb.py %s --makevstack bdb:%s --list %s" % (stack2split, class_init_bdb_template.format(class_num), classdoc_template.format(class_num))
			print_log_msg(cmd, log, verbose)
			...
			if options.filtrad or options.shrink:
				...
				cmd = "e2proc2d.py bdb:%s %s --inplace" % (class_init_bdb_template.format(class_num), outflt.format(class_num))
		...
		# Optionally apply alignment
			...
			cmd = "e2bdb.py %s --makevstack %s --list %s" % (stack2split, class_init_bdb_name, classdoc_template.format(class_num))
			print_log_msg(cmd, log, verbose)
			os.system(cmd)
...
def vomq(classavgstack, classmap, classdoc, log=None, verbose=False):
	...
	Arguments:
		...
		classdoc : Filename template for output lists of particles assigned to a class, one file per class
