...
def MotionParamEstima(movie_names, stackfilename, boxsize, zsize, angout_pix, mc ):
    ...
    print("frequency range to consider for evaluation {} - {} A ({} - {} px)".format(k_eval_angst,
                                                                pixtoang(kout, boxsize, angout_pix), k_eval, kout ))
    ...
    globaldose = 1.277
    prepAlignment(globaldose, angout_pix, boxsize, zsize, k_cutoff)
...
def prepAlignment(globaldose, angout_pix, boxsize, zsize, k_cutoff):

    for i in range(zsize):
        dose = globaldose * (i)
        damageweights = damageWeights(dose, boxsize, angout_pix)
        Filter_dose = Butterworthfilter(damageweights, k_cutoff-1, k_cutoff +1 )

    accCords = []

    for y in range(Filter_dose[1]):
        for x in range(Filter_dose[0]):
...
MotionParamEstima(movie_names, stackfilename, boxsize, zsize, angout_pix, mc)
...
for micro in enumerate(movie_names[ima_start:ima_end]):
    ...
    ref_project_2D_ptcl_vol1 = get_2D_project_for_all_ptcl_from_reference(volft_1, kb_1, project_params, zsize,
                                                                          show=False)
    ref_project_2D_ptcl_vol2 = get_2D_project_for_all_ptcl_from_reference(volft_2, kb_2, project_params, zsize,
                                                                          show=False)

    fsc_val, weight0, weight1, part_shifts = get_fcc_all_particles(micro[1], ref_project_2D_ptcl_vol1,
                                                                   ref_project_2D_ptcl_vol2,
                                                                   nx[0], ny[0], particle_coordinates, zsize,
                                                                   Gainreffile,
...
for micro in enumerate(movie_names[ima_start:ima_end]):
    ...
    local_stack_path = "bdb:%s" % os.path.join(str(os.path.join(str(stack_absolute_path), 'micrographs')),
                                               micro[1].split('.')[0].split('/')[-1] + "_ptcls")
    local_mrc_path = os.path.join(str(os.path.join(str(stack_absolute_path), 'micrographs')),
                                  micro[1].split('.')[0].split('/')[-1] + "_ptcls.mrcs")
