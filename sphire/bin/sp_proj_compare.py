...
def parse_command_line():
    ...
    group_projmatch.add_argument(
        "--delta", type=float, help="Angular search increment for reference projections"
    )
...
def main_proj_compare(
    ...
    if voldim != imgdim:
        ...
        scale = old_div(
            float(imgdim), voldim
        )  # only approximate, since full-sized particle radius is arbitrary
    ...
    if mode == "projmatch":
        ...
        cmd = (
            "sxproject3d.py %s %s --delta=%s --method=S --phiEqpsi=Minus --symmetry=%s"
            % (reconfile, refprojstack, options.delta, options.symmetry)
        )
        ...
        sp_applications.project3d(
            reconfile, refprojstack, delta=options.delta, symmetry=options.symmetry
        )

        # Export projection angles
        print_log_msg(
            "Exporting projection angles from %s to %s" % (refprojstack, refanglesdoc),
            log,
            verbose,
        )
        ...
        if options.align == "ali2d":
            projdir = os.path.join(
                outdir, "Projdir"
            )  # used if input angles no provided
            if os.path.isdir(projdir):
                print_log_msg(
                    "Removing pre-existing directory %s" % projdir, log, verbose
                )
            ...
            print_log_msg(
                "Zeroing out alignment parameters in header of %s" % classavgstack,
                log,
                verbose,
            )
            ...
            # Perform multi-reference alignment
            msg = (
                "Aligning images in %s to projections %s with a radius of %s and a maximum allowed shift of %s"
            ...
            print_log_msg(
                "Exporting angles from %s into %s" % (classavgstack, classangles),
                log,
                verbose,
            )
        ...
        # Diagnostic
        alignlist = sp_utilities.read_text_row(
            classangles
        )  # contain 2D alignment parameters
...
def mode_meridien(
    ...
    for classdoc in classdoclist:  # [classdoclist[32]]:  #
        ...
        [avg_phi_init, avg_theta_init] = average_angles(
            partangleslist, classdoc, selectdoc=selectdoc
        )
        ...
        refprjreal = sp_projection.prgl(
            reconprep,
            [avg_phi_final, avg_theta_final, 0, 0, 0],
            interpolation_method=1,
            return_real=True,
        )
        ...
        # Weird results
            ...
            # Set search range
            currshift = 0
            txrng = tyrng = sp_alignment.search_range(
                idim, outerrad, currshift, maxshift
            )
        ...
        # APSH-like alignment (default)
            ...
            [[ang_align2d, sxs, sys, mirrorflag, scale]] = apsh(
                refprjreal,
                classavg,
        ...
        if mirrorflag == 1:
            tempeulers = list(
                sp_utilities.compose_transform3(
                    avg_phi_final, avg_theta_final, 0, 0, 0, 0, 1, 0, 180, 0, 0, 0, 0, 1
                )
            )
            combinedparams = list(
                sp_utilities.compose_transform3(
