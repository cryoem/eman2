	...
	if center_method == 0:
		...
		if searching_range > 0:
			...
			if abs(cs[1]) > searching_range:
				cs[1] = 0.0
		return sp_fundamentals.fshift(image_to_be_centered, -cs[0], -cs[1]), cs[0], cs[1]
		...
		if searching_range > 0:
			...
			if abs(shifty) > searching_range:
				shifty = 0
		return sp_fundamentals.cyclic_shift(image_to_be_centered, -shiftx, -shifty), shiftx, shifty
		...
		while not_centered:
			...
			if searching_range > 0:
				ccmap = EMAN2_cppwrap.Util.window(ccmap, searching_range, searching_range, 1, 0, 0, 0)
		...
		new_image = sp_morphology.threshold_to_minval(image_to_be_centered, mean + sigma)
