...
def refine_defocus_with_error_est(
	...
	drratio = numpy.abs(best_ctf_no_boostrap.defocus - current_ctf.defocus) / (
			error_sd + 0.0005
	)
...
def _main_():
	...
	output_virtual_stack_path = "bdb:" + multiprocessing.os.path.join(output_folder, "ctf_refined")
	...
	particle_error_path = multiprocessing.os.path.join(output_stats_path, "particle_results.txt")
