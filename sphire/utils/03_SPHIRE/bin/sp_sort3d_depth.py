...
Blockdata["color"], Blockdata["no_of_groups"], balanced_processor_load_on_nodes = sp_utilities.get_colors_and_subsets(
    Blockdata["main_node"], mpi.MPI_COMM_WORLD, Blockdata["myid"], \
...
def depth_box_initialization(box_dir, input_list1, input_list2, log_file):
    ...
    if input_list2 is not None:  # Track 2
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            for indep in range(2):
                sp_utilities.write_text_file(new_assignment[indep], os.path.join(box_dir, "independent_index_%03d.txt" % indep))
            new_assignment = []
            for indep in range(2):
        ...
        if number_of_groups < 2: sp_global_def.ERROR("number_of_groups should be at least larger than 1", "depth_box_initialization",
                                       1, Blockdata["myid"])
...
def get_box_partition(box_dir, ncluster, unaccounted_list):
    if ncluster >= 1:
        ...
        for ic in range(ncluster):
            one_cluster = sp_utilities.read_text_file(os.path.join(box_dir, "Cluster_%03d.txt" % ic))
...
def read_tracker_mpi(current_dir):
    ...
    if open_tracker == 1:
        ...
        sp_global_def.ERROR("Fail to load tracker", "%s" % current_dir, 1, Blockdata["myid"])
    return
...
def depth_clustering_box(work_dir, input_accounted_file, input_unaccounted_file, params, previous_params, nbox,
    ...
    while (keepgoing == 1):
        ...
        iter_total_stack = sp_utilities.bcast_number_to_all(iter_total_stack, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
        iter_number_of_groups = sp_utilities.bcast_number_to_all(iter_number_of_groups, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
        ...
        while ((iter < box_niter) and (converged == 0)):
            for indep_run_iter in range(2):
                ...
                if Blockdata["myid"] == Blockdata["main_node"]:
                    sp_utilities.write_text_row(tmp_final_list, os.path.join(iter_dir, "partition_%03d.txt" % indep_run_iter))
                mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
            ...
            if Blockdata["myid"] == Blockdata["main_node"]:
                ...
                minimum_grp_size1 = 0
                Tracker = 0
            minimum_grp_size1 = sp_utilities.bcast_number_to_all(minimum_grp_size1, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
...
def check_mpi_settings(log_main):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        fsc_refinement = sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"], "fsc_global.txt"))
        ...
        nxinit = 0
    nxinit = sp_utilities.bcast_number_to_all(nxinit, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    ...
    if current_mpi_settings_is_bad == 1:   sp_global_def.ERROR("Initial info is not provided", "check_mpi_settings", 1,
                                                 Blockdata["myid"])
    try:
    ...
    if (
        ...
        sp_global_def.ERROR("Insufficient memory", "Suggestion: set number of processes to: %d" % new_nproc, 1, Blockdata["myid"])
    ...
    if (images_per_cpu < 5): sp_global_def.ERROR("Number of images per processor is less than 5",
...
def get_sorting_image_size(original_data, partids, number_of_groups, sparamstructure, snorm_per_particle, log_main):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        if len(lpartids) == 1:
            ...
            for im in range(len(lpartids[0])):
                iter_assignment.append(random.randint(0, number_of_groups - 1))  # simple version
        else:
            iter_assignment = lpartids[0]
    else:
        iter_assignment = 0
    iter_assignment = sp_utilities.wrap_mpi_bcast(iter_assignment, Blockdata["main_node"])
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        for igroup in range(Tracker["number_of_groups"]):
            for ichunk in range(2):
                tmp_fsc_data = sp_utilities.read_text_file(os.path.join(Tracker["directory"],
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]): sp_utilities.write_text_file(avg_fsc, os.path.join(Tracker["directory"],
                                                                                            "fsc_image_size.txt"))
    del iter_assignment
    del proc_list
    del fsc_data
...
def get_params_for_analysis(orgstack, ali3d_params, smearing_file, smearing_number):
    if ali3d_params is not None:
        ...
        for im in range(len(ali3d_params)):
            vecs_list.append(sp_utilities.getvec(ali3d_params[im][0], ali3d_params[im][1]))
            try:
...
def do_one_way_anova_scipy(clusters, value_list, name_of_variable="variable", log_main=None):
    ...
    if K == 2:
        ...
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12)
    elif K == 14:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13)
    elif K == 15:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14)
    elif K == 16:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15)
    elif K == 17:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16)
    elif K == 18:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17)
    elif K == 19:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18)
    elif K == 20:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19)
    elif K == 21:
        res = scipy.stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19,
                             x20)
    elif K == 22:
...
def import_data(log_main):
    ...
    if Tracker["constants"]["total_stack"] // Tracker["constants"]["img_per_grp"] <= 1:
        sp_global_def.ERROR("Your img_per_grp is too large", "sxsort3d_depth.py", 1, Blockdata["myid"])
    if Tracker["constants"]["minimum_grp_size"] > Tracker["constants"]["img_per_grp"]:
        sp_global_def.ERROR("Minimum_grp_size is too large", "sxsort3d_depth.py", 1, Blockdata["myid"])
    return
...
def Kmeans_minimum_group_size_orien_groups(cdata, fdata, srdata, \
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        if len(lpartids) == 1:  # Not true for sorting
            ...
            for im in range(len(lpartids[0])): iter_assignment.append(
                random.randint(0, number_of_groups - 1))  # simple version
        else:
            iter_assignment = lpartids[0]
    else:
        iter_assignment = 0
    ...
    while total_iter < max_iter:
        ...
        if (Blockdata["myid"] == Blockdata["main_node"]):
            ...
            log_main.add(msg)
            sp_utilities.write_text_file(iter_assignment, os.path.join(Tracker["directory"], "assignment%03d.txt" % total_iter))
        ...
        for iref in range(number_of_groups):
            if (Blockdata["myid"] == Blockdata["last_node"]):
                ...
                if (Tracker["nxinit"] != nnn): ref_vol = sp_fundamentals.fdecimate(ref_vol, Tracker["nxinit"], Tracker["nxinit"],
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        sp_utilities.write_text_row(partition, os.path.join(Tracker["directory"], "list.txt"))
        shutil.rmtree(os.path.join(Tracker["directory"], "tempdir"))
    else:
        partition = 0
    partition = sp_utilities.wrap_mpi_bcast(partition, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    premature = sp_utilities.bcast_number_to_all(premature, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    if (Blockdata["myid"] == Blockdata["last_node"]):
        if clean_volumes:
...
def do_assignment_by_dmatrix_orien_group_minimum_group_size(dmatrix, orien_group_members, number_of_groups,
    ...
    while len(rmatrix[0]) > nima - minimum_group_size * number_of_groups:
        ...
        duplicate_list = (numpy.setdiff1d(numpy.arange(number_of_groups), index_list)).tolist()
    ...
    kmeans_ptl_list = (
        numpy.delete(numpy.array(list(range(nima))), numpy.array(results).ravel())).tolist()  # ravel works only for even size
    del rmatrix
    for iptl in range(len(kmeans_ptl_list)):
        max_indexes = numpy.argwhere(submatrix[:, kmeans_ptl_list[iptl]] <= submatrix[:, kmeans_ptl_list[iptl]][
...
def copy_oldparamstructure_from_meridien_MPI(selected_iteration, log_main):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]): sp_utilities.write_text_row(Tracker["refang"],
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        sp_utilities.write_text_file(my_parstack,
    ...
    for procid in range(2):
        smearing_list = []
        if (Blockdata["myid"] == Blockdata["main_node"]):
            lcore = sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"], "chunk_%d.txt" % procid))
        else:
            lcore = 0
    ...
    if Blockdata["myid"] == Blockdata["main_node"]: sp_utilities.write_text_row(full_dict_list, Tracker["paramstructure_dict"])
    return
...
def precalculate_shifted_data_for_recons3D(prjlist, paramstructure, refang, rshifts, delta, avgnorms, nxinit, nnxo,
    ...
    for im in range(len(prjlist)):
        ...
        if nosmearing:
            ...
            sp_utilities.set_params_proj(prjlist[im], [phi, theta, psi, 0.0, 0.0], xform="xform.projection")
        else:
            ...
            for ii in range(len(tdir)):
                ...
                ipsi = tdir[ii] % 100000
                iang = tdir[ii] / 100000
                sp_utilities.set_params_proj(recdata, [refang[iang][0], refang[iang][1], refang[iang][2] + ipsi * delta, 0.0, 0.0],
...
def downsize_data_for_sorting(original_data, return_real=False, preshift=True, npad=1, norms=None):
    ...
    if Tracker["constants"]["focus3D"]:  # focus mask is applied
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            focus3d = sp_utilities.model_blank(Tracker["nxinit"], Tracker["nxinit"], Tracker["nxinit"])
    ...
    for im in range(nima):
        ...
        if Tracker["constants"]["CTF"]:
            ...
            rimage = sp_fundamentals.fdecimate(rimage, nxinit * npad, nxinit * npad, 1, True, False)
...
def compare_two_images_eucd(data, ref_vol, fdata):
    ...
    for im in range(len(data)):
        ...
        if Tracker["applybckgnoise"]:
            peaks[im] = -EMAN2_cppwrap.Util.sqed(data[im], rtemp, ctfs[im],
                                   Blockdata["unrolldata"][data[im].get_attr("particle_group")]) / qt
        else:
            peaks[im] = -EMAN2_cppwrap.Util.sqed(data[im], rtemp, ctfs[im], Blockdata["unrolldata"]) / qt
    return peaks
...
def compare_two_images_cross(data, ref_vol):
    ...
    for im in range(len(data)):
        ...
        if data[im].get_attr("is_complex") == 1: data[im].set_attr("is_complex", 0)
        if Tracker["constants"]["focus3D"]:
            peaks[im] = EMAN2_cppwrap.Util.innerproduct(ref, data[im], None) / nrmref
        else:
            if Tracker["applybckgnoise"]:
...
def swap_accounted_with_unaccounted_elements_mpi(accounted_file, unaccounted_file, log_file, number_of_groups,
    ...
    if checking_flag == 0:
        ...
        converted_assignment_list = sp_utilities.wrap_mpi_bcast(converted_assignment_list, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    else:
...
def patch_to_do_k_means_match_clusters_asg_new(ptp1, ptp2):
    ...
    if len(ptp1) != len(ptp2):
        for i in range(len(ptp1)): ptp1[i] = numpy.array(ptp1[i], "int32")
        for i in range(len(ptp2)):    ptp2[i] = numpy.array(ptp2[i], "int32")
        alist = []
        blist = []
        for a in ptp1:
...
def do_boxes_two_way_comparison_mpi(nbox, input_box_parti1, input_box_parti2, depth, log_main):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if (len(core1) != len(core2)):  sp_global_def.ERROR("Two partitions have different lengths", "do_boxes_two_way_comparison", 1,
                                              0)
        ...
        for index_of_any in range(len(list_stable)):
            ...
            score3 = float((numpy.intersect1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) \
                     / float(
                (numpy.union1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) * 100.
    ...
    if nclass == 0:
        ...
        if depth > 1:
            ...
            maximum_group_size = sp_utilities.bcast_number_to_all(maximum_group_size, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            ...
            full_list = sp_utilities.wrap_mpi_bcast(full_list, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            stat_list = sp_utilities.wrap_mpi_bcast(stat_list, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            bad_clustering = sp_utilities.bcast_number_to_all(bad_clustering, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            stop_generation = sp_utilities.bcast_number_to_all(stop_generation, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            maximum_group_size = sp_utilities.bcast_number_to_all(maximum_group_size, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
...
def do_withinbox_two_way_comparison(partition_dir, nbox, nrun, niter):
    ...
    log_list.append(
        '----------------------------------------------------------------------------------------------------------------')
    ipair = 0
    core1 = sp_utilities.read_text_row(os.path.join(partition_dir, "partition_%03d.txt" % (2 * ipair)))
    ptp1, tmp1 = split_partition_into_ordered_clusters(core1)
    core2 = sp_utilities.read_text_row(os.path.join(partition_dir, "partition_%03d.txt" % (2 * ipair + 1)))
    ptp2, tmp2 = split_partition_into_ordered_clusters(core2)
    ...
    for im in range(len(ptp2)): msg += '{:8d} '.format(len(ptp2[im]))
    log_list.append(msg)
    if (len(core1) != len(core2)): sp_global_def.ERROR("The two partitions have different lengths", "do_withinbox_two_way_comparison",
                                         1, 0)
    full_list = []
    for a in core1: full_list.append(a[1])
    full_list.sort()
    ...
    for index_of_any in range(len(list_stable)):
        ...
        score2 = float(len(any)) * 100. / float(len(ptp2[newindeces[index_of_any][1]]))
        score3 = float((numpy.intersect1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) \
                 / float((numpy.union1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) * 100.
    ...
    sp_utilities.write_text_file(unaccounted_list, os.path.join(partition_dir, "Unaccounted.txt"))
...
def get_sorting_all_params(data):
    ...
    for myproc in range(Blockdata["nproc"]):
        ...
        mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
    total_attr_value_list = sp_utilities.wrap_mpi_bcast(total_attr_value_list, Blockdata["main_node"])
...
def get_sorting_attr_stack(data_in_core):
    ...
    for idat in range(len(data_in_core)): attr_value_list.append(
        [data_in_core[idat].get_attr("group"), sp_utilities.get_params_proj(data_in_core[idat], xform="xform.projection")])
...
def steptwo_mpi(tvol, tweight, treg, cfsc=None, regularized=True, color=0):
    ...
    n_iter = 10
    ifi = mpi.mpi_iterefa(vol_data.__array_interface__['data'][0], we_data.__array_interface__['data'][0], nx, ny, nz,
                      maxr2, \
    ...
    if (Blockdata["myid_on_node"] == 0):
        ...
        tvol = sp_fundamentals.cyclic_shift(tvol, Tracker["constants"]["nnxo"], Tracker["constants"]["nnxo"],
                            Tracker["constants"]["nnxo"])
        tvol = EMAN2_cppwrap.Util.window(tvol, Tracker["constants"]["nnxo"], Tracker["constants"]["nnxo"],
                           Tracker["constants"]["nnxo"])
        tvol.div_sinc(1)
...
def recons3d_4nnsorting_MPI(myid, main_node, prjlist, random_subset, CTF=True, upweighted=True, mpi_comm=None,
    ...
    # if norm_per_particle == None: norm_per_particle = len(prjlist)*[1.0]
    for im in range(len(prjlist)):
        phi, theta, psi, s2x, s2y = sp_utilities.get_params_proj(prjlist[im],
...
def get_input_from_sparx_ref3d(log_main):  # case one
    ...
    checking_flag = sp_utilities.bcast_number_to_all(checking_flag, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    if checking_flag == 1: sp_global_def.ERROR("SPARX refinement directory does not exist", "get_input_from_sparx_ref3d", 1,
                                 Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
    ...
    import_from_sparx_refinement = sp_utilities.bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0:
        sp_global_def.ERROR("The best solution is not found", "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
        mpi.mpi_finalize()
        exit()
    ...
    import_from_sparx_refinement = sp_utilities.bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: sp_global_def.ERROR("The data stack is not accessible", "get_input_from_sparx_ref3d", 1,
                                                Blockdata["myid"])
    ...
    import_from_sparx_refinement = sp_utilities.bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: sp_global_def.ERROR("The parameter file of the best solution is not accessible",
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if import_from_sparx_refinement: fsc_curve = sp_utilities.read_text_row(
            os.path.join(Tracker["constants"]["masterdir"], "fsc_global.txt"))
        fsc143 = 0
        fsc05 = 0
    ...
    import_from_sparx_refinement = sp_utilities.bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: sp_global_def.ERROR("The driver of the best solution is not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
    ...
    if import_from_sparx_refinement == 0: sp_global_def.ERROR("The index file of the best solution are not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"], "main000/particle_groups_1.txt")):
            ...
            import_from_sparx_refinement == 0
    import_from_sparx_refinement = sp_utilities.bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: sp_global_def.ERROR("The chunk files and partice group files are not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        params = sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"], "sparx_refinement_params.txt"), -1)
    ...
    if Tracker["constants"]["minimum_grp_size"] > Tracker["constants"]["img_per_grp"]:
        sp_global_def.ERROR("User provided img_per_grp is smaller than minimum_grp_size", "get_input_from_sparx_ref3d", 1,
              Blockdata["myid"])
...
def get_input_from_datastack(log_main):  # Case three
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        if (Tracker["nxinit"] > Tracker["constants"]["nnxo"]):
            sp_global_def.ERROR("Image size less than minimum permitted $d" % Tracker["nxinit"], "get_input_from_datastack", 1,
                  Blockdata["myid"])
            nnxo = -1
        else:
            if Tracker["constants"]["CTF"]:
        ...
        Tracker["constants"]["pixel_size"] = 1.0
    Tracker["constants"]["nnxo"] = sp_utilities.bcast_number_to_all(Tracker["constants"]["nnxo"], source_node=Blockdata["main_node"])
    if (Tracker["constants"]["nnxo"] < 0): sp_global_def.ERROR("Image size is negative", "get_input_from_datastack", 1,
                                                 Blockdata["main_node"])
    Tracker["constants"]["pixel_size"] = sp_utilities.bcast_number_to_all(Tracker["constants"]["pixel_size"],
                                                             source_node=Blockdata["main_node"])
    if (Tracker["constants"]["radius"] < 1):
        Tracker["constants"]["radius"] = Tracker["constants"]["nnxo"] // 2 - 2
    elif ((2 * Tracker["constants"]["radius"] + 2) > Tracker["constants"]["nnxo"]):
        sp_global_def.ERROR("Particle radius set too large!", \
...
def do3d_sorting_groups_nofsc_smearing_iter(srdata, partial_rec3d, iteration):
    ...
    keepgoing = sp_utilities.bcast_number_to_all(keepgoing, source_node=Blockdata["main_node"],
                                    mpi_comm=mpi.MPI_COMM_WORLD)  # always check
    Tracker = sp_utilities.wrap_mpi_bcast(Tracker, Blockdata["main_node"])
    if not keepgoing: sp_global_def.ERROR("do3d_sorting_groups_trl_iter  %s" % os.path.join(Tracker["directory"], "tempdir"),
                            "do3d_sorting_groups_trl_iter", 1, Blockdata["myid"])
    return
...
def recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI(myid, main_node, nproc, prjlist, group_ID, refvol_file,
    ...
    if CTF:
        ...
        do_ctf = 0
    if not os.path.exists(refvol_file): sp_global_def.ERROR("refvol does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)
    if not os.path.exists(fftvol_file): sp_global_def.ERROR("fftvol does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)
    if not os.path.exists(weight_file): sp_global_def.ERROR("weight does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)
        ...
        if nosmearing:
            ...
            if current_group_ID != previous_group_ID:
                if current_group_ID == group_ID:
                    flag = 1.0
                    [phi, theta, psi, s2x, s2y] = sp_utilities.get_params_proj(prjlist[im], xform="xform.projection")
                    r.insert_slice(prjlist[im], EMAN2_cppwrap.Transform({"type": "spider", "phi": phi, "theta": theta, "psi": psi}),
                                   flag)
            ...
            if current_group_ID != previous_group_ID:
                if current_group_ID == group_ID:
                    ...
                    for jm in range(len(prjlist[im])):
                        [phi, theta, psi, s2x, s2y] = sp_utilities.get_params_proj(prjlist[im][jm], xform="xform.projection")
                        r.insert_slice(prjlist[im][jm],
                                       EMAN2_cppwrap.Transform({"type": "spider", "phi": phi, "theta": theta, "psi": psi}),
            ...
            if random_subset == 2:
                if nosmearing:
                    ...
                    for ii in range(len(tdir)):
                        ...
                        ipsi = tdir[ii] % 100000
                        iang = tdir[ii] / 100000
                        r.insert_slice(recdata, EMAN2_cppwrap.Transform(
    ...
    for index_of_colors in range(Blockdata["no_of_groups"]):
        ...
        if Blockdata["color"] == index_of_colors:  # It has to be 1 to avoid problem with tvol1 not closed on the disk
            for index_of_group in range(group_start, group_end):
                if Blockdata["myid_on_node"] == 0:
                    tvol0 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_0_0_%d.hdf") % index_of_group)
                    tweight0 = sp_utilities.get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_0_0_%d.hdf") % index_of_group)
                    tvol1 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1_0_%d.hdf") % index_of_group)
                    tweight1 = sp_utilities.get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_1_0_%d.hdf") % index_of_group)
                    EMAN2_cppwrap.Util.fuse_low_freq(tvol0, tvol1, tweight0, tweight1, 2 * Tracker["fuse_freq"])
                    tag = 7007
                    sp_utilities.send_EMData(tvol1, Blockdata["no_of_processes_per_group"] - 1, tag, Blockdata["shared_comm"])
                    sp_utilities.send_EMData(tweight1, Blockdata["no_of_processes_per_group"] - 1, tag, Blockdata["shared_comm"])
                    shrank0 = stepone(tvol0, tweight0)
                    ...
                    tweight1 = sp_utilities.recv_EMData(0, tag, Blockdata["shared_comm"])
                    tvol1.set_attr_dict({"is_complex": 1, "is_fftodd": 1, 'is_complex_ri': 1, 'is_fftpad': 1})
                    shrank1 = stepone(tvol1, tweight1)

                if Blockdata["myid_on_node"] == 1:
                    tvol0 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_0_1_%d.hdf") % index_of_group)
                    tweight0 = sp_utilities.get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_0_1_%d.hdf") % index_of_group)
                    tvol1 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1_1_%d.hdf") % index_of_group)
                    tweight1 = sp_utilities.get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_1_1_%d.hdf") % index_of_group)
                    EMAN2_cppwrap.Util.fuse_low_freq(tvol0, tvol1, tweight0, tweight1, 2 * Tracker["fuse_freq"])
                    tag = 7007
                    sp_utilities.send_EMData(tvol1, Blockdata["no_of_processes_per_group"] - 2, tag, Blockdata["shared_comm"])
                    sp_utilities.send_EMData(tweight1, Blockdata["no_of_processes_per_group"] - 2, tag, Blockdata["shared_comm"])
                    shrank0 = stepone(tvol0, tweight0)
                    ...
                    tweight1 = sp_utilities.recv_EMData(1, tag, Blockdata["shared_comm"])
                    tvol1.set_attr_dict({"is_complex": 1, "is_fftodd": 1, 'is_complex_ri': 1, 'is_fftpad': 1})
                    shrank1 = stepone(tvol1, tweight1)
                ...
                if Blockdata["myid_on_node"] == 0:
                    tag = 7007
                    sp_utilities.send_EMData(shrank0, Blockdata["no_of_processes_per_group"] - 1, tag, Blockdata["shared_comm"])
                    del shrank0
                    lcfsc = 0
                elif Blockdata["myid_on_node"] == Blockdata["no_of_processes_per_group"] - 1:
                    tag = 7007
                    ...
                    cfsc = sp_statistics.fsc(shrank0, shrank1)[1]
                    sp_utilities.write_text_row(cfsc,
                ...
                if Blockdata["myid_on_node"] == 1:
                    tag = 7007
                    sp_utilities.send_EMData(shrank0, Blockdata["no_of_processes_per_group"] - 2, tag, Blockdata["shared_comm"])
                    del shrank0
                    lcfsc = 0
                elif Blockdata["myid_on_node"] == Blockdata["no_of_processes_per_group"] - 2:
                    tag = 7007
                    ...
                    Tracker["fsc143"] = fsc143
                    Tracker["fsc05"] = fsc05
                Tracker = sp_utilities.wrap_mpi_bcast(Tracker, Blockdata["no_of_processes_per_group"] - 1, Blockdata["shared_comm"])
                if (Blockdata["myid_on_node"] == 0):
...
def compute_final_map(work_dir):
    ...
    number_of_groups = sp_utilities.bcast_number_to_all(number_of_groups, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    Tracker = sp_utilities.wrap_mpi_bcast(Tracker, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
    if (number_of_groups == 0): sp_global_def.ERROR("No clusters  found, the program terminates.", "compute_final_map", 1,
                                      Blockdata["myid"])
    compute_noise(Tracker["nxinit"])
    if (Blockdata["myid"] == Blockdata["main_node"]):
...
def do3d_sorting_groups_nofsc_final(rdata, parameterstructure, norm_per_particle):
    ...
    keepgoing = sp_utilities.bcast_number_to_all(keepgoing, source_node=Blockdata["main_node"],
                                    mpi_comm=mpi.MPI_COMM_WORLD)  # always check
    Tracker = sp_utilities.wrap_mpi_bcast(Tracker, Blockdata["main_node"])
    if not keepgoing: sp_global_def.ERROR("do3d_sorting_groups_nofsc_final  %s" % os.path.join(Tracker["directory"], "tempdir"),
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            for ig1, value in list(Tracker["generation"].items()):
                ...
                for ic in range(value):
                    ...
                    cluster = sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"], "generation_%03d" % ig,
                                                          "Cluster_%03d.txt" % ic))
            ...
            Unaccounted_file = os.path.join(Tracker["constants"]["masterdir"], "generation_%03d" % ig,
                                            "Unaccounted.txt")
            shutil.copyfile(Unaccounted_file, os.path.join(Tracker["constants"]["masterdir"], "Unaccounted.txt"))
...
def do_random_groups_simulation_mpi(ptp1, ptp2):
    ...
    for iloop in range(Nloop):
        ...
        for i in range(NT):
            ...
            for j in range(k): new_clusters1[j] = numpy.sort(new_clusters1[j])
            for j in range(k): new_clusters2[j] = numpy.sort(new_clusters2[j])
            ...
            for j in range(k):
                try:
                    clist[j].append(
                        float((numpy.intersect1d(new_clusters1[newindeces[j][0]], new_clusters2[newindeces[j][1]])).size)
                        / float(
                            (numpy.union1d(new_clusters1[newindeces[j][0]], new_clusters2[newindeces[j][1]])).size) * 100.)
                except:
                    pass
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        svar = numpy.sqrt(svar / float(Blockdata["nproc"] * Nloop))
        save = save / float(Blockdata["nproc"] * Nloop)
        for i in range(k):
            gave[i] = gave[i] / float(Blockdata["nproc"] * Nloop)
            gvar[i] = numpy.sqrt(gvar[i] / float(Blockdata["nproc"] * Nloop))
        gave.append(save)
        gvar.append(svar)
    else:
        gave = 0
    ...
    while (keepsorting == 1) and (bad_clustering == 0):
        ...
        if keepchecking == 0:  # new, do it
            if Blockdata["myid"] == Blockdata["main_node"]:
                ...
                within_generation_restart = 0
            within_generation_restart = sp_utilities.bcast_number_to_all(within_generation_restart, Blockdata["main_node"],
                                                            mpi.MPI_COMM_WORLD)
...
def main():
    ...
    if initiate_from_meridien_mode:
        ...
        checking_flag = sp_utilities.bcast_number_to_all(checking_flag, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
        if checking_flag == 1:  sp_global_def.ERROR("The specified refinement_dir does not exist", "sort3d", 1, Blockdata["myid"])

        if options.focus != '':
            if Blockdata["myid"] == Blockdata["main_node"]:
                if not os.path.exists(options.focus):  checking_flag = 1
            checking_flag = sp_utilities.bcast_number_to_all(checking_flag, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            if checking_flag == 1: sp_global_def.ERROR("The specified focus mask file does not exist", "sort3d", 1, Blockdata["myid"])

        if options.mask3D != '':
            if Blockdata["myid"] == Blockdata["main_node"]:
                if not os.path.exists(options.mask3D): checking_flag = 1
            checking_flag = sp_utilities.bcast_number_to_all(checking_flag, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
            if checking_flag == 1: sp_global_def.ERROR("The specified mask3D file does not exist", "sort3d", 1, Blockdata["myid"])

        if options.img_per_grp <= 1:
            sp_global_def.ERROR("Improperiate input paramter for img_per_grp", "sort3d", 1, Blockdata["myid"])
        elif options.img_per_grp < options.minimum_grp_size:
            sp_global_def.ERROR("Img_per_grp should be always larger than minimum_grp_size", "sort3d", 1, Blockdata["myid"])
        ...
        if options.img_per_grp <= 1:
            sp_global_def.ERROR("Improperiate number for img_per_grp", "sort3d", 1, Blockdata["myid"])
        elif options.img_per_grp < options.minimum_grp_size:
            sp_global_def.ERROR("Parameter img_per_grp should be always larger than parameter minimum_grp_size", "sort3d", 1,
                  Blockdata["myid"])
