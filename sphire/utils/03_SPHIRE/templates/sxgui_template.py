...
class SXLookFeelConst(object):
	...
	# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
	default_bg_color = PyQt5.QtGui.QColor(229, 229, 229, 192) # default_bg_color = QColor(229, 229, 229, 242) # Greyish-White Transparent
	default_bg_color_string = 'rgba(229, 229, 229, 192)' # default_bg_color = QColor(229, 229, 229, 242) # Greyish-White Transparent
	default_bg_color_calculator = PyQt5.QtGui.QColor(214, 214, 214) # Greyish-White
	sxinfo_widget_bg_color = PyQt5.QtGui.QColor(0, 0, 0, 10) # Almost-Completely Transparent
	sxcmd_widget_bg_color = PyQt5.QtGui.QColor(0, 0, 0, 0) # Completely Transparent
	sxcmd_tab_bg_color = PyQt5.QtGui.QColor(229, 229, 229, 200) # White Transparent
	...
	def generate_sxcmd_wiki_url(sxcmd, wiki_type = "SPHIRE"):
		if wiki_type == "SPHIRE":
			...
			sxcmd_wiki_url = "%s%s" % (sp_global_def.SPARX_DOCUMENTATION_WEBSITE, sxcmd.name)
			if sxcmd.subname != "":
				sxcmd_wiki_url = "%s_%s" % (sxcmd_wiki_url, sxcmd.subname)
...
class SXMenuItemBtnAreaWidget(QWidget):
	def __init__(self, sxconst_set, sxcmd_category_list, sxinfo, helical, parent = None):
		...
		if helical:
			sxmenu_item_btn_pictograph_file_path = '{0}sxgui_logo_sphire_helix.png'.format(EMAN2.get_image_directory())
		else:
			sxmenu_item_btn_pictograph_file_path = '{0}sxgui_logo_sphire.png'.format(EMAN2.get_image_directory())
	...
	def add_sxmenu_item_btn_widget(self, sxmenu_item, sxmenu_item_btn_subarea_widget):
		...
		sxmenu_item_btn_pictograph_file_path = "{0}sxgui_pictograph_{1}.png".format(EMAN2.get_image_directory(), sxmenu_item.name.replace("sxc_", ""))
...
class SXCmdWidget(QWidget):
	...
	def __init__(self, sxconst_set, sxcmd, helical, parent = None):
		...
		palette.setBrush(PyQt5.QtGui.QPalette.Background, PyQt5.QtGui.QBrush(SXLookFeelConst.sxcmd_widget_bg_color))
			...
			# Then, handle the other cases//
				...
				if sxcmd_token.type == "bool_ignore":
					...
					if sxcmd_token.is_required:
						...
						if not sxcmd_token.is_reversed and (sxcmd_token.widget.checkState() == PyQt5.QtCore.Qt.Checked) != sxcmd_token.default:
							is_flag_required = True
						elif sxcmd_token.is_reversed and (sxcmd_token.widget.checkState() == PyQt5.QtCore.Qt.Checked) == sxcmd_token.default:
							is_flag_required = True
						else:
...
###						if sxcmd_token.type not in ["int", "float", "apix", "ctfwin", "box", "radius", "mass", "displayable_list", "mic_one_list", "any_file_list", "any_image_list", "dir_list"]:
	...
	def generate_cmd_line(self):
		...
		if sxcmd_line:
			...
			cmd_line = ""
			if self.sxcmd_tab_main.qsub_enable_checkbox.checkState() == PyQt5.QtCore.Qt.Checked:
				...
				if self.sxcmd_tab_main.qsub_enable_checkbox.checkState() == PyQt5.QtCore.Qt.Checked: sp_global_def.ERROR("Logical Error: Encountered unexpected condition for sxcmd_tab_main.qsub_enable_checkbox.checkState. Consult with the developer.", "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	def execute_cmd_line(self, execute=True, output_dir='.', number=None):
		...
		PyQt5.QtCore.QTimer.singleShot(5000, lambda: execute_btn.setEnabled(True))
		...
		if cmd_line:
			...
			for sxcmd_token in self.sxcmd.token_list:
				if sxcmd_token.type == "output" or sxcmd_token.type == "output_continue" or sxcmd_token.type == "output_bdb2d_stack":
					if os.path.exists(sxcmd_token.widget.text()) or EMAN2db.db_check_dict(str(sxcmd_token.widget.text())):
			...
			elif self.sxcmd_tab_main.qsub_enable_checkbox.checkState() == PyQt5.QtCore.Qt.Unchecked and not execute:
	...
	def add_to_pipeline(self):
		name = PyQt5.QtGui.QFileDialog.getExistingDirectory(self, 'Pipeline output directory', SXLookFeelConst.file_dialog_dir, options = QFileDialog.DontUseNativeDialog)
	...
	def print_cmd_line(self):
		...
		if cmd_line:
			...
			print(message_line)
			print(cmd_line)
			PyQt5.QtGui.QMessageBox.information(self, "Information","%s \n\n%s" % (message_line, cmd_line))
				...
				if cmd_token.group == group:
					...
					# Then, handle the other cases
						...
						if cmd_token.type in ("bool", "bool_ignore"):
							if cmd_token.widget.checkState() == PyQt5.QtCore.Qt.Checked:
								val_str = "YES"
							else:
								val_str = "NO"
						else:
		...
		if line_in.find("@@@@@ %s gui settings" % (self.sxcmd.get_mode_name_for("human"))) != -1:
			...
			for line_in in file_in:
				...
				if label_in == "MPI processors":
					...
					if val_str_in == "YES":
						self.sxcmd_tab_main.qsub_enable_checkbox.setChecked(PyQt5.QtCore.Qt.Checked)
					else: # assert(val_str_in == "NO")
						self.sxcmd_tab_main.qsub_enable_checkbox.setChecked(PyQt5.QtCore.Qt.Unchecked)
					...
					if not cmd_token.is_locked: 
						...
						if cmd_token.type == "user_func":
							...
							if cmd_token.type in ("bool", "bool_ignore"):
								...
								if val_str_in == "YES":
									cmd_token.widget.setChecked(PyQt5.QtCore.Qt.Checked)
								else: # val_str_in == "NO"
									cmd_token.widget.setChecked(PyQt5.QtCore.Qt.Unchecked)
							# Then, handle the other cases
							else:
...
class SXCmdTab(QWidget):
	def __init__(self, name, helical, parent=None):
		...
		scroll_area.setVerticalScrollBarPolicy(PyQt5.QtCore.Qt.ScrollBarAlwaysOn)
		palette = PyQt5.QtGui.QPalette()
		palette.setBrush(PyQt5.QtGui.QPalette.Background, PyQt5.QtGui.QBrush(SXLookFeelConst.sxcmd_tab_bg_color))
			...
			if curr_group == tab_group:
				...
				if cmd_token.type == "user_func":
					...
					if len(restores) == 1:
						...
						cmd_token_restore_widget[widget_index].clicked.connect(functools.partial(self.handle_restore_widget_event, cmd_token, widget_index))
					else:
						...
						cmd_token_restore_widget[widget_index].lineEdit().setAlignment(PyQt5.QtCore.Qt.AlignCenter)
						cmd_token_restore_widget[widget_index].setStyleSheet(custom_style)
						cmd_token_restore_widget[widget_index].setToolTip('<FONT>'+default_cmd_token_restore_tooltip+'</FONT>')
						grid_layout.addWidget(cmd_token_restore_widget[widget_index], grid_row, grid_col_origin + token_label_col_span, token_widget_row_span, token_widget_col_span)
						cmd_token_restore_widget[widget_index].activated.connect(functools.partial(self.handle_restore_widget_event, cmd_token, widget_index))
					...
					temp_btn.clicked.connect(functools.partial(self.sxcmdwidget.select_file, cmd_token_widget[widget_index], file_format))
				...
				# Then, handle the other cases
					...
					if cmd_token.type in ("bool", "bool_ignore"):
						if len(restores) == 1:
							...
							cmd_token_restore_widget.clicked.connect(functools.partial(self.handle_restore_widget_event, cmd_token))
						else:
							...
							cmd_token_restore_widget.lineEdit().setAlignment(PyQt5.QtCore.Qt.AlignCenter)
							...
							cmd_token_restore_widget.activated.connect(functools.partial(self.handle_restore_widget_event, cmd_token))

						# construct new widget(s) for this command token
						cmd_token_widget = SXCheckBox(cmd_token.key_base)
						if restores[0] == True:
							cmd_token_widget.setCheckState(PyQt5.QtCore.Qt.Checked)
						else:
							cmd_token_widget.setCheckState(PyQt5.QtCore.Qt.Unchecked)
						...
						if cmd_token.type == "displayable_list":
							...
							grid_layout.addWidget(temp_btn, grid_row, grid_col_origin + token_label_col_span + token_widget_col_span * 2, token_widget_row_span, token_widget_col_span)
							temp_btn.clicked.connect(functools.partial(self.sxcmdwidget.select_file, cmd_token_widget, file_format))
							...
							temp_btn.clicked.connect(
								functools.partial(self.sxcmdwidget.select_file, cmd_token_widget,
										file_format))
							file_format = "INVISIBLE"
							temp_btn = QPushButton("%s" % file_format)
							temp_btn.setToolTip('<FONT>' + "This is %s button</FONT>" % file_format)
							...
							grid_layout.addWidget(temp_btn, grid_row, grid_col_origin + token_label_col_span + token_widget_col_span * 2, token_widget_row_span, token_widget_col_span)
							temp_btn.clicked.connect(functools.partial(self.sxcmdwidget.select_dir, cmd_token_widget))
							file_format = "INVISIBLE"
							temp_btn = QPushButton("%s" % file_format)
							temp_btn.setToolTip('<FONT>'+"This is %s button</FONT>" % file_format)
							temp_btn.setEnabled(False)
...
###							grid_layout.addWidget(temp_btn, grid_row, grid_col_origin + token_label_col_span + token_widget_col_span * 3, token_widget_row_span, token_widget_col_span)
		...
		if is_necessary_to_connet_now:
			...
			cmd_token_apix.widget.editingFinished.connect(functools.partial(self.handle_apix_token_editing_finished_event, cmd_token_apix))
		...
		if tab_group == "main":
			...
			if is_qsub_enabled == True:
				self.qsub_enable_checkbox.setCheckState(PyQt5.QtCore.Qt.Checked)
			elif not self.sxcmdwidget.sxcmd.is_submittable:
				self.qsub_enable_checkbox.setCheckState(PyQt5.QtCore.Qt.Unchecked)
			...
			self.qsub_script_open_btn.clicked.connect(functools.partial(self.sxcmdwidget.select_file, self.qsub_script_edit, 'submission_template'))
...
###			grid_row += 1
	...
	def set_text_entry_widget_enable_state(widget, is_enabled):
		...
		default_palette = PyQt5.QtGui.QPalette()
		bg_color = default_palette.color(PyQt5.QtGui.QPalette.Inactive, PyQt5.QtGui.QPalette.Base)
		if is_enabled == False:
			bg_color = default_palette.color(PyQt5.QtGui.QPalette.Disabled, PyQt5.QtGui.QPalette.Base)
	...
	def set_qsub_enable_state(self):
		is_enabled = False
		if self.qsub_enable_checkbox.checkState() == PyQt5.QtCore.Qt.Checked:
			is_enabled = True
		...
		if sxcmd_token.type == "user_func":
			...
			if sxcmd_token.type in ("bool", "bool_ignore"):
				try:
					...
					self.sender().blockSignals(False)
				if text == 'YES':
					sxcmd_token.widget.setChecked(PyQt5.QtCore.Qt.Checked)
				else: # sxcmd_token.restore == False
					sxcmd_token.widget.setChecked(PyQt5.QtCore.Qt.Unchecked)
			else:
				try:
					text = str(self.sender().text())
				except AttributeError:
...
class SXCmdCategoryWidget(QWidget):
	def __init__(self, sxconst_set, sxcmd_category, helical=False, parent = None):
		...
		# Setup Window Layout
		# --------------------------------------------------------------------------------
		self.setup_layout(PyQt5.QtGui.QBrush(SXLookFeelConst.default_bg_color))
...
#				sxcmd.widget.read_params(sxcmd.widget.gui_settings_file_path)
	...
	def setup_layout(self, background_brush):
		...
		self.setAutoFillBackground(True)
		palette = PyQt5.QtGui.QPalette()
		palette.setBrush(PyQt5.QtGui.QPalette.Background, background_brush)
		self.setPalette(palette)
	...
	def add_sxcmd_widgets(self, helical):
		...
		for sxcmd in self.sxcmd_category.cmd_list:
			...
			sxcmd.btn.clicked.connect(functools.partial(self.handle_sxcmd_btn_event, sxcmd))
...
class SXConstSetWidget(QWidget):
	def __init__(self, sxconst_set, sxcmd_category_list, helical=False, parent=None):
		...
		self.setAutoFillBackground(True)
		palette = PyQt5.QtGui.QPalette()
		palette.setBrush(PyQt5.QtGui.QPalette.Background, PyQt5.QtGui.QBrush(SXLookFeelConst.default_bg_color))
		self.setPalette(palette)
		...
		for sxconst in self.sxconst_set.list:
			...
			sxconst_register_widget.clicked.connect(functools.partial(self.handle_regster_widget_event, sxconst))
...
class SXInfoWidget(QWidget):
	def __init__(self, parent = None):
		...
		pic_name = '{0}sxgui_info.png'.format(EMAN2.get_image_directory())
		# Import the picture as pixmap to get the right dimensions
		self.pixmap = PyQt5.QtGui.QPixmap(pic_name)
		...
		scroll_area.setVerticalScrollBarPolicy(PyQt5.QtCore.Qt.ScrollBarAlwaysOn)
		scroll_area.setHorizontalScrollBarPolicy(PyQt5.QtCore.Qt.ScrollBarAlwaysOn)
...
class SXDialogCalculator(QDialog):
	...
	def __init__(self, sxconst_register_widget_apix, sxcmd_token_widget_abs_freq, sxcmd_token_subwidget_left_ares, parent = None):
		...
		self.setAutoFillBackground(True)
		palette = PyQt5.QtGui.QPalette()
		palette.setBrush(PyQt5.QtGui.QPalette.Background, PyQt5.QtGui.QBrush(SXLookFeelConst.default_bg_color_calculator))
		self.setPalette(palette)
...
#		global_grid_row += 1
		...
		for sxoperand in self.sxoperand_set.list:
			...
			if sxoperand.is_input:
				...
				sxoperand_register_widget.clicked.connect(functools.partial(self.handle_regster_widget_event, sxoperand))
			else:
...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def __init__(self, helical, parent = None):
		...
		# Setup Window Layout
		# --------------------------------------------------------------------------------
		background_image_file_path = '{0}sxgui_background.png'.format(EMAN2.get_image_directory())
		...
		for idx in range(2):
			...
			for i, sxcmd_category in enumerate(self.sxcmd_category_list[idx]):
				sxcmd_category.btn.clicked.connect(functools.partial(self.handle_sxmenu_item_btn_event, sxcmd_category, i))
			self.sxconst_set[idx].btn.clicked.connect(functools.partial(self.handle_sxmenu_item_btn_event, self.sxconst_set[idx]))
			self.sxinfo[idx].btn.clicked.connect(functools.partial(self.handle_sxmenu_item_btn_event, self.sxinfo[idx]))
...
def main():
	...
	sxapp.setWindowIcon(PyQt5.QtGui.QIcon(EMAN2.get_image_directory()+"sxgui_icon_sphire.png"))

	sxapp_font = sxapp.font()
	sxapp_font_info = PyQt5.QtGui.QFontInfo(sxapp.font())
...
#	sxapp.setFont(sxapp_font)
	...
	sxmain_window.move(PyQt5.QtCore.QPoint(SXLookFeelConst.sxmain_window_left, SXLookFeelConst.sxmain_window_top));
