    ...
    if options.phase_flip:
        ...
        if options.fl > 0.0:
            inputvol = sp_filter.filt_tanl(
                inputvol, options.pixel_size / options.fl, options.aa
            )
        ...
        if e1.get_zsize() == 1:  # 2D case
            ...
            for i in range(nimage):
                ...
                if options.B_enhance == 0.0 or options.B_enhance == -1.0:
                    ...
                    if options.B_stop == 0.0:
                        freq_max = 1.0 / (2.0 * options.pixel_size)
                    else:
                        freq_max = 1.0 / options.B_stop
                    freq_min = 1.0 / options.B_start
                    log_main.add(
                ...
                sigma_of_inverse = numpy.sqrt(2.0 / global_b)
                ...
                if options.fl > 0.0 and options.fl < 0.5:
                    ...
                    e1 = sp_filter.filt_tanl(
                        e1, options.pixel_size / options.fl, options.aa
                    )
            ...
            for map1_path in input_path_list:
                ...
                def filter_product(B_factor, pixel_size, cutoff, aa, image_size):
                    ...
                    N = image_size // 2
                    sigma_of_inverse = numpy.sqrt(2.0 / (B_factor / pixel_size ** 2))
                    values = []
                    if cutoff > 0.5:
                        cutoff = pixel_size / cutoff  # always uses absolute frequencies
                    for i in range(N):
                        x = float(i) / float(N * 2.0)
                        values.append(
                ...
                if not single_map:
                    ...
                    raw_x_ticks_ang = [
                        int(round(options.pixel_size / float(entry), 0))
                    ...
                    x_ticks_freq.append(
                        options.pixel_size / round(nyquist_resolution, 2)
                    )
                    ...
                    for ifreq in range(nfreq143 + 1, len(fsc_true[1])):
                        if ifreq == nfreq143 + 1:
                            ...
                            fsc_true[1][ifreq] = (fsc_true[1][nfreq143 - 1]) / 5.0
                        else:
                ...
                for ig in range(len(guinierline)):
                    x = ig * 0.5 / float(len(guinierline)) / options.pixel_size
                ...
                if (
                    ...
                    if options.B_enhance == 0.0:  # auto mode
                        ...
                        sigma_of_inverse = numpy.sqrt(
                            2.0 / (global_b / options.pixel_size ** 2)
                        )
                        ...
                        sigma_of_inverse = numpy.sqrt(
                            2.0 / ((abs(options.B_enhance)) / options.pixel_size ** 2)
                        )
                        global_b = options.B_enhance
                ...
                if not single_map:
                    log_main.add(
                        ...
                        % (
                            round((options.pixel_size / resolution_FSChalf), 3),
                            round((options.pixel_size / resolution_FSC143), 3),
                        )
                    )
                    if dip_at_fsc:
                        log_main.add(
                ...
                if cutoff != 0.0:
                    ...
                    log_main.add(
                        ...
                        % (cutoff, options.pixel_size / cutoff)
                    )
                    log_main.add(
