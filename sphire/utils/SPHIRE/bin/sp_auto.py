...
def main(args_as_dict):
	...
	for idx, entry in enumerate(cmds):
		if isinstance(entry, list) and entry[0][0]:
			tmp = line.format('\' \''.join(entry[1:]), subprocess.os.path.join(args_as_dict['output_directory'], '{0:04d}_{1}'.format(dict_idx_dict[idx], entry[0][1])))
		elif isinstance(entry, list):
			tmp = line_no_mpi.format('\' \''.join(entry[1:]), subprocess.os.path.join(args_as_dict['output_directory'], '{0:04d}_{1}'.format(dict_idx_dict[idx], entry[0][1])))
		else:
			tmp = entry
		...
		for key, value in args_as_dict.items():
			if '_OUTPUT_DIR_' in key and key in tmp:
				if key in output_folder_dict:
					...
					value = subprocess.os.path.join(args_as_dict['output_directory'], '{0:04d}_{1}'.format(dict_idx_dict[idx], value))
