...
def AI( fff, anger, shifter, chout = False):
	...
	keepgoing = 1
	if(Blockdata["myid"] == Blockdata["main_node"]): line = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + " =>"
	...
	if(Tracker["mainiteration"] == 1):
		...
		if( chout ): sp_global_def.sxprint("  AI: TR[nxstep], TR[currentres], TR[fsc143], l05, l01, fff[Tracker[nxinit]//2-1]:",Tracker["nxstep"],Tracker["currentres"],Tracker["fsc143"], l05, l01,fff[Tracker["nxinit"]//2-1])
		...
		if( chout ):  sp_global_def.sxprint("  Incoming  parameters  %10.3f  %10.3f  %10.3f  %10.3f   %s"%(Tracker["anger"],anger,Tracker["shifter"],shifter,params_changes))
		...
		if( chout ): sp_global_def.sxprint("  IN AI: nxstep, large at Nyq, outcoming current res, adjusted current, inc, estimated image size",Tracker["nxstep"],Tracker["large_at_Nyquist"],Tracker["currentres"],inc,tmp)
		...
		if((Tracker["no_improvement"]>=Tracker["constants"]["limit_improvement"]) and (Tracker["no_params_changes"]>=Tracker["constants"]["limit_changes"]) and (not Tracker["large_at_Nyquist"])):
			if( Tracker["delta"] < 0.75*Tracker["acc_rot"] ):#<<<----it might cause converge issues when shake is 0.0
				...
				if(Blockdata["myid"] == Blockdata["main_node"]):
					sp_global_def.sxprint(line,"Convergence criterion A is reached (angular step delta smaller than 3/4 changes in angles))")
			else:
				step_range, step = compute_search_params(Tracker["acc_trans"], Tracker["shifter"], Tracker["xr"])
				if( chout ):   sp_global_def.sxprint("  Computed  pares  ",Tracker["anger"] ,anger,Tracker["shifter"],shifter, Tracker["xr"], step_range, step)
				...
				if( Tracker["state"] == "PRIMARY" ) and ( Tracker["delta"] <= 3.75/2.0 ) and Tracker['constants']['do_exhaustive']:
					...
					Tracker['theta_min'] = -1
					Tracker['theta_max'] = -1
					if( Tracker["delta"] <= numpy.degrees(math.atan(0.25/Tracker["constants"]["radius"])) ): Tracker["state"] = "FINAL"
					else:	Tracker["state"] = "RESTRICTED"
				else:
					Tracker["an"] = -1
					if( Tracker["state"] == "PRIMARY" ):  Tracker["state"] = "EXHAUSTIVE"
				if( chout ): sp_global_def.sxprint("  IN AI there was reset due to no changes, adjust stuff  ",Tracker["no_improvement"],Tracker["no_params_changes"],Tracker["delta"],Tracker["xr"],Tracker["ts"], Tracker["state"])
				# check convergence before reset
				if( (Tracker["state"] == "FINAL") and (Tracker["no_improvement"]>=Tracker["constants"]["limit_improvement"]) ):
					keepgoing = 0
					if(Blockdata["myid"] == Blockdata["main_node"]):
						sp_global_def.sxprint(line,"Convergence criterion B is reached (angular step delta smaller than the limit imposed by the structure radius)")
				Tracker["no_improvement"]		= 0
				Tracker["no_params_changes"]	= 0
...
def AI_continuation(fff, anger = -1.0, shifter = -1.0, chout = False):
	...
	if( chout ): sp_global_def.sxprint("  AI: Tracker[nxstep], TR[currentres], Tracker[fsc143], l05, l01, fff[Tracker[nxinit]//2-1]:",Tracker["nxstep"],Tracker["currentres"],Tracker["fsc143"], l05, l01,fff[Tracker["nxinit"]//2-1])
	if(Blockdata["myid"] == Blockdata["main_node"]): line = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + " =>"
	...
	if(Tracker["mainiteration"] == 1):
		...
		Tracker["constants"]["best"] = Tracker["mainiteration"]
		if( Blockdata["myid"] == Blockdata["main_node"] ):
			sp_global_def.sxprint(line, "ITERATION  #%2d. Resolution achieved       : %3d/%3d pixels, %5.2fA/%5.2fA."%\
...
def params_changes( params, oldparams ):
	...
	return round(anger/n,5), round(numpy.sqrt(shifter/2/n),5)
...
def assign_particles_to_groups(minimum_group_size = 10, asubset= None, name_tag='ptcl_source_image'):
	...
	if not asubset:
		try:
			stmp  = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"], name_tag)
			if Tracker["constants"]["CTF"]:
				defstmp = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"],"ctf")
			else:
				defstmp = [-1.0]*len(stmp)
			for i in range(len(defstmp)): defstmp[i] = round(defstmp[i].defocus, 4)
		except:
			if Tracker["constants"]["CTF"]:
				stmp = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"],"ctf")
				for i in range(len(stmp)):  stmp[i] = round(stmp[i].defocus, 4)
				defstmp = stmp[:]
			else:
				sp_global_def.ERROR( "Either ptcl_source_image/filament/filament_id or ctf has to be present in the header." )
	else:
		try:
			stmp_junk = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"], name_tag)
			stmp = [None]*len(asubset)
			for isub in range(len(asubset)): stmp[isub] = stmp_junk[asubset[isub]]
			if Tracker["constants"]["CTF"]:
				defstmp_junk = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"],"ctf")
			...
			if Tracker["constants"]["CTF"]:
				stmp_junk = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"], "ctf")
				stmp = [None]*len(asubset)
	...
	sp_utilities.write_text_row(cross_reference_txt[0], os.path.join(Tracker["constants"]["masterdir"],"main000","groupids.txt") )
	sp_utilities.write_text_row([[sd[cross_reference_txt[0][i][j]][0] for j in range(len(cross_reference_txt[0][i]))]  for i in range(len(cross_reference_txt[0]))], os.path.join(Tracker["constants"]["masterdir"],"main000","micids.txt") )
	...
	for iproc in range(2):
		sp_utilities.write_text_row([cross_reference_txt[0][i] for i in lili[iproc]] , os.path.join(Tracker["constants"]["masterdir"],"main000","groupids_%03d.txt"%iproc) )
		sp_utilities.write_text_row([[sd[cross_reference_txt[0][i][j]][0] for j in range(len(cross_reference_txt[0][i]))]  for i in lili[iproc]], os.path.join(Tracker["constants"]["masterdir"],"main000","micids_%03d.txt"%iproc) )
	del sd

	sp_utilities.write_text_file([len(cross_reference_txt[4][i]) for i in range(len(cross_reference_txt[4]))], os.path.join(Tracker["constants"]["masterdir"],"main000","number_of_particles_per_group.txt") )
	...
	sp_utilities.write_text_file(g0, os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_0.txt") )
	#write_text_file(q1, os.path.join(Tracker["constants"]["masterdir"],"main000","tchunk_1.txt") )
	sp_utilities.write_text_file(g1, os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_1.txt") )
	if asubset:
		sq0 =[None]*len(q0)
		sq1 =[None]*len(q1) 
...
def find_assignments_of_refangles_to_angles(normals_set, ancor_angle, an):
	...
	ancordir = sp_utilities.angles_to_normals(Blockdata["symclass"].symmetry_neighbors([ancor_angle[:3]]))
...
def find_nearest_k_refangles_to_angles(normals_set, ancor_angle, delta, howmany):
	...
	if( Blockdata["symclass"].sym == "c1"):
		...
		ancordir = sp_utilities.angles_to_normals(Blockdata["symclass"].symmetry_neighbors([ancor_angle]))
...
def compute_sigma(projdata, params, first_procid, dryrun = False, myid = -1, mpi_comm = -1):
	...
	if  dryrun:
		...
		if( myid == Blockdata["main_node"] ):
			tsd = sp_utilities.get_im(os.path.join(Tracker["previousoutputdir"],"bckgnoise.hdf"))
		...
		if( myid == Blockdata["main_node"] ):
			ngroups = len(sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"],"main000", "groupids.txt")))
		else: ngroups = 0
		...
		tsd = sp_utilities.model_blank(nv + nv//2, ngroups)
		...
		for i in range(ndata):  # apply_shift; info_mask; norm consistent with get_shrink_data
			...
			stmp = sp_fundamentals.cyclic_shift( projdata[i], int(round(sx)), int(round(sy)))
			st = get_image_statistics(stmp, mask, False)
			stmp -=st[0]
			stmp /=st[1]
			temp = sp_morphology.cosinemask(stmp, radius = Tracker["constants"]["radius"], s = 0.0)
...
def getindexdata(partids, partstack, particle_groups, original_data=None, small_memory=True, nproc =-1, myid = -1, mpi_comm = -1):
	...
	if( mpi_comm < 0 ):  mpi_comm = mpi.MPI_COMM_WORLD
	#  parameters
	if( myid == 0 ):  partstack = sp_utilities.read_text_row(partstack)
	else:  			  partstack = 0
	partstack = sp_utilities.wrap_mpi_bcast(partstack, 0, mpi_comm)
	#  particles IDs
	if( myid == 0 ):  partids = sp_utilities.read_text_file(partids)
	else:          	  partids = 0
	partids = sp_utilities.wrap_mpi_bcast(partids, 0, mpi_comm)
	#  Group assignments
	if( myid == 0 ):	group_reference = sp_utilities.read_text_file(particle_groups)
	...
	if( original_data == None or small_memory):
		original_data = EMAN2_cppwrap.EMData.read_images(Tracker["constants"]["stack"], partids)
...
def get_shrink_data(nxinit, procid, original_data = None, oldparams = None, \
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		...
		sp_global_def.sxprint(  line, "Processing data  onx: %3d, nx: %3d, CTF: %s, applymask: %s, preshift: %s."%(Tracker["constants"]["nnxo"], nxinit, Tracker["constants"]["CTF"], apply_mask, preshift) )
	#  Preprocess the data
	...
	for im in range(nima):
		...
		#  Apply varadj
		if not nonorm:
			EMAN2_cppwrap.Util.mul_scalar(data[im], Tracker["avgvaradj"][procid]/wnorm)
		...
		sp_utilities.set_params_proj(data[im],[phi,theta,psi,max(min(sx*shrinkage,txm),txl),max(min(sy*shrinkage,txm),txl)])
		if not return_real:
...
def get_anger(angle1, angle2):
	...
	for q in ar:
		...
		axes_dis = 0.0
		for i in range(3):
			axes_dis += sp_utilities.lacos(A1[i][0]*A2[i][0] + A1[i][1]*A2[i][1] + A1[i][2]*A2[i][2])
...
def out_fsc(f):
	global Tracker, Blockdata
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("  driver FSC  after  iteration#%3d"%Tracker["mainiteration"])
	sp_global_def.sxprint("  %4d        %7.2f         %7.3f"%(0,1000.00,f[0]))
	for i in range(1,len(f)):
		sp_global_def.sxprint("  %4d        %7.2f         %7.3f"%(i,Tracker["constants"]["pixel_size"]*Tracker["constants"]["nnxo"]/float(i),f[i]))
	sp_global_def.sxprint(" ")
...
def get_refangs_and_shifts():
	...
	refang = Blockdata["symclass"].reduce_anglesets( sp_fundamentals.rotate_params(refang, [-0.5*Tracker["delta"], -0.5*Tracker["delta"], -0.5*Tracker["delta"]]) )
	...
	k = int(numpy.ceil(Tracker["xr"]/ts_coarse))
...
def get_shifts_neighbors(rshifts, cs):
	...
	for l,q in enumerate(rshifts):
		if(sp_utilities.get_dist(q, cs) <= rad): shiftneighbors.append(l)
...
def get_refvol(nxinit):
	...
	if( nxinit != nnn ):
		ref_vol = sp_fundamentals.fdecimate(ref_vol, nxinit, nxinit, nxinit, True, False)
	return ref_vol
...
def do3d(procid, data, newparams, refang, rshifts, norm_per_particle, myid, smearing = True, mpi_comm = -1):
	...
	if procid == 0:
		if(Blockdata["no_of_groups"] >1):
			if myid == Blockdata["nodes"][0] :
				if os.path.exists(os.path.join(Tracker["directory"], "tempdir")): sp_global_def.sxprint("tempdir exists")
				else: 
					try: os.makedirs(os.path.join(Tracker["directory"], "tempdir"))
					except:  sp_global_def.sxprint("tempdir exists")
		else:
			if myid == Blockdata["main_node"]:
				if not os.path.exists(os.path.join(Tracker["directory"],"tempdir")): 
					try: os.makedirs(os.path.join(Tracker["directory"], "tempdir"))
					except: sp_global_def.sxprint("tempdir exists")
				else: sp_global_def.sxprint("tempdir exists")
	mpi.mpi_barrier(mpi_comm)
	"""Multiline Comment10"""
	shrinkage = float(Tracker["nxinit"])/float(Tracker["constants"]["nnxo"])
	if smearing:
		tvol, tweight, trol = sp_reconstruction.recons3d_trl_struct_MPI(myid = myid, main_node = Blockdata["nodes"][procid], prjlist = data, \
	...
	if(Blockdata["no_of_groups"] >1):
		if myid == Blockdata["nodes"][procid]:
			while not os.path.exists(os.path.join(Tracker["directory"],"tempdir")):  time.sleep(5)
			...
			line = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + " =>"
			sp_global_def.sxprint(line,"Executed successfully backprojection for group ",procid)
	else:
		if myid == Blockdata["main_node"]:
			tvol.set_attr("is_complex",0)
			tvol.write_image(os.path.join(   Tracker["directory"], "tempdir", "tvol_%01d_%03d.hdf"%(procid,   Tracker["mainiteration"])))
	...
	for key, value in sorted( dict.items() ):
		...
		for ll in exclude:
			if(key == ll):
				pt = False
				break
		if pt:  sp_global_def.sxprint("                    => ", key+spaces[len(key):],":  ",value)
...
def calculate_2d_params_for_centering(kwargs):
	...
	Finished_initial_2d_alignment = sp_utilities.bcast_number_to_all(Finished_initial_2d_alignment, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	if( Finished_initial_2d_alignment == 1 ):
		...
		image_start, image_end = sp_applications.MPI_start_end(number_of_images_in_stack, Blockdata["nproc"], Blockdata["myid"])
		...
		for im in range(len(original_images)):
			if(shrink_ratio != 1.0):
				original_images[im]  = sp_fundamentals.resample(original_images[im], shrink_ratio)

		nx = original_images[0].get_xsize()
		# nx = int(nx*shrink_ratio + 0.5)

		txrm = (nx - 2*(target_radius+1))//2
		if(txrm < 0):  			sp_global_def.ERROR( "ERROR!!   Radius of the structure larger than the window data size permits   %d"%(radi), "2d prealignment",1, Blockdata["myid"])
		if(txrm/nxrsteps>0):
			tss = ""
			txr = ""
		...
		params2d = sp_utilities.wrap_mpi_gatherv(params2d, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
		if( Blockdata["myid"] == Blockdata["main_node"] ):		
		...
		if (Blockdata["myid"] == Blockdata["main_node"]):
			params2d = sp_utilities.read_text_row(os.path.join(init2dir, "initial2Dparams.txt"))
...
def ali3D_polar_ccc(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	volbuf = sp_helix_sphire.np.frombuffer(sp_helix_sphire.np.core.multiarray.int_asbuffer(base_vol, sizevol*disp_unit), dtype = 'f4')
	...
	buffer = sp_helix_sphire.np.frombuffer(sp_helix_sphire.np.core.multiarray.int_asbuffer(base_ptr, size*disp_unit), dtype = 'f4')
	...
	if( Tracker["nxinit"] != Tracker["nxpolar"] ):
		...
		volbufinit = sp_helix_sphire.np.frombuffer(sp_helix_sphire.np.core.multiarray.int_asbuffer(base_volinit, sizevol*disp_unit), dtype = 'f4')
	...
	nang_start, nang_end = sp_applications.MPI_start_end(n_coarse_ang, Blockdata["no_of_processes_per_group"], Blockdata["myid_on_node"])
	...
	for i in range(nang_start, nang_end, 1):  # This will take care of process on a node less than nang.  Some loops will not be executed
		temp = sp_projection.prgl(volprep,[ coarse_angles[i][0], coarse_angles[i][1],0.0, 0.0,0.0], 1, True)
	...
	mpi.mpi_barrier(Blockdata["shared_comm"])
	if(Blockdata["myid"] == Blockdata["main_node"]):
		sp_global_def.sxprint( "  Reference projections generated : %10.1fmin"%((time.time()-at)/60.))
	
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		sp_global_def.sxprint( "  " )
		#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		line = ''
		sp_global_def.sxprint(  line, "Processing data for polar onx: %3d, nx: %3d, nxpolar: %3d, CTF: %s, preshift: %s."%(Tracker["constants"]["nnxo"], Tracker["nxinit"], Tracker["nxpolar"], Tracker["constants"]["CTF"], preshift) )
	...
	for im in range(nima):
		...
		dataimage = sp_fundamentals.fpol(EMAN2_cppwrap.Util.mulnclreal(sp_fundamentals.fdecimate(dataimage, Tracker["nxinit"], Tracker["nxinit"], 1, False), mask ), Tracker["nxpolar"], Tracker["nxpolar"],1, True)

		if( ( Blockdata["myid"] == Blockdata["main_node"])  and  (im%(max(1,nima/5)) == 0) and (im>0)):
			sp_global_def.sxprint( "  Number of images :%7d   %5d  %5.1f"%(im,nima,float(im)/float(nima)*100.) + "%" +"   %10.1fmin"%((time.time()-at)/60.))
		...
		if( im < min(nima, 1) and procid == 0):
			...
			xod2 = sp_helix_sphire.np.asarray(EMAN2_cppwrap.Util.multiref_Crosrng_msg_stack_stepsi(dataimage, bigbuffer, \
			...
			for iln in range(keepfirst):
				...
				xod1[iln] = EMAN2_cppwrap.Util.innerproduct(data[ishift], temp, None)/nrmref  # peak
				##xod2[iln] = hashparams
			...
			xod1 = sp_helix_sphire.np.ndarray((Tracker["keepfirst"]),dtype='f4',order="C")
...
def ali3D_primary_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		if apply_mask:
			if Tracker["constants"]["hardmask"]:
				...
				bckg.set_attr("is_complex",1)
				bckg.set_attr("is_fftpad",1)
				bckg = sp_fundamentals.fft(sp_filter.filt_table(bckg, oneover[particle_group]))
				#  Normalize bckg noise in real space, only region actually used.
				st = get_image_statistics(bckg, mask2D, False)
				bckg -= st[0]
				bckg /= st[1]
		...
		if( im < min(nima, 1) and procid == 0):
			...
			lina = sp_helix_sphire.np.argwhere(xod1 > Tracker["constants"]["expthreshold"])
			...
			xod1 /= sp_helix_sphire.np.sum(xod1)
			cumprob = 0.0
			lit = len(xod1)
		...
		while(iln<lit):
			...
			while( ipsiandiang == cod2[iln]/1000 ):
				...
				fofa = EMAN2_cppwrap.Util.sqednormbckg(data[ishift], temp, ctfa, bckgnoise, indx, tfrac, tcount)
		...
		cod1 /= sp_helix_sphire.np.sum(cod1)
		cumprob = 0.0
		...
		#  New norm is a sum of eq distances multiplied by their probabilities augmented by PW.
		norm_per_particle[im] = sp_helix_sphire.np.sum(cod1[:lit]*cod3[:lit]) + accumulatepw[im][reachpw]
	...
	for kl in range(nima):
		...
		smax = max(smax, j)
		smin = min(smin, j)
	snum = mpi.mpi_reduce(snum, 1, mpi.MPI_INT, mpi.MPI_SUM, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	sava = mpi.mpi_reduce(sava, 1, mpi.MPI_FLOAT, mpi.MPI_SUM, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	svar = mpi.mpi_reduce(svar, 1, mpi.MPI_FLOAT, mpi.MPI_SUM, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	smax = mpi.mpi_reduce(smax, 1, mpi.MPI_INT, mpi.MPI_MAX, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	smin = mpi.mpi_reduce(smin, 1, mpi.MPI_INT, mpi.MPI_MIN, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	if( Blockdata["myid"] == 0 ):
		sava = float(sava)/snum
		svar = numpy.sqrt(max(0.0,(float(svar) - snum*sava**2)/(snum -1)))
		#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		line = ''
		sp_global_def.sxprint(line, "Smear stat  (number of images, ave, sumsq, min, max)):  %7d    %12.3g   %12.3g  %7d  %7d"%(snum,sava,svar,smin,smax))
	...
	if( procid == 1 ):
		...
		sp_utilities.reduce_EMData_to_root(Blockdata["newbckgnoise"], Blockdata["myid"], Blockdata["main_node"])
		if( Blockdata["myid"] == 0 ):
		...
		sp_utilities.bcast_EMData_to_all(Blockdata["newbckgnoise"], Blockdata["myid"], source_node = Blockdata["main_node"], comm = mpi.MPI_COMM_WORLD)
		for igrp in range(nyb):
			for i in range(nxb):
	...
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		sp_global_def.sxprint( "  Finished sigma2   %10.1fmin"%((time.time()-at)/60.))
...
def ali3D_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		#write_text_row([[newpar[0][2][j][0],newpar[0][2][j][1]] for j in range(len(newpar[0][2]))],os.path.join(Tracker["directory"], "polar%1d.txt"%procid))
		sp_global_def.sxprint( "  Statistics finished : %10.1fmin"%((time.time()-at)/60.))
	return newpar, norm_per_particle
			...
			for icnm in range(max(1,lenass)):  # I have to enter the loop even it there is no assignment
				...
				if( lima == 1 and procid == 0):
					...
					if( lenass > 0):
						...
						xod1 = sp_helix_sphire.np.ndarray((keepfirst),dtype='f4',order="C")
						#xod1.fill(1.0)
						xod2 = sp_helix_sphire.np.ndarray((keepfirst),dtype='int',order="C")
						...
						for iln in range(keepfirst):
							...
							if(ipsiandiang != pre_ipsiandiang):
								...
								temp.set_attr("is_complex",0)
							##junk = time()
							xod1[iln] = -EMAN2_cppwrap.Util.sqed(data[ishift], temp, ctfa, bckgnoise)
							##eat += time()-junk
							##xod2[iln] = hashparams
...
def cerrs(params, ctfs, particle_groups):
	...
	if(Blockdata["myid"] == Blockdata["nodes"][procid]):
		...
		if(Tracker["nxinit"] != nnn ):
			ref_vol = sp_fundamentals.fdecimate(ref_vol,Tracker["nxinit"],Tracker["nxinit"],Tracker["nxinit"], True, False)
	else:
		#log = None
		ref_vol = sp_utilities.model_blank(Tracker["nxinit"], Tracker["nxinit"], Tracker["nxinit"])
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	sp_utilities.bcast_EMData_to_all(ref_vol, Blockdata["myid"], Blockdata["nodes"][procid])
	interpolation_method = 1
	ref_vol = sp_projection.prep_vol(ref_vol, npad = 2, interpolation_method = interpolation_method )
	...
	for itry in range(len(params)):
		...
		F1 = sp_projection.prgl(ref_vol,[ phi1, theta1, psi1, 0.0, 0.0], interpolation_method = 1, return_real= False)
		...
		ctfsbckgnoise = EMAN2_cppwrap.Util.muln_img(EMAN2_cppwrap.Util.unroll1dpw(Tracker["nxinit"],Tracker["nxinit"], [Blockdata["bckgnoise"][i,particle_groups[itry]] for i in range(lb)]), ct)
		...
		for imode in range(2):
			...
			while (peak <= pvalue):
				...
				if (imode == 0):  F2 = sp_projection.prgl(ref_vol,[ phi2, theta2, psi2, 0.0,0.0], 1, False)
				else:             F2 = sp_fundamentals.fshift(F1, xshift*shrinkage, yshift*shrinkage)
	...
	acc_rot = mpi.mpi_reduce(acc_rot, 1, mpi.MPI_FLOAT, mpi.MPI_SUM, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	acc_trans = mpi.mpi_reduce(acc_trans, 1, mpi.MPI_FLOAT, mpi.MPI_SUM, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	acc_rot = mpi.mpi_bcast(acc_rot, 1, mpi.MPI_FLOAT, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	acc_trans = mpi.mpi_bcast(acc_trans, 1, mpi.MPI_FLOAT, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		line = ''
		sp_global_def.sxprint(line,"Estimated accuracy of angles = %6.2f degrees; and shifts = %5.1f pixels"%(acc_rot,acc_trans) )
...
def do3d_final(partids, partstack, original_data, oldparams, oldparamstructure, projdata,\
	...
	if(Blockdata["subgroup_myid"] > -1):
		...
		this_color = 0
	subgroup_comm = mpi.mpi_comm_split(mpi.MPI_COMM_WORLD, this_color, Blockdata['myid'])
	if(Blockdata["subgroup_myid"] > -1):
		...
		if(Blockdata["subgroup_myid"] == Blockdata["main_node"]):
			...
			sp_global_def.sxprint("Reconstruction uses solution of %d iteration"%final_iter)
			sp_global_def.sxprint("Final reconstruction image size is:  %d"%(Tracker["constants"]["nnxo"]))
			sp_global_def.sxprint("Final directory is %s"%(Tracker["directory"]))
		final_dir = Tracker["directory"]
		if(Blockdata["subgroup_myid"] == Blockdata["main_node"]):
			try:
				refang  = sp_utilities.read_text_row( os.path.join(final_dir, "refang.txt"))
				rshifts = sp_utilities.read_text_row( os.path.join(final_dir, "rshifts.txt"))
			except:
				carryon =0
		else:
			refang  = 0
		...
		for procid in range(2):
			...
			im_start, im_end = sp_applications.MPI_start_end(psize, Blockdata["subgroup_size"], Blockdata["subgroup_myid"])
			...
			if Tracker['prior']['apply_prior']:
				outlier_list = sp_utilities.read_text_file(outlier_file)[start_end[0]:start_end[1]]
			else:
			...
			total_left_particles = sp_utilities.wrap_mpi_gatherv(norm_per_particle_outlier, Blockdata["main_node"], subgroup_comm)
			if Blockdata['myid'] == Blockdata['main_node']: 
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		sp_helix_sphire.shutil.copyfile(os.path.join(Tracker["constants"]["masterdir"], "main%03d"%Tracker["mainiteration"], \
		...
		sp_helix_sphire.shutil.rmtree(os.path.join(Tracker["constants"]["masterdir"], "tempdir"))
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	return
...
def recons3d_final(masterdir, do_final_iter_init, memory_per_node, orgstack = None):
	...
	if(do_final_iter_init ==0):
		if(Blockdata["myid"] == Blockdata["main_node"]):
			try:
				...
				sp_global_def.sxprint("The best solution is %d  "%Tracker["constants"]["best"])
				do_final_iter =  Tracker["constants"]["best"] # set the best as do_final iteration
			except: carryon = 0
		carryon = sp_utilities.bcast_number_to_all(carryon)
		if carryon == 0: 
			sp_global_def.ERROR("Best resolution is not found, do_final will not be computed", "recons3d_final", 1, Blockdata["myid"])	# Now work on selected directory
		...
		if(Blockdata["myid"] == Blockdata["main_node"]): sp_global_def.sxprint("User selected %d iteration to compute the 3D reconstruction "%do_final_iter)
		if do_final_iter<=2:
			sp_global_def.ERROR("The selected iteration should be larger than 2", "recons3d_final", 1, Blockdata["myid"])
	...
	if(Blockdata["myid"] == Blockdata["main_node"]): # check json file and load tracker
		...
		Tracker = 0
		stack_prior = None
	carryon = sp_utilities.bcast_number_to_all(carryon, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	if carryon == 0: 
		sp_global_def.ERROR("Failed to load Tracker file %s, program terminates "%os.path.join(final_dir,"Tracker_%03d.json"%do_final_iter), "recons3d_final",1, Blockdata["myid"])
	...
	if(Blockdata["myid"] == Blockdata["main_node"]): # check stack 
		try:  image = sp_utilities.get_im(Tracker["constants"]["stack"],0)
		except:carryon = 0
	carryon = sp_utilities.bcast_number_to_all(carryon, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	if carryon == 0: 
		sp_global_def.ERROR("The orignal data stack for reconstruction %s does not exist, final reconstruction terminates"%Tracker["constants"]["stack"],"recons3d_final", 1, Blockdata["myid"])
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		sp_global_def.sxprint("  MEMORY ESTIMATION.  memory per node = %6.1fGB,  volume size = %6.2fGB, data size per node = %6.2fGB, estimated number of CPUs = %d"%(memory_per_node,volume_size,data_size,nnprocs))
	...
	nogo =    sp_utilities.bcast_number_to_all(nogo,    source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
	if( nogo == 1 ):
		sp_global_def.ERROR("Insufficient memory to compute final reconstruction","recons3d_final", 1, Blockdata["myid"])
	nnprocs = sp_utilities.bcast_number_to_all(nnprocs, source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
...
def recons3d_trl_struct_MPI_nosmearing(myid, main_node, prjlist, parameters, CTF, upweighted, mpi_comm, target_size):
	...
	for im in range(len(prjlist)):
		...
		phi,theta,psi,s2x,s2y                   = sp_utilities.get_params_proj(prjlist[im], xform = "xform.projection")
...
def rec3d_continuation_nosmearing(original_data, mpi_comm):
	...
	for procid in range(2):
		#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		line = ''
		if(Blockdata["myid"] == Blockdata["nodes"][procid]): sp_global_def.sxprint(line, "3-D reconstruction of group %d"%procid)
...
def update_memory_estimation():
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		try: total_stack = EMAN2_cppwrap.EMUtil.get_image_count(Tracker["constants"]["stack"])
		except: sp_global_def.sxprint(Tracker["constants"]["stack"], "does not exist")
		...
		sp_global_def.sxprint("  MEMORY ESTIMATION.  memory per node = %6.1fGB,  volume size = %6.2fGB, data size per node = %6.2fGB, estimated number of CPUs = %d"%(Tracker["constants"]["memory_per_node"],volume_size,data_size,nnprocs))
		memory_per_cpu_3d = data_size/Blockdata["no_of_processes_per_group"]*1.2 + volume_size
		sp_global_def.sxprint("  Estimated memory consumption per CPU in reconstruction %6.2f"%memory_per_cpu_3d)
		...
		nogo    = 0
	nogo = sp_utilities.bcast_number_to_all(nogo, source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
	if( nogo == 1 ):  sp_global_def.ERROR("Insufficient memory to continue refinement from subset","continue_from_subset", 1, Blockdata["myid"])
	nnprocs = sp_utilities.bcast_number_to_all(nnprocs, source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
	Blockdata["ncpuspernode"] 	= nnprocs
	Blockdata["nsubset"] 		= Blockdata["ncpuspernode"]*Blockdata["no_of_groups"]
	create_subgroup()
...
def rec3d_make_maps(compute_fsc = True, regularized = True):
	...
	if compute_fsc:
		if(Blockdata["no_of_groups"] == 1):
			if( Blockdata["myid"] == Blockdata["nodes"][0] ):
				tvol0 		= sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_0_%03d.hdf"%(   Tracker["mainiteration"])))
				tweight0 	= sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tweight_0_%03d.hdf"%(Tracker["mainiteration"])))
				tvol1 		= sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1_%03d.hdf"%(   Tracker["mainiteration"])))
				tweight1 	= sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "tweight_1_%03d.hdf"%(Tracker["mainiteration"])))
		...
		if( Blockdata["myid"] == Blockdata["main_node"]):
			sp_utilities.write_text_file(cfsc, os.path.join(Tracker["directory"] ,"driver_%03d.txt"%(Tracker["mainiteration"])))
			out_fsc(cfsc)
	...
	if regularized:
		for iorder in range(2):
			if(iorder == lorder[0]):
				if(Blockdata["color"] == Blockdata["node_volume"][1]):
					...
					if( Blockdata["myid_on_node"] == 0 ):
						treg0 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "trol_0_%03d.hdf"%(Tracker["mainiteration"])))
					else:
			...
			if(iorder == lorder[1]):
				if( Blockdata["color"] == Blockdata["node_volume"][0] ):
					...
					#  compute filtered volume
					if( Blockdata["myid_on_node"] == 0 ):
						treg1 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir",\
		...
		if( Blockdata["myid"] == Blockdata["nodes"][0]): sp_helix_sphire.shutil.rmtree(os.path.join(Tracker["directory"], "tempdir"))
		mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	else:
		if(Blockdata["no_of_groups"] == 1):
			for iproc in range(2):
				if(Blockdata["myid_on_node"] == 0):
					tvol0 		= sp_utilities.get_im(os.path.join(Tracker["directory"],os.path.join("tempdir", "tvol_0_%03d.hdf"%(Tracker["mainiteration"]))))
					tweight0 	= sp_utilities.get_im(os.path.join(Tracker["directory"],os.path.join("tempdir","tweight_0_%03d.hdf"%(Tracker["mainiteration"]))))
					tvol1 		= sp_utilities.get_im(os.path.join(Tracker["directory"],os.path.join("tempdir", "tvol_1_%03d.hdf"%(Tracker["mainiteration"]))))
					tweight1 	= sp_utilities.get_im(os.path.join(Tracker["directory"],os.path.join("tempdir","tweight_1_%03d.hdf"%(Tracker["mainiteration"]))))
					EMAN2_cppwrap.Util.fuse_low_freq(tvol0, tvol1, tweight0, tweight1, 2*Tracker["constants"]["fuse_freq"])
					treg  = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "trol_%d_%03d.hdf"%((iproc, Tracker["mainiteration"]))))
				else:
			...
			if(Blockdata["myid"] == Blockdata["main_shared_nodes"][1]):
				tag = 7007
				sp_utilities.send_EMData(tvol1,    Blockdata["main_shared_nodes"][0], tag, mpi.MPI_COMM_WORLD)
				sp_utilities.send_EMData(tweight1, Blockdata["main_shared_nodes"][0], tag, mpi.MPI_COMM_WORLD)
				...
				tag = 7007
				tvol1    	= sp_utilities.recv_EMData(Blockdata["main_shared_nodes"][1], tag, mpi.MPI_COMM_WORLD)
				tweight1    = sp_utilities.recv_EMData(Blockdata["main_shared_nodes"][1], tag, mpi.MPI_COMM_WORLD)
				tvol1.set_attr_dict( {"is_complex":1, "is_fftodd":1, 'is_complex_ri': 1, 'is_fftpad': 1})
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			...
			if( Blockdata["color"] == Blockdata["node_volume"][1]):
				...
				if( Blockdata["myid"] == Blockdata["main_shared_nodes"][0]):
					treg1 = sp_utilities.get_im(os.path.join(Tracker["directory"], "tempdir", "trol_1_%03d.hdf"%(Tracker["mainiteration"])))
				else:
					tvol1 		= sp_utilities.model_blank(1)
					tweight1 	= sp_utilities.model_blank(1)
					treg1 		= sp_utilities.model_blank(1)
...
def refinement_one_iteration(partids, partstack, original_data, oldparams, projdata, general_mode = True, continuation_mode = False):
	...
	for procid in range(2):
		original_data[procid], oldparams[procid], start_end[procid] = getindexdata(partids[procid], partstack[procid], \
			os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_%01d.txt"%procid), \
			original_data[procid], small_memory = Tracker["constants"]["small_memory"],\
			nproc = Blockdata["nproc"], myid = Blockdata["myid"], mpi_comm = mpi.MPI_COMM_WORLD)
	...
	if( Tracker["constants"]["shake"] > 0.0 ):
		if(Blockdata["myid"] == Blockdata["main_node"]):
			shakenumber = random.uniform( -Tracker["constants"]["shake"], Tracker["constants"]["shake"])
		else:
			shakenumber = 0.0
		...
		refang        = Blockdata["symclass"].reduce_anglesets( sp_fundamentals.rotate_params(refang, [-rangle,-rangle,-rangle]) )
		coarse_angles = Blockdata["symclass"].reduce_anglesets( sp_fundamentals.rotate_params(coarse_angles, [-rangle,-rangle,-rangle]) )
		shakegrid(rshifts, rshift)
		shakegrid(coarse_shifts, rshift)

		if(Blockdata["myid"] == Blockdata["main_node"]):
			sp_utilities.write_text_row([[shakenumber, rangle, rshift]], os.path.join(Tracker["directory"] ,"randomize_search.txt") )
	else:
		rangle = 0.0
		rshift = 0.0

	if(Blockdata["myid"] == Blockdata["main_node"]):
		sp_utilities.write_text_row( refang,  os.path.join(Tracker["directory"] , "refang.txt"))
		sp_utilities.write_text_row( rshifts, os.path.join(Tracker["directory"] , "rshifts.txt"))
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	...
	for procid in range(2):
		...
		if general_mode:
			if( Tracker["state"] == "INITIAL" ):
				...
				newparamstructure[procid], norm_per_particle[procid] = \
						...
						preshift = True, apply_mask = True, nonorm = Tracker["constants"]["nonorm"], applyctf = True)
			else:  sp_global_def.ERROR("sxmeridien","Incorrect state  %s"%Tracker["state"],1,Blockdata["myid"])
		...
		params = sp_utilities.wrap_mpi_gatherv(params, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
		#  store params
		if(Blockdata["myid"] == Blockdata["main_node"]):
			#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			line = ''
			sp_global_def.sxprint(line,"Executed successfully: ","Projection matching, state: %s, number of images:%7d"%(Tracker["state"],len(params)))
			sp_utilities.write_text_row(params, os.path.join(Tracker["directory"], "params-chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"])) )
		del params
		...
		total_left_particles = sp_utilities.wrap_mpi_gatherv(norm_per_particle_outlier, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
		if Blockdata['myid'] == Blockdata['main_node']: 
			sp_global_def.sxprint('Use {0} particles for final reconstruction!'.format(len(total_left_particles)))

		do3d(procid, projdata_outlier, newparamstructure_outlier, refang, rshifts, norm_per_particle_outlier, Blockdata["myid"], smearing = True, mpi_comm = mpi.MPI_COMM_WORLD)
		projdata_outlier = []
		newparamstructure_outlier = []
		norm_per_particle_outlier = []
		if Tracker["changed_delta"]:
		...
		if( Blockdata["myid_on_node"] == 0 ):
			...
			sp_utilities.wrap_mpi_send(newparamstructure[procid], 0, Blockdata["shared_comm"])
	...
	if( Blockdata["myid"] == Blockdata["main_node"]):
		pinids = sp_utilities.read_text_file(partids[0])  + sp_utilities.read_text_file(partids[1])
		params = sp_utilities.read_text_row(partstack[0]) + sp_utilities.read_text_row(partstack[1])
		...
		sp_utilities.write_text_file([pinids[i][0] for i in range(len(pinids))], os.path.join(Tracker["directory"] ,"indexes_%03d.txt"%(Tracker["mainiteration"])))
		sp_utilities.write_text_row( [pinids[i][1] for i in range(len(pinids))], os.path.join(Tracker["directory"] ,"params_%03d.txt"%(Tracker["mainiteration"])))
		del pinids
	...
	for procid in range(2):
		if Blockdata['myid'] == Blockdata['main_node'] and Tracker['constants']['plot_ang_dist']:
			...
			outlier_full = []
			sp_global_def.sxprint('Create angular distribution plot for chunk {0}'.format(procid))
	...
	if(Tracker["mainiteration"] == 1 ):
		...
		if( Blockdata["myid"] == Blockdata["main_node"] ):
			Blockdata["bckgnoise"]= sp_utilities.get_im(os.path.join(Tracker["directory"],"bckgnoise.hdf"))
		...
		if(Blockdata["myid"] == Blockdata["main_node"]):
			params = sp_utilities.read_text_row(os.path.join(Tracker["directory"],"params-chunk_0_%03d.txt"%(Tracker["mainiteration"])))+sp_utilities.read_text_row(os.path.join(Tracker["directory"],"params-chunk_1_%03d.txt"%(Tracker["mainiteration"])))
			li     = sp_utilities.read_text_file(os.path.join(Tracker["directory"],"chunk_0_%03d.txt"%(Tracker["mainiteration"])))+sp_utilities.read_text_file(os.path.join(Tracker["directory"],"chunk_1_%03d.txt"%(Tracker["mainiteration"])))
			ctfs   = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"],'ctf')
			ctfs   = [ctfs[i] for i in li]
			particle_groups = sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_0.txt") ) + sp_utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_1.txt") )
		...
		particle_groups = sp_utilities.wrap_mpi_bcast(particle_groups, Blockdata["main_node"])
		...
		ctfs = [sp_utilities.generate_ctf(ctfs[i]) for i in range(Blockdata["myid"]*npart,(Blockdata["myid"]+1)*npart)]
		...
		if(Blockdata["myid"] == Blockdata["main_node"]):
			sp_utilities.write_text_row( [[Tracker["acc_rot"], Tracker["acc_trans"]]], os.path.join(Tracker["directory"] ,"accuracy_%03d.txt"%(Tracker["mainiteration"])) )

	if(Blockdata["myid"] == Blockdata["main_node"]):
		anger, shifter = params_changes( sp_utilities.read_text_row(os.path.join(Tracker["directory"],"params_%03d.txt"%(Tracker["mainiteration"]))), sp_utilities.read_text_row(os.path.join(Tracker["previousoutputdir"],"params_%03d.txt"%(Tracker["mainiteration"]-1))) )
		sp_utilities.write_text_row( [[anger, shifter]], os.path.join(Tracker["directory"] ,"error_thresholds_%03d.txt"%(Tracker["mainiteration"])) )

		#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		line = ''
		sp_global_def.sxprint(line,"Average displacements for angular directions = %6.2f degrees; and shifts = %5.1f pixels"%(anger, shifter) )
...
def get_image_statistics(image, mask, invert):
	if Tracker['constants']['filament_width'] is None:
		...
		mask2d = sp_utilities.model_rotated_rectangle2D(
			radius_long=int(sp_helix_sphire.np.sqrt(2 * image.get_xsize()**2) // 2),
...
def calculate_prior_values(tracker, blockdata, outlier_file, chunk_file, params_file, im_start, im_end, procid):
	...
	if(blockdata["myid"] == blockdata["main_node"] ):
		...
		sp_global_def.sxprint('Chunk {0}: Discarded {1}|{2:.1f}%; Kept {3}|{4:.1f}%; Nr. particles {5}'.format(
			procid,
			nr_outliers,
		...
		if Tracker["constants"]["pixel_size"]*Tracker["constants"]["nnxo"]/float(Tracker["fsc143"]) > 10:
			sp_global_def.sxprint('Resolution not sufficient to remove outliers! Do not discard outlier!')
			outliers = [0] * len_data
		elif 100*no_outliers/float(len_data) < 50:
			sp_global_def.sxprint('Number of outliers too large! Do not discard outlier!')
			outliers = [0] * len_data
...
def prior_stack_fmt(sphire_prior_stack):
	...
	for entry in sphire_prior_stack.dtype.names:
		if isinstance(sphire_prior_stack[entry][0], basestring):
			fmt.append('% {0}s'.format(sp_helix_sphire.np.max([len(str_entry) for str_entry in sphire_prior_stack[entry]])+3))
		elif isinstance(sphire_prior_stack[entry][0], int):
			fmt.append('% {0}d'.format(len(str(sp_helix_sphire.np.max(sphire_prior_stack[entry])))+3))
		elif isinstance(sphire_prior_stack[entry][0], float):
			fmt.append('% {0}.6f'.format(len(str(sp_helix_sphire.np.max(sphire_prior_stack[entry])))+3))
		else:
	...
	if((not do_final_mode) and (not do_continuation_mode)):
		...
		if Blockdata["myid"]  == Blockdata["main_node"]:
			if( os.path.exists(os.path.join(masterdir, "main000", "Tracker_000.json")) ):
				...
				restart_flag  = 0
		restart_flag = sp_utilities.bcast_number_to_all(restart_flag, source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
		keepgoing1   = sp_utilities.bcast_number_to_all(keepgoing1,   source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
		keepgoing2   = sp_utilities.bcast_number_to_all(keepgoing2,   source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
		if keepgoing1== 0:
			sp_global_def.ERROR("To restart, meridien requires only the name of existing refinement directory.", "meridien",1, Blockdata["myid"])
		if keepgoing2 ==0:
			sp_global_def.ERROR("To start, meridien requires at least the stack name and the name of reference structure", "meridien",1, Blockdata["myid"])
		...
		if not balanced_processor_load_on_nodes:
			sp_global_def.ERROR("Nodes do not have the same number of CPUs, please check configuration of the cluster.", "meridien",1, Blockdata["myid"])
		if Blockdata["myid"]  == Blockdata["main_node"]:
			line = ""
			for a in sys.argv:
				line +=a+"  "
		...
		if not restart_mode: #<<<-------Fresh run
			...
			nnxo = sp_utilities.bcast_number_to_all(nnxo, source_node = Blockdata["main_node"])
			if( nnxo < 0 ):
				sp_global_def.ERROR("Incorrect image size  ", "meridien", 1, Blockdata["myid"])
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				if( Tracker["constants"]["mask3D"] and (not os.path.exists(Tracker["constants"]["mask3D"]))):
					checking_flag = 0
			checking_flag = sp_utilities.bcast_number_to_all(checking_flag, source_node = Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
			if checking_flag==0:
				sp_global_def.ERROR("mask3D file does  not exists ","meridien",1,Blockdata["myid"])
			
			if( options.xr/options.ts<1.0 ): 
				sp_global_def.ERROR("Incorrect translational searching settings, search range cannot be smaller than translation step ","meridien", 1, Blockdata["myid"])
			if( 2*(Tracker["currentres"] + Tracker["nxstep"]) > Tracker["constants"]["nnxo"] ):
				sp_global_def.ERROR("Image size less than what would follow from the initial resolution provided %d  %d  %d"%(Tracker["currentres"], Tracker["nxstep"],\
				 2*(Tracker["currentres"] + Tracker["nxstep"])),"sxmeridien", 1, Blockdata["myid"])

			if(Tracker["constants"]["radius"]  < 1):
				Tracker["constants"]["radius"]  = Tracker["constants"]["nnxo"]//2-2
			elif((2*Tracker["constants"]["radius"] +2) > Tracker["constants"]["nnxo"]):
				sp_global_def.ERROR("Particle radius set too large!","sxmeridien", 1, Blockdata["myid"])
			###<-----end of sanity check <----------------------
			###<<<----------------------------- parse program
			...
			if (radi < 1):  sp_global_def.ERROR("Particle radius has to be provided!", "2d prealignment", 1, Blockdata["myid"])
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				...
				if(options.initialshifts):
					...
					sp_utilities.write_text_row(tp_list, os.path.join(initdir,"params_000.txt"))
					...
					#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
					line = ''
					sp_global_def.sxprint(line,"Executed successfully: Imported initial parameters from the input stack")
					del tp_list

				else:
					sp_utilities.write_text_row([[0,0,0,params2d[i][1],params2d[i][2], 0.0, 1.0] for i in l1], partstack[0])
					sp_utilities.write_text_row([[0,0,0,params2d[i][1],params2d[i][2], 0.0, 1.0] for i in l2], partstack[1])
					sp_utilities.write_text_row([[0,0,0,params2d[i][1],params2d[i][2], 0.0, 1.0] for i in range(len(l1)+len(l2))], os.path.join(initdir,"params_000.txt"))

				del l1, l2

				# Create reference models for each particle group
				if(Tracker["constants"]["mask3D"] == None):
					viv = sp_filter.filt_table(sp_morphology.cosinemask(sp_utilities.get_im(volinit),radius = Tracker["constants"]["radius"]), [1.0]*Tracker["constants"]["inires"] + [0.5] + [0.0]*Tracker["constants"]["nnxo"])
				else:
					viv = sp_filter.filt_table(sp_utilities.get_im(volinit)*sp_utilities.get_im(Tracker["constants"]["mask3D"]), [1.0]*Tracker["constants"]["inires"] + [0.5] + [0.0]*Tracker["constants"]["nnxo"])
		...
		while(keepgoing):
			...
			if( not doit ):			
				...
				if doit2:
					doit = True
					if( Blockdata["myid"] == Blockdata["main_node"] ): sp_helix_sphire.shutil.rmtree(Tracker["directory"])
					mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			if doit :
				if(Blockdata["myid"] == Blockdata["main_node"]):
				...
				if(Tracker["mainiteration"] == 1):
					...
					if(Blockdata["myid"] == Blockdata["main_node"]):
						fff = sp_utilities.read_text_file(os.path.join(Tracker["previousoutputdir"],"driver_%03d.txt"%(Tracker["mainiteration"]-1)))
						[anger, shifter] = sp_utilities.read_text_row( os.path.join(Tracker["previousoutputdir"] ,"error_thresholds_%03d.txt"%(Tracker["mainiteration"]-1)) )[0]
					else:
						fff = []
						anger   = 0.0
						shifter = 0.0
				...
				if keepgoing == 1: # not converged
					if Blockdata["myid"] == Blockdata["main_node"]:
						if( Tracker["mainiteration"] > 1 ):
							#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
							line = ''
							sp_global_def.sxprint(line,"Resolution achieved in ITERATION  #%2d: %3d/%3d pixels, %5.2fA/%5.2fA."%
						...
						#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
						line = ''
						sp_global_def.sxprint(line,"ITERATION  #%2d. Current state: %14s, nxinit: %3d, delta: %9.4f, xr: %9.4f, ts: %9.4f"%\
					...
					if(Blockdata["myid"] == Blockdata["main_node"] and doit2):
						...
						junk = sp_utilities.cmdexecute(cmd)
					if(not doit2):  sp_global_def.ERROR("There was a gap in main directories, program cannot proceed","sxmeridien",1,Blockdata["myid"])
					...
					if( Blockdata["subgroup_myid"] > -1 ): mpi.mpi_comm_free(Blockdata["subgroup_comm"])
					if(Blockdata["myid"] == Blockdata["main_node"]):
						sp_global_def.sxprint(line,"Resolution achieved in ITERATION  #%2d: %3d/%3d pixels, %5.2fA/%5.2fA."%
								(Tracker["mainiteration"]-1, \
								Tracker["currentres"], Tracker["fsc143"], Tracker["constants"]["pixel_size"]*Tracker["constants"]["nnxo"]/float(Tracker["currentres"]), \
						...
						sp_global_def.sxprint("The iteration that contains the best resolution is %d"%Tracker["constants"]["best"])
						if Tracker["constants"]["best"] ==2:  sp_global_def.sxprint("No resolution improvement in refinement ")
		...
		if keepgoing1== 0:
			sp_global_def.ERROR("To restart, meridien requires only the name of existing refinement directory.", "meridien local",1, Blockdata["myid"])
			
		if keepgoing2 ==0:
			sp_global_def.ERROR("To start, meridien requires at least the stack name and the name of reference structure", "meridien local",1, Blockdata["myid"])
		...
		if( options.delta> 3.75 ):
			sp_global_def.ERROR("Local searches requested, delta cannot be larger than 3.73.", "meridien",1, Blockdata["myid"])
		if( options.memory_per_node < 0.0 ): options.memory_per_node = 2.0*Blockdata["no_of_processes_per_group"]
		#  For the time being we use all CPUs during refinement
		Blockdata["ncpuspernode"] = Blockdata["no_of_processes_per_group"]
		Blockdata["nsubset"]      = Blockdata["ncpuspernode"]*Blockdata["no_of_groups"]
		...
		if not restart_mode: #<<<-------Fresh run
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				if( Tracker["constants"]["mask3D"] and (not os.path.exists(Tracker["constants"]["mask3D"]))):
					checking_flag = 0
			checking_flag = sp_utilities.bcast_number_to_all(checking_flag, Blockdata["main_node"], mpi_comm = mpi.MPI_COMM_WORLD)
			if checking_flag==0:
				sp_global_def.ERROR("mask3D file does  not exists ","meridien",1,Blockdata["myid"])
			...
			initdir = os.path.join(Tracker["constants"]["masterdir"],"main000")
			if(Blockdata["myid"] == Blockdata["main_node"]):
				if os.path.exists(initdir): sp_helix_sphire.shutil.rmtree(initdir)
				os.makedirs(initdir)
			...
			if doit :
				...
				if keepgoing == 1: # not converged
					...
					if(Blockdata["myid"] == Blockdata["main_node"] and doit2):
						...
						junk = sp_utilities.cmdexecute(cmd)
					if(not doit2):
						sp_global_def.ERROR("There was a gap in main directories, program cannot proceed","sxmeridien",1,Blockdata["myid"])
					mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
					...
					if(Blockdata["myid"] == Blockdata["main_node"]):
						...
						sp_global_def.sxprint("Iteration that contains the best resolution is %d"%Tracker["constants"]["best"])
						if Tracker["constants"]["best"] ==2:
							sp_global_def.sxprint("No resolution improvement in refinement ")
		...
		orgstack      = None
		if( len(args) == 3):
			sp_global_def.ERROR("do_final option requires only one or two arguments ","meridien", 1, Blockdata["myid"])
			...
			if checking_flag ==0:  sp_global_def.ERROR("do_final: refinement directory for final reconstruction does not exist ","meridien", 1, Blockdata["myid"])
			...
			checking_flag = sp_utilities.bcast_number_to_all(checking_flag, source_node = Blockdata["main_node"])
			if checking_flag ==0:
				sp_global_def.ERROR("do_final: refinement directory for final reconstruction does not exist ","meridien", 1, Blockdata["myid"])
		...
		if( options.do_final < 0):
			sp_global_def.ERROR("Incorrect iteration number in do_final  %d"%options.do_final,"meridien",1,Blockdata["myid"])
