...
def main():
    ...
    if not os.path.exists(unblur_path):
        sp_global_def.ERROR( "Unblur directory does not exist, please change the name and restart the program" )
        return
    ...
    if not file_list:
        sp_global_def.ERROR( "Input file does not exist, please change the name and restart the program" )
        return
    ...
    if not options.skip_dose_filter and not os.path.exists(options.summovie_path):
        sp_global_def.ERROR( "Path to the SumMovie executable is necessary when dose weighting is performed" )
        return
...
def run_unblur(
    ...
    if mic_list:
        ...
        if len(file_list) == 0:
            sp_global_def.ERROR( 'no files in {0} matched the file pattern:\n'.format(mic_list), 1)
    ...
    for index, inputfile in enumerate(sorted(file_list)):
        ...
        if clean_unblur:
            ...
            sp_global_def.ERROR( 'unblur error. check the logfile for more information: {0}'.format(log_name), action=0)
