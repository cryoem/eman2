...
def ang_n(tot, mode, maxrin):
	...
	"""
    if mode == "f" or mode == "F":
        return numpy.fmod(((tot - 1.0) / maxrin + 1.0) * 360.0, 360.0)
    else:
        return numpy.fmod(((tot - 1.0) / maxrin + 1.0) * 180.0, 180.0)
...
def ringwe(numr, mode="F"):
    ...
    for i in range(0, nring):
        wr[i] = (
            numr[i * 3] * dpi / float(numr[2 + i * 3]) * maxrin / float(numr[2 + i * 3])
        )
    return wr
...
def kbt(nx, npad=2):
    ...
    r = nx / 2
    v = K / 2.0 / N
    return EMAN2_cppwrap.Util.KaiserBessel(alpha, K, r, K / (2.0 * N), N)
...
def Numrinit(first_ring, last_ring, skip=1, mode="F"):
    ...
    for k in range(first_ring, last_ring + 1, skip):
        ...
        if k + skip <= last_ring and jp > ip + ip // 2:
            ip = min(MAXFFT, 2 * ip)
        if k + skip > last_ring and jp > ip + ip // 5:
            ip = min(MAXFFT, 2 * ip)
...
def align2d(
    ...
    ny = refim.get_ysize()
    if last_ring == 0:
        last_ring = nx / 2 - 2 - int(max(max(xrng), max(yrng)))
...
def parabl(Z):
    ...
    C5 = (Z[0, 0] - Z[0, 2] - Z[2, 0] + Z[2, 2]) / 4.0
    ...
    YSH = (C4 * C5 - 2.0 * C2 * C6) / DENOM - 2.0
    XSH = (C2 * C5 - 2.0 * C4 * C3) / DENOM - 2.0
...
def align2d_direct3(
    ...
    for image in input_images:
        ...
        bang = (bang // 2 - nc) * psistep + 180.0 * (bang % 2)
...
def ali_nvol(v, mask):
    ...
    while gogo:
        ...
        crit = p[1]
        if (crit - ocrit) / (crit + ocrit) / 2.0 > -1.0e-2 or niter > 10:
            gogo = False
        niter += 1
        ocrit = crit
