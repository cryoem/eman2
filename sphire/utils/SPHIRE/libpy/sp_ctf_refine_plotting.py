...
def create_particles_plot_map(
	...
	matplotlib.pyplot.setp(axis.get_xticklabels(), rotation=30, horizontalalignment="right")
	return axis
...
def create_and_save_particle_plots(
	...
	all_errors = []
	with tqdm.tqdm(total=len(refinement_results_per_micrograph), file=sys.stdout) as pbar:
		for mic_name in refinement_results_per_micrograph:
			...
			width = int(numpy.lib.function_base.np.round(numpy.lib.function_base._hist_bin_fd(numpy.lib.function_base.np.array(particle_defocus))))

			width = max(10, width)
			n, _, _ = axis.hist(particle_defocus, bins=width, facecolor='green', alpha=0.75)
			matplotlib.pyplot.axvline(old_defocus, color='k', linestyle='dashed', linewidth=1)
			matplotlib.pyplot.text(old_defocus, max(n) / 2, "Old defocus", rotation=90,
		...
		out_img_name = matplotlib.os.path.join(
			matplotlib.os.path.dirname(matplotlib.os.path.dirname(path_output_img)), "error_cdf.png"
		)
		matplotlib.pyplot.hist(
			all_errors,
			bins=numpy.lib.function_base.np.arange(0, numpy.lib.function_base.np.max(all_errors) - 0.001, 0.001),
			normed=True,
			cumulative=True,
			label="CDF",
			histtype="step",
