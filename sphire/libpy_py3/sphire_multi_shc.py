...
def orient_params(params, refparams, indexes=None, symmetry_class=None):
    ...
    pass  # IMPORTIMPORTIMPORT from pixel_error import angle_diff, angle_diff_sym
    ...
    if (symmetry_class.sym[0] == "c" and symmetry_class.nsym > 1):
        ...
        phi = sparx_pixel_error.angle_diff_sym([params[j] for j in tindexes], [refparams[j] for j in tindexes],
                                               symmetry_class.nsym)
        out = copy.deepcopy(params)
        for j in range(n):
            out[j][0] = (out[j][0] + phi) % divic
        # mirror checking
        psi_diff = sparx_pixel_error.angle_diff([out[j][2] for j in tindexes], [refparams[j][2] for j in tindexes])
        if (abs(psi_diff - 180.0) < 90.0):
            for j in range(n):
                # apply mirror
                out[j][2] = (out[j][2] + 180.0) % 360.0
    elif (symmetry_class.sym[0] == "c"):
        t1, t2, t3 = sparx_utilities.rotation_between_anglesets([params[j] for j in tindexes],
                                                                [refparams[j] for j in tindexes])
        out = sparx_fundamentals.rotate_params([params[i][:3] for i in range(n)], [-t3, -t2, -t1])
...
def find_common_subset(projs, target_threshold=2.0, minimal_subset_size=3, symmetry_class=None):
    ...
    pass  # IMPORTIMPORTIMPORT from utilities import getvec, getfvec, getang3, lacos, angles_to_normals
        ...
        for i in subset:
            ...
            for k in range(sc - 1):
                for l in range(k + 1, sc):
                    ...
                    dmin = 180.0
                    for q in neisym: dmin = min(dmin, sparx_utilities.getang3(q, outp[k][i]))
                    qt += dmin
    ...
    while (True):
        ...
        if (symmetry_class.sym[0] == "c"):  # c including cn symmetry
            ...
            for i in range(sc - 1, -1, -1):
                tv = sparx_utilities.angles_to_normals(projs[i])
                    ...
                    if (symmetry_class.nsym > 1):
                        ...
                        for k in subset:
                            ...
                            for l in range(symmetry_class.nsym):
                                u1, u2, u3 = sparx_utilities.getfvec(out[k][0] + l * divic, out[k][1])
                                qt = sparx_utilities.lacos(tv[k][0] * u1 + tv[k][1] * u2 + tv[k][2] * u3)
                                mind = min(mind, qt)
                        ...
                        # for k in xrange(n):
                        for k in subset:
                            u1, u2, u3 = sparx_utilities.getfvec(out[k][0], out[k][1])
                            avg_diff_per_image[k] += sparx_utilities.lacos(
            ...
            outp = copy.deepcopy(projs)
            for l in range(1, sc):
                psi_diff = sparx_pixel_error.angle_diff([outp[l][j][2] for j in subset],
                                                        [outp[0][j][2] for j in subset])
...
def ali3d_multishc(stack, ref_vol, ali3d_options, symmetry_class, mpi_comm=None, log=None, number_of_runs=2):
    pass  # IMPORTIMPORTIMPORT from alignment    import Numrinit, prepare_refrings, proj_ali_incore_local, shc
    pass  # IMPORTIMPORTIMPORT from utilities    import model_circle, get_input_from_string, get_params_proj, set_params_proj
    pass  # IMPORTIMPORTIMPORT from utilities    import write_text_row
    pass  # IMPORTIMPORTIMPORT from utilities    import wrap_mpi_gatherv, wrap_mpi_bcast, wrap_mpi_send, wrap_mpi_recv, wrap_mpi_split
    ...
    pass  # IMPORTIMPORTIMPORT from global_def   import Util, ERROR
    ...
    if number_of_proc < number_of_runs:
        sparx_global_def.ERROR("number_of_proc < number_of_runs", "VIPER1", 1, myid)
    ...
    mpi_subcomm = sparx_utilities.wrap_mpi_split(mpi_comm, number_of_runs)
    ...
    if mpi_subrank == 0:
        mpi_subroots = sparx_utilities.wrap_mpi_gatherv([myid], 0, mpi_comm)
    else:
        mpi_subroots = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_comm)
    mpi_subroots = sparx_utilities.wrap_mpi_bcast(mpi_subroots, main_node, mpi_comm)

    xrng = sparx_utilities.get_input_from_string(xr)
    if yr == "-1":
        yrng = xrng
    else:
        yrng = sparx_utilities.get_input_from_string(yr)
    ...
    image_start, image_end = sparx_applications.MPI_start_end(total_nima, mpi_subsize, mpi_subrank)
    ...
    for N_step in range(lstp):  # At this point there is just one value here, it cannot loop.
        ...
        while not terminate:
            ...
            refrings = sparx_alignment.prepare_refrings(volft, kb, nx, -1.0, reference_angles, "", numr,
                                                        MPI=mpi_subcomm)
            del volft, kb
            # =========================================================================
            ...
            if orient_and_shuffle:
                ...
                vecs = [[refrings[lr].get_attr("n1"), refrings[lr].get_attr("n2"), refrings[lr].get_attr("n3")] for lr
                        in range(len(refrings))]
                for im in range(nima):
                    #  For testing purposes make sure that directions did not change
                    t1, t2, t3, t4, t5 = sparx_utilities.get_params_proj(data[im])
                    pass  # IMPORTIMPORTIMPORT from utilities import nearest_fang
                    iqa = sparx_utilities.nearest_fang(vecs, t1,
                                                       t2)  # Here I could use more sophisticated distance for symmetries
                    # if myid == 0 :
            ...
            while True:
                ...
                if mpi_subrank == 0:
                    ...
                    proj_ids = None
                proj_ids = sparx_utilities.wrap_mpi_bcast(proj_ids, 0, mpi_subcomm)
                if len(proj_ids) == 0:
                    break
                if mpi_subrank < len(proj_ids):
                    # -------- alignment
                    im = proj_ids[mpi_subrank]
                    peak, pixel_error, checked_refs, iref = sparx_alignment.shc(data[im], refrings, [[1.0, 1.0]], numr,
                                                                                xrng[N_step], yrng[N_step],
                                                                                step[N_step], sym="nomirror")
                    # -------- gather results to root
                    vector_assigned_refs = sparx_utilities.wrap_mpi_gatherv([iref], 0, mpi_subcomm)
                    vector_previousmax = sparx_utilities.wrap_mpi_gatherv([data[im].get_attr("previousmax")], 0,
                                                                          mpi_subcomm)
                    vector_xformprojs = sparx_utilities.wrap_mpi_gatherv([data[im].get_attr("xform.projection")], 0,
                                                                         mpi_subcomm)
                    vector_pixel_error = sparx_utilities.wrap_mpi_gatherv([pixel_error], 0, mpi_subcomm)
                    vector_checked_ref = sparx_utilities.wrap_mpi_gatherv([checked_refs], 0, mpi_subcomm)
                else:
                    # -------- no projection assigned, send to root empty lists
                    vector_assigned_refs = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_subcomm)
                    vector_previousmax = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_subcomm)
                    vector_xformprojs = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_subcomm)
                    vector_pixel_error = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_subcomm)
                    vector_checked_ref = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_subcomm)
                # -------- merge results
                if mpi_subrank == 0:
                    used_refs = set()
                    ...
                    used_refs = None
                # ------- broadcast results
                used_refs = sparx_utilities.wrap_mpi_bcast(used_refs, 0, mpi_subcomm)
                vector_previousmax = sparx_utilities.wrap_mpi_bcast(vector_previousmax, 0, mpi_subcomm)
                vector_xformprojs = sparx_utilities.wrap_mpi_bcast(vector_xformprojs, 0, mpi_subcomm)
            ...
            if mpi_subrank == 0:
                ...
                total_checked_refs = sparx_utilities.wrap_mpi_gatherv([number_of_checked_refs], main_node, mpi_comm)
            else:
                all_pixer = sparx_utilities.wrap_mpi_gatherv([], 0, mpi_comm)
                total_checked_refs = sparx_utilities.wrap_mpi_gatherv([], main_node, mpi_comm)
            if myid == main_node:
                ...
                orient_and_shuffle = (percent_of_pixerr_below_one > doga) and (afterGAcounter < 0)
                afterGAcounter -= 1
                ## if total_iter%3 == 0:  orient_and_shuffle = True
                ## else:   orient_and_shuffle = False
                # terminate          = ( percent_of_pixerr_below_one > 0.9 )  #
                log.add("=================================================")
                log.add("Percent of positions with pixel error below 1.0 = ", (int(percent_of_pixerr_below_one * 100)),
                        "%", "   Mutations: ", orient_and_shuffle)
            orient_and_shuffle = sparx_utilities.wrap_mpi_bcast(orient_and_shuffle, 0, mpi_comm)
            # =========================================================================
            ...
            if center == -1 and symmetry_class.sym[0] == 'c':
                ...
                cs[0], cs[1], cs[2], dummy, dummy = sparx_utilities.estimate_3D_center_MPI(data[image_start:image_end],
                                                                                           total_nima, mpi_subrank,
                                                                                           mpi_subsize, 0,
                                                                                           mpi_comm=mpi_subcomm)  # estimate_3D_center_MPI(data, number_of_runs*total_nima, myid, number_of_proc, main_node, mpi_comm=mpi_comm)
                if myid == main_node:
            ...
            if orient_and_shuffle:  # DO orient
                params = []
                for im in data:
                    phi, theta, psi, sx, sy = sparx_utilities.get_params_proj(im)
                ...
                params_0 = sparx_utilities.wrap_mpi_bcast(params, mpi_subroots[0], mpi_comm)
                if (mpi_subrank == 0) and (myid != 0):
                    ...
                    subset, avg_diff_per_image, params = find_common_subset([params_0, params], delta[N_step] * 1.5,
                                                                            len(params) / 3, symmetry_class)
                    params = params[1]
                    #   neither is needed
                ...
                params = sparx_utilities.wrap_mpi_bcast(params, 0, mpi_subcomm)
                ...
                for i in range(nima):  sparx_utilities.set_params_proj(data[i], params[i])
                ...
                if mpi_subrank == 0:
                    L2 = vol.cmp("dot", vol, dict(negative=0, mask=sparx_utilities.model_circle(last_ring, nx, nx, nx)))
                ...
                if myid == 0:
                    ...
                    for sr in mpi_subroots:
                        if sr == myid:
                            ...
                            all_L2s.append(sparx_utilities.wrap_mpi_recv(sr, mpi_comm))
                            all_params.append(sparx_utilities.wrap_mpi_recv(sr, mpi_comm))
                else:
                    if mpi_subrank == 0:
                        sparx_utilities.wrap_mpi_send(L2, 0, mpi_comm)
                        sparx_utilities.wrap_mpi_send(params, 0, mpi_comm)
                ...
                if myid == 0:
                    ...
                    if (all_L2s[0] < GA[number_of_runs - 1][0]):
                        ...
                        q1, q2, q3, q4 = sparx_statistics.table_stat([GA[i][0] for i in range(number_of_runs)])
                        # Terminate if variation of L2 norms less than (L2threshold*100)% of their average
                        crit = numpy.sqrt(max(q2, 0.0)) / q1
                        for i in range(number_of_runs):
                        ...
                        if (Iter < max_iter) and (firstcheck and crit):
                            ...
                            pass  # IMPORTIMPORTIMPORT import numpy.random
                            all_params = [[[numpy.random.random() * 360.0, numpy.random.random() * 180.0,
                                            numpy.random.random() * 360.0, 0.0, 0.0] \
                    ...
                    if not terminate and noimprovement == 0 and noreseeding:
                        ...
                        pass  # IMPORTIMPORTIMPORT from utilities import nearest_many_full_k_projangles, angles_to_normals
                        ...
                        for ip in range(0, 2 * (len(ipl) / 2) + len(ipl) % 2, 2):
                            ...
                            keepset = sparx_utilities.nearest_many_full_k_projangles(
                                sparx_utilities.angles_to_normals(GA[ipl[ip]][1]), [GA[ipl[ip]][1][itmp]],
                                howmany=total_nima / 2, sym_class=symmetry_class)[0]
                        ...
                        for i in range(keepset):
                            ...
                            pass  # IMPORTIMPORTIMPORT import numpy.random
                            all_params[-1 - i] = [[numpy.random.random() * 360.0, numpy.random.random() * 180.0,
                                                   numpy.random.random() * 360.0, 0.0, 0.0] \
                    ...
                    if myid == 0:
                        for i in range(number_of_runs):
                            ...
                            if sr == myid:
                                ...
                                sparx_utilities.wrap_mpi_send(all_params[i], sr, mpi_comm)
                    else:
                        if mpi_subrank == 0:
                            params = sparx_utilities.wrap_mpi_recv(0, mpi_comm)

                    params = sparx_utilities.wrap_mpi_bcast(params, 0, mpi_subcomm)
                    for i in range(nima):
                        sparx_utilities.set_params_proj(data[i], params[i])
                    """Multiline Comment5"""
                    # =========================================================================
                    #
                    mpi.mpi_barrier(mpi_comm)
            ...
            if not terminate:
                ...
                if len(ali3d_options.moon_elimination) > 0:
                    pass  # IMPORTIMPORTIMPORT from utilities import eliminate_moons
                    vol = sparx_utilities.eliminate_moons(vol, ali3d_options.moon_elimination)
...
def ali3d_multishc_2(stack, ref_vol, ali3d_options, symmetry_class, mpi_comm=None, log=None):
    pass  # IMPORTIMPORTIMPORT from alignment       import Numrinit, prepare_refrings, proj_ali_incore_local, shc
    pass  # IMPORTIMPORTIMPORT from utilities       import model_circle, get_input_from_string, get_params_proj, wrap_mpi_gatherv, wrap_mpi_bcast, wrap_mpi_split
    ...
    symmetry_class = sparx_fundamentals.symclass(sym)
    ...
    if myid == main_node:
        ...
        L2 = qvol.cmp("dot", qvol, dict(negative=0, mask=sparx_utilities.model_circle(last_ring, nx, nx, nx)))
    ...
    if myid == main_node:
        ...
        L2 = ref_vol.cmp("dot", ref_vol, dict(negative=0, mask=sparx_utilities.model_circle(last_ring, nx, nx, nx)))
    ...
    for N_step in range(lstp):
        ...
        while Iter < max_iter and terminate == 0:
            ...
            refrings = sparx_alignment.prepare_refrings(volft, kb, nx, -1.0, reference_angles, "", numr, MPI=mpi_comm)
            del volft, kb
            # =========================================================================
            ...
            for im in range(nima):
                ...
                peak, pixer[im], checked_refs, iref = sparx_alignment.shc(data[im], refrings, [[1.0, 1.0]], numr,
                                                                          xrng[N_step], yrng[N_step], step[N_step],
                                                                          sym="nomirror")  # cannot use 'an' here
            ...
            if center == -1 and sym[0] == 'c':
                pass  # IMPORTIMPORTIMPORT from utilities      import estimate_3D_center_MPI, rotate_3D_shift
                cs[0], cs[1], cs[2], dummy, dummy = sparx_utilities.estimate_3D_center_MPI(data, total_nima, myid,
                                                                                           number_of_proc, main_node,
                                                                                           mpi_comm=mpi_comm)
                if myid == main_node:
                    msg = " Average center x = %10.3f        Center y = %10.3f        Center z = %10.3f\n" % (
                    cs[0], cs[1], cs[2])
    ...
    par_r = sparx_utilities.wrap_mpi_gatherv(par_r, 0, mpi_comm)
...
def multi_shc(all_projs, subset, runs_count, ali3d_options, mpi_comm, log=None, ref_vol=None):
    pass  # IMPORTIMPORTIMPORT from applications import MPI_start_end
    pass  # IMPORTIMPORTIMPORT from utilities import set_params_proj, wrap_mpi_bcast, write_text_row, drop_image, write_text_file
    pass  # IMPORTIMPORTIMPORT from utilities import bcast_EMData_to_all, bcast_list_to_all, bcast_number_to_all
    pass  # IMPORTIMPORTIMPORT from fundamentals import symclass
    ...
    if (mpi_size < runs_count):  sparx_global_def.ERROR("multi_shc", "mpi_size < runs_count", 1, mpi_rank)
    ...
    symmetry_class = sparx_fundamentals.symclass(ali3d_options.sym)
    ...
    if mpi_rank == 0:
        ...
        if (len(prms) < len(subset)):
            ...
            for i in subset:
                ...
                sparx_utilities.set_params_proj(all_projs[i], prms[i] + [0., 0.])
                # all_projs[i].set_attr("stable", 0)
                all_projs[i].set_attr("previousmax", -1.e23)
                projections.append(all_projs[i])
        del prms

    error = sparx_utilities.bcast_number_to_all(error, source_node=0)
    if (error == 1): sparx_global_def.ERROR("multi_shc", "Angular step too large, decrease delta", 1, mpi_rank)

    ###from sys import exit
    # if mpi_rank == 0:   print "  NEW   ",mpi_rank
    ###exit()
    projections = sparx_utilities.wrap_mpi_bcast(projections, 0, mpi_comm)
    ...
    out_params = ali3d_multishc(projections, ref_vol, ali3d_options, symmetry_class, mpi_comm=mpi_comm, log=log,
    ...
    if mpi_rank == 0:
        ...
        for i in range(n_projs):
            sparx_utilities.set_params_proj(projections[i], out_params[i])
        sparx_utilities.write_text_row(out_params, log.prefix + "refparams2.txt")
        """Multiline Comment13"""
    """Multiline Comment14"""
    ...
    if mpi_rank == 0:
        ...
        L2 = ref_vol.cmp("dot", ref_vol,
                         dict(negative=0, mask=sparx_utilities.model_circle(ali3d_options.ou, nx, nx, nx)))
    ...
    if (mpi_size > n_projs):
        ...
        mpi_subrank = mpi.mpi_comm_rank(mpi_subcomm)
        if (mpi_rank < n_projs):
            out_params, out_vol, previousmax, out_r = ali3d_multishc_2(projections, ref_vol, ali3d_options,
                                                                       symmetry_class, mpi_comm=mpi_subcomm, log=log)
        else:
            out_params = None
            out_vol = None
        mpi.mpi_barrier(mpi_comm)
    else:
        out_params, out_vol, previousmax, out_r = ali3d_multishc_2(projections, ref_vol, ali3d_options, symmetry_class,
                                                                   mpi_comm=mpi_comm, log=log)

    if mpi_rank == 0:
        sparx_utilities.write_text_file(previousmax, log.prefix + "previousmax.txt")
        sparx_utilities.write_text_row(out_params, log.prefix + "params.txt")
        sparx_utilities.drop_image(out_vol, log.prefix + "volf.hdf")
...
def mirror_and_reduce_dsym(params, indexes, symmetry_class):
    ...
    pass  # IMPORTIMPORTIMPORT from utilities import getang3
    pass  # IMPORTIMPORTIMPORT from pixel_error import angle_diff
    ...
    for i in range(1, sc):
        psi_diff = sparx_pixel_error.angle_diff([params[i][j][2] for j in indexes], [params[0][j][2] for j in indexes])
        #  adjust psi if necessary
        if (abs(psi_diff - 180.0) < 90.0):
            for j in range(ns):
                # apply mirror
        ...
        for j in indexes:
            ...
            dmin = 180.0
            for q in neisym: dmin = min(dmin, sparx_utilities.getang3(q, params[0][j]))
            per1 += dmin
...
def do_volume(data, options, iter, mpi_comm):
    ...
    if (type(data) == list):
        if CTF:
            vol = sparx_reconstruction.recons3d_4nn_ctf_MPI(myid, data, snr, symmetry=sym, npad=npad, mpi_comm=mpi_comm)
        else:
            vol = sparx_reconstruction.recons3d_4nn_MPI(myid, data, symmetry=sym, snr=snr, npad=npad, mpi_comm=mpi_comm)
    else:
        vol = data
    ...
    if myid == 0:
        ...
        if (options.mask3D == None):
            ...
            if (type(options.mask3D) == bytes):
                mask3D = sparx_utilities.get_im(options.mask3D)
            else:
                mask3D = (options.mask3D).copy()
            nxm = mask3D.get_xsize()
            if (nx != nxm):
                pass  # IMPORTIMPORTIMPORT from fundamentals import rot_shift3D
                mask3D = EMAN2_cppwrap.Util.window(sparx_fundamentals.rot_shift3D(mask3D, scale=float(nx) / float(nxm)),
                                                   nx, nx, nx)
                nxm = mask3D.get_xsize()
                assert (nx == nxm)
        ...
        if (options.pwreference):
            ...
            rt = sparx_utilities.read_text_file(options.pwreference)
            ...
            if (type(options.fl) == list):
                vol = sparx_fundamentals.fft(sparx_filter.filt_table(sparx_filter.filt_table(vol, options.fl), ro))
            else:
                vol = sparx_fundamentals.fft(
                    sparx_filter.filt_table(sparx_filter.filt_tanl(vol, options.fl, options.aa), ro))
        else:
            if (type(options.fl) == list):
                vol = sparx_filter.filt_table(vol, options.fl)
            else:
                vol = sparx_filter.filt_tanl(vol, options.fl, options.aa)
        stat = EMAN2_cppwrap.Util.infomask(vol, mask3D, False)
        vol -= stat[0]
        EMAN2_cppwrap.Util.mul_scalar(vol, 1.0 / stat[1])
        vol = sparx_morphology.threshold(vol)
