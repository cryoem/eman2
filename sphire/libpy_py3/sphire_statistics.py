...
def add_ave_varf_MPI(myid, data, mask=None, mode="a", CTF=False, ctf_2_sum=None, ali_params="xform.align2d",
                     main_node=0, comm=-1):
    """
    ...
    if CTF:
        ...
        for i in range(n):
            if mode == "a":
                ...
                if mask:
                    ima = sparx_fundamentals.fft(EMAN2_cppwrap.Util.muln_img(data[i], mask))
                else:
                    ima = sparx_fundamentals.fft(data[i])
...
def sum_oe(data, mode="a", CTF=False, ctf_2_sum=None, ctf_eo_sum=False, return_params=False):
    ...
    if CTF:
        ...
        if data[0].get_attr_default('ctf_applied', 1) == 1:  sparx_global_def.ERROR("data cannot be ctf-applied",
                                                                                    "sum_oe", 1)
        if ctf_2_sum:
            get_ctf2 = False
        else:
...
def ave_series(data, pave=True, mask=None):
    ...
    if pave:  EMAN2_cppwrap.Util.mul_scalar(ave, old_div(1.0, float(n)))
    return ave
...
def varf3d_MPI(prjlist, ssnr_text_file=None, mask2D=None, reference_structure=None, ou=-1, rw=1.0, npad=1, CTF=False,
    ...
    if myid == 0:
        [ssnr1, vol_ssnr1] = sparx_reconstruction.recons3d_nn_SSNR_MPI(myid, prjlist, mask2D, rw, npad, sign, sym, CTF,
                                                                       mpi_comm=mpi_comm)
    else:
        sparx_reconstruction.recons3d_nn_SSNR_MPI(myid, prjlist, mask2D, rw, npad, sign, sym, CTF, mpi_comm=mpi_comm)

    nx = prjlist[0].get_xsize()
    if ou == -1:
        radius = int(old_div(nx, 2)) - 2
    else:
        radius = int(ou)
    if (reference_structure == None):
        if CTF:
            snr = 1.0  # e20
            if myid == 0:
                reference_structure = sparx_reconstruction.recons3d_4nn_ctf_MPI(myid, prjlist, snr, sign, sym,
                                                                                mpi_comm=mpi_comm)
            else:
            ...
            snr = 1.0
            if myid == 0:
                reference_structure = sparx_reconstruction.recons3d_4nn_MPI(myid, prjlist, sym, snr=snr,
                                                                            mpi_comm=mpi_comm)
            else:
    ...
    del volft
    if myid == 0:
        [ssnr2, vol_ssnr2] = sparx_reconstruction.recons3d_nn_SSNR_MPI(myid, re_prjlist, mask2D, rw, npad, sign, sym,
                                                                       CTF, mpi_comm=mpi_comm)
    else:
        sparx_reconstruction.recons3d_nn_SSNR_MPI(myid, re_prjlist, mask2D, rw, npad, sign, sym, CTF, mpi_comm=mpi_comm)
    del re_prjlist
    ...
    if myid == 0 and ssnr_text_file != None:
        ...
        outf.close()
    if myid == 0:
        vol_ssnr1 = EMAN2_cppwrap.Util.subn_img(EMAN2_cppwrap.Util.pack_complex_to_real(vol_ssnr1),
                                                EMAN2_cppwrap.Util.pack_complex_to_real(vol_ssnr2))
        del vol_ssnr2
    ...
    # return  threshold_to_minval( Util.subn_img(Util.pack_complex_to_real(vol_ssnr1), Util.pack_complex_to_real(vol_ssnr2)), 1.0)
        ...
        return sparx_utilities.model_blank(2, 2, 2)
...
def fsc_mask(img1, img2, mask=None, w=1.0, filename=None):
    ...
    ny = img1.get_ysize()
    nz = img1.get_zsize()
    if (mask == None):  mask = sparx_utilities.model_circle(old_div(nx, 2), nx, ny, nz)
    ...
    for i in range(myid, lp, number_of_proc):
        ...
        if i > 0:
            ...
            do = EMAN2_cppwrap.Util.infomask(
                sparx_morphology.square_root(sparx_morphology.threshold(EMAN2_cppwrap.Util.muln_img(tmp1, tmp2))), m,
                True)[0]
        ...
        if (myid == main_node):
            for k in range(number_of_proc):
                if (k != main_node):
                    ...
                    tag_node = k + 1001
                    dis = mpi.mpi_recv(2, mpi.MPI_FLOAT, k, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL,
                                       mpi.MPI_COMM_WORLD)
...
def k_means_match_clusters_asg_new(asg1, asg2, T=0):
    ...
    for k1 in range(K):
        for k2 in range(K):
            MAT[k1][k2] = EMAN2_cppwrap.Util.k_means_cont_table(asg1[k1], asg2[k2], dummy, asg1[k1].size, asg2[k2].size,
                                                                0)
            if MAT[k1][k2] <= T:
                MAT[k1][k2] = 0
    for i in range(K):
...
def k_means_stab_bbenum(PART, T=10, nguesses=5, J=50, max_branching=40, stmult=0.25, branchfunc=2, LIM=-1,
                        doMPI_init=False, njobs=-1, do_mpi=False, K=-1, cdim=[], nstart=-1, nstop=-1, top_Matches=[]):
    ...
    if np > 2:
        MATCH = k_means_match_bbenum(PART, T=T, J=J, max_branching=max_branching, stmult=stmult, branchfunc=branchfunc,
                                     LIM=LIM, nguesses=nguesses, DoMPI=False)
...
def k_means_match_bbenum(PART, T=10, J=1, max_branching=40, stmult=0.25, nguesses=5, branchfunc=2, LIM=-1,
                         DoMPI_init=False, Njobs=-1, DoMPI=False, K=-1, np=-1, c_dim=[], N_start=-1, N_stop=-1,
                         topMatches=[]):
    ...
    # output=Util.branchMPIpy(ar_newParts,ar_class_dim,np,K,T,ar_levels,K,nguesses,N_stop-N_start+1, array(firstmatches, 'int32'))
        ...
        output = EMAN2_cppwrap.Util.bb_enumerateMPI(ar_argParts, ar_class_dim, np, K, T, nguesses, LARGEST_CLASS, J,
                                                    max_branching, stmult, branchfunc, LIM)
