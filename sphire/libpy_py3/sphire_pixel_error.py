...
def pixel_error_2D(ali_params1, ali_params2, r=1.0):
    ...
    return old_div((numpy.sin(old_div(numpy.radians(ali_params1[0] - ali_params2[0]) , 2)) * (2 * r + 1)) ** 2 , 2)\
           + (ali_params1[1] - ali_params2[1]) ** 2 + (ali_params1[2] - ali_params2[2]) ** 2
...
def align_diff_params(ali_params1, ali_params2):
    ...
    for i in range(nima):
        ...
        if abs(mirror1 - mirror2) == mirror:
            ...
            alpha12, sx12, sy12, mirror12 = sparx_utilities.combine_params2(alpha1, sx1, sy1, int(mirror1), alphai, 0.0,
                                                                            0.0, 0)
            if mirror1 == 0:
                sxi += sx2 - sx12
            else:
...
def multi_align_stability(ali_params, mir_stab_thld=0.0, grp_err_thld=10000.0, err_thld=1.732, print_individual=False,
                          d=64):
    def sqerr(a):
        n = len(a)
        avg = sum(a)
        ...
        for i in range(N):
            ...
            for j in range(L):
                if int(ali_params[j][i * 4 + 3]) == 0:
                    ...
                    sx[j] = args_list[j * 3 + 1] + ali_params[j][i * 4 + 1] * cosa[j] + ali_params[j][i * 4 + 2] * sina[
                        j]
                    sy[j] = args_list[j * 3 + 2] - ali_params[j][i * 4 + 1] * sina[j] + ali_params[j][i * 4 + 2] * cosa[
                        j]
                else:
                    sum_cosa += numpy.cos(numpy.radians(-args_list[j * 3] + ali_params[j][i * 4]))
                    sum_sina += numpy.sin(numpy.radians(-args_list[j * 3] + ali_params[j][i * 4]))
                    sx[j] = -args_list[j * 3 + 1] + ali_params[j][i * 4 + 1] * cosa[j] - ali_params[j][i * 4 + 2] * \
                            sina[j]
                    sy[j] = args_list[j * 3 + 2] + ali_params[j][i * 4 + 1] * sina[j] + ali_params[j][i * 4 + 2] * cosa[
                        j]
            sqrtP = numpy.sqrt(sum_cosa ** 2 + sum_sina ** 2)
            sqr_pixel_error[i] = max(0.0, old_div(d * d , 4.) * (1 - old_div(sqrtP , L)) + sqerr(sx) + sqerr(sy))
            # Get ave transform params
            H = EMAN2_cppwrap.Transform({"type": "2D"})
            H.set_matrix(
                [old_div(sum_cosa, sqrtP), old_div(sum_sina, sqrtP), 0.0, old_div(sum(sx), L), old_div(-sum_sina, sqrtP), old_div(sum_cosa, sqrtP), 0.0,
                 old_div(sum(sy), L), 0.0, 0.0, 1.0, 0.0])
            dd = H.get_params("2D")
            #  We are using here mirror of the LAST SET.
            H = EMAN2_cppwrap.Transform({"type": "2D", "alpha": dd["alpha"], "tx": dd["tx"], "ty": dd["ty"],
                                         "mirror": int(ali_params[L - 1][i * 4 + 3]), "scale": 1.0})
            dd = H.get_params("2D")
