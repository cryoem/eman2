...
def prepare_volume(volume_path, mask=None, resolution=None, pixel_size=None):
	...
	if mask:
		vol = vol * mask
	vol = prep_vol(vol, interpolation_method=1)
	return vol
...
def read_volume(
	...
	if mask_vol:
		mask_vol = prep_vol(mask_vol, interpolation_method=1)
...
def write_statistics(output_stats_path, mic_stats):
	...
	footer = (
		...
		"{6: ^10.5f} {7: ^10.5f} {8: ^10.5f} {9: ^10.5f} {10: ^10.5f}".format(
			"AVERAGE",
			np.mean(diff_mean_list),
			np.mean(diff_abs_mean_list),
			np.mean(diff_std_list),
			np.mean(diff_abs_std_list),
			np.mean(diff_p25_list),
			np.mean(diff_p50_list),
			np.mean(diff_p75_list),
			np.mean(diff_min_list),
			np.mean(diff_max_list),
			np.mean(mean_error_list),
		)
	)
