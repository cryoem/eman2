...
def get_timestamp( file_format=False ):
	...
	"""
	if file_format:
		return time.time.strftime( "%Y-%m-%d_%H-%M-%S", time.time.localtime() )
	else:
		return time.time.strftime( "%Y-%m-%d %H:%M:%S", time.time.localtime() )
...
pass#IMPORTIMPORTIMPORT from EMAN2  import Util, EMData, EMUtil, Transform
pass#IMPORTIMPORTIMPORT from EMAN2_meta import DATESTAMP

pass#IMPORTIMPORTIMPORT import sp_utilities as util

pass#IMPORTIMPORTIMPORT from random import seed

pass#IMPORTIMPORTIMPORT import mpi  # NOTE: put this import _after_ the EMAN2 imports
