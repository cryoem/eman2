...
def ali2d_single_iter(data, numr, wr, cs, tavg, cnx, cny, \
	...
	"""
	pass#IMPORTIMPORTIMPORT from sp_utilities import combine_params2, inverse_transform2, get_params2D, set_params2D
	pass#IMPORTIMPORTIMPORT from sp_alignment import ormq, ornq
	...
	if random_method == "SCF":
		pass#IMPORTIMPORTIMPORT from sp_fundamentals import fft, scf
		pass#IMPORTIMPORTIMPORT from sp_alignment import multalign2d_scf
		frotim = [sp_fundamentals.fft(tavg)]
		xrng = int(xrng+0.5)
		yrng = int(yrng+0.5)
		cimage = EMAN2_cppwrap.Util.Polar2Dm(sp_fundamentals.scf(tavg), cnx, cny, numr, mode)
	...
	for im in range(len(data)):
		...
		if( random_method == "PCP"):
			...
			nx = ima.get_xsize()
			ny = ima.get_ysize()
			alpha, sx, sy, mirror, dummy = sp_utilities.get_params2D(data[im], ali_params)
			alpha, sx, sy, dummy         = sp_utilities.combine_params2(alpha, sx, sy, mirror, 0.0, -cs[0], -cs[1], 0)
		...
		if random_method == "SHC":
			...
			if(data[im].get_attr("previousmax")<olo[5]):
				...
				[alphan, sxn, syn, mn] = sp_utilities.combine_params2(0.0, -sxi, -syi, 0, angt, sxst, syst, mirrort)
				...
				sp_utilities.set_params2D(data[im], [alpha, sx, sy, mirror, 1.0], ali_params)
				nope += 1
				mn = 0
				sxn = 0.0
				syn = 0.0
			...
			dummy,sxs,sys, dummy = sp_utilities.inverse_transform2(-angt,sx+sxst,sy+syst)
			sp_utilities.set_params2D(data[im][0][0], [angt, sxs, sys, mirrort, 1.0], ali_params)
		else:
...
def crit2d(args, data):
	...
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import rtshgkb
...
def ormq_fast(dimage, crefim, xrng, yrng, step, numr, mode, delta = 0.0):
	...
	#MULTILINEMULTILINEMULTILINE 6
	#MULTILINEMULTILINEMULTILINE 6
	if( peak < -1.0e20): sp_global_def.ERROR("ormq_fast","failed, most likely due to search ranges",1)
...
def prepref(data, maskfile, cnx, cny, numr, mode, maxrangex, maxrangey, step):
	...
	for im in range(nima):
		...
		alphai, sxi, syi, dummy      = sp_utilities.combine_params2(0.0, sx, sy, 0, -alpha, 0,0, 0)
		#  introduce constraints on parameters to accomodate use of cs centering
		sxi = min(max(sxi,-mashi),mashi)
		syi = min(max(syi,-mashi),mashi)	
...
def prepare_refrings( volft, kb, nz = -1, delta = 2.0, ref_a = "P", sym = "c1", numr = None, MPI=False, \
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities    import even_angles, getfvec
	...
	if(type(ref_a) is types.ListType):
		...
		if initial_theta and initial_phi :
			ref_angles = sp_utilities.even_angles(delta, theta1 = initial_theta, phi1 = initial_phi, symmetry=sym, method = ref_a, phiEqpsi = phiEqpsi)
		else:
	...
	if(nz <1):  sp_global_def.ERROR("Data size has to be given (nz)", "prepare_refrings", 1, myid)
	...
	if kbx is None:
		...
		for i in range(ref_start, ref_end):
			prjref = sp_projection.prgs(volft, kb, [ref_angles[i][0], ref_angles[i][1], ref_angles[i][2], 0.0, 0.0], kbx, kby)
			cimage = EMAN2_cppwrap.Util.Polar2Dm(prjref, cnx, cny, numr, mode)  # currently set to quadratic....
			EMAN2_cppwrap.Util.Normalize_ring(cimage, numr, 0 )
			EMAN2_cppwrap.Util.Frngs(cimage, numr)
			EMAN2_cppwrap.Util.Applyws(cimage, numr, wr_four)
...
def fine_2D_refinement(data, br, mask, tavg, group = -1):
	...
	pass#IMPORTIMPORTIMPORT from sp_fundamentals 	import rtshgkb, prepg
	...
	for im in range(len(data)):
		...
		params = [alpha, sx, sy]
		outparams =  sp_utilities.amoeba(params, weights, crit2d, 1.e-4, 1.e-4, 500, stuff)
		del stuff[3]
		del stuff[3]
		# set parameters to the header
		data[im].set_attr_dict({'alpha':outparams[0][0], 'sx':outparams[0][1], 'sy':outparams[0][2],'mirror': mirror})
		# update the average
		temp = sp_fundamentals.rtshgkb(ddata, outparams[0][0], outparams[0][1], outparams[0][2], kb)
		if  mirror: temp.process_inplace("xform.mirror",{"axis":'x'})
...
def align2d_scf(image, refim, xrng=-1, yrng=-1, ou = -1):
	...
	pass#IMPORTIMPORTIMPORT from sp_alignment import Numrinit, ringwe, ornq
	# center in SPIDER convention
	cnx = nx//2+1
	cny = ny//2+1
	#precalculate rings
	...
	alpha2, sxs, sys, mirr, peak2 = ornq(sp_fundamentals.mirror(sci), crefim, [0.0], [0.0], 1.0, "H", numr, cnx, cny)
	...
	ccf1 = EMAN2_cppwrap.Util.window(sp_fundamentals.ccf(sp_fundamentals.rot_shift2D(image, alpha, 0.0, 0.0, mirr), frotim),nrx,nry)
	p1 = sp_utilities.peak_search(ccf1)
	
	ccf2 = EMAN2_cppwrap.Util.window(sp_fundamentals.ccf(sp_fundamentals.rot_shift2D(image, alpha+180.0, 0.0, 0.0, mirr), frotim),nrx,nry)
...
def multalign2d_scf(image, refrings, frotim, numr, xrng=-1, yrng=-1, ou = -1):
	...
	for iki in range(len(refrings)):
		...
		ccf2 = EMAN2_cppwrap.Util.window(sp_fundamentals.ccf(sp_fundamentals.rot_shift2D(image, alpha+180.0, 0.0, 0.0, mirr), frotim[iki]), nrx, nry)
		p2 = sp_utilities.peak_search(ccf2)
		#print p1
		#print p2
...
def align2d_direct3(input_images, refim, xrng=1, yrng=1, psimax=180, psistep=1, ou = -1, CTF = None):
	...
	for i in range(nm):
		...
		refs[2*i] = [sp_fundamentals.fft(temp), sp_fundamentals.fft(sp_fundamentals.mirror(temp))]
		temp = sp_fundamentals.rot_shift2D(refim, (i-nc)*psistep+180.0)*mask
		refs[2*i+1] = [sp_fundamentals.fft(temp), sp_fundamentals.fft(sp_fundamentals.mirror(temp))]
	del temp

	results = []
	mir = 0
	for image in input_images:
		if CTF:  ims = sp_filter.filt_ctf(sp_fundamentals.fft(image), image.get_attr("ctf"))
		else:    ims = sp_fundamentals.fft(image)
		ama = -1.e23
		bang = 0.
		bsx = 0.
		bsy = 0.
		...
		bang, bsx, bsy, _ = sp_utilities.inverse_transform2(bang, (1 - 2*mir)*bsx, bsy, mir)
...
def search_range(n, radius, shift, range, location = ""):
	...
	if( ql < 0 or qe < 0 ):
		sp_global_def.ERROR("Shift of particle too large, results may be incorrect:  %4d   %3d   %f  %f  %f  %f  %f"%(n, cn, radius, shift, range, ql, qe),"search_range  "+location,0)
		ql = max(ql,0)
		qe = max(qe,0)
