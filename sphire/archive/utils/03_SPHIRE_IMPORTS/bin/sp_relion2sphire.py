...
def main():
	...
	if len(args) != 2:
		...
		sp_global_def.ERROR( "Missing paths to input STAR file and output directory. Please see usage information above" )
		return
	...
	if (not os.path.exists(file_path_relion_star)):
		sp_global_def.ERROR( "Input RELION STAR file \'"+file_path_relion_star+"\' is not found." )
		return

	if (os.path.exists(dir_path_work)):
		sp_global_def.ERROR( "Output directory \'"+dir_path_work+"\' already exists. Please delete it or use a different output directory" )
		return
	
	if dir_path_relion_project is not None:
		if not os.path.exists(dir_path_relion_project):
			sp_global_def.ERROR( "Specified RELION project directory \'"+dir_path_relion_project+"\' does not exist." )
			return
		...
		# Process item list and data entries 
			...
			if str_line.find('_rln') != -1:
				...
				if relion_key in list(relion_dict.keys()):
					...
					sp_global_def.sxprint(relion_dict[relion_key][idx_title] % (relion_dict[relion_key][idx_col], relion_key))
			...
			# Then, read the data entries
				...
				if i_relion_particle == 0:
					...
					for category_key in list(relion_category_dict.keys()):
						for key in relion_category_dict[category_key][idx_required_key_list]:
							if relion_dict[key][idx_col] < 0:
								sp_global_def.sxprint('#     %s entry for %s is not found' % (key, relion_category_dict[category_key][idx_relion_process]))
						...
						if relion_category_dict[category_key][idx_is_category_found] == True:
							for key in relion_category_dict[category_key][idx_denpended_key_list]:
								if relion_category_dict[key][idx_is_category_found] == False:
									sp_global_def.sxprint('#     %s required for %s is not found' % (relion_category_dict[key][idx_relion_process], relion_category_dict[category_key][idx_relion_process]))
									relion_category_dict[category_key][idx_is_category_found] = False
					
					if relion_category_dict['mic'][idx_is_category_found] == False:
						sp_global_def.sxprint('# ')
						sp_global_def.sxprint('# ERROR!!! Input STAR file must contain all entries for %s as the minimum requirement. Aborting execution ...' % (relion_category_dict['mic'][idx_relion_process]))
						is_success = False
						break;
					...
					for category_key in list(relion_category_dict.keys()):
						if relion_category_dict[category_key][idx_is_category_found] == True:
							sp_global_def.sxprint('# ')
							sp_global_def.sxprint('# Parameters associated with %s will be extracted.' % (relion_category_dict[category_key][idx_relion_process]))
							if category_key == 'helical':
								...
								sp_global_def.sxprint('#     NOTE: %s is under development!!!' % (relion_category_dict['helical'][idx_relion_process]))
								sp_global_def.sxprint('#           Only the information necessary for SPHIRE helical reconstruction development will be stored in the image header.')
							...
							sp_global_def.sxprint('# ')
							sp_global_def.sxprint('# WARNING!!! %s cannot be extracted!!! Some of required paramters are missing (see above).' % (relion_category_dict[category_key][idx_relion_process]))
					sp_global_def.sxprint('# ')
					
					if is_enable_create_stack and not relion_category_dict['window'][idx_is_category_found]:
						sp_global_def.sxprint('# WARNING!!! Input STAR file must contain at least all entries for %s and %s to create a particle stack!!! No stack will be created.' % (relion_category_dict['mic'][idx_relion_process], relion_category_dict['window'][idx_relion_process]))
						sp_global_def.sxprint('# ')
						is_enable_create_stack = False
				
				if i_relion_particle % 1000 == 0:
					sp_global_def.sxprint('# Processing RELION entries from %7d to %7d ...' % (i_relion_particle, i_relion_particle + 1000 - 1))
				...
				if relion_category_dict['ctf'][idx_is_category_found] == True:
					...
					sphire_const_ac_phase_shift = sp_morphology.ampcont2angle(sphire_cter_entry[idx_cter_const_ac])  # must pass amplitude contrast in [%]
					sphire_total_phase_shift = sphire_cter_entry[idx_cter_phase_shift] + sphire_const_ac_phase_shift
					sphire_cter_entry[idx_cter_total_ac] = sp_morphology.angle2ampcont(sphire_total_phase_shift)
				...
				if is_enable_create_stack:
					...
					if not os.path.exists(relion_local_stack_path):
						sp_global_def.sxprint('# WARNING!!! Image name %s specified in STAR file is not found from the current directory. Skipping STAR file entry %d...' % (relion_local_stack_path, i_relion_particle))
					else:
						sphire_header = {}
						...
						if relion_category_dict['ctf'][idx_is_category_found] or options.negative_stain:
							...
							sphire_header['ctf'] = sp_utilities.generate_ctf(sphire_cter_entry_list)
				...
				sp_global_def.sxprint('# An empty line is detected after data entries. Breaking the loop...')
				break;
	
	if is_found_section == False:
		sp_global_def.sxprint('# ERROR!!! Specified --star_section (%s) is not found!!!' % (str_relion_start_section))
		sp_global_def.sxprint('#          Please check section name in STAR file.')
		is_success = False
	elif is_found_loop == False:
		sp_global_def.sxprint('# ERROR!!! loop_ line after specified --star_section (%s) is not found!!!' % (str_relion_start_section))
	...
	if is_success:
		...
		sp_global_def.sxprint('# ')
		sp_global_def.sxprint('# Detected RELION column counts                     := {} '.format(i_relion_item_col))
		sp_global_def.sxprint('# Detected RELION entry counts                      := {} '.format(i_relion_particle))
		sp_global_def.sxprint('# Processed SPHIRE mircrograph directory counts     := {} '.format(sphire_micrographs_dict_dirname_counts))
		sp_global_def.sxprint('# Processed SPHIRE mircrograph entry total counts   := {} '.format(sphire_mircrograph_entry_total_counts))
		sp_global_def.sxprint('# Processed SPHIRE CTER directory counts            := {} '.format(sphire_cter_dict_dirname_counts))
		sp_global_def.sxprint('# Processed SPHIRE CTER entry total counts          := {} '.format(sphire_cter_entry_total_counts))
		sp_global_def.sxprint('# Processed SPHIRE coorinates directory counts      := {} '.format(sphire_coordinates_dict_dirname_counts))
		sp_global_def.sxprint('# Processed SPHIRE coorinates entry total counts    := {} '.format(sphire_coordinates_entry_total_counts))
		sp_global_def.sxprint('# Processed SPHIRE 3D projection directory counts   := {} '.format(sphire_proj3d_dict_dirname_counts))
		sp_global_def.sxprint('# Processed SPHIRE 3D projection entry total counts := {} '.format(sphire_proj3d_entry_total_counts))
		sp_global_def.sxprint('# Processed SPHIRE chunk directory counts           := {} '.format(sphire_chunk_dict_dirname_counts))
		sp_global_def.sxprint('# Processed SPHIRE chunk ID total counts            := {} '.format(sphrie_chunk_id_total_counts))

		if is_enable_create_stack:
			sp_global_def.sxprint('# Processed SPHIRE header directory counts          := {} '.format(sphire_header_dict_dirname_counts))
			sp_global_def.sxprint('# Processed SPHIRE header total counts              := {} '.format(sphire_header_total_counts))
		
		sp_global_def.sxprint('# Processed SPHIRE stack CTF entry counts           := {} '.format(i_sphire_stack_ctf))
		sp_global_def.sxprint('# Processed SPHIRE stack coorinates entry counts    := {} '.format(i_sphire_stack_coordinates))
		sp_global_def.sxprint('# Processed SPHIRE stack 3D Proj. entry counts      := {} '.format(i_sphire_stack_proj3d))
		sp_global_def.sxprint('# Image counts added to SPHIRE stack                := {} '.format(i_sphire_stack_particle_img))
		...
		if is_enable_create_stack:
			if i_sphire_stack_particle_img < i_relion_particle:
				sp_global_def.sxprint('# WARNING!!! The number of particles in generated stack (%d) is different from the number of entries in input RELION STAR file (%d)!!!' % (i_relion_particle, i_sphire_stack_particle_img))
				sp_global_def.sxprint('#            Please check if there are all images specified by _rlnImageName in STAR file')
			else:
		...
		for micrograph_dirname in sorted(sphire_cter_dict):
			...
			for micrograph_basename in sorted(sphire_cter_dict[micrograph_dirname]):
				...
				sphire_cter_stats[idx_cter_def], sd, _,_ = sp_statistics.table_stat(sphire_cter_table[idx_cter_def])
				...
				sphire_cter_stats[idx_cter_total_ac], sd, _,_ = sp_statistics.table_stat(sphire_cter_table[idx_cter_total_ac])
				sphire_cter_stats[idx_cter_sd_total_ac] = numpy.sqrt(max(0.0,sd))
				
				sphire_cter_stats[idx_cter_astig_amp] , sd, _,_ = sp_statistics.table_stat(sphire_cter_table[idx_cter_astig_amp])
				...
				sphire_cter_stats[idx_cter_astig_ang], sphire_cter_stats[idx_cter_sd_astig_ang] = sp_pixel_error.angle_ave(sphire_cter_table[idx_cter_astig_ang])
				
				sphire_cter_stats[idx_cter_max_freq], _, _, _ = sp_statistics.table_stat(sphire_cter_table[idx_cter_max_freq])
				
				sphire_cter_stats[idx_cter_reserved], _, _, _ = sp_statistics.table_stat(sphire_cter_table[idx_cter_reserved])
				
				sphire_cter_stats[idx_cter_const_ac], _, _, _ = sp_statistics.table_stat(sphire_cter_table[idx_cter_const_ac])
				
				sphire_cter_stats[idx_cter_phase_shift], _, _, _ = sp_statistics.table_stat(sphire_cter_table[idx_cter_phase_shift])
		...
		if not relion_category_dict['helical'][idx_is_category_found]:
			...
			if relion_category_dict['window'][idx_is_category_found]:
				...
				# Write rebox parameters to files (doing here to avoid repeating open/close files in loop)
				sp_global_def.sxprint('# ')
				sp_global_def.sxprint('# Particle coodinates are not found! Skipping save SPHIRE rebox files ...')
		else:
			# assert relion_category_dict['helical'][idx_is_category_found]:
			sp_global_def.sxprint('# ')
			sp_global_def.sxprint('# For helical reconstruction, SPHIRE rebox files are not supported yet ...')
		...
		if is_enable_create_stack:
			...
			sp_global_def.sxprint('# ')
			sp_global_def.sxprint('# Creating local particle stack for each micrograph...')
			
			for micrograph_dirname in sorted(sphire_header_dict):
				sp_global_def.sxprint('# ')
				sp_global_def.sxprint('# Processing {} directory ...'.format(micrograph_dirname))
				...
				for i_micrograph, micrograph_basename in enumerate(sorted(sphire_header_dict[micrograph_dirname])):
					if i_micrograph % 100 == 0:
						sp_global_def.sxprint('# Processing micrographs from {:5d} to {:5d} in {} directory...'.format(i_micrograph, i_micrograph + 100 - 1, micrograph_dirname))
					...
					for sphire_header_id, sphire_header in enumerate(sphire_header_dict[micrograph_dirname][micrograph_basename]):
						...
						img_particle = sp_utilities.get_im(relion_local_stack_path, sphire_local_particle_id)
						...
						if relion_category_dict['proj3d'][idx_is_category_found] == True:
							sp_utilities.set_params_proj(img_particle, sphire_header['xform.projection'])
				...
				e2bdb_command = 'e2bdb.py  {}  --makevstack={}'.format(dir_path_local_bdb_stacks, file_path_sphire_mic_dir_stack)
				sp_global_def.sxprint('# ')
				sp_global_def.sxprint('# Please execute the following command line to create single virtual stack by combining all local stacks in {} directory'.format(micrograph_dirname))
