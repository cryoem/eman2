...
pass#IMPORTIMPORTIMPORT from sp_global_def import sxprint, ERROR, SPARXVERSION
pass#IMPORTIMPORTIMPORT from sp_global_def import *

pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, string_found_in_file, get_latest_directory_increment_value, store_value_of_simple_vars_in_json_file
pass#IMPORTIMPORTIMPORT from sp_utilities import cmdexecute, if_error_then_all_processes_exit_program, wrap_mpi_bcast
pass#IMPORTIMPORTIMPORT from sp_utilities import read_text_row, read_text_file, write_text_file, write_text_row, getindexdata#, print_program_start_information
pass#IMPORTIMPORTIMPORT from sp_multi_shc import find_common_subset, do_volume, multi_shc
...
pass#IMPORTIMPORTIMPORT from sp_applications import cpy
pass#IMPORTIMPORTIMPORT import sp_applications

pass#IMPORTIMPORTIMPORT from shutil import copyfile
...
def calculate_list_of_independent_viper_run_indices_used_for_outlier_elimination(no_of_viper_runs_analyzed_together, 
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities import combinations_of_n_taken_by_k
	...
	number_of_additional_combinations_for_this_viper_iteration = sp_utilities.combinations_of_n_taken_by_k(no_of_viper_runs_analyzed_together - 1,
	...
	if (my_rank == 0):
		...
		f = open(mainoutputdir + "list_of_viper_runs_included_in_outlier_elimination.json", 'w'); pass#IMPORTIMPORTIMPORT import json
...
def identify_outliers(myid, main_node, rviper_iter, no_of_viper_runs_analyzed_together, 
	...
	if (myid == main_node):
		...
		if(os.path.exists(mainoutputdir + DIR_DELIM + "list_of_viper_runs_included_in_outlier_elimination.json")):
			...
			f = open(mainoutputdir + "list_of_viper_runs_included_in_outlier_elimination.json", 'r'); pass#IMPORTIMPORTIMPORT import json
	...
	if (myid == main_node):
		...
		if list_of_independent_viper_run_indices_used_for_outlier_elimination[0] == EMPTY_VIPER_RUN_INDICES_LIST:
			if no_of_viper_runs_analyzed_together > MAXIMUM_NO_OF_VIPER_RUNS_ANALYZED_TOGETHER:
				error_status = 1
				sp_global_def.sxprint("RVIPER reached maximum number of VIPER runs analyzed together without finding a core set of stable projections for the current RVIPER iteration (%d)! Finishing."%rviper_iter)
				cmd = "{} {}".format("mkdir ", masterdir + "MAXIMUM_NO_OF_VIPER_RUNS_ANALYZED_TOGETHER__Reached"); junk = sp_utilities.cmdexecute(cmd)
			else:
				...
				cmd = "{} {}".format("rm ", mainoutputdir + "list_of_viper_runs_included_in_outlier_elimination.json"); junk = sp_utilities.cmdexecute(cmd)
...
def plot_errors_between_any_number_of_projections(masterdir, rviper_iter, list_of_projection_indices, error_value, symc, index, runs_iter):
	...
	for i1 in list_of_projection_indices:
		p.append(sp_utilities.read_text_row(mainoutputdir + NAME_OF_RUN_DIR + "%03d"%(i1) + DIR_DELIM + "params.txt"))
	...
	matplotlib.pyplot.title("Sorted errors between projections\nError value = %s" % error_value)
	...
	matplotlib.pyplot.savefig( os.path.join(mainoutputdir, 'sorted_errors_between_projections' + "_%03d"%(runs_iter) + "_%03d"%(index) + '.png') )
...
def find_index_of_discontinuity_in_derivative(error_curve_func, list_of_projection_indices, mainoutputdir,
	...
	split_point_set = matplotlib.pyplot.np.linspace(0.8,outlier_percentile/100.0,split_point_resolution)
	...
	for split_index, split_point in enumerate(split_point_set):
		first_line_x = list(map(int, matplotlib.pyplot.np.linspace(0,split_point,resolution)*data_set_length))
		first_line_y = matplotlib.pyplot.np.array([error_curve_func[x] for x in first_line_x])
		first_line_z = matplotlib.pyplot.np.poly1d( matplotlib.pyplot.np.polyfit(first_line_x, first_line_y, degree_of_the_fitting_polynomial) )

		second_line_x = list(map(int, matplotlib.pyplot.np.linspace(split_point,1, resolution)*data_set_length))
		second_line_y = matplotlib.pyplot.np.array([error_curve_func[x-1] for x in second_line_x])
		second_line_z = matplotlib.pyplot.np.poly1d( matplotlib.pyplot.np.polyfit(second_line_x, second_line_y, degree_of_the_fitting_polynomial) )
...
def measure_for_outlier_criterion(criterion_name, masterdir, rviper_iter, list_of_viper_run_indices, symc):
	...
	for i1 in list_of_viper_run_indices:
		p.append(sp_utilities.read_text_row(mainoutputdir + NAME_OF_RUN_DIR + "%03d" % (i1) + DIR_DELIM + "params.txt"))
	subset, avg_diff_per_image, outp = sp_multi_shc.find_common_subset(p, 0, symmetry_class = symc)
...
def found_outliers(list_of_projection_indices, outlier_percentile, rviper_iter, masterdir,  bdb_stack_location,
	...
	projs = []
	for i1 in list_of_projection_indices:
		projs.append(sp_utilities.read_text_row(mainoutputdir + NAME_OF_RUN_DIR + "%03d"%(i1) + DIR_DELIM + "params.txt"))

	subset, avg_diff_per_image, rotated_params = sp_multi_shc.find_common_subset(projs, target_threshold = 0, symmetry_class = symc)
	...
	sp_global_def.sxprint("outlier_index_threshold_method: ", outlier_index_threshold_method)
	...
	sp_utilities.write_text_file(index_outliers, mainoutputdir + "this_iteration_index_outliers.txt")
	sp_utilities.write_text_file(index_keep_images, mainoutputdir + "this_iteration_index_keep_images.txt")
	...
	sp_utilities.write_text_file([dat[i].get_attr("original_image_index")  for i in index_outliers],mainoutputdir + "index_outliers.txt")
	sp_utilities.write_text_file([dat[i].get_attr("original_image_index")  for i in index_keep_images],mainoutputdir + "index_keep_images.txt")
	...
	for i1 in range(len(list_of_projection_indices)):
		...
		sp_utilities.write_text_row(rotated_params[i1], mainoutputdir + NAME_OF_RUN_DIR + "%03d"%(list_of_projection_indices[i1]) + DIR_DELIM + NAME_OF_PARAMS_FILE)  # 5 columns
...
def calculate_volumes_after_rotation_and_save_them(ali3d_options, rviper_iter, masterdir, bdb_stack_location, mpi_rank, mpi_size,
	...
	if len(list_of_independent_viper_run_indices_used_for_outlier_elimination)==0:
		sp_global_def.sxprint("Error: len(list_of_independent_viper_run_indices_used_for_outlier_elimination)==0")
		mpi.mpi_finalize()
		sys.exit()
	...
	if (mpi_size > n_projs):
		...
		if (mpi_rank < n_projs):
			...
			for idx, i in enumerate(list_of_independent_viper_run_indices_used_for_outlier_elimination):
				projdata = sp_utilities.getindexdata(bdb_stack_location + "_%03d"%(rviper_iter - 1), partids_file_name, partstack[idx], mpi_rank, mpi_subsize)
				vol = sp_multi_shc.do_volume(projdata, ali3d_options, 0, mpi_comm = mpi_subcomm)
				del projdata
				if( mpi_rank == 0):
					vol.write_image(mainoutputdir + DIR_DELIM + NAME_OF_RUN_DIR + "%03d"%(i) + DIR_DELIM + "rotated_volume.hdf")
					#line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " => "
					line = ''
					sp_global_def.sxprint(line  + "Generated rec_ref_volume_run #%01d \n"%i)
				del vol

		mpi.mpi_barrier(mpi_comm)
	else:
		for idx, i in enumerate(list_of_independent_viper_run_indices_used_for_outlier_elimination):
			projdata = sp_utilities.getindexdata(bdb_stack_location + "_%03d"%(rviper_iter - 1), partids_file_name, partstack[idx], mpi_rank, mpi_size)
	...
	if( mpi_rank == 0):
		...
		pass#IMPORTIMPORTIMPORT from sp_utilities import get_params3D, set_params3D, get_im, model_circle
		pass#IMPORTIMPORTIMPORT from sp_statistics import ave_var
		pass#IMPORTIMPORTIMPORT from sp_applications import ali_vol
		...
		# for i in xrange(no_of_viper_runs_analyzed_together):
		for idx, i in enumerate(list_of_independent_viper_run_indices_used_for_outlier_elimination):
			vls[idx] = sp_utilities.get_im(mainoutputdir + DIR_DELIM + NAME_OF_RUN_DIR + "%03d"%(i) + DIR_DELIM + "rotated_volume.hdf")
			sp_utilities.set_params3D(vls[idx],[0.,0.,0.,0.,0.,0.,0,1.0])
		asa,sas = sp_statistics.ave_var(vls)
		# do the alignment
		nx = asa.get_xsize()
		radius = nx/2 - .5
		st = EMAN2_cppwrap.Util.infomask(asa*asa, sp_utilities.model_circle(radius,nx,nx,nx), True)
		goal = st[0]
		going = True
		while(going):
			sp_utilities.set_params3D(asa,[0.,0.,0.,0.,0.,0.,0,1.0])
			# for i in xrange(no_of_viper_runs_analyzed_together):
			for idx, i in enumerate(list_of_independent_viper_run_indices_used_for_outlier_elimination):
				o = sp_applications.ali_vol(vls[idx],asa,7.0,5.,radius)  # range of angles and shifts, maybe should be adjusted
...
def get_already_processed_viper_runs(run_get_already_processed_viper_runs):
	...
	if run_get_already_processed_viper_runs:
		...
		sp_global_def.sxprint("get_already_processed_viper_runs.counter: " + str(get_already_processed_viper_runs.counter))
...
"""
	...
	parser.add_option("--function", type="string", default="ref_ali3d",         help=optparse.SUPPRESS_HELP)
	...
	##### XXXXXXXXXXXXXXXXXXXXXX option does not exist in docs XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	parser.add_option("--my_random_seed",      type="int",  default=123,  help = optparse.SUPPRESS_HELP)
	##### XXXXXXXXXXXXXXXXXXXXXX option does not exist in docs XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	parser.add_option("--run_get_already_processed_viper_runs", action="store_true", dest="run_get_already_processed_viper_runs", default=False, help = optparse.SUPPRESS_HELP)
	##### XXXXXXXXXXXXXXXXXXXXXX option does not exist in docs XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	parser.add_option("--use_latest_master_directory", action="store_true", dest="use_latest_master_directory", default=False, help = optparse.SUPPRESS_HELP)
	...
	try:
		...
		if(myid == main_node):
			if options.resample_ratio != 1.0:
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("Resample ratio {} is specified with --resample_ratio option...".format(options.resample_ratio))
			else:
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("Resample ratio is {}. The program does not resample the input volume...".format(options.resample_ratio))
	...
	if len(args) == 2:
		...
		if use_latest_master_directory:
			...
			r = matplotlib.re.compile("^master.*$"); pass#IMPORTIMPORTIMPORT import re
	...
	if mpi_size % no_of_shc_runs_analyzed_together != 0:
		sp_global_def.ERROR('Number of processes needs to be a multiple of the number of quasi-independent runs (shc) within each viper run. '
	...
	if(myid == main_node):
		...
		if( masterdir == ""):
			timestring = time.strftime("%Y_%m_%d__%H_%M_%S" + DIR_DELIM, time.localtime())
		...
		if ':' in args[0]:
			...
			if(not os.path.exists(os.path.join(masterdir,"EMAN2DB" + DIR_DELIM) ) ):
				...
				try:
					sp_applications.header(bdb_stack_location + suffix, params='original_image_index', fprint=True)
					sp_global_def.sxprint("Images were already indexed!")
				except KeyError:
					sp_global_def.sxprint("Indexing images")
					sp_applications.header(bdb_stack_location + suffix, params='original_image_index', consecutive=True)
		else:
	...
	iteration_start = sp_utilities.get_latest_directory_increment_value(masterdir, "main")

	if iteration_start_default != 0:
		iteration_start = iteration_start_default
	if (myid == main_node):
		if (iteration_start < iteration_start_default):
			sp_global_def.ERROR('Starting iteration provided is greater than last iteration performed. Quitting program' )
			error_status = 1
		if (number_of_rrr_viper_runs < iteration_start):
			sp_global_def.ERROR('Please provide number of rviper runs (--n_rv_runs) greater than number of iterations already performed.' )
			error_status = 1
	...
	for rviper_iter in range(iteration_start, number_of_rrr_viper_runs + 1):
		if(myid == main_node):
			all_projs = EMAN2_cppwrap.EMData.read_images(bdb_stack_location + "_%03d"%(rviper_iter - 1))
			sp_global_def.sxprint("XXXXXXXXXXXXXXXXX")
			sp_global_def.sxprint("Number of projections (in loop): " + str(len(all_projs)))
			sp_global_def.sxprint("XXXXXXXXXXXXXXXXX")
		...
		runs_iter = sp_utilities.get_latest_directory_increment_value(masterdir + NAME_OF_MAIN_DIR + "%03d"%rviper_iter, DIR_DELIM + NAME_OF_RUN_DIR, start_value=0) - 1
		...
		while True:
			...
			if (myid == main_node):
				...
				if run_get_already_processed_viper_runs:
					cmd = "{} {}".format("mkdir -p", masterdir + DIR_DELIM + NAME_OF_MAIN_DIR + ('%03d' + DIR_DELIM)%(rviper_iter)); junk = sp_utilities.cmdexecute(cmd)
					cmd = "{} {}".format("rm -rf", independent_run_dir); junk = sp_utilities.cmdexecute(cmd)
					cmd = "{} {}".format("cp -r", get_already_processed_viper_runs() + " " +  independent_run_dir); junk = sp_utilities.cmdexecute(cmd)

				if os.path.exists(independent_run_dir + "log.txt") and (sp_utilities.string_found_in_file("Finish VIPER2", independent_run_dir + "log.txt")):
					this_run_is_NOT_complete = 0
				else:
					this_run_is_NOT_complete = 1
					cmd = "{} {}".format("rm -rf", independent_run_dir); junk = sp_utilities.cmdexecute(cmd)
					cmd = "{} {}".format("mkdir -p", independent_run_dir); junk = sp_utilities.cmdexecute(cmd)
			...
			if this_run_is_NOT_complete:
				...
				if (myid == main_node):
					sp_utilities.store_value_of_simple_vars_in_json_file(masterdir + 'program_state_stack.json', locals(), exclude_list_of_vars=["usage"], 
						vars_that_will_show_only_size = ["subset"])
					sp_utilities.store_value_of_simple_vars_in_json_file(masterdir + 'program_state_stack.json', options.__dict__, write_or_append='a')

				out_params, out_vol, out_peaks = sp_multi_shc.multi_shc(all_projs, subset, no_of_shc_runs_analyzed_together, options,
		...
		if increment_for_current_iteration == MUST_END_PROGRAM_THIS_ITERATION:
			if (myid == main_node):
				sp_global_def.sxprint("RVIPER found a core set of stable projections for the current RVIPER iteration (%d), the maximum angle difference between corresponding projections from different VIPER volumes is less than %.2f. Finishing."%(rviper_iter, ANGLE_ERROR_THRESHOLD))
				...
				sp_global_def.sxprint('Copying average and variance from iteration %03d to output directory %s' % (rviper_iter, masterdir) )
				shutil.copyfile(iter_avg, master_avg) 
				shutil.copyfile(iter_var, master_var) 
			break
	else:
		if (myid == main_node):
			sp_global_def.sxprint("After running the last iteration (%d), RVIPER did not find a set of projections with the maximum angle difference between corresponding projections from different VIPER volumes less than %.2f Finishing."%(rviper_iter, ANGLE_ERROR_THRESHOLD))
