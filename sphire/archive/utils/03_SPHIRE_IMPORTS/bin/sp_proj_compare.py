...
pass#IMPORTIMPORTIMPORT from sp_applications import header, project3d, mref_ali2d
pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, write_header, model_circle, read_text_row, set_params2D, write_text_row, set_params_proj, compose_transform3, model_blank
pass#IMPORTIMPORTIMPORT from sp_statistics import ccc
pass#IMPORTIMPORTIMPORT from sp_fundamentals import rops_table, fft, rot_shift2D
pass#IMPORTIMPORTIMPORT from sp_projection import prep_vol, prgl
pass#IMPORTIMPORTIMPORT from math import sqrt, degrees, atan2
...
pass#IMPORTIMPORTIMPORT from sp_alignment import Numrinit, ringwe, search_range, align2d, align2d_scf, align2d_direct3
pass#IMPORTIMPORTIMPORT from sp_global_def import Util, sxprint
...
def main_proj_compare(classavgstack, reconfile, outdir, options, mode='viper', prjmethod='trilinear', 
	...
	if mode=='projmatch':
		...
		cmd += '\n'
		print_log_msg(cmd, log, verbose)
		sp_applications.project3d(reconfile, refprojstack, delta=options.delta, symmetry=options.symmetry)
		...
		if options.align=='ali2d':
			...
			print_log_msg(cmd, log, verbose)
			sp_applications.mref_ali2d(classavgstack, refprojstack, projdir, ou=options.matchrad, xrng=options.matchshift, yrng=options.matchshift)
		...
		# Diagnostic
		alignlist = sp_utilities.read_text_row(classangles)  # contain 2D alignment parameters
...
def apsh(refimgs, imgs2align, outangles=None, refanglesdoc=None, outaligndoc=None, outerradius=-1, 
	...
	for imgindex in range(numimg):
		...
		best2dparamslist = [angt, sxst, syst, mirrorfloat, bestreffloat, peakt] = EMAN2_cppwrap.Util.multiref_polar_ali_2d(
		...
		# Check for mirroring
		if mirrorflag == 1:
			tempeulers = list(sp_utilities.compose_transform3(besteulers[0],besteulers[1],besteulers[2], besteulers[3],besteulers[4],0, 1,
						0,180,0, 0,0,0, 1))
			combinedparams = list(sp_utilities.compose_transform3(tempeulers[0],tempeulers[1],tempeulers[2], tempeulers[3],tempeulers[4],0, 1, 
						0,0,-angt, 0,0,0, 1))
		else:
			combinedparams = list(sp_utilities.compose_transform3(besteulers[0],besteulers[1],besteulers[2], besteulers[3],besteulers[4],0, 1, 
						0,0,-angt, 0,0,0, 1))
...
def mode_meridien(reconfile, classavgstack, classdocs, partangles, selectdoc, maxshift, outerrad, 
	...
	for classdoc in classdoclist:  # [classdoclist[32]]:  # 
		...
		refprjreal = sp_projection.prgl(reconprep, [avg_phi_final,avg_theta_final,0,0,0], interpolation_method=1, return_real=True)
		...
		if alignopt == 'scf':
			ang_align2d, sxs, sys, mirrorflag, peak = sp_alignment.align2d_scf(classavg, refprjreal, maxshift, maxshift, ou=outerrad)
		...
		# Weird results
			...
			# Perform alignment
			ang_align2d, sxs, sys, mirrorflag, peak = sp_alignment.align2d(classavg, refprjreal, txrng, tyrng, last_ring=outerrad)  
		
		# Direct3 (angles seemed to be quantized)
		elif alignopt == 'direct3':
			[[ang_align2d, sxs, sys, mirrorflag, peak]] = sp_alignment.align2d_direct3([classavg], refprjreal, maxshift, maxshift, ou=outerrad)
		...
		# Check for mirroring
		if mirrorflag == 1:
			tempeulers = list(sp_utilities.compose_transform3(avg_phi_final,avg_theta_final,0, 0,0,0, 1,
						0,180,0, 0,0, 0,1))
			combinedparams = list(sp_utilities.compose_transform3(tempeulers[0],tempeulers[1],tempeulers[2], tempeulers[3],tempeulers[4],0, 1, 
						0,0,-ang_align2d, 0,0,0, 1))
		else:
			combinedparams = list(sp_utilities.compose_transform3(avg_phi_final,avg_theta_final,0, 0,0,0, 1,
						0,0,-ang_align2d, 0,0, 0,1))
		# compose_transform3: returns phi,theta,psi, tx,ty,tz, scale
	...
	sp_utilities.write_text_row(outangleslist, outanglesdoc)
	sp_utilities.write_text_row(outalignlist, outaligndoc)
...
def average_angles(alignlist, partdoc, selectdoc=None, init_angles=None, threshold=None, goodpartdoc=None, 
	...
	if isinstance(alignlist, str): alignlist = sp_utilities.read_text_row(outaligndoc)
...
def compare_projs(reconfile, classavgstack, inputanglesdoc, outdir, interpolation_method=1, log=None, verbose=False):
	...
	for imgnum in range(nimg1):
		...
		# Compute re-projection
		prjimg = sp_projection.prgl(reconprep, angleslist[imgnum], interpolation_method=1, return_real=False)
		...
		prjimg = sp_fundamentals.fft(sp_filter.filt_table(prjimg, table))  # match FFT amplitudes of re-projection and class average
...
def montage2(inputstack, ncol, marginwidth=0, bkgd=0, outfile=None):
	...
	if isinstance(inputstack, str): inputstack = EMAN2_cppwrap.EMData.read_images(inputstack)
