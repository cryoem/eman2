...
pass#IMPORTIMPORTIMPORT from   string import atoi,replace
...
def main():
	...
	pass#IMPORTIMPORTIMPORT from sp_applications import recons3d_n, recons3d_trl_MPI

	sp_global_def.BATCH = True
	if options.interpolation_method == "4nn":
		sp_applications.recons3d_n(prj_stack, pid_list, vol_stack, options.CTF, options.snr, 1, options.npad,\
		...
		if options.MPI is False:
			sp_global_def.ERROR( "Trilinear interpolation reconstruction has MPI version only!" )
			return
		sp_applications.recons3d_trl_MPI(prj_stack, pid_list, vol_stack, options.CTF, options.snr, 1, options.npad,\
		 options.sym, options.verbose, options.niter, options.compensate, options.target_window_size)
		
	else:
		sp_global_def.ERROR( "Wrong interpolation method. The current options are 4nn, and tril. 4nn is the default one." )
		return
