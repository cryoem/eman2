...
pass#IMPORTIMPORTIMPORT import ctypes
pass#IMPORTIMPORTIMPORT import string
pass#IMPORTIMPORTIMPORT import subprocess
pass#IMPORTIMPORTIMPORT import numpy  as np
pass#IMPORTIMPORTIMPORT import random as rnd

# EMAN2 & sparx base
pass#IMPORTIMPORTIMPORT import EMAN2 as E2  #  <----------------------- TODO: get rid of this
pass#IMPORTIMPORTIMPORT from sp_sparx import * #  <----------------------- TODO: get rid of this

pass#IMPORTIMPORTIMPORT import mpi  # NOTE: this needs to be imported AFTER the EMAN2 import

# EMAN2 & sparx general utility
pass#IMPORTIMPORTIMPORT import sp_global_def
pass#IMPORTIMPORTIMPORT import optparse
pass#IMPORTIMPORTIMPORT import sp_fundamentals
pass#IMPORTIMPORTIMPORT import sp_utilities as util

pass#IMPORTIMPORTIMPORT from sp_logger import Logger, BaseLogger_Files
pass#IMPORTIMPORTIMPORT from sp_global_def import ERROR, EMData, sxprint, SPARXVERSION

# EMAN2 & sparx specific packages
pass#IMPORTIMPORTIMPORT import sp_isac
pass#IMPORTIMPORTIMPORT import sp_alignment as align
pass#IMPORTIMPORTIMPORT import sp_applications as apps
pass#IMPORTIMPORTIMPORT import sp_statistics as stats

pass#IMPORTIMPORTIMPORT from sp_filter import filt_tanl as filter_tanl
pass#IMPORTIMPORTIMPORT from sp_pixel_error import multi_align_stability
pass#IMPORTIMPORTIMPORT from EMAN2db import db_open_dict as E2_db_open_dict
...
Blockdata["color"], Blockdata["no_of_groups"], balanced_processor_load_on_nodes = ctypes.util.get_colors_and_subsets( Blockdata["main_node"], 
...
def create_zero_group():
	...
	submyids = ctypes.util.wrap_mpi_gatherv( submyids, Blockdata["main_node"], mpi.MPI_COMM_WORLD )
	submyids = ctypes.util.wrap_mpi_bcast  ( submyids, Blockdata["main_node"], mpi.MPI_COMM_WORLD )
...
def normalize_particle_images( aligned_images, shrink_ratio, target_radius, target_dim, align_params, filament_width=-1, ignore_helical_mask=False ):
	...
	if filament_width == -1:
		if new_dim >= target_dim:
			...
			mask = ctypes.util.model_circle( new_dim//2-2, new_dim, new_dim )
	...
	for im in range( len(aligned_images) ):
		...
		if filament_width != -1:
			mask = ctypes.util.model_rotated_rectangle2D( radius_long=int( numpy.sqrt(2*current_dim**2) )//2, # long  edge of the rectangular mask
		...
		if new_dim < target_dim:
			aligned_images[im] = ctypes.util.pad( aligned_images[im], target_dim, target_dim, 1, 0.0 )
...
def iter_isac_pap(alldata, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, match_first, max_round, match_second, stab_ali, thld_err, indep_run, thld_grp, img_per_grp, generation, candidatesexist = False, random_seed=None, new = False):
	...
	if os.path.exists(ali_params_dir):  
		sp_global_def.ERROR( "Output directory %s for alignment parameters exists, please either change its name or delete it and restart the program" % ali_params_dir, myid=myid )
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	...
	d = numpy.frombuffer(numpy.core.multiarray.int_asbuffer(base_ptr, size*disp_unit), dtype = 'f4')
	d = d.reshape(orgsize)
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
...
def isac_MPI_pap(stack, refim, d, maskfile = None, ir=1, ou=-1, rs=1, xrng=0, yrng=0, step=1, maxit=30, isac_iter=10, CTF=False, snr=1.0, rand_seed=-1, color=0, comm=-1, stability=False, stab_ali=5, iter_reali=1, thld_err=1.732, FL=0.1, FH=0.3, FF=0.2, dst=90.0, method = ""):
	...
	while( (main_iter < max_iter) and (terminate == 0) ):
		...
		for j in range(numref):
			...
			cimage = EMAN2_cppwrap.Util.Polar2Dm(refi[j] , cnx, cny, numr, mode) # converting reference images to polar cordinates 
			EMAN2_cppwrap.Util.Frngs(cimage, numr) # Applying FFT on the reference images 
			EMAN2_cppwrap.Util.Applyws(cimage, numr, wr) # apply weights to FTs of rings
			refi[j] = cimage.copy()
		...
		for im in range(image_start, image_end):
			alpha, sx, sy, mirror, scale = ctypes.util.get_params2D(alldata[im])  # retrieves 2D alignment parameters
			##  TEST WHETHER PARAMETERS ARE WITHIN RANGE
			alphai, sxi, syi, scalei = ctypes.util.inverse_transform2(alpha, sx, sy)  # returns the inverse  geometric transformof the 2d rot and trans matrix
			# If shifts are outside of the permissible range, reset them
			if(abs(sxi)>mashi or abs(syi)>mashi):
				sxi = 0.0
				syi = 0.0
				ctypes.util.set_params2D(alldata[im],[0.0,0.0,0.0,0,1.0])  # set 2D alignment parameters (img, alpha, tx, ty, mirror, scale)
			# normalize
			alldata[im].process_inplace("normalize.mask", {"mask":mask, "no_sigma":0}) # subtract average under the mask
			txrng = sp_alignment.search_range(nx, ou, sxi, xrng, "ISAC2")   # Find permissible ranges for translational searches by resampling into polar coordinates
			...
			for iref in range(numref):
				[alphan, sxn, syn, mn] = \
				   ctypes.util.combine_params2(0.0, -sxi, -syi, 0, temp[iref*5+1], temp[iref*5+2], temp[iref*5+3], int(temp[iref*5+4]))  #  Combine 2D alignment parameters including mirror
		...
		for im in range(image_start, image_end):
			...
			if mn == 0:
				ctypes.util.set_params2D(alldata[im], [alphan, sxn-sx_sum[matchref], syn-sy_sum[matchref], mn, scale])  # set 2D alignment parameters (img, alpha, tx, ty, mirror, scale)
			else:
				ctypes.util.set_params2D(alldata[im], [alphan, sxn+sx_sum[matchref], syn-sy_sum[matchref], mn, scale])
		...
		for im in range(image_start, image_end):
			...
			alphan, sxn, syn, mirrorn = ctypes.util.combine_params2(alpha, sx, sy, mirror, ref_ali_params[matchref*4+0], 
		...
		if do_within_group == 1:
			...
			for im in range(nima):
				...
				ali_params = list(map(float, ali_params))
				ctypes.util.set_params2D(alldata[im], [ali_params[0], ali_params[1], ali_params[2], int(ali_params[3]), 1.0])
...
def do_generation(main_iter, generation_iter, target_nx, target_xr, target_yr, target_radius, options):
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		plist = ctypes.util.read_text_file( os.path.join(Blockdata["masterdir"], 
												  "main%03d"%main_iter,
	...
	nimastack = ctypes.util.bcast_number_to_all(nimastack, source_node = Blockdata["main_node"], mpi_comm=mpi.MPI_COMM_WORLD)
	...
	if( Blockdata["myid_on_node"] == 0 ):
		...
		for i in range(nimastack):
			bigbuffer.insert_clip( sp_utilities.get_im(Blockdata["stack_ali2d"],plist[i]),(0,0,i) )
		del plist
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		...
		if(len(good)> 0):
			...
			if( os.path.exists( os.path.join(Blockdata["masterdir"], "processed_images.txt") ) ):
				...
				lprocessed.sort()
				ctypes.util.write_text_file(lprocessed, os.path.join(Blockdata["masterdir"], "processed_images.txt" ))
			else:
				ctypes.util.write_text_file(good, os.path.join(Blockdata["masterdir"], "processed_images.txt" ))
		
		if(len(bad)> 0):
			bad.sort()
			ctypes.util.write_text_file(bad, os.path.join(Blockdata["masterdir"], "main%03d"%main_iter, \
			...
			if( (int(len(bad)*1.2) < 2*options.img_per_grp) or ( (len(good) == 0) and (generation_iter == 1) ) ):
				...
				ctypes.util.write_text_file(leftout, os.path.join(Blockdata["masterdir"], "not_processed_images.txt" ))
...
def main(args):
	...
	if(myid == main_node): 
		Blockdata["total_nima"] = EMAN2_cppwrap.EMUtil.get_image_count(Blockdata["stack"])
	else: 
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		...
		if( not os.path.exists( os.path.join(Blockdata["masterdir"], "main001", "generation000") ) ):
			...
			ctypes.util.write_text_row( [[0.0,0.0,0.0,-1] for i in range(Blockdata["total_nima"])], os.path.join(Blockdata["masterdir"], "all_parameters.txt") )
		...
		# continue ISAC from a previous run
				...
				while(keepdoing_main):
					...
					if( os.path.exists( os.path.join(Blockdata["masterdir"], "main%03d"%main_iter ) ) ):
						if( not  os.path.exists( os.path.join(Blockdata["masterdir"], "main%03d"%main_iter, "finished" ) ) ):
							sp_utilities.cmdexecute( "{} {}".format("rm -rf", os.path.join(Blockdata["masterdir"], "main%03d"%main_iter)) )
							keepdoing_main = False
					else: 
						keepdoing_main = False
			...
			if( os.path.exists(os.path.join(Blockdata["masterdir"], "finished")) ):
				sp_utilities.cmdexecute( "{} {}".format("rm -rf", os.path.join(Blockdata["masterdir"], "finished")) )

	error = sp_utilities.bcast_number_to_all(error, source_node = Blockdata["main_node"])
	if(error == 1):  
		sp_global_def.ERROR( "Good Sir or Madam, for thine restart value %d there exists no ISAC directory for thine continuation. Please doth choose a restart value of -1 to start a fresh run or 0 to continue the iteration last completed." % options.restart, myid=Blockdata["myid"])
	elif(error == 2):  
		sp_global_def.ERROR( "Cannot restart from unfinished main iteration number %d. To automatically detect the latest ISAC directory for continuation please choose a restart value of 0." % main_iter, myid=Blockdata["myid"])
	...
	if not checkitem(os.path.join(init2dir, "Finished_initial_2d_alignment.txt")):
		...
		if options.VPP:
			...
			if(myid == 0):
				...
				rpw[0] = 1.0
				ctypes.util.write_text_file(rpw,os.path.join(Blockdata["masterdir"], "rpw.txt"))
			else:  rpw = []
			...
			if(myid == 0):
				ntp = len(sp_fundamentals.rops_table(original_images[0]))
				ctypes.util.write_text_file([0.0]*ntp,os.path.join(Blockdata["masterdir"], "rpw.txt"))
		...
		if options.skip_prealignment:
			...
			if(txrm < 0):
				sp_global_def.ERROR( "Radius of the structure larger than the window data size permits %d"%(radi), myid=myid )
			if(txrm/nxrsteps>0):
				tss = ""
				txr = ""
			...
			for i in range(len(params2d)):
				alpha, sx, sy, mirror = ctypes.util.combine_params2(0, params2d[i][1],params2d[i][2], 0, -params2d[i][0], 0, 0, 0)
				sx /= shrink_ratio
				sy /= shrink_ratio
				params2d[i][0] = 0.0
				params2d[i][1] = sx
		...
		if( myid == main_node ):		
			if options.skip_prealignment:
				sp_global_def.sxprint("=========================================")
				sp_global_def.sxprint("There is no alignment step, '%s' params are set to zero for later use."%os.path.join(init2dir, "initial2Dparams.txt"))
				sp_global_def.sxprint("=========================================")
			sp_utilities.write_text_row(tmp,os.path.join(init2dir, "initial2Dparams.txt"))
		del tmp
		mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
		...
		aligned_images = EMAN2_cppwrap.EMData.read_images(Blockdata["stack"], list(range(image_start,image_end)))
		...
		for im in range(nima):
			...
			if options.filament_width != -1:
				mask = ctypes.util.model_rotated_rectangle2D( radius_long=int(numpy.sqrt(2*nx**2))//2, # use a length that will be guaranteed to cross the whole image
			...
			if options.CTF:
				aligned_images[im] = sp_filter.filt_ctf(aligned_images[im], aligned_images[im].get_attr("ctf"), binary = True)
			# for phase plate data we force images to match the rotational power spectrum (provided to us from the outside)
			elif options.VPP:
				aligned_images[im] = sp_fundamentals.fft( sp_filter.filt_table(sp_filter.filt_ctf(sp_fundamentals.fft(aligned_images[im]), aligned_images[im].get_attr("ctf"), binary = True), rpw) )
		...
		ctypes.util.gather_compacted_EMData_to_root(Blockdata["total_nima"], aligned_images, myid)
			...
			if( not checkitem(os.path.join(Blockdata["masterdir"], "main%03d"%main_iter )) ):
				...
				if( Blockdata["myid"] == 0 ):
					...
					if( os.path.exists( os.path.join(Blockdata["masterdir"], "not_processed_images.txt")) ):
						...
						ctypes.util.write_text_file( list(range(Blockdata["total_nima"])),
											 os.path.join(Blockdata["masterdir"], 
														  "main%03d"%main_iter, 
														  "generation%03d"%generation_iter, 
														  "to_process_next_%03d_%03d.txt"%(main_iter,generation_iter)) )
