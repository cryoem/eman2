...
def estimate_angle(coords_a, coords_b):
	...
	delta_x = coords_b[0] - coords_a[0]
	delta_y = coords_b[1] - coords_a[1]
	angle = -1 * numpy.arctan2(delta_y, delta_x) * 180 / numpy.pi
...
"""
	...
	while my_mpi_proc_id == main_mpi_proc:
		...
		if error_status is None and not coords_path_list:
			error_status = ( "No coordinates files are found in the directory specified by coordinates file path pattern (%s). Please check input_coordinates_pattern argument. Run %s -h for help."
							 % (os.path.dirname(coords_pattern), program_name), inspect.getframeinfo(inspect.currentframe()) )
			break
		...
		if use_real_ctf_params:
			...
			if error_status is None and not cter_entry_list:
				error_status = ( "No CTER partres entries are found in %s. Please check input_ctf_params_source argument. Run %s -h for help."
								 % (ctf_params_src, program_name), inspect.getframeinfo(inspect.currentframe()) )
				break
			...
			if len(cter_entry_list[0]) == n_idx_cter:
				...
				for old_cter_entry in cter_entry_list:
					...
					if cter_entry[idx_cter_sd_astig_ang] > options.astigmatism_error:
						sp_global_def.sxprint( "    NOTE: Astigmatism angular SD of %s (%f degree) exceeds specified limit (%f degree). Resetting astigmatism parameters to zeros..."
		...
		sp_global_def.sxprint( "Rejected by no coordinates file    : %6d" % (len(no_coords_mic_id_substr_list)) )
	...
	sp_utilities.if_error_then_all_processes_exit_program(error_status) # here three should just be thrown an exception
	...
	for mic_id_substr_idx, mic_id_substr in enumerate(valid_mic_id_substr_list):
		...
		if options.limit_ctf:
			...
			if abs_ctf_limit < 0.5:
				...
				# --------------------------------------------------------------------------------
		mic_img = sp_fundamentals.fft(sp_filter.filt_gaussh(mic_img, resample_ratio / box_size))
		...
		if coords_accepted:
			...
			for coords_id, entry in enumerate(coords_accepted):
				...
				if is_filament:
					long_axis_radius = numpy.int(numpy.sqrt(2 * box_size ** 2)) // 2
	...
	if options.limit_ctf:
		...
		if my_mpi_proc_id == main_mpi_proc:
			...
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Global summary of CTF limit absolute frequency (--limit_ctf)...")
			sp_global_def.sxprint(
	...
	if main_mpi_proc == my_mpi_proc_id:
		...
		sp_global_def.sxprint("Detected                           : %6d" % (n_global_coords_detect))
		sp_global_def.sxprint("Processed                          : %6d" % (n_global_coords_process))
		sp_global_def.sxprint(
