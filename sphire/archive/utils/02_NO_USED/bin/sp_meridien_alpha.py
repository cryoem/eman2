...
def Xali3D_direct_ccc(data, refang, shifts, ctfs = None, bckgnoise = None, kb3D = None):
	...
	for i in range(nang):
		...
		for j in range(npsi):
			...
			img_buffer = sp_helix_sphire.np.frombuffer(sp_helix_sphire.np.core.multiarray.int_asbuffer(pointer_location, size_of_one_image*disp_unit), dtype = 'f4')
			...
			for icnm in range(max(1,lenass)):  # I have to enter the loop even it there is no assignment
				...
				if( lima == 1 and procid == 0):
					...
					if(lenass>0):
						...
						#xod1.fill(1.0)
						xod2 = sp_helix_sphire.np.ndarray((Tracker["keepfirst"]),dtype='int',order="C")
