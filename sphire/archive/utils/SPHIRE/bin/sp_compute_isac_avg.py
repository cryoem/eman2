...
def compute_average(mlist, radius, CTF):
    if CTF:
        ...
        for ifreq in range(1, len(frc[0])):
            ...
            frc[1][ifreq] = 2.0 * frc[1][ifreq] / (1.0 + frc[1][ifreq])
...
def adjust_pw_to_model(image, pixel_size, roo):
    ...
    if roo is None:  # adjusted to the analytic model, See Penczek Methods Enzymol 2010
        ...
        for ifreq in range(len(rot1)):
            ...
            v = numpy.exp(c1 + c2 / (x / c3 + 1) ** 2) + numpy.exp(
                c4 - 0.5 * (((x - c5) / c6 ** 2) ** 2)
            )
            pu.append(v)
        s = sum(pu)
...
def apply_enhancement(avg, B_start, pixel_size, user_defined_Bfactor):
    if user_defined_Bfactor > 0.0:
        ...
        global_b = b * 4.0  #
    return (
        sp_filter.filt_gaussinv(sp_fundamentals.fft(avg), numpy.sqrt(2.0 / global_b)),
        global_b,
    )
...
def main():
    ...
    x_range = max(
        Tracker["constants"]["xrange"], int(1.0 / Tracker["ini_shrink"] + 0.99999)
    )
        ...
        for iavg in range(navg):
            ...
            for im in range(len(members)):
                ...
                P = sp_utilities.combine_params2(
                    ...
                    init_dict[abs_id][3],
                    parameters[abs_id][0],
                    parameters[abs_id][1] / Tracker["ini_shrink"],
                    parameters[abs_id][2] / Tracker["ini_shrink"],
                    parameters[abs_id][3],
                )
