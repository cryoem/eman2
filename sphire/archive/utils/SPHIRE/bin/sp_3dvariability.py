...
def main():
    ...
    if options.symmetrize:
        ...
        if options.VAR:  # 2D variance images have no shifts
            ...
            vol_size2 = nx ** 3 * 4.0 * 8 / 1.0e9
            vol_size1 = 2.0 * nnxo ** 3 * 4.0 * 8 / 1.0e9
            proj_size = (
                nnxo * nnyo * len(proj_list) * 4.0 * 2.0 / 1.0e9
            )  # both aveList and varList
            orig_data_size = nnxo * nnyo * 4.0 * tnumber / 1.0e9
            reduced_data_size = nx * nx * 4.0 * tnumber / 1.0e9
            ...
            for index_of_proj in range(len(all_proj)):
                ...
                if myid == heavy_load_myid and index_of_proj % 100 == 0:
                    log_main.add(
                        " ...... %6.2f%% "
                        % (index_of_proj / float(len(all_proj)) * 100.0)
                    )
            mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
            if myid == heavy_load_myid:
                log_main.add(
                    "All_proj preprocessing cost %7.2f m" % ((time.time() - ttt) / 60.0)
                )
            ...
            for i in range(len(proj_list)):
                ...
                if (myid == heavy_load_myid) and (i % 100 == 0):
                    log_main.add(
                        " ......%6.2f%%  " % (i / float(len(proj_list)) * 100.0)
                    )
            ...
            if options.ave3D:
                ...
                if myid == main_node:
                    if current_decimate != 1.0:
                        ave3D = sp_fundamentals.resample(ave3D, 1.0 / current_decimate)
        ...
        if options.var3D:
            ...
            if myid == main_node:
                if current_decimate != 1.0:
                    res = sp_fundamentals.resample(res, 1.0 / current_decimate)
                ...
                log_main.add(
                    "Var3D reconstruction took %f12.1 [m]" % ((time.time() - t6) / 60.0)
                )
                log_main.add(
                    "Total computation time %f12.1 [m]" % ((time.time() - t0) / 60.0)
                )
