            ...
            if (
                ...
                if (
                    ...
                    if Tracker["delta"] <= numpy.degrees(
                        math.atan(0.25 / Tracker["constants"]["radius"])
                    ):
                        Tracker["state"] = "FINAL"
                    else:
...
def AI_continuation(fff, anger=-1.0, shifter=-1.0, chout=False):
    ...
    if Tracker["mainiteration"] == 1:
        ...
        Tracker["fsc143"] = l01
        Tracker["large_at_Nyquist"] = (
            fff[Tracker["nxinit"] // 2] > 0.1 or fff[Tracker["nxinit"] // 2 - 1] > 0.2
        )
        Tracker["nxinit"] = min(
...
def compute_search_params(acc_trans, shifter, old_range):
    ...
    if new_range > 8.0 * step:
        new_range /= 2.0  # change 4 to 8
    if new_range > 8.0 * step:
        step = new_range / 8.0  # change 4 to 8
    ...
    if dryrun:
        ...
        if myid == Blockdata["main_node"]:
            EMAN2_cppwrap.Util.mul_scalar(
                tavg, 1.0 / float(sum(Tracker["nima_per_chunk"]))
            )
            ...
            for i in range(ngroups):
                ...
                for ism in range(0):  # 2
                    for k in range(1, nv - 1):
                        tmp2[k] = (tmp1[k - 1] + tmp1[k] + tmp1[k + 1]) / 3.0
                    for k in range(1, nv - 1):
                        tmp1[k] = tmp2[k]
                """Multiline Comment4"""
...
def get_shrink_data(
    ...
    for im in range(nima):
        ...
        if not nonorm:
            EMAN2_cppwrap.Util.mul_scalar(
                data[im], Tracker["avgvaradj"][procid] / wnorm
            )
...
def steptwo_mpi(tvol, tweight, treg, cfsc=None, regularized=True, color=0):
    ...
    if Blockdata["myid_on_node"] == 0:
        ...
        tvol = sp_morphology.cosinemask(
            tvol, Tracker["constants"]["nnxo"] // 2 - 1 - 5, 5, None
...
def ali3D_polar_ccc(
    ...
    nang = len(refang)
    npsi = int(360.0 / Tracker["delta"])
    nshifts = len(shifts)
    n_fine_shifts = 4

    nima = len(original_data)
    mask = EMAN2_cppwrap.Util.unrollmask(Tracker["nxinit"], Tracker["nxinit"])
    for j in range(Tracker["nxinit"] // 2, Tracker["nxinit"]):
        mask[0, j] = 1.0
    mask2D = sp_utilities.model_circle(
        Tracker["constants"]["radius"],
        Tracker["constants"]["nnxo"],
    ...
    n_coarse_psi = int(360.0 / coarse_delta)
    ...
    for im in range(nima):
        ...
        if Tracker["constants"]["CTF"]:
            ...
            ctf_params.apix = ctf_params.apix / (
        ...
        if (
            ...
            and (im % (max(1, nima / 5)) == 0)
            and (im > 0)
        ):
            sp_global_def.sxprint(
                "  Number of images :%7d   %5d  %5.1f"
                % (im, nima, float(im) / float(nima) * 100.0)
                + "%"
                + "   %10.1fmin" % ((time.time() - at) / 60.0)
            )
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        sp_global_def.sxprint(
            "  Finished projection matching   %10.1fmin" % ((time.time() - at) / 60.0)
        )
    # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
def ali3D_primary_polar(
    ...
    for im in range(nima):
        ...
        if not nonorm:
            EMAN2_cppwrap.Util.mul_scalar(
                dataimage, Tracker["avgvaradj"][procid] / wnorm
            )
        ...
        for j in range(Tracker["nxinit"] // 2 + 1, Tracker["nxinit"]):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        sp_global_def.sxprint(
            "  Finished sigma2   %10.1fmin" % ((time.time() - at) / 60.0)
        )
...
def ali3D_polar(
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        # write_text_row([[newpar[0][2][j][0],newpar[0][2][j][1]] for j in range(len(newpar[0][2]))],os.path.join(Tracker["directory"], "polar%1d.txt"%procid))
        sp_global_def.sxprint(
            "  Statistics finished : %10.1fmin" % ((time.time() - at) / 60.0)
        )
    return newpar, norm_per_particle
...
def ali3D_primary_local_polar(
    ...
    numberofrefs_inmem = int(
        ...
        / 4
        / ((size_of_one_image * disp_unit) / 1.0e9)
    )
    ...
    for icone in range(max_number_of_cones):
        ...
        if (
            ...
            for icnm in range(
                ...
                if (
                    (Blockdata["myid"] == Blockdata["main_node"])
                    and (lima % (max(1, nima / 5)) == 0)
                    and (lima > 0)
                ):
                    sp_global_def.sxprint(
                        "  Number of images :%7d   %5d  %5.1f"
                        % (lima, nima, float(lima) / float(nima) * 100.0)
                        + "%"
                        + "   %10.1fmin" % ((time.time() - at) / 60.0)
                    )
...
def recons3d_final(masterdir, do_final_iter_init, memory_per_node, orgstack=None):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        nnprocs = min(
            Blockdata["no_of_processes_per_group"],
            int(((memory_per_node - data_size * 1.2) / volume_size)),
        )
        sp_global_def.sxprint(
...
def recons3d_trl_struct_MPI_nosmearing(
    ...
    for im in range(len(prjlist)):
        ...
        try:
            ...
            bckgn = [1.0] * (Tracker["constants"]["nnxo"] // 2)
        if not upweighted:
...
def update_memory_estimation():
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        image_size = max(Tracker["nxinit"], Tracker["constants"]["nnxo"] * 1.0 / 2.0)
        data_size = (
            total_stack
            * 4
            * float(image_size ** 2)
        ...
        volume_size = (1.5 * 4 * (2.0 * image_size + 3.0) ** 3) / 1.0e9
        ...
        memory_per_cpu_3d = (
            data_size / Blockdata["no_of_processes_per_group"] * 1.2 + volume_size
        )
        sp_global_def.sxprint(
...
def reduce_shifts(sx, sy, img):
    ...
    if Tracker["constants"]["helical_rise"] is not None:
        try:
            ...
            rise = Tracker["constants"]["helical_rise"] / float(
                Tracker["constants"]["pixel_size"]
            )
            rise_half = rise / 2.0
...
5.  mpirun -np 64 --hostfile four_nodes.txt  sxmeridien.py vton3 --do_final=21
    ...
    if (not do_final_mode) and (not do_continuation_mode):
        ...
        if not restart_mode:  # <<<-------Fresh run
            ...
            Constants["ccfpercentage"] = options.ccfpercentage / 100.0
