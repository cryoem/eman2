...
def check_mpi_settings(log_main):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        for ifreq in range(len(fsc_refinement)):
            if fsc_refinement[ifreq] < 0.143:
                break
        res = float(ifreq) / 2.0 / float(len(fsc_refinement))
    ...
    images_per_cpu = int(
        float(Tracker["constants"]["total_stack"]) / Blockdata["nproc"]
    )
...
def do_one_way_anova_scipy(
    ...
    for i in range(K):
        ...
        summed_squared_elements_within_groups[i] = ssa_per_group ** 2 / float(
            len(replicas[i])
        )
    ...
    log_main.add(
        ...
        % (name_of_variable, round(global_mean / (float(nsamples)), 4))
    )
...
def check_3dmask(log_main):
    ...
    ###########################################################################
    Tracker["nxinit"] = Tracker["nxinit_refinement"]
    Tracker["currentres"] = float(Tracker["constants"]["fsc05"]) / float(
        Tracker["nxinit"]
    )
    ...
    while total_iter < max_iter:
        ...
        for iref in range(number_of_groups):
            if Blockdata["myid"] == Blockdata["last_node"]:
                ...
                ref_vol -= stat[0]
                if stat[1] != 0.0:
                    EMAN2_cppwrap.Util.mul_scalar(ref_vol, 1.0 / stat[1])
                ref_vol *= mask3D
            else:
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            for im in range(len(local_peaks)):
                dmatrix[im // nima][im % nima + image_start] = local_peaks[im]
        else:
            dmatrix = 0
        if Blockdata["myid"] != Blockdata["main_node"]:
...
def downsize_data_for_sorting(
        ...
        for i in range(len(Blockdata["bckgnoise"])):
            ...
            for k in range(nnx):
                if Blockdata["bckgnoise"][i][k] > 0.0:
                    temp[k] = 1.0 / numpy.sqrt(Blockdata["bckgnoise"][i][k])
            oneover.append(temp)
        del temp
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            if (
                ...
                focus3d = sp_fundamentals.resample(
                    focus3d, float(Tracker["nxinit"]) / float(focus3d_nx)
                )
        else:
    ...
    for im in range(nima):
        ...
        if not Tracker["nosmearing"] and norms:
            cimage *= Tracker["avgnorm"][chunk_id] / norms[im]  # norm correction
    ...
    if len(glist) <= Blockdata["nproc"] * 30:
        ...
        for ic in range(len(clusters)):
            if len(clusters[ic]) <= img_per_grp:
                slist.append(
                    max(1.0 - float(len(clusters[ic])) / float(img_per_grp), 0.05)
                )
            else:
                slist.append(0.05)
        ...
        for ichunk in range(3):
            ...
            while len(ulist) > 0:
                ...
                if slist[im] > random.random():
                    ...
                    if len(clusters[ic]) <= img_per_grp:
                        slist[im] = max(
                            1.0 - float(len(clusters[im])) / float(img_per_grp), 0.05
                        )
                    else:
                        slist[im] = 0.05
                    del ulist[0]
            ...
            if Blockdata["myid"] == Blockdata["main_node"]:
                for ic in range(len(clusters)):
                    ...
                    if len(clusters[ic]) <= img_per_grp:
                        slist[ic] = max(
                            1.0 - float(len(clusters[ic])) / float(img_per_grp), 0.05
                        )
                    else:
                        slist[ic] = 0.05
            else:
...
def assign_unaccounted_inverse_proportion_to_size(glist, clusters, img_per_grp):
    ...
    for ic in range(len(clusters)):
        if len(clusters[ic]) <= img_per_grp:
            slist.append(max(1.0 - float(len(clusters[ic])) / float(img_per_grp), 0.05))
        else:
            slist.append(0.05)
    nc = 0
    while len(ulist) > 0:
...
def do_boxes_two_way_comparison_mpi(
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        ratio_unaccounted = 100.0 - nb_tot_objs / float(total_data) * 100.0
        ratio_accounted = nb_tot_objs / float(total_data) * 100.0
        new_list = []
...
def do_withinbox_two_way_comparison(partition_dir, nbox, nrun, niter):
    ...
    for index_of_any in range(len(list_stable)):
        ...
        any.tolist()
        any.sort()
        score1 = float(len(any)) * 100.0 / float(len(ptp1[newindeces[index_of_any][0]]))
        score2 = float(len(any)) * 100.0 / float(len(ptp2[newindeces[index_of_any][1]]))
        score3 = (
            float(
                (
                    numpy.intersect1d(
...
def steptwo_mpi(tvol, tweight, treg, cfsc=None, regularized=True, color=0):
    ...
    if Blockdata["myid_on_node"] == 0:
        ...
        tvol = sp_morphology.cosinemask(
            tvol, Tracker["constants"]["nnxo"] // 2 - 1, 5, None
        )
        return tvol
    else:
        return None
...
def get_input_from_sparx_ref3d(log_main):  # case one
    ...
    if update_sym == 1:
        ...
        Tracker["constants"]["orientation_groups"] = max(
            4, Tracker["constants"]["orientation_groups"] // Blockdata["symclass"].nsym
        )
...
def get_input_from_datastack(log_main):  # Case three
    ...
    Tracker["shrinkage"] = float(Tracker["nxinit"]) / float(
        Tracker["constants"]["nnxo"]
    )
    Tracker["bckgnoise"] = None
    ...
    if Blockdata["no_of_groups"] == 1:
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            for i in range(len(cfsc), Tracker["constants"]["nnxo"] // 2 + 1):
                cfsc.append(0.0)
            sp_utilities.write_text_row(
...
def recons3d_trl_struct_group_MPI(
    ...
    if not nosmearing:
        ...
        nshifts = len(rshifts_shrank)
        for im in range(len(rshifts_shrank)):
            rshifts_shrank[im][0] *= float(Tracker["nxinit"]) / float(
                Tracker["constants"]["nnxo"]
            )
            rshifts_shrank[im][1] *= float(Tracker["nxinit"]) / float(
                Tracker["constants"]["nnxo"]
            )
    nnx = prjlist[0].get_xsize()
    nny = prjlist[0].get_ysize()
            ...
            for index_of_group in range(group_start, group_end):
                ...
                if Blockdata["myid_on_node"] == 0:
                    ...
                    if Tracker["nxinit"] < Tracker["constants"]["nnxo"]:
                        cfsc = cfsc[: Tracker["nxinit"]]
                        for i in range(
                            len(cfsc), Tracker["constants"]["nnxo"] // 2 + 1
                        ):
                            cfsc.append(0.0)
                    lcfsc = len(cfsc)
                    fsc05 = 0
...
def check_sorting(total_data, keepsorting, log_file):
    ...
    while n <= Tracker_main["constants"]["img_per_grp_split_rate"]:
        if (
            Tracker_main["constants"]["img_per_grp"] // 2 ** n
            > Tracker_main["constants"]["minimum_grp_size"]
        ):
            Tracker["number_of_groups"] = (2 ** n * total_data) // Tracker_main[
                "constants"
            ]["img_per_grp"]
...
def get_MGR_from_two_way_comparison(newindeces, clusters1, clusters2, N):
    ...
    for i in range(K):
        diagonal_k[i] = (sum_rows[i] + sum_cols[i]) // 2  # nk
    min_sizes = [None for i in range(K)]
    for i in range(K):
        min_sizes[i] = diagonal_k[i] ** 2 / N
    return min_sizes
...
def do_random_groups_simulation_mpi(ptp1, ptp2):
    ...
    for i1 in range(len(ptp1)):
        plist1.append(
            [nsize1, nsize1 + max(int(float(len(ptp1[i1])) / tsize * 100.0), 1)]
        )
        nsize1 += max(int(float(len(ptp1[i1])) / tsize * 100.0), 1)
    ...
    for i1 in range(len(ptp2)):
        plist2.append(
            [nsize2, nsize2 + max(int(float(len(ptp2[i1])) / tsize * 100.0), 1)]
        )
        nsize2 += max(int(float(len(ptp2[i1])) / tsize * 100.0), 1)
    ...
    for iloop in range(Nloop):
        ...
        for i in range(NT):
            ...
            tlist.append(nb_tot_objs / float(len(set(ts))) * 100.0)
            for j in range(k):
                try:
                    clist[j].append(
                        float(
