...
def fill_soft_edge_kernel_mask(kernel_mask, length, mode):
    ...
    if mode.lower() == "c":
        ...
        scipy.ndimage.morphology.numpy.exp(
            Q * (kernel_mask / float(length)) ** 2, out=kernel_mask
        )
    ...
    if dimension == 2:
        ...
        kernel_mask = (
            scipy.ndimage.morphology.numpy.sqrt(
                ((x - length) ** 2 + (y - length) ** 2) / float(edge_norm)
            )
            * cosine_falloff
        )
    elif dimension == 3:
    ...
    if Cs == 0.0:
        for ix in range(nx):
            ...
            for iy in range(nx):
                ...
                r2 = x * x + y * y
                if r2 < nr2:
                    dfa = defc - astigmamp / 2 * numpy.sin(
...
def ctflimit(nx, defocus, cs, voltage, pix):
    ...
    z1 = defocus * 10000.0
    ctfper = ctfperiod(defocus, cs, lam, 1.0 / (2 * pix))
...
def adaptive_mask(
    ...
    ny = vol.get_ysize()
    nz = vol.get_zsize()
    mc = sp_utilities.model_circle(nx // 2, nx, ny, nz) - sp_utilities.model_circle(
        nx // 3, nx, ny, nz
    )
        ...
        if stack == None:
            ...
            if my_mpi_proc_id == main_mpi_proc:
                sp_global_def.sxprint(
                    (
                        "    Processing %s ---> %6.2f%%"
                        % (img_name, (ifi - set_start) / progress_percent_step)
                    )
                )
        ...
        if len(adefocus) < 2:
            ...
            if len(reject_img_messages) > 0:
                ...
                sp_utilities.write_text_file(
                    [
                        list(range(lnsb)),
                        [float(i) / wn / pixel_size for i in range(lnsb)],
                        pwrot1,
                        crot1,
                        pwrot2,
                        crot2,
...
def ampcont2angle(A):
    ...
    if A == 100.0:
        ...
        return numpy.degrees(math.atan(A / numpy.sqrt(1.0e4 - A ** 2))) + 180.0
    else:
        return numpy.degrees(math.atan(A / numpy.sqrt(1.0e4 - A ** 2)))
...
def simpw1d_print(defocus, data):
    ...
    for i in range(len(data[0])):
        sp_global_def.sxprint(
            i, i + data[8], data[0][i], ct[i], data[1][i], data[0][i] / data[1][i]
        )
    return -sum(data[0] * ct / data[1]) / numpy.linalg.norm(ct, 2)
...
def simctf2out(dz, data):
    ...
    mm = sp_utilities.pad(
        sp_utilities.model_blank(nx // 2, nx, 1, 1.0), nx, nx, 1, 0.0, -nx // 4
    )
...
def defocusgett_vpp(
    ...
    envelope = (
        defocus_baseline_fit(
            roo, i_start, min(int(i_stop * 1.45), nx // 2 - 2), int(nr2), 2
        )
        - baseline
    )
    #  Process envelope
    qm = numpy.max(envelope[5:])
    dc = max(numpy.min(envelope[5:]), qm / 1000.0)
