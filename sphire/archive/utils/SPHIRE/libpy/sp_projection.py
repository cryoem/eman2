...
def prep_vol(vol, npad=2, interpolation_method=-1):
    ...
    if interpolation_method == -1:
        ...
        if Mx == Mz & My == Mz:
            ...
            kb = EMAN2_cppwrap.Util.KaiserBessel(alpha, K, M / 2, K / (2.0 * N), N)
            volft = vol.copy()
            volft.divkbsinh(kb)
            ...
            Nz = Mz * npad
            # support of the window
            kbx = EMAN2_cppwrap.Util.KaiserBessel(alpha, K, Mx / 2, K / (2.0 * Nx), Nx)
            kby = EMAN2_cppwrap.Util.KaiserBessel(alpha, K, My / 2, K / (2.0 * Ny), Ny)
            kbz = EMAN2_cppwrap.Util.KaiserBessel(alpha, K, Mz / 2, K / (2.0 * Nz), Nz)
            volft = vol.copy()
