...
def spruce_up(ref_data):
    ...
    volf = sp_morphology.threshold(volf)
    # Apply B-factor
    B = 1.0 / math.sqrt(2.0 * 14.0)
    volf = sp_filter.filt_gaussinv(volf, B, False)
    nx = volf.get_xsize()
    stat = EMAN2_cppwrap.Util.infomask(
        volf,
        sp_utilities.model_circle(nx // 2 - 2, nx, nx, nx)
        - sp_utilities.model_circle(nx // 2 - 6, nx, nx, nx),
        True,
    )
...
def constant(ref_data):
    ...
    stat = EMAN2_cppwrap.Util.infomask(
        ref_data[2], sp_utilities.model_circle(nx // 2 - 2, nx, nx), False
    )
    ref_data[2] -= stat[0]
        ...
        if Tracker["constants"]["mask3D"] == None:
            ...
            if nx != nxm:
                mask3D = EMAN2_cppwrap.Util.window(
                    sp_fundamentals.rot_shift3D(mask3D, scale=float(nx) / float(nxm)),
                    nx,
                    nx,
                    nx,
                )
            ...
            if Tracker["constants"]["sausage"]:
                ...
                for i in range(len(ro)):
                    ro[i] *= (
                        ...
                        * math.exp(
                            -(
                                ((i / y / Tracker["constants"]["pixel_size"]) - 0.10)
                                / 0.025
                            )
                            ** 2
                        )
                        ...
                        * math.exp(
                            -(
                                ((i / y / Tracker["constants"]["pixel_size"]) - 0.215)
                                / 0.025
                            )
                            ** 2
                        )
            ...
            if lx != nx:
                if lx < nx:
                    mask = EMAN2_cppwrap.Util.window(
                        sp_fundamentals.rot_shift3D(mask, scale=float(lx) / float(nx)),
                        lx,
                        lx,
                        lx,
                    )
