...
def common_line_in3D(phiA, thetaA, phiB, thetaB):
	...
	ny = numpy.sin(th1) * numpy.cos(th2) * numpy.cos(ph1) * numpy.cos(ph2) - numpy.cos(th1) * numpy.sin(th2) * numpy.cos(
		ph1
	) * numpy.cos(ph2)
...
def drop_png_image(im, trg):
	...
	if trg[-4:] != ".png":
		sp_global_def.ERROR("destination name must be png extension", "drop_png_image", 1)
	...
	if isinstance(trg, str):
		...
		sp_global_def.ERROR("destination is not a file name", "drop_png_image", 1)
...
def hist_func(args, data):
	...
	return EMAN2_cppwrap.Util.hist_comp_freq(
		args[0],
		args[1],
		data[4],
		data[5],
...
def info(image, mask=None, Comment=""):
	...
	if e.is_complex():
		...
		if e.is_shuffled():
			s = " (shuffled)"
		if e.is_fftodd():
			sp_global_def.sxprint("Complex odd image%s: nx = %i, ny = %i, nz = %i" % (s, nx, ny, nz))
		else:
			sp_global_def.sxprint("Complex even image%s: nx = %i, ny = %i, nz = %i" % (s, nx, ny, nz))

	else:
		sp_global_def.sxprint("Real image: nx = %i, ny = %i, nz = %i" % (nx, ny, nz))

	sp_global_def.sxprint("avg = %g, std dev = %g, min = %g, max = %g" % (mean, sigma, imin, imax))
...
def print_row(input, ix=0, iz=0):
	...
	ny = image.get_ysize()
	nz = image.get_zsize()
	sp_global_def.sxprint("(z = %d slice, x = %d row)" % (iz, ix))
	line = []
	for iy in range(ny):
		...
		if (iy + 1) % 10 == 0:
			line.append("\n")
	line.append("\n")
	sp_global_def.sxprint("".join(line))
...
def print_col(input, iy=0, iz=0):
	...
	ny = image.get_ysize()
	nz = image.get_zsize()
	sp_global_def.sxprint("(z = %d slice, y = %d col)" % (iz, iy))
	line = []
	for ix in range(nx):
		line.append("%12.5g  " % (image.get_value_at(ix, iy, iz)))
		if (ix + 1) % 10 == 0:
...
def print_slice(input, iz=0):
	...
	ny = image.get_ysize()
	nz = image.get_zsize()
	sp_global_def.sxprint("(z = %d slice)" % (iz))
	line = []
	for iy in range(ny):
		line.append("Row ")
		line.append("%4i " % iy)
...
def print_image_slice_3d(input, num=0, direction="z"):
	...
	if direction == "x":
		# print "xxxxx"
		ix = num
		sp_global_def.sxprint("(x = %d slice)" % (ix))
		line = []
		...
		# print "yyy"
		iy = num
		sp_global_def.sxprint("(y = %d slice)" % (iy))
		line = []
		for iz in range(nz - 1, -1, -1):
			line.append("Z ")
			line.append("%4i " % iz)
...
def reconstitute_mask(
	...
	if nima > 1:
		...
		if save_file_on_disk:
			image_in_reconstituted_mask = EMAN2_cppwrap.Util.reconstitute_image_mask(
				image_mask_applied, new_mask
			)
			image_in_reconstituted_mask.write_image(saved_file_name)
		else:
			return EMAN2_cppwrap.Util.reconstitute_image_mask(image_mask_applied, new_mask)
...
def finish_time(start_time):
	...
	sp_global_def.sxprint(("Running time is"), finish_time - start_time)
	return finish_time
...
def gather_compacted_EMData_to_root_with_header_info_for_each_image(
	...
	ref_start, ref_end = sp_applications.MPI_start_end(
		...
		if sender_id == myid:
			...
			for i in range(ref_start + 1, ref_end):
				data = numpy.concatenate(
					[data, EMAN2_cppwrap.EMNumPy.em2numpy(list_of_em_objects_for_myid_process[i])]
				)
...
def gather_compacted_EMData_to_root( number_of_all_em_objects_distributed_across_processes, list_of_em_objects_for_myid_process, mpi_rank, comm=-1 ):
	...
	data = matplotlib.pyplot.np.array( data )
	...
	for sender_id in range(1, mpi_size):
		...
		# transfer/receive: root process blocks until data from process w/ rank <sender_id> has been received
			...
			ref_start, ref_end = sp_applications.MPI_start_end( number_of_all_em_objects_distributed_across_processes, mpi_size, sender_id )
...
def gather_EMData(data, number_of_proc, myid, main_node):
	...
	if myid == main_node:
			...
			if i == main_node:
				...
				for k in range(l):
					...
					mem_len = mpi.mpi_recv(
						1, mpi.MPI_INT, i, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD
					)
		...
		for k in range(l):
			...
			mpi.mpi_send(
				len(mem), 1, mpi.MPI_INT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD
			)
			mpi.mpi_send(
				mem,
				len(mem),
...
def nearestk_projangles(projangles, whichone=0, howmany=1, sym="c1"):
	...
	if sym == "c1":
		...
		sp_global_def.sxprint("  ERROR:  symmetry not supported  ", sym)
		assignments = []
...
def nearest_full_k_projangles(reference_ang, angles, howmany=1, sym_class=None):
	...
	if sym_class == None or sym_class.sym[:2] == "c1":
		...
		assignments = EMAN2_cppwrap.Util.nearest_fang_select(
			reference_normals, ref[0], ref[1], ref[2], howmany
		)
	else:
		ancordir = angles_to_normals(sym_class.symmetry_neighbors([angles[:3]]))
		assignments = EMAN2_cppwrap.Util.nearest_fang_sym(
			ancordir, reference_normals, len(ancordir), howmany
		)
...
def cone_ang(projangles, phi, tht, ant, symmetry="c1"):
	...
	if symmetry == "c1":
		...
		sp_global_def.sxprint("Symmetry not supported ", symmetry)
	return la
...
def angles_between_anglesets(angleset1, angleset2, indexes=None):
	...
	angleset1 = sp_fundamentals.rotate_params(
...
def group_proj_by_phitheta_slow(
	...
	for grp in range(N / img_per_grp):
		...
		if N_remain >= nref1 * L:
			...
			for i in range(nref):
				if len(assignments[i]) > max_group_size:
					...
					max_group = i
			sp_global_def.sxprint(max_group_size, max_group, previous_group, end=" ")
		...
		if max_group != previous_group:
			...
			sp_global_def.sxprint(Nn, True, end=" ")
		else:
			sp_global_def.sxprint(Nn, False, end=" ")
		...
		sp_global_def.sxprint(t2 - t1, t3 - t2, t22 - t21, t3 - t22, t4 - t3)
	...
	sp_global_def.sxprint("Total time used = ", time.time() - t0)
...
def get_shrink_data_huang(
	...
	for im in range(nima):
		...
		data[im] = sp_fundamentals.resample(data[im], shrinkage)
...
def print_program_start_information():
	...
	if myid == 0:
		sp_global_def.sxprint("Location: " + os.getcwd())
...
def program_state_stack(
	...
	program_state_stack.start_executing = mpi.mpi_bcast(
		program_state_stack.start_executing, 1, mpi.MPI_INT, 0, mpi.MPI_COMM_WORLD
	)
...
def get_sorting_params(Tracker, data):
	...
	for inode in range(nproc):
		...
		if myid == main_node:
			image_start, image_end = sp_applications.MPI_start_end(ndata, nproc, inode)
...
def get_resolution_mrk01(vol, radi, nnxo, fscoutputdir, mask_option):
	...
	if type(radi) == int:
		...
		mask = radi
	nfsc = sp_statistics.fsc(vol[0] * mask, vol[1] * mask, 1.0, os.path.join(fscoutputdir, "fsc.txt"))
	currentres = -1.0
	ns = len(nfsc[1])
	...
	"""Multiline Comment34"""
		...
		#MULTILINEMULTILINEMULTILINE 34
				#MULTILINEMULTILINEMULTILINE 34
						...
						#MULTILINEMULTILINEMULTILINE 34
		#MULTILINEMULTILINEMULTILINE 34
	lowpass, falloff = sp_filter.fit_tanh1(nfsc, 0.01)
...
def margin_of_error(P, size_of_this_sampling):
	...
	return numpy.sqrt(P * (1.0 - P) / size_of_this_sampling)
...
def create_random_list(Tracker):
	...
	for irandom in range(Tracker["constants"]["indep_runs"]):
		ll = copy.copy.copy(Tracker["this_data_list"])
		random.shuffle(ll)
...
def recons_mref(Tracker):
	...
	for igrp in range(number_of_groups):
		...
		if myid == main_node:
			sp_global_def.sxprint("reconstructed %3d" % igrp)
		ref_list.append(vol)
...
def apply_low_pass_filter(refvol, Tracker):
	...
	for iref in range(len(refvol)):
		refvol[iref] = sp_filter.filt_tanl(refvol[iref], Tracker["low_pass_filter"], 0.1)
	return refvol
...
def get_complementary_elements(total_list, sub_data_list):
	if len(total_list) < len(sub_data_list):
		sp_global_def.sxprint("Wrong input list!")
		return []
	else:
		sub_data_dict = {}
		complementary = []
...
def set_filter_parameters_from_adjusted_fsc(n1, n2, Tracker):
	...
	for i in range(1, ns - 1):
		if adjusted_fsc[1][i] < fsc_cutoff:
			...
			break
	lowpass, falloff = sp_filter.fit_tanh1(adjusted_fsc, 0.01)
...
def get_stable_members_from_two_runs(SORT3D_rootdirs, ad_hoc_number, log_main):
	...
	sort3d_rootdir_list = string.split(SORT3D_rootdirs)
	dict1 = []
	maximum_elements = 0
...
def two_way_comparison_single(partition_A, partition_B, Tracker):
	...
	ptp[0] = numpy32_A
	ptp[1] = numpy32_B
	newindexes, list_stable, nb_tot_objs = sp_statistics.k_means_match_clusters_asg_new(
		ptp[0], ptp[1]
	)
	if myid == main_node:
		log_main.add(
...
def Kmeans_exhaustive_run(ref_vol_list, Tracker):
	...
	while (
		...
		empty_group, res_classes, data_list = sp_applications.ali3d_mref_Kmeans_MPI(
		...
		if empty_group == 1:
			...
			if Tracker["constants"]["mask3D"]:
				mask3D = sp_morphology.get_shrink_3dmask(
			...
			for igrp in range(len(new_class)):
				...
				volref, fsc_kmref = sp_reconstruction.rec3D_two_chunks_MPI(
					data,
					snr,
...
def print_a_line_with_timestamp(string_to_be_printed):
	line = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + " =>"
	sp_global_def.sxprint((line, string_to_be_printed))
...
def split_a_group(workdir, list_of_a_group, Tracker):
	...
	for index in range(2):
		...
		vol = sp_filter.filt_tanl(vol, Tracker["constants"]["low_pass_filter"], 0.1)
		ref_list.append(vol)
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	### EQ-Kmeans
	outdir = os.path.join(workdir, "EQ-Kmeans")
	sp_applications.mref_ali3d_EQ_Kmeans(ref_list, outdir, this_particle_text_file, Tracker)
...
def angular_distribution_old(params_file, output_folder, prefix, method, pixel_size, delta, symmetry, box_size,
	...
	if error:
		...
		if do_print:
			sp_global_def.sxprint('Most values are valid')
	...
	def to_cartesian(angles):
		...
		column_phi = 0
		column_theta = 1
		angles_radians = matplotlib.numpy.radians(angles)
		cartesian_array = matplotlib.numpy.empty((angles_radians.shape[0], 3))

		sinus = matplotlib.numpy.sin(angles_radians[:, column_theta])
		cartesian_array[:, COLUMN_X] = matplotlib.numpy.cos(angles_radians[:, column_phi]) * sinus
		cartesian_array[:, COLUMN_Y] = matplotlib.numpy.sin(angles_radians[:, column_phi]) * sinus
		cartesian_array[:, COLUMN_Z] = matplotlib.numpy.cos(angles_radians[:, column_theta])
	...
	def markus(args, data, data_cart, sym_class):
		if do_print:
			sp_global_def.sxprint('Create reference angles')
		...
		if do_print:
			sp_global_def.sxprint('Find nearest neighbours')
		# Find the nearest neighbours of the reduced data to the reference angles on the C1 sphere.
		_, knn_data = scipy.spatial.cKDTree(angles_cart, balanced_tree=False).query(data_cart)
		# Find the nearest neighbours of the reduced reference data to the reference angles that do not contain mirror projections.
		_, knn_angle = scipy.spatial.cKDTree(angles_no_mirror_cart, balanced_tree=False).query(angles_reduce_cart)
		...
		radius = matplotlib.numpy.bincount(knn_data, minlength=angles_cart.shape[0])
		# New output histogram array that needs to be filled later
		radius_array = matplotlib.numpy.zeros(angles_cart.shape[0], dtype=int)
		...
		matplotlib.numpy.add.at(radius_array, knn_angle, radius)
		...
		radius_array = radius_array[nonzero_mask]
		if do_print:
			sp_global_def.sxprint(matplotlib.numpy.sort(radius_array))
	...
	if do_print:
		sp_global_def.sxprint('Reduce data to symmetry - This might take some time for high symmetries')
	# Reduce the parameter data by moving mirror projections into the non-mirror region of the sphere.
	data = matplotlib.numpy.array(sym_class.reduce_anglesets(data_params.tolist(), inc_mirror=inc_mirror))
	# Create cartesian coordinates
	...
	radius_array = matplotlib.numpy.array(occupy)
	angles_no_mirror = matplotlib.numpy.array(eva)
	angles_no_mirror_cart = to_cartesian(angles_no_mirror)
...
def tabessel(nx, nnxo, nbel=5000):
	...
	alpha = 15
	# order = 0
	normk = EMAN2_cppwrap.Util.bessel0(0.0, radius, alpha)
	for i in range(nbel):
