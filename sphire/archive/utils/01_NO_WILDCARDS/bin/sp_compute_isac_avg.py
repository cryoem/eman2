...
pass#IMPORTIMPORTIMPORT from sp_utilities 		import get_im, bcast_number_to_all, write_text_file, read_text_file, wrap_mpi_bcast, write_text_row
pass#IMPORTIMPORTIMPORT from sp_utilities 		import cmdexecute
...
def compute_average(mlist, radius, CTF):
	...
	pass#IMPORTIMPORTIMPORT from sp_statistics   import fsc, sum_oe
	if CTF:
		avge, avgo, ctf_2_sume, ctf_2_sumo, params_list = \
		     sp_statistics.sum_oe(mlist, "a", CTF, None, True, True)
		...
		frc = sp_statistics.fsc(sp_fundamentals.fft(sumavgo), sp_fundamentals.fft(sumavge))
		frc[1][0] = 1.0
		for ifreq in range(1, len(frc[0])):
			frc[1][ifreq] = max(0.0, frc[1][ifreq])
			frc[1][ifreq] = 2.*frc[1][ifreq]/(1.+frc[1][ifreq])
		sumavg  =  EMAN2_cppwrap.Util.addn_img(sp_fundamentals.fft(avgo), sp_fundamentals.fft(avge))
		...
		avge, avgo, params_list = sp_statistics.sum_oe(mlist, "a", False,  None, False, True)
...
def apply_enhancement(avg, B_start, pixel_size, user_defined_Bfactor):
	...
	if user_defined_Bfactor>0.0:
		...
		guinierline = sp_fundamentals.rot_avg_table(sp_morphology.power(EMAN2_cppwrap.periodogram(sp_fundamentals.fft(avg)),.5))
		freq_max    =  1./(2.*pixel_size)
		freq_min    =  1./B_start
		b, junk, ifreqmin, ifreqmax = sp_morphology.compute_bfactor(guinierline, freq_min, freq_max, pixel_size)
		#print(ifreqmin, ifreqmax)
		global_b = b*4. #
	return sp_filter.filt_gaussinv(sp_fundamentals.fft(avg), numpy.sqrt(2./global_b)), global_b
...
def main():
	...
	if adjust_to_given_pw2:
		...
		if checking_flag ==1: 
			sp_global_def.ERROR( "User provided power spectrum does not exist", myid=Blockdata["myid"] )
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		if not masterdir:
			...
			if os.path.exists(masterdir): sp_global_def.sxprint("%s already exists"%masterdir)
			else:
				os.makedirs(masterdir)
	...
	while not os.path.exists(Tracker["constants"]["masterdir"]):
		sp_global_def.sxprint("Node ", Blockdata["myid"], "  waiting...", Tracker["constants"]["masterdir"])
		time.sleep(1)
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		core = sp_utilities.read_text_row(os.path.join(Tracker["directory"], "initial2Dparams.txt"))
		for im in range(len(core)): init_dict[im]  = core[im]
		del core
	else: init_dict = 0
	init_dict = sp_utilities.wrap_mpi_bcast(init_dict, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	###
	do_ctf = True
	if options.noctf: do_ctf = False 
	if(Blockdata["myid"] == Blockdata["main_node"]):
		if do_ctf: sp_global_def.sxprint("CTF correction is on")
		else:      sp_global_def.sxprint("CTF correction is off")
		if options.local_alignment: sp_global_def.sxprint("local refinement is on")
		else:                       sp_global_def.sxprint("local refinement is off")
		if B_enhance: sp_global_def.sxprint("Bfactor is to be applied on averages")
		elif adjust_to_given_pw2: sp_global_def.sxprint("PW of averages is adjusted to a given 1D PW curve")
		elif adjust_to_analytic_model: sp_global_def.sxprint("PW of averages is adjusted to analytical model")
		else: sp_global_def.sxprint("PW of averages is not adjusted")
		...
		if Tracker["constants"]["pixel_size"] == -1.0:
			sp_global_def.sxprint("Pixel size value is not provided by user. extracting it from ctf header entry of the original stack.")
			try:
				ctf_params = image.get_attr("ctf")
				Tracker["constants"]["pixel_size"] = ctf_params.apix
			except: 
				sp_global_def.ERROR( "Pixel size could not be extracted from the original stack.", myid=Blockdata["myid"] )
		...
		if not os.path.exists(isac_shrink_path):
			sp_global_def.ERROR( "%s does not exist in the specified ISAC run output directory"%(isac_shrink_path), myid=Blockdata["myid"] )
	...
	Tracker = sp_utilities.wrap_mpi_bcast(Tracker, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	...
	if(Blockdata["myid"] == Blockdata["main_node"]): parameters = sp_utilities.read_text_row(os.path.join(Tracker["constants"]["isac_dir"], "all_parameters.txt"))
	else: parameters = 0
	parameters = sp_utilities.wrap_mpi_bcast(parameters, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)		
	params_dict = {}
	list_dict   = {}
	...
	navg = min(Tracker["constants"]["navg"], EMAN2_cppwrap.EMUtil.get_image_count(os.path.join(Tracker["constants"]["isac_dir"], "class_averages.hdf")))
	global_dict = {}
	ptl_list    = []
	memlist     = []
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		for iavg in range(navg):
			...
			image   = sp_utilities.get_im(os.path.join(Tracker["constants"]["isac_dir"], "class_averages.hdf"), iavg)
			members = sorted(image.get_attr("members"))
			memlist.append(members)
			for im in range(len(members)):
				abs_id =  members[im]
				global_dict[abs_id] = [iavg, im]
				P = sp_utilities.combine_params2( init_dict[abs_id][0], init_dict[abs_id][1], init_dict[abs_id][2], init_dict[abs_id][3], \
				...
				if parameters[abs_id][3] ==-1: 
					sp_global_def.sxprint("WARNING: Image #{0} is an unaccounted particle with invalid 2D alignment parameters and should not be the member of any classes. Please check the consitency of input dataset.".format(abs_id)) # How to check what is wrong about mirror = -1 (Toshio 2018/01/11)
			...
			sp_utilities.write_text_row(params_of_this_average, os.path.join(Tracker["constants"]["masterdir"], "params_avg", "params_avg_%03d.txt"%iavg))
		ptl_list.sort()
		...
		sp_utilities.write_text_row(init_params, os.path.join(Tracker["constants"]["masterdir"], "init_isac_params.txt"))
	else:  
		params_dict = 0
		list_dict   = 0
		memlist     = 0
	params_dict = sp_utilities.wrap_mpi_bcast(params_dict, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	list_dict   = sp_utilities.wrap_mpi_bcast(list_dict, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	memlist     = sp_utilities.wrap_mpi_bcast(memlist, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	# Now computing!
	del init_dict
	...
	image_start,image_end = sp_applications.MPI_start_end(navg, Blockdata["nproc"], Blockdata["myid"])

	if Blockdata["myid"] == Blockdata["main_node"]:
		cpu_dict = {}
		for iproc in range(Blockdata["nproc"]):
			local_image_start, local_image_end = sp_applications.MPI_start_end(navg, Blockdata["nproc"], iproc)
			for im in range(local_image_start, local_image_end):
				cpu_dict [im] = iproc
	else:
		cpu_dict = 0
	
	cpu_dict = sp_utilities.wrap_mpi_bcast(cpu_dict, Blockdata["main_node"], communicator = mpi.MPI_COMM_WORLD)
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		if B_enhance: 
			sp_global_def.sxprint("Avg ID   B-factor  FH1(Res before ali) FH2(Res after ali)")	
		else: 
	...
	for iavg in range(image_start,image_end):
		...
		for im in range(len(mlist)):
			sp_utilities.set_params2D(mlist[im], params_dict[iavg][im], xform = "xform.align2d")
		
		if options.local_alignment:
			new_avg, plist, FH2 = sp_applications.refinement_2d_local(mlist, Tracker["constants"]["radius"], a_range,x_range, y_range, CTF = do_ctf, SNR=1.0e10)
		...
		if B_enhance:
			...
			sp_global_def.sxprint("  %6d      %6.3f  %4.3f  %4.3f"%(iavg, gb, FH1, FH2))
		...
		if Tracker["constants"]["low_pass_filter"] != -1.0:
			...
			if enforced_to_H1: new_avg = sp_filter.filt_tanl(new_avg, FH1, 0.02)
		if B_enhance: new_avg = sp_fundamentals.fft(new_avg)
			
		new_avg.set_attr("members",   list_dict[iavg])
		new_avg.set_attr("n_objects", len(list_dict[iavg]))
		slist[iavg]    = new_avg
		sp_global_def.sxprint(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + " =>",  "Refined average %7d"%iavg)
	...
	for im in range(navg):
		...
		if options.local_alignment:
			if cpu_dict[im] == Blockdata["myid"]:
				sp_utilities.write_text_row(plist_dict[im], os.path.join(Tracker["constants"]["masterdir"], "ali2d_local_params_avg", "ali2d_local_params_avg_%03d.txt"%im))
			
			if cpu_dict[im] == Blockdata["myid"] and cpu_dict[im]!= Blockdata["main_node"]:
				sp_utilities.wrap_mpi_send(plist_dict[im], Blockdata["main_node"], mpi.MPI_COMM_WORLD)
				sp_utilities.wrap_mpi_send(FH_list, Blockdata["main_node"], mpi.MPI_COMM_WORLD)
	...
	if options.local_alignment:
		if Blockdata["myid"] == Blockdata["main_node"]:
			...
			sp_utilities.write_text_row(ali3d_local_params, os.path.join(Tracker["constants"]["masterdir"], "ali2d_local_params.txt"))
			sp_utilities.write_text_row(FH_list, os.path.join(Tracker["constants"]["masterdir"], "FH_list.txt"))
	else:
		if Blockdata["myid"] == Blockdata["main_node"]:
			sp_utilities.write_text_row(FH_list, os.path.join(Tracker["constants"]["masterdir"], "FH_list.txt"))
