...
def depth_clustering_box(work_dir, input_accounted_file, input_unaccounted_file, params, previous_params, nbox,
                         log_main):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import read_text_file, wrap_mpi_bcast, write_text_file, write_text_row
    pass#IMPORTIMPORTIMPORT import copy, time, shutil
...
def check_mpi_settings(log_main):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import wrap_mpi_bcast, read_text_file, bcast_number_to_all
    pass#IMPORTIMPORTIMPORT import os
...
def get_sorting_image_size(original_data, partids, number_of_groups, sparamstructure, snorm_per_particle, log_main):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import wrap_mpi_bcast, read_text_file, write_text_file
...
def compute_noise(image_size):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, model_blank
...
def get_shrink_data_sorting(partids, partstack, return_real=False, preshift=True, apply_mask=True, npad=1):
    ...
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import wrap_mpi_bcast, read_text_row
...
def read_data_for_sorting(partids, partstack, previous_partstack):
    ...
    pass#IMPORTIMPORTIMPORT from sp_applications import MPI_start_end
    pass#IMPORTIMPORTIMPORT from EMAN2 import Region
    pass#IMPORTIMPORTIMPORT from sp_utilities import wrap_mpi_bcast, read_text_row, get_im, set_params_proj
    # functions:
    # read in data
    if (Blockdata["myid"] == Blockdata["main_node"]):
        lpartids = sp_utilities.read_text_file(partids, -1)
...
def read_paramstructure_for_sorting(partids, paramstructure_dict_file, paramstructure_dir):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import read_text_row, read_text_file, wrap_mpi_bcast
    pass#IMPORTIMPORTIMPORT from sp_applications import MPI_start_end
    if (Blockdata["myid"] == Blockdata["main_node"]):
...
def copy_oldparamstructure_from_meridien_MPI(selected_iteration, log_main):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import read_text_row, write_text_row, read_text_file, wrap_mpi_bcast
    pass#IMPORTIMPORTIMPORT from sp_applications import MPI_start_end
    pass#IMPORTIMPORTIMPORT import json
...
def precalculate_shifted_data_for_recons3D(prjlist, paramstructure, refang, rshifts, delta, avgnorms, nxinit, nnxo,
    ...
    pass#IMPORTIMPORTIMPORT from sp_utilities import random_string, get_im, findall, info, model_blank
...
def downsize_data_for_sorting(original_data, return_real=False, preshift=True, npad=1, norms=None):
    ...
    # So, the lengths of partids and partstack are the same.
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_fundamentals import resample, fshift, cyclic_shift
    pass#IMPORTIMPORTIMPORT from sp_filter import filt_ctf
    pass#IMPORTIMPORTIMPORT from sp_applications import MPI_start_end
    pass#IMPORTIMPORTIMPORT from EMAN2 import Region
    # functions:
...
def compare_two_images_cross(data, ref_vol):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import same_ctf
    ny = data[0].get_ysize()
    peaks = len(data) * [None]
...
def get_angle_step_and_orien_groups_mpi(params_in, partids_in, angstep):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_applications import MPI_start_end
    pass#IMPORTIMPORTIMPORT from sp_utilities import wrap_mpi_recv, wrap_mpi_bcast, wrap_mpi_send, bcast_number_to_all
    pass#IMPORTIMPORTIMPORT from sp_utilities import read_text_row, read_text_file
...
def recons3d_4nnsorting_group_MPI(myid, main_node, prjlist, random_subset, group_ID, CTF=True, upweighted=True,
    ...
    pass#IMPORTIMPORTIMPORT from EMAN2 import Reconstructors
    pass#IMPORTIMPORTIMPORT from sp_utilities import model_blank, info
    pass#IMPORTIMPORTIMPORT from sp_filter import filt_table
    pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_barrier
    pass#IMPORTIMPORTIMPORT from sp_statistics import fsc
    pass#IMPORTIMPORTIMPORT from sp_reconstruction import insert_slices_pdf
...
def do3d_sorting_groups_trl_iter(data, iteration):
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, write_text_row, bcast_number_to_all, wrap_mpi_bcast
    keepgoing = 1
    if (Blockdata["myid"] == Blockdata["last_node"]):
        if not os.path.exists(os.path.join(Tracker["directory"], "tempdir")): os.mkdir(
            os.path.join(Tracker["directory"], "tempdir"))
...
def get_input_from_datastack(log_main):  # Case three
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import write_text_file, write_text_row, wrap_mpi_bcast
    pass#IMPORTIMPORTIMPORT import json
    pass#IMPORTIMPORTIMPORT from string import split, atoi
    pass#IMPORTIMPORTIMPORT from random import shuffle
...
def recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI(myid, main_node, nproc, prjlist, group_ID, refvol_file,
    ...
    """
    pass#IMPORTIMPORTIMPORT from sp_utilities import reduce_EMData_to_root, random_string, get_im, findall, info, model_blank
    pass#IMPORTIMPORTIMPORT from EMAN2 import Reconstructors
    pass#IMPORTIMPORTIMPORT from sp_filter import filt_table
    pass#IMPORTIMPORTIMPORT from sp_fundamentals import fshift
    pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_barrier
...
def recons3d_trl_struct_group_MPI(myid, main_node, prjlist, random_subset, group_ID, paramstructure,
    ...
    """
    global Tracker, Blockdata
    pass#IMPORTIMPORTIMPORT from sp_utilities import reduce_EMData_to_root, random_string, get_im, findall, model_blank, info
    pass#IMPORTIMPORTIMPORT from EMAN2 import Reconstructors
    pass#IMPORTIMPORTIMPORT from sp_filter import filt_table
    pass#IMPORTIMPORTIMPORT from sp_fundamentals import fshift
    pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_barrier
...
def main():
    ...
    if initiate_from_meridien_mode:
        ...
        #
        pass#IMPORTIMPORTIMPORT from sp_statistics import k_means_match_clusters_asg_new, k_means_stab_bbenum
        pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, bcast_number_to_all, write_text_file, read_text_file, wrap_mpi_bcast, \
