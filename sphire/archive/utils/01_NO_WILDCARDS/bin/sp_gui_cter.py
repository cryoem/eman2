...
###		self.is_enable_max_power = False # MRK_TEST: Can this be an option in future?
		...
		self.setWindowIcon(PyQt5.QtGui.QIcon(EMAN2.get_image_directory()+"sparxicon.png"))
		...
		EMAN2.E2loadappwin("sp_gui_cter","plotcoarse",self.wplotrotavgcoarse.qt_parent)
...
###		if self.is_enable_max_power == True: 
	...
	def initialize_popup_windows(self):
		...
		self.wfft.qt_parent.setWindowFlags((self.wfft.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
		...
		self.wimgmicthumb.qt_parent.setWindowFlags((self.wimgmicthumb.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
		...
		self.wplotrotavgcoarse.qt_parent.setWindowFlags((self.wplotrotavgcoarse.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
		...
		self.wplotrotavgfine.qt_parent.setWindowFlags((self.wplotrotavgfine.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
		...
		self.whistparam.qt_parent.setWindowFlags((self.whistparam.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
		...
		self.wscatterparam.qt_parent.setWindowFlags((self.wscatterparam.qt_parent.windowFlags()| PyQt5.QtCore.Qt.CustomizeWindowHint) & ~PyQt5.QtCore.Qt.WindowMinimizeButtonHint) # Disabled minimize icon button in window title bar
	...
	def draw_main_window(self):
		...
		self.cbrotavgdisplay = eman2_gui.valslider.CheckBox(None,None,self.curplotrotavgdisplay)
		self.add_label_with_checkbox("Rot. Avg. Plot", self.cbrotavgdisplay, leftcolumn, labelwidth=sublabelwidth)
		
		self.cbrotzoomdisplay = eman2_gui.valslider.CheckBox(None,None,self.curplotrotzoomdisplay)
		self.add_label_with_checkbox("Rot. Avg. Plot Zoom", self.cbrotzoomdisplay, leftcolumn, labelwidth=sublabelwidth)
		
		self.cbhistogramdisplay = eman2_gui.valslider.CheckBox(None,None,self.curhistogramdisplay)
		self.add_label_with_checkbox("Histogram", self.cbhistogramdisplay, leftcolumn, labelwidth=sublabelwidth)
		
		self.cbscatterdisplay = eman2_gui.valslider.CheckBox(None,None,self.curscatterdisplay)
		self.add_label_with_checkbox("Sort Plot", self.cbscatterdisplay, leftcolumn, labelwidth=sublabelwidth)
		
		self.cbmicthumbdisplay = eman2_gui.valslider.CheckBox(None,None,self.curimgmicthumbdisplay)
		...
		for item in self.graph_map:
			self.graph_map[item].item_widget = eman2_gui.valslider.CheckBox(None,None,True)
		...
		self.vbplotfixscale = eman2_gui.valslider.ValBox(self,(0,99999),None,self.curplotfixscale)  # default <- self.curplotfixscale
		...
		self.lbentry.setSizePolicy(PyQt5.QtWidgets.QSizePolicy.Preferred,PyQt5.QtWidgets.QSizePolicy.Expanding)
		...
		unappliedLimitFrame = PyQt5.QtWidgets.QWidget(self)  # QGridLayout doesn't have borders, so I'm enclosing it inside a QFrame, which can
		...
		# Applied limits
		appliedLimitFrame = PyQt5.QtWidgets.QWidget(self)  # QGridLayout doesn't have borders, so I'm enclosing it inside a QFrame, which can
	...
	def set_size_popup_windows(self):
		...
		self.wimgmicthumb.set_data(sp_utilities.model_blank(img_size,img_size, bckg=1.0)) # resize does not work if no image is set, img_size appears to affect centering
		...
		self.wfft.set_data(sp_utilities.model_blank(img_size,img_size, bckg=1.0)) # resize does not work if no image is set
	...
	def add_centered_label(self, labeltext, target, labelwidth=None, style_sheet=None):
		...
		temp_label.setAlignment(PyQt5.QtCore.Qt.AlignHCenter|PyQt5.QtCore.Qt.AlignVCenter)
	...
	def add_label_with_value(self, labeltext, valueentry, target, 
		...
		temp_label = PyQt5.QtWidgets.QLabel(labeltext, self)
		temp_label.setAlignment(PyQt5.QtCore.Qt.AlignRight|PyQt5.QtCore.Qt.AlignVCenter)
	...
	def add_label_with_param(self, labeltext, target, val_min, val_max, 
		...
		# Initialize ValBox
		val_default = val_min
		valueentry = eman2_gui.valslider.ValBox(self,(val_min,val_max),None,val_default)
	...
	def add_threshold_widgets(self, param, target1, target2, labelwidth=80, editwidth=80):
		...
		curr_hist_params.unapply_widget_lower = eman2_gui.valslider.ValBox(self,(val_min,val_max),None,val_min,labelwidth)
		self.add_threshold_widget(curr_hist_params.unapply_widget_lower, temp_hbox, stylesheet="color: rgb(0,0,255);")
		
		curr_hist_params.unapply_widget_upper = eman2_gui.valslider.ValBox(self,(val_min,val_max),None,val_max,labelwidth)
		...
		curr_hist_params.apply_widget_lower = eman2_gui.valslider.ValBox(self,(val_min,val_max),None,val_min,labelwidth)
		self.add_threshold_widget(curr_hist_params.apply_widget_lower, temp_hbox, stylesheet="color: rgb(0,0,255);")
		
		curr_hist_params.apply_widget_upper = eman2_gui.valslider.ValBox(self,(val_min,val_max),None,val_max,labelwidth)
	...
	def add_label_with_pulldown(self, labeltext, target, 
		...
		for index, (control, map_entry) in enumerate(self.threshold_control_map.items()):
			...
			self.sthresholdcontrol.setItemData(map_entry.index, PyQt5.QtGui.QColor(map_entry.color), PyQt5.QtCore.Qt.TextColorRole)
	...
	def readCterPartresFile(self, file_path):
		...
		if not os.path.exists(file_path):
			PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Cannot find CTER partres file (%s). Please check the file path." % (file_path))
			return
		
		if os.path.basename(file_path).find("partres") == -1:
			PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Invalid file name for CTER partres file (%s). The file name must contain \"partres\"." % (file_path))
			return
		
		if file_path[-1*len(".txt"):] != ".txt":
			PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Invalid file extension for CTER partres file (%s). The file extension must be \".txt\"." % (file_path))
			return
		
		new_entry_list = sp_utilities.read_text_row(file_path)
		if len(new_entry_list) == 0:
			PyQt5.QtWidgets.QMessageBox.warning(self, "Warning", "Specified CTER partres file (%s) does not contain any entry. Please check the file." % (file_path))
			return
		assert len(new_entry_list) > 0, "MRK_DEBUG"
		
		if len(new_entry_list[0]) != self.n_idx_cter - self.n_idx_cter_extra:
			PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","The number of columns (%d) has to be %d in %s." % (len(new_entry_list[0]), self.n_idx_cter - self.n_idx_cter_extra, file_path))
			return
...
###					new_entry_list[cter_id][self.idx_cter_max_power] = max(new_rotinf_table[self.idx_rotinf_exp_with_astig]) # MRK_TEST: 
		...
		if selected_hist_param.val_min == selected_hist_param.val_max:
			...
			PyQt5.QtWidgets.QMessageBox.information(self, "Information",
		...
		if os.path.exists(cter_pwrot_dir):
			...
			PyQt5.QtWidgets.QMessageBox.warning(None,"Warning",
	...
	def openCterPartres(self,val=None):
		...
		file_path = PyQt5.QtWidgets.QFileDialog.getOpenFileName(self, "Open CTER partres File", options = PyQt5.QtWidgets.QFileDialog.DontUseNativeDialog)
	...
	def updatePlotCurves(self, error_display = True):
		...
		if not os.path.exists(self.cter_pwrot_file_path):
			...
			if error_display and os.path.exists(os.path.dirname(self.cter_pwrot_file_path)):
				PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Cannot find file cter_pwrot_file_path (%s). Please check the contents of pwrot directory. \n\nPlots will not be shown." 
		...
		if not self.use_ctffind:
			...
			self.rotinf_table = sp_utilities.read_text_file(self.cter_pwrot_file_path, ncol=-1)
	...
	def draw_limits(self, error_name, error_label, shape_r,shape_g,shape_b, y_offset, nyquist_freq):
		...
		if error_freq > 0.0 and error_freq <= nyquist_freq:
			...
			self.wplotrotavgcoarse.add_shape(error_name, eman2_gui.emshape.EMShape(("scrline", shape_r,shape_g,shape_b, error_scr_x, coarseLimits[1], error_scr_x, coarseLimits[1]+coarseLimits[3], 1) ) )
			self.wplotrotavgcoarse.add_shape("%s_freq"%(error_name),
								  eman2_gui.emshape.EMShape(("scrlabel",0,0,0, error_scr_x-260, coarseLimits[1]+coarseLimits[3]-y_offset, "%s %1.5g (%1.5g)"%(error_label, error_freq, 1.0/error_freq), 120.0, -1) ) )
			error_scr_x, error_scr_y = self.wplotrotavgfine.plot2scr(error_freq, 0.0)
			self.wplotrotavgfine.add_shape(error_name, eman2_gui.emshape.EMShape(("scrline", shape_r,shape_g,shape_b, error_scr_x, fineLimits[1], error_scr_x, fineLimits[1]+fineLimits[3], 1) ) )
			self.wplotrotavgfine.add_shape("%s_freq"%(error_name),
								  eman2_gui.emshape.EMShape(("scrlabel", 0,0,0, error_scr_x-260, fineLimits[1]+fineLimits[3]-y_offset, "%s %1.5g (%1.5g)"%(error_label, error_freq, 1.0/error_freq), 120.0, -1) ) )
	...
	def updateEntryList(self):
		...
		self.lbentry.clear()
		newItemflags = PyQt5.QtCore.Qt.ItemFlags(PyQt5.QtCore.Qt.ItemIsSelectable)|PyQt5.QtCore.Qt.ItemFlags(PyQt5.QtCore.Qt.ItemIsEnabled)|PyQt5.QtCore.Qt.ItemFlags(PyQt5.QtCore.Qt.ItemIsUserCheckable)
		for cter_entry in self.cter_entry_list:
			newItem = PyQt5.QtWidgets.QListWidgetItem(os.path.basename(cter_entry[self.cter_params['mic_name'].idx_cter]))
	...
	def updateImgMicThumb(self, error_display = True):
		...
		if not os.path.exists(self.cter_micthumb_file_path):
			...
			if error_display and os.path.exists(os.path.dirname(self.cter_micthumb_file_path)):
				PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Cannot find micrograph thumbnail (%s). Please check your micrograph thumbnail directory. \n\nMicrograph thumbnail will not be shown." % (self.cter_micthumb_file_path))
			return
	...
	def updateFFT(self, error_display = True):
		...
		if not os.path.exists(self.cter_fft_file_path):
			...
			if error_display and os.path.exists(os.path.dirname(self.cter_fft_file_path)):
				PyQt5.QtWidgets.QMessageBox.warning(None,"Warning","Cannot find power spectrum (%s). Please check your power spectrum directory. \n\nPower spectrum will not be shown." % (self.cter_micthumb_file_path))
			return
	...
	def applyAllThresholds(self,item = None):
		...
		reply = PyQt5.QtWidgets.QMessageBox.question(self, "Warning", "Applying all threshold setting will wipe the previous selection states including manual setting. Do you really want to continue?", PyQt5.QtWidgets.QMessageBox.Yes | PyQt5.QtWidgets.QMessageBox.No, PyQt5.QtWidgets.QMessageBox.No)
	...
	def readThresholdSet(self, file_path_in, idx_thresholdset):
		...
		if line_in.find("@@@@@ gui_cter thresholds") != -1:
			...
			PyQt5.QtWidgets.QMessageBox.warning(self, "Warning", "The specified file is not threshold file.")
	...
	def saveThresholdSet(self,item = None):
		...
		file_path_out = PyQt5.QtWidgets.QFileDialog.getSaveFileName(self, title_string, options = PyQt5.QtWidgets.QFileDialog.DontUseNativeDialog)  
	...
	def loadThresholdSet(self,item = None):
		...
		reply = PyQt5.QtWidgets.QMessageBox.question(self, "Warning", "Loading thresholds will wipe the previous threshold setting. Do you really want to continue?", PyQt5.QtWidgets.QMessageBox.Yes | PyQt5.QtWidgets.QMessageBox.No, PyQt5.QtWidgets.QMessageBox.No)
		if reply == PyQt5.QtWidgets.QMessageBox.No:
			return
		
		title_string = "Load %s Thresholds" % list( self.threshold_status_labels.items() )[self.curthresholdset][1]
		file_path_in = PyQt5.QtWidgets.QFileDialog.getOpenFileName(self, title_string, options = PyQt5.QtWidgets.QFileDialog.DontUseNativeDialog)
	...
	def saveSelection(self,item = None):
		...
		if existing_file_path != None:
			reply = PyQt5.QtWidgets.QMessageBox.question(self, "Warning", "The file (%s) already exists. Do you want to overwrite the file?" % (existing_file_path), PyQt5.QtWidgets.QMessageBox.Yes | PyQt5.QtWidgets.QMessageBox.No, PyQt5.QtWidgets.QMessageBox.No)
			if reply == PyQt5.QtWidgets.QMessageBox.No:
				return
	...
	def closeEvent(self,event):
		...
		if self.cter_entry_list != None:
			...
			EMAN2.E2saveappwin("sp_gui_cter","imgmicthumb",self.wimgmicthumb.qt_parent)
			EMAN2.E2saveappwin("sp_gui_cter","plotcoarse",self.wplotrotavgcoarse.qt_parent)
			EMAN2.E2saveappwin("sp_gui_cter","plotfine",self.wplotrotavgfine.qt_parent)
