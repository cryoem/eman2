...
pass#IMPORTIMPORTIMPORT from   time import localtime, strftime, sleep, time
...
def assign_particles_to_groups(minimum_group_size = 10, asubset= None):
	...
	if not asubset:
		try:
			stmp  = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"], "ptcl_source_image")
			...
			if Tracker["constants"]["CTF"]:
				...
				sp_global_def.ERROR("Either ptcl_source_image or ctf has to be present in the header.","meridien",1)
	else:
		try:
			...
			if Tracker["constants"]["CTF"]:
				...
				defstmp[:] = stmp[:]
			else:  sp_global_def.ERROR("Either ptcl_source_image or ctf has to be present in the header.","meridien",1)
...
def get_anger(angle1, angle2):
	...
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import rotmatrix
...
def calculate_2d_params_for_centering(kwargs):
	pass#IMPORTIMPORTIMPORT from sp_utilities 		import wrap_mpi_gatherv, read_text_row, write_text_row, bcast_number_to_all, get_im, combine_params2, model_circle, gather_compacted_EMData_to_root
	pass#IMPORTIMPORTIMPORT from sp_applications 	import MPI_start_end, ali2d_base
	pass#IMPORTIMPORTIMPORT from sp_fundamentals 	import resample, rot_shift2D
...
def ali3D_polar_ccc(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		if( im < min(nima, 1) and procid == 0):
			...
			xod2 = numpy.asarray(EMAN2_cppwrap.Util.multiref_Crosrng_msg_stack_stepsi(dataimage, bigbuffer, \
...
def recons3d_trl_struct_MPI_nosmearing(myid, main_node, prjlist, parameters, CTF, upweighted, mpi_comm, target_size):
	global Tracker, Blockdata
	pass#IMPORTIMPORTIMPORT from sp_utilities      import reduce_EMData_to_root, random_string, get_im, findall, model_blank, info, get_params_proj
...
def refinement_one_iteration(partids, partstack, original_data, oldparams, projdata, general_mode = True, continuation_mode = False):
	...
	for procid in range(2):
		...
		do3d(procid, projdata[procid], newparamstructure[procid], refang, rshifts, norm_per_particle[procid], Blockdata["myid"], smearing = True, mpi_comm = mpi.MPI_COMM_WORLD)
		projdata[procid] = []
		if Tracker["changed_delta"]:
...
def main():
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities import write_text_row, drop_image, model_gauss_noise, get_im, set_params_proj, wrap_mpi_bcast, model_circle
	pass#IMPORTIMPORTIMPORT import sp_user_functions
	pass#IMPORTIMPORTIMPORT from sp_applications  import MPI_start_end
	pass#IMPORTIMPORTIMPORT from optparse      import OptionParser
	pass#IMPORTIMPORTIMPORT from sp_global_def    import SPARXVERSION
	pass#IMPORTIMPORTIMPORT from EMAN2         import EMData
	pass#IMPORTIMPORTIMPORT from sp_multi_shc     import multi_shc
