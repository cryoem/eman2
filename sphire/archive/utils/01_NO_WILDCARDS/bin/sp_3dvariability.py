...
pass#IMPORTIMPORTIMPORT from sp_global_def import SPARX_MPI_TAG_UNIVERSAL
...
def main():
	...
	def params_3D_2D_NEW(phi, theta, psi, s2x, s2y, mirror):
		...
		if mirror:
			m = 1
			alpha, sx, sy, scalen = sp_utilities.compose_transform2(0, s2x, s2y, 1.0, 540.0-psi, 0, 0, 1.0)
		else:
			m = 0
			alpha, sx, sy, scalen = sp_utilities.compose_transform2(0, s2x, s2y, 1.0, 360.0-psi, 0, 0, 1.0)
	...
	#####
	pass#IMPORTIMPORTIMPORT from mpi import mpi_comm_rank, mpi_comm_size, mpi_recv, MPI_COMM_WORLD
	pass#IMPORTIMPORTIMPORT from mpi import mpi_barrier, mpi_reduce, mpi_bcast, mpi_send, MPI_FLOAT, MPI_SUM, MPI_INT, MPI_MAX
	#from mpi import *
	pass#IMPORTIMPORTIMPORT from sp_applications   import MPI_start_end
	pass#IMPORTIMPORTIMPORT from sp_reconstruction import recons3d_em, recons3d_em_MPI
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities      import read_text_row, get_image, get_im, wrap_mpi_send, wrap_mpi_recv
	pass#IMPORTIMPORTIMPORT from sp_utilities      import bcast_EMData_to_all, bcast_number_to_all
	pass#IMPORTIMPORTIMPORT from sp_utilities      import get_symt
	...
	if options.output_dir =="./": current_output_dir = optparse.os.path.abspath(options.output_dir)
	...
	if options.symmetrize :
		...
		if optparse.os.path.exists(optparse.os.path.join(current_output_dir, "log.txt")): optparse.os.remove(optparse.os.path.join(current_output_dir, "log.txt"))
		...
		for k in range(ks):
			...
			junk = sp_utilities.cmdexecute("e2bdb.py  "+stack+"  --makevstack=bdb:"+Qfile)
		...
		junk = sp_utilities.cmdexecute("e2bdb.py   " + current_output_dir +"  --makevstack="+sdata +" --filt=Q")
		...
		color, no_of_groups, balanced_processor_load_on_nodes = sp_utilities.get_colors_and_subsets(main_node, mpi.MPI_COMM_WORLD, myid, \
		...
		if options.fl > 0.0 and options.aa == 0.0:
			sp_global_def.ERROR( "Fall off has to be given for the low-pass filter", myid=myid )
		...
		if options.VAR and (options.ave2D or options.ave3D or options.var2D): 
			sp_global_def.ERROR( "When VAR is set, the program cannot output ave2D, ave3D or var2D", myid=myid )
		...
		if current_decimate>1.0 or current_decimate<0.0:
			sp_global_def.ERROR( "Decimate rate should be a value between 0.0 and 1.0", myid=myid )
		
		if current_window < 0.0:
			sp_global_def.ERROR( "Target window size should be always larger than zero", myid=myid )
		...
		keepgoing = sp_utilities.bcast_number_to_all(keepgoing, main_node, mpi.MPI_COMM_WORLD)
		if keepgoing == 0: 
			sp_global_def.ERROR( "The target window size cannot be larger than the size of decimated image", myid=myid )
		...
		if myid == main_node:
			if not optparse.os.path.exists(current_output_dir): optparse.os.makedirs(current_output_dir)# Never delete output_dir in the program!
		...
		if myid == main_node:
			...
			if options.sym != "c1" :
				...
				try:
					...
					if(i != options.sym):
						sp_global_def.ERROR( "The symmetry provided does not agree with the symmetry of the input stack", myid=myid )
				except:
					sp_global_def.ERROR( "Input stack is not prepared for symmetry, please follow instructions", myid=myid )
				pass#IMPORTIMPORTIMPORT from sp_utilities import get_symt
				i = len(sp_utilities.get_symt(options.sym))
				if((nima/i)*i != nima):
					sp_global_def.ERROR( "The length of the input stack is incorrect for symmetry processing", myid=myid )
				symbaselen = nima/i
		...
		if not is_fft_friendly:
			...
			if current_decimate == 1.:
				...
				if current_window == 0:
					...
					nx = sp_fundamentals.smallprime(int(current_window*current_decimate+0.5))
					ny = nx
		...
		if options.VAR: # 2D variance images have no shifts
			...
			for index_of_particle in range(img_begin,img_end):
				...
				if current_window > 0: varList.append(sp_fundamentals.fdecimate(sp_fundamentals.window2d(image,current_window,current_window), nx,ny))
				else:   varList.append(sp_fundamentals.fdecimate(image, nx,ny))
				
		else:
			pass#IMPORTIMPORTIMPORT from sp_utilities		import bcast_number_to_all, bcast_list_to_all, send_EMData, recv_EMData
			pass#IMPORTIMPORTIMPORT from sp_utilities		import set_params_proj, get_params_proj, params_3D_2D, get_params2D, set_params2D, compose_transform2
			pass#IMPORTIMPORTIMPORT from sp_utilities		import model_blank, nearest_proj, model_circle, write_text_row, wrap_mpi_gatherv
			pass#IMPORTIMPORTIMPORT from sp_applications	import pca
			pass#IMPORTIMPORTIMPORT from sp_statistics		import avgvar, avgvar_ctf, ccc
			pass#IMPORTIMPORTIMPORT from sp_filter		    import filt_tanl
			pass#IMPORTIMPORTIMPORT from sp_morphology		import threshold, square_root
			pass#IMPORTIMPORTIMPORT from sp_projection 	import project, prep_vol, prgs
			pass#IMPORTIMPORTIMPORT from sets		    import Set
			pass#IMPORTIMPORTIMPORT from sp_utilities      import wrap_mpi_recv, wrap_mpi_bcast, wrap_mpi_send
			...
			proj_angles_list = sp_utilities.wrap_mpi_bcast(proj_angles_list, main_node, mpi.MPI_COMM_WORLD)
			proj_angles      = []
			for i in range(nima):
			...
			proj_list, mirror_list = sp_utilities.nearest_proj(proj_angles, img_per_grp, range(img_begin, img_end))
			all_proj = sets.Set()
			for im in proj_list:
				for jm in im:
			...
			if myid == main_node:
				...
				log_main.add("Grouping projections took:  %12.1f [m]"%((time.time()-t2)/60.))
			...
			full_data         = numpy.full((number_of_proc, 2), -1., dtype=numpy.float16)
			full_data[myid]   = orig_data_size, reduced_data_size
			if myid != main_node: sp_utilities.wrap_mpi_send(full_data, main_node, mpi.MPI_COMM_WORLD)
			if myid == main_node:
				for iproc in range(number_of_proc):
					if iproc != main_node:
						dummy = sp_utilities.wrap_mpi_recv(iproc, mpi.MPI_COMM_WORLD)
						full_data[numpy.where(dummy>-1)] = dummy[numpy.where(dummy>-1)]
				del dummy
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			full_data = sp_utilities.wrap_mpi_bcast(full_data, main_node, mpi.MPI_COMM_WORLD)
			...
			for index_of_proj in range(len(all_proj)):
				...
				if( current_window > 0): imgdata[index_of_proj] = sp_fundamentals.fdecimate(sp_fundamentals.window2d(sp_utilities.get_im(stack, all_proj[index_of_proj]),current_window,current_window), nx, ny)
				else:                    imgdata[index_of_proj] = sp_fundamentals.fdecimate(sp_utilities.get_im(stack, all_proj[index_of_proj]), nx, ny)
			...
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			if myid == heavy_load_myid:
				log_main.add("All_proj preprocessing cost %7.2f m"%((time.time()-ttt)/60.))
			...
			#MULTILINEMULTILINEMULTILINE 1
			#MULTILINEMULTILINEMULTILINE 1
			pass#IMPORTIMPORTIMPORT from sp_applications import prepare_2d_forPCA
			...
			for i in range(len(proj_list)):
				...
				for j in range(img_per_grp):
					...
					if thetaM <= 90:
						if mirror == 0:  alpha, sx, sy, scale = sp_utilities.compose_transform2(alpha, sx, sy, 1.0, phiM - cpar["phi"], 0.0, 0.0, 1.0)
						else:            alpha, sx, sy, scale = sp_utilities.compose_transform2(alpha, sx, sy, 1.0, 180-(phiM - cpar["phi"]), 0.0, 0.0, 1.0)
					else:
						if mirror == 0:  alpha, sx, sy, scale = sp_utilities.compose_transform2(alpha, sx, sy, 1.0, -(phiM- cpar["phi"]), 0.0, 0.0, 1.0)
						else:            alpha, sx, sy, scale = sp_utilities.compose_transform2(alpha, sx, sy, 1.0, -(180-(phiM - cpar["phi"])), 0.0, 0.0, 1.0)
				...
				if options.fl > 0.0:
					for k in range(img_per_grp):
						grp_imgdata[k] = sp_filter.filt_tanl(grp_imgdata[k], options.fl, options.aa)

				#  Because of background issues, only linear option works.
				if options.CTF:  ave, var = sp_statistics.aves_wiener(grp_imgdata, SNR = 1.0e5, interpolation_method = "linear")
				else:  ave, var = sp_statistics.ave_var(grp_imgdata)
				# Switch to std dev
			...
			if (myid == heavy_load_myid):
				log_main.add("Computing aveList and varList took %12.1f [m]"%((time.time()-ttt)/60.))
			
			xform_proj_for_2D = sp_utilities.wrap_mpi_gatherv(xform_proj_for_2D, main_node, mpi.MPI_COMM_WORLD)
			if (myid == main_node):
				sp_utilities.write_text_row([str(entry) for entry in xform_proj_for_2D], optparse.os.path.join(current_output_dir, "params.txt"))
					...
					for i in range(number_of_proc):
						if i == main_node :
							for im in range(len(aveList)):
								aveList[im].write_image(optparse.os.path.join(current_output_dir, options.ave2D), km)
								km += 1
						else:
							nl = mpi.mpi_recv(1, mpi.MPI_INT, i, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
							nl = int(nl[0])
							for im in range(nl):
								...
								tmpvol.write_image(optparse.os.path.join(current_output_dir, options.ave2D), km)
								km += 1
				else:
					mpi.mpi_send(len(aveList), 1, mpi.MPI_INT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
					for im in range(len(aveList)):
						sp_utilities.send_EMData(aveList[im], main_node,im+myid+70000)
						"""Multiline Comment4"""
						...
						#MULTILINEMULTILINEMULTILINE 4
				if myid == main_node:
					sp_applications.header(optparse.os.path.join(current_output_dir, options.ave2D), params='xform.projection', fimport = optparse.os.path.join(current_output_dir, "params.txt"))
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)	
			if options.ave3D:
				pass#IMPORTIMPORTIMPORT from sp_fundamentals import fpol
				t5 = time.time()
				if myid == main_node: log_main.add("Reconstruct ave3D ... ")
				ave3D = sp_reconstruction.recons3d_4nn_MPI(myid, aveList, symmetry=options.sym, npad=options.npad)
				sp_utilities.bcast_EMData_to_all(ave3D, myid)
				if myid == main_node:
					if current_decimate != 1.0: ave3D = sp_fundamentals.resample(ave3D, 1./current_decimate)
					ave3D = sp_fundamentals.fpol(ave3D, nnxo, nnxo, nnxo) # always to the orignal image size
					sp_utilities.set_pixel_size(ave3D, 1.0)
					ave3D.write_image(optparse.os.path.join(current_output_dir, options.ave3D))
					log_main.add("Ave3D reconstruction took %12.1f [m]"%((time.time()-t5)/60.0))
			...
			if options.var2D:
				...
				if myid == main_node:
					...
					for i in range(number_of_proc):
						if i == main_node :
							for im in range(len(varList)):
								...
								tmpvol.write_image(optparse.os.path.join(current_output_dir, options.var2D), km)
								km += 1
						else:
							nl = mpi.mpi_recv(1, mpi.MPI_INT, i, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
							nl = int(nl[0])
					...
					mpi.mpi_send(len(varList), 1, mpi.MPI_INT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
					for im in range(len(varList)):
						sp_utilities.send_EMData(varList[im], main_node, im+myid+70000)#  What with the attributes??
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
				if myid == main_node:
					pass#IMPORTIMPORTIMPORT from sp_applications import header
					sp_applications.header(optparse.os.path.join(current_output_dir, options.var2D), params = 'xform.projection',fimport = optparse.os.path.join(current_output_dir, "params.txt"))
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
		if options.var3D:
			...
			res = sp_reconstruction.recons3d_4nn_MPI(myid, varList, symmetry = options.sym, npad=options.npad)
			#res = recons3d_em_MPI(varList, vol_stack, options.iter, radiusvar, options.abs, True, options.sym, options.squ)
			if myid == main_node:
				pass#IMPORTIMPORTIMPORT from sp_fundamentals import fpol
				if current_decimate != 1.0: res	= sp_fundamentals.resample(res, 1./current_decimate)
				res = sp_fundamentals.fpol(res, nnxo, nnxo, nnxo)
				sp_utilities.set_pixel_size(res, 1.0)
				res.write_image(optparse.os.path.join(current_output_dir, options.var3D))
				log_main.add("%-70s:  %s\n"%("The reconstructed var3D is saved as ", options.var3D))
				log_main.add("Var3D reconstruction took %f12.1 [m]"%((time.time()-t6)/60.0))
				log_main.add("Total computation time %f12.1 [m]"%((time.time()-t0)/60.0))
