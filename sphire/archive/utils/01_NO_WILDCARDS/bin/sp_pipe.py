...
pass#IMPORTIMPORTIMPORT from sp_sparx import *
...
class SXmpi_run(object):
	...
	def setup():
		...
		if SXmpi_run.RUNNING_UNDER_MPI:
			...
			pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_comm_rank, mpi_comm_size
	...
	def cleanup():
		if SXmpi_run.RUNNING_UNDER_MPI:
			pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_barrier, mpi_finalize
...
def isac_substack(args):
	pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, read_text_row, write_text_row, write_text_file, combine_params2, cmdexecute
	...
	if not EMAN2db.db_check_dict(args.input_bdb_stack_path, readonly=True):
		sp_global_def.ERROR( "Input BDB image stack file does not exist. Please check the file path and restart the program.", where=subcommand_name)
	args.input_run_dir = args.input_run_dir.strip()
	if not os.path.exists(args.input_run_dir):
		sp_global_def.ERROR( "ISAC or Beautifier run output directory does not exist. Please check the directory path and restart the program.", where=subcommand_name)
	...
	if args.isac_class_avgs_path != "": # User provided name
		...
		if not os.path.exists(args.isac_class_avgs_path):
			sp_global_def.ERROR( "The specifed ISAC class average stack file does not exist. Please check the file path and restart the program.", where=subcommand_name)
	else: # Default name of ISAC or Beautifier
		args.isac_class_avgs_path = default_isac_class_avgs_path
		if not os.path.exists(args.isac_class_avgs_path):
			sp_global_def.ERROR( "ISAC or Beautifier run output directory does not contain the default ISAC class average stack file ({}). Please check the directory path or specify ISAC class average stack file, then restart the program.".format(args.isac_class_avgs_path), where=subcommand_name)
	args.substack_basename = args.substack_basename.strip()
	if args.substack_basename == "":
		sp_global_def.ERROR( "Substack basename cannot be empty string or only white spaces.", where=subcommand_name)
	...
	if n_fullstack_img == 0:
		sp_global_def.ERROR( "Input BDB image stack file does contain no images. Please check the file path and restart the program.", where=subcommand_name)
	...
	if len(isac_missing_path_list) > 0 and len(beautifier_missing_path_list) > 0:
		...
		sp_global_def.ERROR(error_message, where=subcommand_name) # action=1 - fatal error, exit
	...
	if len(isac_missing_path_list) == 0:
		...
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("ISAC run output directory is specified. The program assumes the ISAC class averages are shrunk and not beautified with the original image size.")
		sp_global_def.sxprint("Extracting the shrink ratio and 2D alignment parameters of this ISAC run...")
		...
		fullstack_prealign2d_list = sp_utilities.read_text_row(fullstack_prealign2d_path)
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Found {} entries in {}.".format(len(fullstack_prealign2d_list), fullstack_prealign2d_path))
		if len(fullstack_prealign2d_list) != n_fullstack_img:
			sp_global_def.ERROR("The number of entries in {} is not consistent with {}. Please check the consistency of input datasets.".format(fullstack_prealign2d_path, args.input_bdb_stack_path), where=subcommand_name) # action=1 - fatal error, exit
		...
		fullstack_shrunk_core_align2d_list = sp_utilities.read_text_row(fullstack_shrunk_core_align2d_path)
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Found {} entries in {}.".format(len(fullstack_shrunk_core_align2d_list), fullstack_shrunk_core_align2d_path))
		if len(fullstack_shrunk_core_align2d_list) != n_fullstack_img:
			sp_global_def.ERROR("The number of entries in {} is not consistent with {}. Please check the consistency of input datasets.".format(fullstack_shrunk_core_align2d_path, args.input_bdb_stack_path), where=subcommand_name) # action=1 - fatal error, exit
		...
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Registering scaled back and combined 2D alignment parameters of this ISAC run...")
		for fullstack_img_id in range(n_fullstack_img):
			...
			if len(prealign2d) != n_idx_isac_align2d:
				sp_global_def.ERROR("Invalid number of columns {} at entry #{} in {}. It should be {}. The parameter file might be corrupted. Please consider to rerun ISAC.".format(len(prealign2d), fullstack_img_id, fullstack_prealign2d_path, n_idx_isac_align2d), where=subcommand_name) # action=1 - fatal error, exit
			...
			if len(shrunk_core_align2d) != n_idx_isac_align2d:
				sp_global_def.ERROR("Invalid number of columns {} at entry #{} in {}. It should be {}. The parameter file might be corrupted. Please consider to rerun ISAC.".format(len(shrunk_core_align2d), fullstack_img_id, fullstack_shrunk_core_align2d_path, n_idx_isac_align2d), where=subcommand_name) # action=1 - fatal error, exit
			...
			if shrunk_core_align2d[idx_isac_align2d_mirror] != -1:
				...
				isac_total_align2d = list(sp_utilities.combine_params2(alpha1, sx1, sy1, mirror1, alpha2, sx2, sy2, mirror2))
		...
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Beautifier run output directory is specified. The program assumes the ISAC class averages are beautified with the original image size.")
		sp_global_def.sxprint("Extracting the 2D alignment parameters of this Beautifier run...")
		...
		accounted_local_total_align2d_list = sp_utilities.read_text_row(accounted_local_total_align2d_path)
		n_accounted_img = len(accounted_local_total_align2d_list)
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Found {} entries in {}.".format(n_accounted_img, accounted_local_total_align2d_path))
		...
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Registering 2D alignment parameters of this Beautifier run...")
		for accounted_img_id in range(n_accounted_img):
			...
			if len(local_total_param2d) not in (n_idx_beautifier_align2d, n_idx_beautifier_align2d-1):
				sp_global_def.ERROR("Invalid number of columns {} at entry #{} in {}. It should be {}. The parameter file might be corrupted. Please consider to rerun ISAC.".format(len(local_total_param2d), accounted_img_id, accounted_local_total_align2d_path, n_idx_beautifier_align2d), where=subcommand_name) # action=1 - fatal error, exit
			if local_total_param2d[idx_beautifier_align2d_mirror] == -1: # An Unaccounted Particle
				sp_global_def.ERROR("Invalid alignment parameters of an unaccounted particle is detected at entry #{} in {}. The parameter files might be corrupted. Please consider to rerun Beautifier.".format(accounted_img_id, accounted_local_total_align2d_path), where=subcommand_name) # action=1 - fatal error, exit
	...
	if len(fullstack_total_align2d_list) == 0:
		sp_global_def.ERROR("No alignment parameters are detected. Please check the contents of ISAC or Beautifier run output directory.", where=subcommand_name) # action=1 - fatal error, exit

	if len(accounted_total_align2d_list) == 0:
		sp_global_def.ERROR("No alignment parameters of accounted particles are detected. Please check the contents of ISAC or Beautifier run output directory.", where=subcommand_name) # action=1 - fatal error, exit
	...
	sp_utilities.write_text_row(fullstack_total_align2d_list, fullset_total_align2d_path)
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Saved the total 2D alignment parameters of all particles in original fullstack to {}, using the same format as ISAC 2D alignment file.".format(fullset_total_align2d_path))
	...
	sp_utilities.write_text_row(accounted_total_align2d_list, accounted_total_align2d_path)
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Saved the total 2D alignment parameters of all accounted particles to {}, using the same format as Beautifier 2D alignment file.".format(accounted_total_align2d_path))
	...
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Checking the number of default ISAC class averages {} in ISAC or Beautifier run output directory...".format(default_isac_class_avgs_path))
	n_default_class_avg = 0
	if os.path.exists(default_isac_class_avgs_path):
		n_default_class_avg = EMAN2_cppwrap.EMUtil.get_image_count(default_isac_class_avgs_path)
	else: 
		sp_global_def.sxprint("WARNING! The default ISAC class averages file does not exist.")
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Detected {} default ISAC class averages in {}".format(n_default_class_avg, default_isac_class_avgs_path))
	
	# Retrieve original fullstack particle IDs of members listed in ISAC class averages
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Extracting original fullstack particle IDs of members listed in ISAC class averages...")
	n_class_avg = EMAN2_cppwrap.EMUtil.get_image_count(args.isac_class_avgs_path)

	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Detected {} ISAC class averages in {}".format(n_class_avg, args.isac_class_avgs_path))
	...
	for class_avg_id in range(n_class_avg):
		fullstack_img_id_list_of_isac_class = sp_utilities.get_im(args.isac_class_avgs_path, class_avg_id).get_attr("members")
	...
	sp_utilities.write_text_file( class_membership, class_membership_file_path ) # NOTE: here we only write a single column; later we overwrite this file w/ two columns

	n_isac_substack_img = len(fullstack_img_id_list_of_isac_substack)
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Extracted {} ISAC class members from {}".format(n_isac_substack_img, args.isac_class_avgs_path))
	if not n_isac_substack_img <= n_accounted_img:
		sp_global_def.ERROR("Invalid number of ISAC class members {}. It must be smaller than or equal to the total number of ISAC accounted particles {}. The stack header might be corrupted. Please consider to rerun ISAC.".format(n_isac_substack_img, n_accounted_img), where=subcommand_name) # action=1 - fatal error, exit
	...
	sp_utilities.write_text_file(fullstack_img_id_list_of_isac_substack, fullstack_img_id_path_of_isac_substack)
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Saved original fullstack particle IDs of all members listed in ISAC class averages to {}.".format(fullstack_img_id_path_of_isac_substack))
	
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Converting 2D alignment parameters of all members listed in ISAC class averages to 2D alignment parameters header entry format...")
	...
	sp_utilities.write_text_row(isac_substack_total_header_align2d_list, isac_substack_total_header_align2d_path)
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Saved the converted 2D alignment parameters to {}.".format(isac_substack_total_header_align2d_path))
	...
	if status == 0: 
		sp_global_def.ERROR("\"{}\" execution failed. Exiting...".format(cmd_line), where=subcommand_name) # action=1 - fatal error, exit
	
	# Import the total 2D alignment parameters to xform.align2d
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Importing the total 2D alignment parameters in the original scale to the header entry...")
	...
	status = sp_utilities.cmdexecute(cmd_line)
	if status == 0: sp_global_def.ERROR("\"{}\" execution failed. Exiting...".format(cmd_line), where=subcommand_name) # action=1 - fatal error, exit
	
	# Transform xform.align2d to xform.projection
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Creating projection parameters header entry from imported 2D alignment parameters using 2D-to-3D transformation...")
	...
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Importing class membership information (also found in file \'particle_membership.txt\')...")
	cmd_line = "sp_header.py {} --import={} --params={}".format( virtual_bdb_substack_path, # target stack
	...
	sp_global_def.sxprint("  ISAC substack size      : %6d"%(EMAN2_cppwrap.EMUtil.get_image_count(virtual_bdb_substack_path)))
	sp_global_def.sxprint(" ")
...
def resample_micrographs(args):
	...
	# ------------------------------------------------------------------------------------
	if SXmpi_run.RUNNING_UNDER_MPI:
		pass#IMPORTIMPORTIMPORT from mpi import MPI_COMM_WORLD, mpi_barrier, mpi_reduce, MPI_INT, MPI_SUM
	...
	while True:
		...
		if error_status is None and mic_pattern is None:
			error_status = ("Missing required argument input_micrograph_pattern. Please run %s -h for help." % (program_name), inspect.getframeinfo(inspect.currentframe()))
			break
		...
		if error_status is None and root_out_dir is None:
			error_status = ("Missing required argument output_directory. Please run %s -h for help." % (program_name), inspect.getframeinfo(inspect.currentframe()))
			break
		...
		if error_status is None and args.resample_ratio is None:
			error_status = ("Missing required option --resample_ratio. Please run %s -h for help." % (program_name), inspect.getframeinfo(inspect.currentframe()))
			break
		
		if error_status is None and (args.resample_ratio <= 0.0 or args.resample_ratio >= 1.0):
			error_status = ("Invalid option value: --resample_ratio=%s. Please run %s -h for help." % (args.resample_ratio, program_name), inspect.getframeinfo(inspect.currentframe()))
			break
	...
	while SXmpi_run.is_main_proc():
		...
		if args.selection_list == None:
			...
			if os.path.splitext(args.selection_list)[1] == ".txt":
				...
				sp_global_def.sxprint("Found %d microgarph entries in %s." % (len(selected_mic_path_list), args.selection_list))
				...
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("Processing a single micrograph: %s..." % (args.selection_list))
			...
			if selected_mic_directory != "":
				sp_global_def.sxprint("    NOTE: Program disregards the directory paths in the selection list (%s)." % (selected_mic_directory))
		...
		for mic_id_substr in global_entry_dict:
			...
			if subkey_selected_mic_basename in mic_id_entry: 
				...
				if len(warinnig_messages) > 0:
					sp_global_def.sxprint("WARNING!!! Micrograph ID %s does not have:" % (mic_id_substr))
		...
		if args.check_consistency:
			...
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Generating consistency report of the provided dataset in %s..."%(mic_consistency_check_info_path))
		...
		sp_global_def.sxprint("Detected                           : %6d" % (len(global_entry_dict)))
		sp_global_def.sxprint("Valid                              : %6d" % (len(valid_mic_id_substr_list)))
	...
	if SXmpi_run.RUNNING_UNDER_MPI:
		...
		global_entry_dict = sp_utilities.wrap_mpi_bcast(global_entry_dict, SXmpi_run.main_mpi_proc)
		valid_mic_id_substr_list = sp_utilities.wrap_mpi_bcast(valid_mic_id_substr_list, SXmpi_run.main_mpi_proc)
		
		# Slice the list of valid micrograph id substrings for this mpi process
		mic_start, mic_end = sp_applications.MPI_start_end(len(valid_mic_id_substr_list), SXmpi_run.n_mpi_procs, SXmpi_run.my_mpi_proc_id)
		valid_mic_id_substr_list = valid_mic_id_substr_list[mic_start:mic_end]
		
	if SXmpi_run.is_main_proc():
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Micrographs processed by main process (including percent of progress):")
	...
	for mic_id_substr_idx, mic_id_substr in enumerate(valid_mic_id_substr_list):
		...
		if SXmpi_run.is_main_proc():
			sp_global_def.sxprint("%s ---> % 2.2f%%" % (mic_basename, mic_id_substr_idx / progress_percent_step))
		...
		try:
			...
			sp_global_def.sxprint("Failed to read the associate micrograph %s for %s. The file might be corrupted. Skipping..." % (mic_path, mic_basename))
			continue
	...
	# ------------------------------------------------------------------------------------
	if SXmpi_run.RUNNING_UNDER_MPI:
		n_mic_process = mpi.mpi_reduce(n_mic_process, 1, mpi.MPI_INT, mpi.MPI_SUM, SXmpi_run.main_mpi_proc, mpi.MPI_COMM_WORLD)
	
	# Print out the summary of all micrographs
	if SXmpi_run.is_main_proc():
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Summary of processing...")
		sp_global_def.sxprint("Valid                              : %6d" % (len(unsliced_valid_serial_id_list)))
		sp_global_def.sxprint("Processed                          : %6d" % (n_mic_process))
...
def organize_micrographs(args):
	...
	if src_mic_pattern.find("*") == -1:
		sp_global_def.ERROR("The source micrograph path pattern must contain wild card (*). Please correct source_micrograph_pattern argument and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	
	if os.path.splitext(select_list_path)[1] != ".txt":
		sp_global_def.ERROR("The extension of source micrograph selecting list file must \'.txt\'. Please choose a correct file path or change the file extension, then restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	
	if not os.path.exists(select_list_path):
		sp_global_def.ERROR("The micrograph selecting list file does not exist. Please choose a correct file path and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	...
	if os.path.exists(dst_dir):
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("The destination directory (%s) already exists. "%(dst_dir))
	...
	if len(src_mic_path_list) == 0:
		sp_global_def.ERROR("No micrograph files are found in the directory specified by the micrograph path pattern (%s). Please check source_micrograph_pattern argument and restart the program."%(src_dir), where=subcommand_name) # action=1 - fatal error, exit
	...
	# Check error condition of micrograph entry lists
	sp_global_def.sxprint("Found %d microgarph entries in %s."%(len(select_mic_path_list), select_list_path))
	if len(select_mic_path_list) == 0:
		sp_global_def.ERROR("No micrograph entries are found in the selection list file (%s). Please correct selection_list option and restart the program."%(select_list_path), where=subcommand_name) # action=1 - fatal error, exit
	
	select_mic_dir = os.path.dirname(select_mic_path_list[0])
	if select_mic_dir != "":
		sp_global_def.sxprint("    NOTE: Program disregards the directory paths in the source selection list (%s)."%(select_mic_dir))
	...
	if dst_mic_pattern is None:
		...
		for mic_id_substr in global_entry_dict:
			...
			if subkey_select_mic_basename in mic_id_entry: 
				...
				if len(warinnig_messages) > 0:
					sp_global_def.sxprint("WARNING!!! Micrograph ID %s has problems with consistency among the provided dataset:"%(mic_id_substr))
					for warinnig_message in warinnig_messages:
						sp_global_def.sxprint(warinnig_message)
					sp_global_def.sxprint("    Ignores this as an invalid entry.")
				else:
		...
		sp_global_def.sxprint("Detected                           : %6d"%(len(global_entry_dict)))
		sp_global_def.sxprint("Valid                              : %6d"%(len(valid_mic_id_substr_list)))
		sp_global_def.sxprint("Rejected by no source micrographs  : %6d"%(len(no_src_mic_id_substr_list)))
		sp_global_def.sxprint(" ")
		...
		sp_global_def.sxprint("Detected                           : %6d"%(len(global_entry_dict)))
		sp_global_def.sxprint("Valid                              : %6d"%(len(valid_mic_id_substr_list)))
		sp_global_def.sxprint("Rejected by not found in both dirs : %6d"%(len(no_mic_in_both_dirs_id_substr_list)))
		sp_global_def.sxprint("Rejected by already in dst dir     : %6d"%(len(already_in_dst_dir_mic_id_substr_list)))
		sp_global_def.sxprint("Rejected by duplicated in dst dir  : %6d"%(len(duplicated_in_dst_dir_mic_id_substr_list)))
		sp_global_def.sxprint(" ")
		...
		if len(duplicated_in_dst_dir_mic_id_substr_list) > 0:
			...
			sp_global_def.sxprint("Storing the list of duplicated micrographs in %s."%(duplicated_mic_list_path))
			sp_global_def.sxprint(" ")
	...
	if len(valid_mic_id_substr_list) > 0:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Moving micrographs in the selecting list (%s) from the source directory (%s) to the destination directory (%s)..."%(select_list_path, src_dir, dst_dir))
...
def restacking(args):
	...
	# import math
	pass#IMPORTIMPORTIMPORT from EMAN2db   import db_check_dict
	pass#IMPORTIMPORTIMPORT from sp_utilities import get_im, get_params_proj
	...
	# Check error conditions of arguments
	if not EMAN2db.db_check_dict(args.input_bdb_stack_path, readonly=True):
		sp_global_def.ERROR("Input BDB image stack file does not exist. Please check the input stack path and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	if os.path.exists(args.output_directory):
		sp_global_def.ERROR("Output directory exists. Please change the name and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	
	# Check error conditions of options
	if args.selection_list is not None:
		if not os.path.exists(args.selection_list):
			sp_global_def.ERROR("Micrograph selecting list does not exist. Please check the micrograph selecting list path and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
		if os.path.splitext(args.selection_list)[1] != ".txt":
			sp_global_def.ERROR("The extention of micrograph selecting list file must be .txt. Please check the micrograph selecting list path and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
		args.sv_vstack_basename = args.sv_vstack_basename.strip()
		if args.sv_vstack_basename == "":
			sp_global_def.ERROR("Output virtual stack basename cannot be empty string or only white spaces.", where=subcommand_name) # action=1 - fatal error, exit
	if not args.reboxing:
		...
		if abs(args.shift3d_x) >= img_size//2:
			sp_global_def.ERROR("Invalid 3D x-shift {}. 3D x-shift must be smaller than half of image size {}. Please provide valid values for x-shift and/or box size. Then, restart the program.".format(args.shift3d_x, args.img_size//2), where=subcommand_name) # action=1 - fatal error, exit
		if abs(args.shift3d_y) >= img_size//2:
			sp_global_def.ERROR("Invalid 3D y-shift {}. 3D y-shift must be smaller than half of image size {}. Please provide valid values for y-shift and/or box size. Then, restart the program.".format(args.shift3d_y, args.img_size//2), where=subcommand_name) # action=1 - fatal error, exit
		if abs(args.shift3d_z) >= img_size//2:
			sp_global_def.ERROR("Invalid 3D z-shift {}. 3D z-shift must be smaller than half of image size {}. Please provide valid values for z-shift and/or box size. Then, restart the program.".format(args.shift3d_z, args.img_size//2), where=subcommand_name) # action=1 - fatal error, exit
	if args.reboxing:
		if args.rb_box_size <= 0:
			sp_global_def.ERROR("Invalid box size {}. Box size must be larger than zero. Please provide valid box size and restart the program.".format(args.rb_box_size), where=subcommand_name) # action=1 - fatal error, exit
		if abs(args.shift3d_x) >= args.rb_box_size//2:
			sp_global_def.ERROR("Invalid 3D x-shift {}. 3D x-shift must be smaller than half of box size {}. Please provide valid values for x-shift and/or box size. Then, restart the program.".format(args.shift3d_x, args.rb_box_size//2), where=subcommand_name) # action=1 - fatal error, exit
		if abs(args.shift3d_y) >= args.rb_box_size//2:
			sp_global_def.ERROR("Invalid 3D y-shift {}. 3D y-shift must be smaller than half of box size {}. Please provide valid values for y-shift and/or box size. Then, restart the program.".format(args.shift3d_y, args.rb_box_size//2), where=subcommand_name) # action=1 - fatal error, exit
		if abs(args.shift3d_z) >= args.rb_box_size//2:
			sp_global_def.ERROR("Invalid 3D z-shift {}. 3D z-shift must be smaller than half of box size {}. Please provide valid values for z-shift and/or box size. Then, restart the program.".format(args.shift3d_z, args.rb_box_size//2), where=subcommand_name) # action=1 - fatal error, exit
	...
	if args.selection_list is not None:
		...
		sp_global_def.sxprint("----- Running in Selected Micrographs Mode -----")
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Checking the selection list {}...".format(args.selection_list))
		selected_mic_path_list = sp_utilities.read_text_file(args.selection_list)
		
		# Check error condition of micrograph entry lists
		sp_global_def.sxprint("Found %d microgarph entries in %s." % (len(selected_mic_path_list), args.selection_list))
		if len(selected_mic_path_list) == 0:
			sp_global_def.ERROR("No micrograph entries are found in the selection list file. Please check the micrograph selecting list and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
		if not isinstance(selected_mic_path_list[0], str):
			sp_global_def.ERROR("Invalid format of the selection list file. The first column must contain micrograph paths in string type. Please check the micrograph selecting list and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
		
		selected_mic_directory = os.path.dirname(selected_mic_path_list[0])
		if selected_mic_directory != "":
			sp_global_def.sxprint("    NOTE: Program disregards the directory paths in the selection list ({}).".format(selected_mic_directory))
		
		# Register micrograph basename found in the selection list
		# to the global entry dictionary
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Registering all micrographs in the selection list {}...".format(args.selection_list))
		for mic_path in selected_mic_path_list:
			mic_basename = os.path.basename(mic_path)
			if mic_basename in global_mic_dict:
				sp_global_def.sxprint("WARNING!!! Micrograph {} is duplicated in the selection list {}. Ignoring this duplicated entry...".format(mic_path, args.selection_list))
				continue
	...
	# --------------------------------------------------------------------------------
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Checking the input stack {}...".format(args.input_bdb_stack_path))
	n_img = EMAN2_cppwrap.EMUtil.get_image_count(args.input_bdb_stack_path)
	# sxprint(" ")
	sp_global_def.sxprint("Found {} particle images in the input stack {}".format(n_img, args.input_bdb_stack_path))
	...
	for img_id in range(n_img):
		...
		if img.has_attr("ctf"):
			defocus, cs, voltage, apix, bfactor, ampcont, dfdiff, dfang = sp_utilities.get_ctf(img)
		else:
		...
		if img.has_attr("xform.projection"):
			proj_phi, proj_theta, proj_psi, proj_tx, proj_ty = sp_utilities.get_params_proj(img)
		else:
		...
		# Transform the coordinates according to projection parameters and user-provided 3D shift (corresponding to shifting the 3D volume)
		trans3x3 = EMAN2_cppwrap.Transform({"phi":float(proj_phi), "theta":float(proj_theta), "psi":float(proj_psi), "tx":float(resampled_proj_tx), "ty":float(resampled_proj_ty), "tz":0.0, "type":"spider"})
		origin_vec3d = EMAN2_cppwrap.Vec3f(float(resampled_shift3d_x), float(resampled_shift3d_y), float(resampled_shift3d_z))
		transformed_vec3d = trans3x3 * origin_vec3d
		shift2d_x = -1 * transformed_vec3d[0]
		shift2d_y = -1 * transformed_vec3d[1]
...
#	sxprint("Found total of {} assocaited micrographs in the input stack {}.".format(len(global_mic_dict), args.input_bdb_stack_path))
	...
	if missing_ctf_params_counter > 0:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("WARNING!!! The CTF parameters (ctf header entry) are missing from {} out of {} particle images in the input stack {}.".format(missing_proj_params_counter, n_img, args.input_bdb_stack_path))
		sp_global_def.sxprint("           The program automatically sets dummy CTF parameters for these particle images.")
		
	if missing_proj_params_counter > 0:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("WARNING!!! The projection parameters (xform.projection header entry) are missing from {} out of {} particle images in the input stack {}.".format(missing_proj_params_counter, n_img, args.input_bdb_stack_path))
		sp_global_def.sxprint("           The program automatically sets the projection parameters to all zeros (null alignment).")
	...
	for mic_basename in global_mic_dict:
		...
		if mic_entry.is_in_stack:
			...
			if mic_entry.is_in_list:
				...
				sp_global_def.sxprint("    Micrograph {} is in the stack but not in the selection list.".format(mic_basename))
		else:
			if mic_entry.is_in_list:
				sp_global_def.sxprint("    Micrograph {} is in the selection list but not in the stack.".format(mic_basename))
	
	os.mkdir(args.output_directory)
	sp_global_def.write_command(args.output_directory)

	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Found total of {} micrographs in the input stack {}.".format(len(mic_basename_list_of_input_stack), args.input_bdb_stack_path))
	...
	if args.selection_list is not None:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Found total of {} valid micrographs for the output.".format(len(mic_basename_list_of_output_stack)))
	...
	sp_global_def.sxprint("Saving the list of micrographs found in the input stack {} to {}...".format(args.input_bdb_stack_path, input_mic_list_file_path))
	...
	sp_global_def.sxprint("Saving the list of valid micrograph names for the output to {}...".format(output_mic_list_file_path))
	...
	for mic_basename in mic_basename_list_of_output_stack:
		...
		sp_global_def.sxprint(" {:6d} particles in {}...".format(len(mic_entry.img_id_list), mic_basename))
		#if args.reboxing:
	...
	sp_global_def.sxprint("Saving the list of input stack particle IDs for the output dataset to {}...".format(output_particle_id_list_file_path))
	...
	sp_global_def.sxprint("Num. of extracted micrographs in selection list       : {:6d}".format(len(selected_mic_path_list)))
	sp_global_def.sxprint("Num. of extracted micrographs in input stack          : {:6d}".format(len(mic_basename_list_of_input_stack)))
	sp_global_def.sxprint("Saved input stack micrograph list to                  : {}".format(input_mic_list_file_path))
	sp_global_def.sxprint("Num. of valid micrographs for output dataset          : {:6d}".format(len(mic_basename_list_of_output_stack)))
	sp_global_def.sxprint("Saved output dataset micrograph list to               : {}".format(output_mic_list_file_path))
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Num. of detected input stack particles                : {:6d}".format(n_img))
	sp_global_def.sxprint("Num. of particle IDs in output dataset                : {:6d}".format(len(global_output_image_id_list)))
	sp_global_def.sxprint("Saved particle ID list of output dataset to           : {}".format(output_particle_id_list_file_path))
	sp_global_def.sxprint("Num. of CTF params in output dataset                  : {:6d}".format(global_ctf_params_counters))
	sp_global_def.sxprint("Saved CTF params list of output dataset to            : {}".format(ctf_params_list_file_path))
	sp_global_def.sxprint("Num. of original proj. params in output dataset       : {:6d}".format(global_original_proj_params_counters))
	sp_global_def.sxprint("Saved original proj. params list of output dataset to : {}".format(original_proj_params_list_file_path))
	sp_global_def.sxprint("Num. of ceneterd proj. params in output dataset       : {:6d}".format(global_centered_proj_params_counters))
	sp_global_def.sxprint("Saved ceneterd proj. params list of output dataset to : {}".format(centered_proj_params_list_file_path))
	if args.reboxing:
		sp_global_def.sxprint("Num. of original coordinates in output dataset        : {:6d}".format(global_original_coords_counters))
		sp_global_def.sxprint("Saved original coordinates files in                   : {}".format(os.path.join(args.output_directory, original_coords_list_subdir)))
		sp_global_def.sxprint("Num. of original rebox coordinates in output dataset  : {:6d}".format(global_original_rebox_coords_counters))
		sp_global_def.sxprint("Saved original rebox coordinates files in             : {}".format(os.path.join(args.output_directory, original_rebox_coords_list_subdir)))
		sp_global_def.sxprint("Num. of centered coordinates in output dataset        : {:6d}".format(global_centered_coords_counters))
		sp_global_def.sxprint("Saved centered coordinates files in                   : {}".format(os.path.join(args.output_directory, centered_coords_list_subdir)))
		sp_global_def.sxprint("Num. of centered rebox coordinates in output dataset  : {:6d}".format(global_centered_rebox_coords_counters))
		sp_global_def.sxprint("Saved centered rebox coordinates files in             : {}".format(os.path.join(args.output_directory, centered_rebox_coords_list_subdir)))
	if args.save_vstack:
		sp_global_def.sxprint("Save output stack as                                  : {}".format(virtual_bdb_stack_path))
...
def moon_eliminator(args):
	...
	if not os.path.exists(args.input_volume_path):
		sp_global_def.ERROR("Input volume file {} does not exist. Please check the file path and restart the program.".format(args.input_volume_path), where=subcommand_name) # action=1 - fatal error, exit
	...
	if args.input_volume_path_2nd is not None:
		...
		if not os.path.exists(args.input_volume_path_2nd):
			sp_global_def.ERROR("Second input volume file {} does not exist. Please check the file path and restart the program.".format(args.input_volume_path_2nd), where=subcommand_name) # action=1 - fatal error, exit
	
	args.output_directory = args.output_directory.strip()
	if os.path.exists(args.output_directory):
		sp_global_def.ERROR("Output directory {} exists. Please change the name and restart the program.".format(args.output_directory), where=subcommand_name) # action=1 - fatal error, exit
	
	# Check error conditions of options
	if args.pixel_size is None:
		sp_global_def.ERROR("Pixel size [A] is required. Please set a pasitive value larger than 0.0 to --pixel_size option.", where=subcommand_name) # action=1 - fatal error, exit
	else:
		if args.pixel_size <= 0.0:
			sp_global_def.ERROR("Invalid pixel size {}[A]. Please set a pasitive value larger than 0.0 to --pixel_size option.".format(args.pixel_size), where=subcommand_name) # action=1 - fatal error, exit
	
	nyquist_res = args.pixel_size * 2
	
	if args.mol_mass is None and args.use_density_threshold is None:
		sp_global_def.ERROR("Molecular mass [kDa] or density_threshold is required. Please set a pasitive value larger than 0.0 to --mol_mass option or --use_density_threshold option.", where=subcommand_name) # action=1 - fatal error, exit
	elif args.mol_mass is not None and args.use_density_threshold is not None:
		sp_global_def.ERROR("You provided values for both --mol_mass and --use_density_threshold. --mol_mass is used and --use_density_threshold ignored.", where=subcommand_name , action=0)
		...
		if args.mol_mass <= 0.0:
			sp_global_def.ERROR("Invalid molecular mass {}[A]. Please set a pasitive value larger than 0.0 to --mol_mass option.".format(args.mol_mass), where=subcommand_name) # action=1 - fatal error, exit
	...
	try:
		...
		if not os.path.exists(args.resample_ratio):
			sp_global_def.ERROR("Specified ISAC2 run output directory {} does not exist. Please check --resample_ratio option.".format(args.resample_ratio), where=subcommand_name) # action=1 - fatal error, exit
		
		isac_shrink_path = os.path.join(args.resample_ratio, "README_shrink_ratio.txt")
		if not os.path.exists(isac_shrink_path):
			sp_global_def.ERROR("{} does not exist in the specified ISAC2 run output directory. Please check ISAC2 run directory and --resample_ratio option.".format(isac_shrink_path), where=subcommand_name) # action=1 - fatal error, exit
	else:
		if float(args.resample_ratio) <= 0.0:
			sp_global_def.ERROR("Invalid resample ratio {}. Please set a value larger than 0.0 to --resample_ratio option.".format(args.resample_ratio), where=subcommand_name) # action=1 - fatal error, exit
	
	if args.box_size is not None:
		if args.box_size <= 0.0:
			sp_global_def.ERROR("Invalid box size {}[Pixels]. Please set a pasitive value larger than 0 to --box_size option.".format(args.box_size), where=subcommand_name) # action=1 - fatal error, exit
		
	if args.fl != -1.0:
		if args.fl < nyquist_res:
			sp_global_def.ERROR("Invalid low-pass filter resolution {}[A] for 3D volume. Please set a value larger than or equal to Nyquist resolution {}[A].".format(args.fl, nyquist_res), where=subcommand_name) # action=1 - fatal error, exit
	
	args.outputs_root = args.outputs_root.strip()
	if args.outputs_root == "":
		sp_global_def.ERROR("Root name of outputs cannot be empty string or only white spaces.", where=subcommand_name) # action=1 - fatal error, exit
	...
	if args.input_volume_path_2nd:
		EMAN2_cppwrap.Util.add_img(vol3d, sp_utilities.get_im(args.input_volume_path_2nd))
	...
	if isac_shrink_path is not None:
		...
		isac_shrink_file.close()
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("ISAC2 run directory path is specified with --resample_ratio option...")
		sp_global_def.sxprint("Extracted parameter values")
		sp_global_def.sxprint("  ISAC shrink ratio    : {}".format(isac_shrink_ratio))
		sp_global_def.sxprint("  ISAC particle radius : {}".format(isac_radius))
		...
		if resample_ratio != 1.0:
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Resample ratio {} is specified with --resample_ratio option...".format(resample_ratio))
		else:
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Resample ratio is {}. The program does not resample the input volume...".format(resample_ratio))
	...
	if resample_ratio != 1.0:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Resampling the input volume with resample ratio {}...".format(resample_ratio))
		vol3d = sp_fundamentals.resample(vol3d, resample_ratio)
		
		vol3d_dims = vol3d.get_xsize()
		sp_global_def.sxprint("  Dimensions of resampled 3D volume : {}".format(vol3d_dims))
	...
	if args.box_size is not None:
		if args.box_size != vol3d_dims:
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Adjusting the dimensions of 3D volume to {}...".format(args.box_size))
			if args.box_size > vol3d_dims:
				vol3d = EMAN2_cppwrap.Util.pad(vol3d, args.box_size, args.box_size, args.box_size, 0, 0, 0, "circumference")
			else:
				vol3d = EMAN2_cppwrap.Util.window(vol3d, args.box_size, args.box_size, args.box_size, 0, 0, 0)

		vol3d_dims = vol3d.get_xsize()
		sp_global_def.sxprint("  The dimensions of adjusted 3D volume : {}".format(vol3d_dims))
	...
	if not (args.shift3d_x == 0 and args.shift3d_y == 0 and args.shift3d_z == 0):
		...
		if not args.resampled_shift3d and resample_ratio != 1.0:
			sp_global_def.sxprint("  Resampling provided 3D shift (x, y, z) = ({}, {}, {}) with resample ratio {}...".format(args.shift3d_x, args.shift3d_y, args.shift3d_z, resample_ratio))
			args.shift3d_x *= resample_ratio
			args.shift3d_y *= resample_ratio
			args.shift3d_z *= resample_ratio
		sp_global_def.sxprint("  Applying 3D shift (x, y, z) = ({}, {}, {}) to the volume...".format(args.shift3d_x, args.shift3d_y, args.shift3d_z))
		vol3d = sp_fundamentals.rot_shift3D(vol3d, sx = args.shift3d_x, sy = args.shift3d_y, sz = args.shift3d_z)
	...
	if args.fl != -1.0:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Low-pass filtration of the input volume using cutoff resolution {}[A] and fall-off {}[1/Pixels]...".format(args.fl, args.aa))
		vol3d = sp_filter.filt_tanl(vol3d, args.pixel_size/args.fl, args.aa)
	else:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("The program does not low-pass filter input volume...".format(args.fl))
	...
	if args.use_density_threshold is None:
		...
		sp_global_def.sxprint("The density threshold corresponing to the specified molecular mass {}[kDa] and pixel size {}[A/Pixels] is  {}".format(args.mol_mass, args.pixel_size,density_threshold))
	else:
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Using user-provided density threshold {}...".format(args.use_density_threshold))
		density_threshold = args.use_density_threshold

	# Eliminate moons
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Eliminating moons of the input volume using density threshold of {} with {} edge...".format(density_threshold, args.edge_type))
	...
	if( volume_difference.get_value_at(volume_difference.calc_max_index()) != 0 or \
		...
		mask_no_moon = sp_morphology.adaptive_mask(my_volume_binarized_with_no_moons, 0.0, 0.5, args.ndilation, args.edge_width, mode)
	...
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Saving moon eliminated 3D reference {}...".format(ref3d_moon_eliminated_file_path))
...
def desymmetrize(args):
	...
	if args.input_bdb_stack_path[:len("bdb:")].lower() != "bdb:":
		sp_global_def.ERROR("Invalid input BDB stack file path %s.  The path must start with \'bdb:\'. Please check the file path and restart the program." % (args.input_bdb_stack_path), where=subcommand_name) # action=1 - fatal error, exit
	if not EMAN2db.db_check_dict(args.input_bdb_stack_path, readonly=True):
		sp_global_def.ERROR("Input BDB image stack file %s does not exist. Please check the file path and restart the program." % (args.input_bdb_stack_path), where=subcommand_name) # action=1 - fatal error, exit
	if not os.path.exists(args.input_cluster_path):
		sp_global_def.ERROR("Specified input cluster text file %s does not exist. Please check the file path and restart the program." %(args.input_cluster_path), where=subcommand_name) # action=1 - fatal error, exit
	if os.path.exists(args.output_directory):
		sp_global_def.ERROR("Output directory %s exists. Please change the name and restart the program." %(args.output_directory), where=subcommand_name) # action=1 - fatal error, exit
	...
	symmetrized_particle_id_list = sp_utilities.read_text_file(args.input_cluster_path)  # Cluster#.txt
	sp_global_def.sxprint("MRK_DEBUG: len(symmetrized_particle_id_list) := ", len(symmetrized_particle_id_list))
	sp_global_def.sxprint("Detected %d symmetrized particle IDs in the specified cluster text file %s."%(len(symmetrized_particle_id_list), args.input_cluster_path))
	sp_global_def.sxprint(" ")
	...
	sp_utilities.write_text_file(symmetrized_particle_id_list, symmetrized_particle_id_list_file_path)
	...
	input_bdb_stack = EMAN2db.db_open_dict(args.input_bdb_stack_path, ro=True) # Read only
	
	n_img_detected = EMAN2_cppwrap.EMUtil.get_image_count(args.input_bdb_stack_path)
	sp_global_def.sxprint("Detected %d particles in symmetrized input BDB stack %s."%(n_img_detected, args.input_bdb_stack_path))
	sp_global_def.sxprint(" ")
	...
	try: 
		...
		sp_global_def.ERROR("Failed to read image header of particle #%d from %s. Aborting..."%(symmetrized_particle_id, args.input_bdb_stack_path), where=subcommand_name) # action=1 - fatal error, exit
	
	symmetry_type = ""
	n_symmetry = 0
	n_presymmetriezed_img = 0
	if "variabilitysymmetry" in img_header:
		sp_global_def.sxprint("Detected %s point-group symmetry in specified input BDB stack %s."%(img_header["variabilitysymmetry"], args.input_bdb_stack_path))
		sp_global_def.sxprint(" ")
		
		symmetry_type = img_header["variabilitysymmetry"][:1].lower()
		if symmetry_type != "c":
			sp_global_def.ERROR("Unsupported point-group symmetries. Other than cn are not supported yet.", where=subcommand_name) # action=1 - fatal error, exit
		n_symmetry = int(img_header["variabilitysymmetry"][-1:])
		if n_symmetry < 2:
			sp_global_def.ERROR("Point-group symmetry have to be higher than c1.", where=subcommand_name) # action=1 - fatal error, exit
	else:
		sp_global_def.ERROR("Specified input BDB stack is not symmetrized. Please choose a symmetrized stack and restart the program.", where=subcommand_name) # action=1 - fatal error, exit
	
	
	n_presymmetriezed_img = n_img_detected // n_symmetry
	sp_global_def.sxprint("The computed number of particles in the pre-symmetrized stack is %d."%(n_presymmetriezed_img))
	
	if len(symmetrized_particle_id_list) > n_presymmetriezed_img:
		sp_global_def.ERROR("Input symmetrized particle ID list contains more entries (%d) than expected (%d)."%(len(symmetrized_particle_id_list), n_presymmetriezed_img), where=subcommand_name, action=0) # action = 0 - non-fatal, print a warning;
	...
	for i_img_detected, symmetrized_particle_id in enumerate(symmetrized_particle_id_list):
		...
		if i_img_detected % 1000 == 0:
			try:
				sp_global_def.sxprint("Progress %5.2f%%: Processing %6dth entry (Symmetrized Particle ID %6d)."%(float(i_img_detected)/n_img_detected*100.0, i_img_detected, symmetrized_particle_id))
				sys.stdout.flush()
			except:
				pass
		...
		if args.check_duplication:
			if desymmetrized_particle_id in desymmetrized_particle_id_info_dict:
				sp_global_def.sxprint("WARNING!!! Desymmetrized particle ID %d is duplicated."%(desymmetrized_particle_id))
			else:
	...
	if args.check_duplication:
		...
		for desymmetrized_particle_id in desymmetrized_particle_id_info_dict:
			if len(desymmetrized_particle_id_info_dict[desymmetrized_particle_id]) > 1:
				sp_global_def.sxprint("- Desymmetrized Particle ID %d"%(desymmetrized_particle_id))
				for desymmetrized_particle_id_info in desymmetrized_particle_id_info_dict[desymmetrized_particle_id]:
					sp_global_def.sxprint("-- Symmetrized Particle ID %d in %s (%d) "%(desymmetrized_particle_id_info[1], desymmetrized_particle_id_info[2], desymmetrized_particle_id_info[3]))
		sp_global_def.sxprint(" ")
	...
	sp_utilities.write_text_file(desymmetrized_particle_id_list, desymmetrized_particle_id_list_file_path)
	...
	sp_utilities.write_text_file(nodupilicated_desymmetrized_particle_id_list, nodupilicated_desymmetrized_particle_id_list_file_path)

	# Print summary of processing
	sp_global_def.sxprint(" ")
	sp_global_def.sxprint("Summary of processing...")
	sp_global_def.sxprint("Symmetrized particle IDs in symmetrized stack               : %6d"%(n_img_detected))
	sp_global_def.sxprint("Symmetrized particle IDs in sorted cluster                  : %6d"%(len(symmetrized_particle_id_list)))
	sp_global_def.sxprint("Desymmetrized particle IDs in sorted cluster                : %6d"%(len(desymmetrized_particle_id_list)))
	sp_global_def.sxprint("No-duplicated desymmetrized particle IDs in sorted cluster  : %6d"%(len(nodupilicated_desymmetrized_particle_id_list)))
	sp_global_def.sxprint(" ")
...
def main():
	...
	parser.add_argument("--version", action="version", version=sp_global_def.SPARXVERSION)
