...
def main():
	...
	if len(args) <3 or len(args) > 4:
		...
		sp_global_def.ERROR( "Wrong number of parameters. Please see usage information above." )
		return
	...
	if options.MPI:
		...
		filteredvol = sp_filter.filterlocal(ui, vi, m, options.falloff, myid, main_node, number_of_proc)
		...
		ui = sp_utilities.get_im(args[1])  # resolution volume, values are assumed to be from 0 to 0.5
		...
		while(cutoff < st[3] ):
			...
			pt = EMAN2_cppwrap.Util.infomask( sp_morphology.threshold_outside(ui, cutoff - 0.00501, cutoff + 0.005), m, True)
			if(pt[0] != 0.0):
