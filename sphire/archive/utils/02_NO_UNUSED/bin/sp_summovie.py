...
pass#IMPORTIMPORTIMPORT from sp_global_def import SPARXVERSION, ERROR
pass#IMPORTIMPORTIMPORT from optparse import OptionParser, SUPPRESS_HELP
pass#IMPORTIMPORTIMPORT from sp_utilities import create_summovie_command
...
def main():
    ...
    if not os.path.exists(summovie_path):
        sp_global_def.ERROR( "Summovie directory does not exist, please change the name and restart the program." )
        return

    # If the output directory exists, stop the script
    if os.path.exists(output_dir):
        sp_global_def.ERROR( "Output directory exists, please change the name and restart the program." )
        return
    ...
    if not file_list:
        sp_global_def.ERROR( "Input micrograph file(s) does not exist, please change the name and restart the program." )
        return

    if not shift_list:
        sp_global_def.ERROR( "Input shift file(s) does not exist, please change the name and restart the program." )
        return
    ...
    if len(input_mic_name) != 2 or len(input_shift_name) != 2:
        sp_global_def.ERROR( "Too many wildcard arguments. Please use exactly one \'*\' in the pattern." )
        return
    ...
    if selection_file:
        ...
        try:
            ...
            sp_global_def.ERROR( "No entrys in micrograph list file \'"+selection_file+"\'" )
            return
        ...
        if len(mic_list) == 0:
            sp_global_def.ERROR( "No files in \'"+selection_file+"\' matched the micrograph file pattern" )
            return
...
def run_summovie( opt ):
    ...
    for index, inputfile in enumerate(sorted(opt['mic_list'])):
        ...
        if clean:
            ...
            sp_global_def.ERROR( 'sum movie error. check the logfile for more information: {0}'.format(log_name), action=0 )
