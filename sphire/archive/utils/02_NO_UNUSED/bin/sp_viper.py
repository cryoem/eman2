...
pass#IMPORTIMPORTIMPORT from sp_utilities import if_error_then_all_processes_exit_program, write_text_row, drop_image, model_gauss_noise, get_im, set_params_proj, wrap_mpi_bcast, model_circle, bcast_number_to_all
...
"""
	...
	##### XXXXXXXXXXXXXXXXXXXXXX option does not exist in docs XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	parser.add_option("--return_options", action="store_true", dest="return_options", default=False, help = optparse.SUPPRESS_HELP)	
	...
	if mpi_rank == 0:
		if mpi_size % options.nruns != 0:
			sp_global_def.ERROR( "Number of processes needs to be a multiple of total number of runs. Total runs by default are 3, you can change it by specifying --nruns option.", action=0 )
			error = 1

		if optparse.os.path.exists(outdir):
			sp_global_def.ERROR( "Output directory \'%s\' exists, please change the name and restart the program"%outdir, action=0 )
			error = 1
		pass#IMPORTIMPORTIMPORT import sp_global_def
	...
	error = sp_utilities.bcast_number_to_all(error, source_node = 0, mpi_comm = mpi.MPI_COMM_WORLD)
	if error == 1 :
		return
	...
	options.an  = -1.0
	pass#IMPORTIMPORTIMPORT from sp_multi_shc import multi_shc
	out_params, out_vol, out_peaks = sp_multi_shc.multi_shc(all_projs, subset, runs_count, options, mpi_comm=mpi.MPI_COMM_WORLD, log=log)
