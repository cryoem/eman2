...
def mrk_resample2d(img, sub_rate, target_size = None):
	...
	"""
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import subsample, prepi
	...
	if sub_rate == 1.0: 
		...
		resample_img, kb = sp_fundamentals.prepi(EMAN2_cppwrap.Util.pad(img, nn, nn, 1, 0, 0, 0, "circumference"))
...
"""
	...
	while my_mpi_proc_id == main_mpi_proc:
		...
		if options.selection_list == None:
			...
			if os.path.splitext(options.selection_list)[1] == ".txt":
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("----- Running with Selected Micrographs Mode -----")
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("Checking the selection list...")
				assert (os.path.exists(options.selection_list))
				selected_mic_path_list = sp_utilities.read_text_file(options.selection_list)
				
				# Check error condition of micrograph entry lists
				sp_global_def.sxprint("Found %d microgarph entries in %s." % (len(selected_mic_path_list), options.selection_list))
				...
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("Processing a single micrograph: %s..." % (options.selection_list))
		...
		if ctf_params_src is not None:
			if use_real_ctf_params:
				# This should be string for CTER partres (CTF parameter) file
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("The program will use the CTF parameters stored in the CTER partres file specified through -swap_ctf_params option, while ignoring the CTF parameters in the input rebox parameters file...")
				sp_global_def.sxprint("  It will also ignores all defocus errors extracted from the input rebox paramters file and reset it to zero...")
				sp_global_def.sxprint(" ")
				...
				sp_global_def.sxprint("Found %d CTER partres entries in %s." % (len(cter_entry_list), ctf_params_src))
				...
				if cter_mic_directory != "":
					sp_global_def.sxprint("    NOTE: Program disregards the directory paths in the CTER partres file (%s)." % (cter_mic_directory))
				...
				# This should be string for CTER partres (CTF parameter) file
				sp_global_def.sxprint(" ")
				sp_global_def.sxprint("The program will use the simulated ideal CTF with the pixel size %f specified through -swap_ctf_params option, while ignoring the CTF parameters in the input rebox parameters file..." % (float(ctf_params_src)))
				sp_global_def.sxprint("  It will also ignores all defocus errors extracted from the input rebox paramters file and reset it to zero...")
		...
		sp_global_def.sxprint("Rejected by no rebox file : %6d" % (len(no_rebox_mic_id_substr_list)))
		if ctf_params_src is not None:
			if use_real_ctf_params:
				sp_global_def.sxprint("Rejected by no CTER partres entry  : %6d" % (len(no_cter_entry_mic_id_substr_list)))
	...
	box_half = box_size // 2
	mask2d = sp_utilities.model_circle(box_size//2, box_size, box_size) # Create circular 2D mask to Util.infomask of particle images
	...
	if RUNNING_UNDER_MPI:
		...
		global_entry_dict = sp_utilities.wrap_mpi_bcast(global_entry_dict, main_mpi_proc)
		valid_mic_id_substr_list = sp_utilities.wrap_mpi_bcast(valid_mic_id_substr_list, main_mpi_proc)
		
		# Slice the list of valid micrograph id substrings for this mpi process
		mic_start, mic_end = sp_applications.MPI_start_end(len(valid_mic_id_substr_list), n_mpi_procs, my_mpi_proc_id)
	...
	for mic_id_substr_idx, mic_id_substr in enumerate(valid_mic_id_substr_list):
		...
		if (len(rebox_list) == 0):
			sp_global_def.sxprint("For %s, the associate rebox file %s does not contain any entries. Skipping..." % (mic_basename, rebox_path))
			n_mic_reject_no_rebox_entry += 1
			continue
		assert (len(rebox_list) > 0)
		if (len(rebox_list[0]) != n_idx_rebox):
			sp_global_def.sxprint("For %s, the format of the associate rebox file %s is not valid. It should contain %d columns. Skipping..." % (mic_basename, rebox_path, n_idx_rebox))
		...
		# --------------------------------------------------------------------------------
		mic_img = sp_fundamentals.fft(sp_filter.filt_gaussh(mic_img, mic_resample_ratio / box_size))
		...
		for rebox_id in range(len(rebox_list)):
			...
			if rebox_resample_ratio < 1.0:
				...
				if my_mpi_proc_id == main_mpi_proc:
					...
					sp_global_def.sxprint("MRK_DEBUG: BEFORE applying rebox_resample_ratio := {}".format(rebox_resample_ratio))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				ctf_obj.apix *= rebox_resample_ratio
				proj_entry[idx_proj_sx] /= rebox_resample_ratio
				proj_entry[idx_proj_sy] /= rebox_resample_ratio
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: AFTER applying rebox_resample_ratio := {}".format(rebox_resample_ratio))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
			else:
			...
			if mic_resample_ratio < 1.0:
				...
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: ")
					sp_global_def.sxprint("MRK_DEBUG: BEFORE applying mic_resample_ratio := {}".format(mic_resample_ratio))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				...
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: AFTER applying mic_resample_ratio := {}".format(mic_resample_ratio))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("Resample micrograph to pixel size %6.4f [A/Pixels] from %6.4f [A/Pixels] and window segments from resampled micrograph." % (ctf_obj.apix, src_pixel_size))
			else:
			...
			if pp_mag_error_accum != 1.0:
				...
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: ")
					sp_global_def.sxprint("MRK_DEBUG: BEFORE applying pp_mag_error_accum := {}".format(pp_mag_error_accum))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				proj_entry[idx_proj_sx] /= pp_mag_error_accum
				proj_entry[idx_proj_sy] /= pp_mag_error_accum
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: AFTER applying pp_mag_error_accum := {}".format(pp_mag_error_accum))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
			else:
			...
			if( (0 <= x - resampled_box_half) and ( x + resampled_box_half < nx ) and (0 <= y - resampled_box_half) and ( y + resampled_box_half < ny ) ):
				try:
					...
					sp_global_def.sxprint("MRK_DEBUG: ")
					sp_global_def.sxprint("MRK_DEBUG: mic_resample_ratio := {}, box_size := {}, box_half := {}".format(mic_resample_ratio, box_size, box_half))
					sp_global_def.sxprint("MRK_DEBUG: pp_mag_error_accum := {}, resampled_box_size := {}, resampled_box_half := {}".format(pp_mag_error_accum, resampled_box_size, resampled_box_half))
					sp_global_def.sxprint("MRK_DEBUG: nx := {}, ny := {}".format(nx, ny))
					sp_global_def.sxprint("MRK_DEBUG: x := {}, y := {}, x0 := {}, y0 := {}, x-x0 := {}, y-y0 := {}".format(x, y, x0, y0, x-x0, y-y0))
					raise
			else:
			...
			if pp_def_error_accum != 0.0:
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: ")
					sp_global_def.sxprint("MRK_DEBUG: BEFORE applying pp_def_error_accum := {}".format(pp_def_error_accum))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.defocus := {}".format(ctf_obj.defocus))
				ctf_obj.defocus += pp_def_error_accum
				if my_mpi_proc_id == main_mpi_proc:
					sp_global_def.sxprint("MRK_DEBUG: AFTER applying pp_def_error_accum := {}".format(pp_def_error_accum))
					sp_global_def.sxprint("MRK_DEBUG: ctf_obj.defocus := {}".format(ctf_obj.defocus))
			...
			try:
				...
				sp_global_def.sxprint("The standard deviation of the particle image associated with %dth rebox entry in micrograph %s for %s is zero unexpectedly. Skipping..." % (rebox_id, mic_path, mic_basename))
				continue
	...
	if main_mpi_proc == my_mpi_proc_id:
		...
		sp_global_def.sxprint("Rejected by no rebox entries       : %6d" % (n_mic_reject_no_rebox_entry))
		sp_global_def.sxprint("Rejected by invalid rebox format   : %6d" % (n_mic_reject_invalid_rebox_format))
		sp_global_def.sxprint(" ")
		sp_global_def.sxprint("Global summary of rebox level processing...")
		sp_global_def.sxprint("Detected                           : %6d" % (n_global_rebox_detect))
		sp_global_def.sxprint("Processed                          : %6d" % (n_global_rebox_process))
		sp_global_def.sxprint("Rejected by out of boundary        : %6d" % (n_global_rebox_reject_out_of_boundary))
		if n_global_rebox_reject_out_of_boundary > 0:
			assert os.path.exists(reject_out_of_boundary_dir), "MRK_DEBUG"
			sp_global_def.sxprint("    NOTE: Information of rejected rebox by out of boundary are saved in %s files." % (os.path.join(reject_out_of_boundary_dir, os.path.splitext(os.path.basename(rebox_pattern))[0] + "_reject_out_of_boundary.txt" )))
		else:
	...
	if main_mpi_proc == my_mpi_proc_id:
		...
		if RUNNING_UNDER_MPI:
			...
			sp_global_def.sxprint(" ")
			sp_global_def.sxprint("Please execute from the command line :  ", e2bdb_command)
		else:
