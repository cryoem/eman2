...
pass#IMPORTIMPORTIMPORT import sp_filter as sparx_filter
...
def sanity_checks(command_args, input_vol):
	...
	if os.path.isfile(
		...
		if not command_args.overwrite:
			...
			sp_global_def.ERROR( "Output mask already exists! Please provide the overwrite option if you want to overwrite the existing mask." )
	...
	if command_args.second_mask_shape in ('cylinder', 'sphere'):
		...
		if command_args.s_radius > nx // 2 or command_args.s_radius > ny // 2:
			sp_global_def.ERROR( "Provided radius is larger than input image dimensions!" )
	...
	if command_args.second_mask_shape is not None:
		...
		if command_args.s_nx > nx or command_args.s_ny > ny or command_args.s_nz > nz:
			sp_global_def.ERROR( "Provided s_nx, s_ny, s_nz mask dimension is larger than input image dimensions!" )
...
def main():
	...
	if command_args.mol_mass:
		...
		sp_global_def.sxprint('Mask molecular mass translated into binary threshold: ', density_threshold)
	...
	if command_args.second_mask is not None:
		...
		if command_args.s_mol_mass:
			...
			sp_global_def.sxprint('Second mask molecular mass translated into binary threshold: ', s_density_threshold)
