...
def add_ave_varf_MPI(myid, data, mask = None, mode = "a", CTF = False, ctf_2_sum = None, ali_params = "xform.align2d", main_node = 0, comm = -1):
	...
	if get_ctf2: sp_utilities.reduce_EMData_to_root(ctf_2_sum, myid, main_node, comm)
	nima = n
	...
	if myid == main_node:
		...
		if st[2] < 0.0:  sp_global_def.ERROR("Negative variance!", "add_ave_varf_MPI", 1)
	else:
...
def sum_oe(data, mode = "a", CTF = False, ctf_2_sum = None, ctf_eo_sum = False, return_params = False):
	...
	"""
	pass#IMPORTIMPORTIMPORT from sp_utilities    import    model_blank, get_params2D, same_ctf
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import    rot_shift2D, fft
	pass#IMPORTIMPORTIMPORT from copy import deepcopy
	if CTF: sp_global_def.ERROR("DEBUG_TEST","sum_oe",0)
	n      = len(data)
	...
	if CTF:
		...
		if data[0].get_attr_default('ctf_applied', 1) == 1:  sp_global_def.ERROR("data cannot be ctf-applied", "sum_oe", 1)
		if ctf_2_sum:  get_ctf2 = False
		else:          get_ctf2 = True
		for im in range(n):
			...
			if mode == "a":
				...
				ima = sp_fundamentals.rot_shift2D(data[im], alpha, sx, sy, mirror, scale, "quadratic")
				if return_params:
...
def aves_wiener(input_stack, mode="a", SNR=1.0, interpolation_method="linear"):
	...
	pass#IMPORTIMPORTIMPORT from  sp_morphology	import ctf_img
	pass#IMPORTIMPORTIMPORT from  sp_filter		import filt_ctf
	pass#IMPORTIMPORTIMPORT from  sp_utilities		import pad, get_params2D, get_im, model_blank
	...
	for i in range(n):
		...
		if mode == "a":
			...
			oc = sp_fundamentals.rot_shift2D(oc, alpha, sx, sy, mirror, interpolation_method=interpolation_method)
			ctf_params.dfang += alpha
			if mirror == 1:  ctf_params.dfang = 270.0-ctf_params.dfang
		EMAN2_cppwrap.Util.mul_scalar(oc, SNR)
		EMAN2_cppwrap.Util.add_img(ave, oc)
		EMAN2_cppwrap.Util.add_img2(ctf_2_sum, snrsqrt*sp_morphology.ctf_img(nx, ctf_params, ny = ny, nz = 1))
	ctf_2_sum += 1.0
	ave = EMAN2_cppwrap.Util.divn_filter(sp_fundamentals.fft(ave), ctf_2_sum)  # result in Fourier space
	# variance
	...
	for i in range(n):
		...
		ima = EMAN2_cppwrap.Util.divn_filter(sp_filter.filt_ctf(EMAN2_cppwrap.Util.subn_img(sp_fundamentals.fft(ima), sp_filter.filt_ctf(ave, ctf_params, dopad=False)), ctf_params, dopad=False), ctf_2_sum)
...
def varf2d_MPI(myid, data, ave, mask = None, mode = "a", CTF = False, main_node = 0, comm = -1):
	...
	if myid == main_node:
		...
		st = EMAN2_cppwrap.Util.infomask(var, None, True)
		if st[2] < 0.0:  sp_global_def.ERROR("Negative variance!", "varf2_MPI", 1)
...
def fsc_mask(img1, img2, mask = None, w = 1.0, filename=None):
	...
	ny = img1.get_ysize()
	nz = img1.get_zsize()
	if( mask == None):  mask = sp_utilities.model_circle(nx//2, nx, ny, nz)
...
def locres(vi, ui, m, nk, cutoff, step, myid, main_node, number_of_proc):
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities import model_blank, bcast_EMData_to_all, recv_EMData, send_EMData, bcast_number_to_all, info
	pass#IMPORTIMPORTIMPORT from sp_filter import filt_tophatb
	pass#IMPORTIMPORTIMPORT from EMAN2 import rsconvolution
	pass#IMPORTIMPORTIMPORT from sp_morphology import square_root, threshold
		...
		if(myid == main_node):
			for k in range(number_of_proc):
				if(k != main_node):
					...
					tag_node = k+1001
					dis = mpi.mpi_recv(2, mpi.MPI_FLOAT, k, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
			...
			mpi.mpi_send(dis, 2, mpi.MPI_FLOAT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
