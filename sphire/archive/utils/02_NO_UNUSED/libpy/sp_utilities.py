...
def even_angles(
	...
	symmetry_string = string.split(symmetry)[0]
	if symmetry_string[0] == "c":
		...
		sp_global_def.ERROR("even_angles", "Symmetry not supported: " + symmetry_string, 0)
	"""Multiline Comment5"""
...
def even_angles_cd(
	...
	if method == "P":
		...
		z1 = numpy.cos(theta1 * numpy.pi / 180.0)
...
def get_symt(symmetry):
	...
	for q in scl.symangles:
		trans.append(
			EMAN2_cppwrap.Transform({"type": "spider", "phi": q[0], "theta": q[1], "psi": q[2]})
		)
	return trans
...
def get_input_from_string(str_input):
	...
	"""
	pass#IMPORTIMPORTIMPORT from re import split
...
def print_list_format(m, narray=0):
	...
	if narray > len(m):
		narray = 0
		sp_global_def.ERROR("improper input narray number, use default value", "print_list_foramt", 0)
	if narray == 0:
...
def chooseformat(t, form_float="  %12.5f"):
	pass#IMPORTIMPORTIMPORT import string
	pass#IMPORTIMPORTIMPORT from string import replace, strip, split, atoi

	e_form = string.replace(form_float, "f", "e")
	ee = string.strip(form_float) % t
	if len(ee) > string.atoi(inspect.string.split(string.strip(form_float), ".")[0][1:]):
		return e_form
	df1 = float(ee)
	df2 = float(string.strip(e_form) % t)
...
def write_text_row(data, file_name, form_float="  %14.6f", form_int="  %12d"):
	...
	"""
	pass#IMPORTIMPORTIMPORT import types
	pass#IMPORTIMPORTIMPORT from string import find
...
def estimate_3D_center_MPI(data, nima, myid, number_of_proc, main_node, mpi_comm=None):
	...
	if myid == main_node:
		...
		image_start_proc, image_end_proc = sp_applications.MPI_start_end(nima, number_of_proc, myid)
		n_params = (image_end_proc - image_start_proc) * 5
...
def bcast_compacted_EMData_all_to_all(list_of_em_objects, myid, comm=-1):
	...
	if size_of_one_refring_assumed_common_to_all * (ref_end - ref_start) > (
		...
		sp_global_def.sxprint("Sending refrings: size of data to broadcast is greater than 2GB")

	for sender_id in range(ncpu):
		sender_ref_start, sender_ref_end = sp_applications.MPI_start_end(num_ref, ncpu, sender_id)
...
def recv_attr_dict(
	...
	for n in range(number_of_proc):
		if n != main_node:
			dis = mpi.mpi_recv(2, mpi.MPI_INT, n, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, comm)
			value = mpi.mpi_recv(
...
def send_attr_dict(main_node, data, list_params, image_start, image_end, comm=-1):
	...
	TransType = type(EMAN2_cppwrap.Transform())
	mpi.mpi_send(
		[image_start, image_end], 2, mpi.MPI_INT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, comm
	)
	nvalue = []
	for im in range(image_start, image_end):
		...
		for il in range(len(value)):
			if type(value[il]) is int:
				...
				for f in m:
					nvalue.append(f)
	mpi.mpi_send(nvalue, len(nvalue), mpi.MPI_FLOAT, main_node, sp_global_def.SPARX_MPI_TAG_UNIVERSAL, comm)
...
def nearest_many_full_k_projangles(
	...
	#
	pass#IMPORTIMPORTIMPORT from sp_utilities import getfvec, angles_to_normals
...
def angular_occupancy(params, angstep=15.0, sym="", method="S", inc_mirror=0):
	...
	pass#IMPORTIMPORTIMPORT from sp_utilities import nearest_fang, angles_to_normals
...
def balance_angular_distribution(params, max_occupy=-1, angstep=15.0, sym="c1"):
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import symclass
	data = sp_fundamentals.symclass(sym).reduce_anglesets(matplotlib.numpy.array(params)[:,0:3], inc_mirror=0, tolistconv=False)
	...
	if max_occupy > 0:
		...
		# for l, q in enumerate(occupancy):
			idxs = matplotlib.numpy.where(matplotlib.numpy.array(occupancy).flatten()==i)[0]
...
def rotation_between_anglesets(agls1, agls2):
	...
	N = len(agls1)
	if N != len(agls2):
		sp_global_def.ERROR("rotation_between_anglesets", "Both lists must have the same length", 1)
		return -1
	if N < 2:
		sp_global_def.ERROR(
	...
	pass#IMPORTIMPORTIMPORT from sp_fundamentals import recmat
...
def nearest_proj(proj_ang, img_per_grp=100, List=[]):
	...
	if len(List) == 0:
		List = list(range(N))
	if N < img_per_grp:
		sp_global_def.sxprint("Error: image per group larger than the number of particles!")
		exit()
...
def eliminate_moons(my_volume, moon_elimination_params):
	...
	if (
		...
		pass#IMPORTIMPORTIMPORT from sp_utilities import gauss_edge
...
def combinations_of_n_taken_by_k(n, k):
	...
	return int(
		reduce(lambda x, y: x * y, (fractions.Fraction(n - i, i + 1) for i in range(k)), 1)
	)
...
def store_value_of_simple_vars_in_json_file(
	...
	ordered_my_vars = matplotlib.collections.OrderedDict(sorted(my_vars.items()))
...
def convert_json_fromunicode(data):
	...
	if isinstance(data, six.string_types):
		return str(data)
	elif isinstance(data, matplotlib.collections.Mapping):
		return dict(list(map(convert_json_fromunicode, iter(list(data.items())))))
	elif isinstance(data, matplotlib.collections.Iterable):
...
def angular_distribution(params_file, output_folder, prefix, method, pixel_size, delta, symmetry, box_size, particle_radius, dpi, do_print=True, exclude=None):
	...
	def get_color(sorted_array):
		...
		"""
		sorted_normalized = matplotlib.numpy.true_divide(sorted_array, matplotlib.numpy.max(sorted_array))
		color_list = []
	...
	data_params = matplotlib.numpy.atleast_2d(matplotlib.numpy.genfromtxt(params_file, usecols=(0, 1, 2)))
	...
	width = (pixel_size * particle_radius * matplotlib.numpy.radians(delta) * 2) / float(2 * 3)
	length = particle_radius * 0.2

	# Normalize the radii so that the maximum value is 1
	radius_normalized = matplotlib.numpy.true_divide(radius_array, matplotlib.numpy.max(radius_array))

	# Vector pointing to the center of the Box in chimera
	vector_center = 0.5 * matplotlib.numpy.array([
		box_size,
		box_size,
		box_size
		])
...
# EMAN2 / sparx modules
pass#IMPORTIMPORTIMPORT import sp_morphology
