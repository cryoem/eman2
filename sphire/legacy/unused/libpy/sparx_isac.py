...
"""2
		...
		for im in xrange(image_start, image_end):
			...
			for iref in xrange(numref):
				pass#IMPORTIMPORTIMPORT from utilities import inverse_transform2
				[alphan, sxn, syn, mn] = \
...
def isac_stability_check_mpi(alldata, numref, belongsto, stab_ali, thld_err, mask, first_ring, last_ring, rstep, xrng, yrng, step, \
	...
	pass#IMPORTIMPORTIMPORT from applications import within_group_refinement
	pass#IMPORTIMPORTIMPORT from mpi		  import mpi_comm_size, mpi_comm_rank, mpi_barrier, mpi_bcast, mpi_send, mpi_recv, MPI_FLOAT
	pass#IMPORTIMPORTIMPORT from pixel_error  import multi_align_stability
	pass#IMPORTIMPORTIMPORT from utilities	import get_params2D, set_params2D, model_blank
...
#	print "rest of code..."
	...
	return refi
'''
"""  I have no idea why would anybody want to see it  03/25/2014  PAP5
...
def iter_isac(stack, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, \
	...
	pass#IMPORTIMPORTIMPORT from global_def   import ERROR, EMData, Transform
	pass#IMPORTIMPORTIMPORT from pixel_error  import multi_align_stability
	pass#IMPORTIMPORTIMPORT from utilities    import model_blank, write_text_file, get_params2D
	pass#IMPORTIMPORTIMPORT from utilities    import gather_EMData, bcast_EMData_to_all, send_EMData, recv_EMData
	...
	if myid == main_node:
		print("****************************************************************************************************")
		print("*                                                                                                  *")
		print("*                 Beginning of the ISAC program                "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"           *")
		print("*                                                                                                  *")
	...
	if  not candidatesexist:
		if myid == main_node:
			print("******************************************************************************************")
			print("*            Beginning of the first phase           "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"            *")
			print("*                                                                                        *")
		...
		while Iter <= max_round and couldnt_find_stable < 3 and K > 3:
			if myid == main_node: 
				print("################################################################################")
				print("#           Beginning of Round %2d           "%Iter+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"          #")
				print("################################################################################")
			...
			if myid == main_node:
				print("**********************************************************************")
				print("     Processing of candidate averages   "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
				print("**********************************************************************")
	
			for mloop in range(1, match_first+1):
				if myid == main_node:
					print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
					print("     Loop %3d for 2-way matching   "%mloop+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
					print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
		...
		if myid == main_node:
			...
			print("******************************************************************************************")
			print("*              End of the first phase             "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"              *")
			print("******************************************************************************************")
		return
	...
	if myid == main_node:
		print("")
		print("******************************************************************************************")
		print("*           Beginning of the second phase         "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"             *")
		print("*                                                                                        *")
	...
	for mloop in range(1, match_second+1):
		...
		if myid == main_node:		
			print("################################################################################")
			print("#       Iteration %2d for %d-way matching       "%(mloop, wayness)+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"        #")
			print("################################################################################")
...
#					refim[i].write_image("init_group%d_2nd_phase_round%d.hdf"%(color, mloop), i)
		...
		if myid == main_node:
			print("**********************************************************************")
			print("     Run the main part of ISAC program   "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
			print("**********************************************************************")
...
#				refim[i].write_image("log_mainPart_" + str(color) + "_" + str(mloop) + ".hdf", i)
		...
		if mloop != match_second:
			if myid == main_node:
				print("**********************************************************************")
				print("     Run the %d-way matching algorithm  "%wayness+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
				print("**********************************************************************")
...
#				refim[i].write_image("log_afterMatching_" + str(color) + "_" + str(mloop) + ".hdf", i)
	...
	if myid == main_node:
		print("**********************************************************************")
		print("       Run the final %d-way matching algorithm  "%indep_run+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
		print("**********************************************************************")
	...
	if myid == main_node:
		print("****************************************************************************************************")
		print("*                                                                                                  *")
		print("*                   End of the ISAC program                 "+time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())+"            *")
		print("*                                                                                                  *")
		print("****************************************************************************************************")
	return
...
def isac_MPI(stack, refim, maskfile = None, outname = "avim", ir=1, ou=-1, rs=1, xrng=0, yrng=0, step=1, 
	...
	pass#IMPORTIMPORTIMPORT from global_def   import EMData, Util
	pass#IMPORTIMPORTIMPORT from alignment	  import Numrinit, ringwe, search_range
	pass#IMPORTIMPORTIMPORT from applications import MPI_start_end, within_group_refinement
	pass#IMPORTIMPORTIMPORT from filter	      import filt_tanl
	pass#IMPORTIMPORTIMPORT from fundamentals import rot_shift2D, fshift, fft
	pass#IMPORTIMPORTIMPORT from pixel_error  import multi_align_stability
	pass#IMPORTIMPORTIMPORT from statistics   import ave_series
	pass#IMPORTIMPORTIMPORT from utilities	  import model_circle, model_blank, combine_params2, inverse_transform2, get_image
...
def match_independent_runs(data, refi, n_group, T):
	...
	pass#IMPORTIMPORTIMPORT from statistics  import k_means_stab_bbenum
...
def match_2_way(data, refi, indep_run, thld_grp, FH, FF, find_unique=True, wayness=2, suffix=""):
	...
	pass#IMPORTIMPORTIMPORT from utilities  import read_text_row, set_params2D
	pass#IMPORTIMPORTIMPORT from statistics import ave_series, k_means_stab_bbenum
