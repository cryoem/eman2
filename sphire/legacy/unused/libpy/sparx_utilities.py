...
def center_2D(image_to_be_centered, center_method = 1, searching_range = -1, Gauss_radius_inner = 2, Gauss_radius_outter = 7, self_defined_reference = None):
	...
	if    center_method == 0 :  return  image_to_be_centered,0.,0.
		...
		return sparx_fundamentals.fshift(image_to_be_centered, -cs[0], -cs[1]), cs[0], cs[1]
		...
		cc = sparx_morphology.binarize(EMAN2_cppwrap.rsconvolution(sparx_morphology.binarize(image_to_be_centered,p[0]+p[1]),model_blank(5,5,1,1.0/(5.0*5.0))),0.5)
		c = sparx_fundamentals.ccf(cc, self_defined_reference)
		...
		return sparx_fundamentals.cyclic_shift(image_to_be_centered, -shiftx, -shifty), shiftx, shifty
		...
		shifty = 0
		while (not_centered):
			reference = sparx_fundamentals.rot_avg_image(tmp_image)
			ccmap = sparx_fundamentals.ccf(tmp_image, reference)
			if searching_range > 0:  ccmap = EMAN2_cppwrap.Util.window(ccmap, searching_range, searching_range, 1, 0, 0, 0)
			peak  = peak_search(ccmap)
			centered_image = sparx_fundamentals.fshift(tmp_image, -peak[0][4], -peak[0][5])
		...
		new_image = sparx_morphology.threshold_to_minval(image_to_be_centered, mean+sigma)
		...
		ccmap = sparx_fundamentals.ccf(image_to_be_centered, reference)
		if searching_range > 1: ccmap = EMAN2_cppwrap.Util.window(ccmap, searching_range, searching_range, 1, 0, 0, 0)
		peak  = peak_search(ccmap)
		return sparx_fundamentals.fshift(image_to_be_centered, -peak[0][4], -peak[0][5]), peak[0][4], peak[0][5]
...
def drop_png_image(im, trg):
	...
	if trg[-4:] != '.png':
		sparx_global_def.ERROR('destination name must be png extension', 'drop_png_image', 1)
	...
	if isinstance(trg, str):
		...
		sparx_global_def.ERROR('destination is not a file name', 'drop_png_image', 1)
...
def gather_compacted_EMData_to_root_with_header_info_for_each_image(number_of_all_em_objects_distributed_across_processes, list_of_em_objects_for_myid_process, myid, comm=-1):
	...
	for sender_id in range(1,ncpu):
		...
		if myid == 0:
			...
			data = mpi.mpi_recv(sender_size_of_refrings, mpi.MPI_FLOAT, sender_id, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
		elif sender_id == myid:
			wrap_mpi_send(str(em_dict_to_send_list), 0)
			# print "sender_id = ", sender_id, "sender_size_of_refrings = ", sender_size_of_refrings
			mpi.mpi_send(data, sender_size_of_refrings, mpi.MPI_FLOAT, 0, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
...
def gather_EMData(data, number_of_proc, myid, main_node):
	...
	if myid == main_node:
		for i in range(0, number_of_proc*inc, inc):
			if i == main_node:
				...
				for k in range(l):
					...
					mem_len = mpi.mpi_recv(1, mpi.MPI_INT, i, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
					members = mpi.mpi_recv(int(mem_len[0]), mpi.MPI_INT, i, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
		...
		for k in range(l):
			...
			mpi.mpi_send(len(mem), 1, mpi.MPI_INT, main_node, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
			mpi.mpi_send(mem, len(mem), mpi.MPI_INT, main_node, sparx_global_def.SPARX_MPI_TAG_UNIVERSAL, mpi.MPI_COMM_WORLD)
...
def angles_between_anglesets(angleset1, angleset2, indexes=None):
	...
	angleset1 = sparx_fundamentals.rotate_params([angleset1[i] for i in range(len(angleset1))],[-rot[2],-rot[1],-rot[0]])
...
def get_groups_from_partition_dep(partition, initial_ID_list, number_of_groups):
...
def remove_small_groups_dep1(class_list,minimum_number_of_objects_in_a_group):
	new_class  = []
	final_list = []
...
def remove_small_groups_dep2(class_list,minimum_number_of_objects_in_a_group):
	new_class  = []
	final_list = []
	for one_class in class_list:
		if len(one_class)>=minimum_number_of_objects_in_a_group:
...
def print_a_line_with_timestamp_dep(string_to_be_printed ):
...
def get_number_of_groups_dep1(total_particles,number_of_images_per_group, round_off=.2):
...
def get_number_of_groups_dep2(total_particles,number_of_images_per_group):
...
def set_filter_parameters_from_adjusted_fsc(n1,n2,Tracker):
	...
	for i in range(1,ns-1):
		if adjusted_fsc[1][i] < fsc_cutoff:
			...
			break
	lowpass, falloff    = sparx_filter.fit_tanh1(adjusted_fsc, 0.01)
		...
		for jsort3d in range(len(sort3d_rootdir_list)):
			if isort3d != jsort3d:
				...
				for jlj in range(len(lj)):
					...
					new_lj.append(t)
				ptp=[new_li,new_lj]
				newindeces, list_stable, nb_tot_objs = sparx_statistics.k_means_match_clusters_asg_new(ptp[0],ptp[1])
				log_main.add("*************************************************************")
...
def two_way_comparison_single(partition_A, partition_B,Tracker):
	...
	ptp[0] = numpy32_A
	ptp[1] = numpy32_B
	newindexes, list_stable, nb_tot_objs = sparx_statistics.k_means_match_clusters_asg_new(ptp[0],ptp[1])
	if myid == main_node:
...
def Kmeans_exhaustive_run(ref_vol_list,Tracker):
	...
	while empty_group ==1 and kmref<=5:## In case pctn of Kmeans jumps between 100% to 0%, stop the program
		...
		empty_group, res_classes, data_list = sparx_applications.ali3d_mref_Kmeans_MPI(ref_vol_list, outdir, final_list_text_file, Tracker)
		kmref +=1
		if empty_group ==1:
			if myid ==main_node:
			...
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			ref_vol_list = []
			if  Tracker["constants"]["mask3D"]: mask3D = sparx_morphology.get_shrink_3dmask(Tracker["constants"]["nxinit"],Tracker["constants"]["mask3D"])
			else: mask3D = None
			...
			for igrp in range(len(new_class)):
				...
				volref, fsc_kmref = sparx_reconstruction.rec3D_two_chunks_MPI(data,snr,Tracker["constants"]["sym"],mask3D,\
...
def split_a_group(workdir,list_of_a_group,Tracker):
	...
	for index in range(2):
		...
		vol = sparx_filter.filt_tanl(vol,Tracker["constants"]["low_pass_filter"],.1)
		ref_list.append(vol)
	mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	### EQ-Kmeans
	outdir = os.path.join(workdir,"EQ-Kmeans")
	sparx_applications.mref_ali3d_EQ_Kmeans(ref_list,outdir,this_particle_text_file,Tracker)
