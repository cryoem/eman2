...
class SXsubcmd_config(object):
    def __init__(
        ...
        self.token_edit_list = (
            token_edit_list
        )  # To edit some attributes of tokens (label, help, group, is_required, default). If the original value should be kept, set to None. First entry should be mode token.
        self.is_modeless = (
            is_modeless
        )  # For special command subset, set this to True to suppress setting of SXcmd.mode.
        self.subset_config = (
            subset_config
        )  # Unique name to differentiate subset configuration of this command. For example, to name a command argument mode, which dependes on the number of input arguments. If not necessary, use empty string
        self.mpi_support = (
            mpi_support
        )  # Flag to indicate if this command suppors MPI version
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXcmd_config(object):
    def __init__(
        ...
        self.category = (
            category
        )  # Category of this command: sxc_movie, sxc_cter, sxc_window, sxc_isac, sxc_viper, sxc_meridien, sxc_sort3d, sxc_localres, sxc_utilities
        self.role = (
            role
        )  # Role of this command; sxr_pipe (pipeline), sxr_alt (alternative) sxr_util (utility)
        self.is_submittable = (
            is_submittable
        )  # External GUI Application (e.g. sxgui_cter.py) should not be submitted to job queue. If it is true, this command will be registered to child_application_list of SXCmdWidget
        self.exclude_list = exclude_list  # token key base list to be excluded
        self.subconfig = (
            subconfig
        )  # Subset configuration of this command (e.g. sxprocess and sxlocres). None includes all command tokens, and do not make subcmd
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXkeyword_map(object):
    def __init__(self, priority, token_type):
        if priority >= 100:
            sp_global_def.ERROR(
                "Priority should be lower than 100",
        ...
        self.priority = (
            priority
        )  # Priority of this keyword. Highest priority is 0. The type of higher priority will be used to avoid the conflict among keywords
...
def construct_keyword_dict():
    ...
    keyword_dict["--chunkdir"] = SXkeyword_map(
        0, "dir"
    )  # --chunkdir=chunkdir (contains keyworkd 'chunk' but this should be directory type)
    keyword_dict["destination_directory"] = SXkeyword_map(
        0, "dir"
    )  # destination_directory (contains keyworkd 'directory' but this should be directory type)
    keyword_dict["main000"] = SXkeyword_map(
        0, "dir"
    )  # destination_directory (contains keyworkd 'directory' but this should be directory type)
    ...
    keyword_dict["--stack_mode"] = SXkeyword_map(
        0, "bool"
    )  # stack_mode (contains keyworkd 'stack' but this should be bool type)
    keyword_dict["--skip_mask_rviper"] = SXkeyword_map(
        0, "bool"
    )  # --generate_mask (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--fill_mask"] = SXkeyword_map(
        0, "bool"
    )  # --generate_mask (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--generate_mask"] = SXkeyword_map(
        0, "bool"
    )  # --generate_mask (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--binary_mask"] = SXkeyword_map(
        0, "bool"
    )  # --binary_mask (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--symmetrize"] = SXkeyword_map(
        0, "bool"
    )  # --symmetrize (contains keyworkd '--sym' but this should be bool type)
    keyword_dict["--do_adaptive_mask"] = SXkeyword_map(
        0, "bool"
    )  # --do_adaptive_mask (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--adaptive_mask"] = SXkeyword_map(
        0, "bool"
    )  # --adaptive_mask (contains keyworkd 'mask' but this should be bool type)
    ...
    keyword_dict["--no_virtual_stack"] = SXkeyword_map(
        0, "bool"
    )  # --no_virtual_stack (contains keyworkd 'stack' but this should be bool type)
    keyword_dict["--create_stack"] = SXkeyword_map(
        0, "bool"
    )  # --create_stack (contains keyworkd 'stack' but this should be bool type)
    keyword_dict["--do_not_create_stack"] = SXkeyword_map(
        0, "bool"
    )  # --do_not_create_stack (contains keyworkd 'stack' but this should be bool type)
    keyword_dict["--save_vstack"] = SXkeyword_map(
        0, "bool"
    )  # --save_vstack (contains keyworkd 'stack' but this should be bool type)
    keyword_dict["--use_mol_mass"] = SXkeyword_map(
        0, "bool_ignore"
    )  # if one wants to use the --mol_mass=KILODALTON
    keyword_dict["--s_use_mol_mass"] = SXkeyword_map(
        0, "bool_ignore"
    )  # if one wants to use the --mol_mass=KILODALTON
    keyword_dict["--use_second_mask"] = SXkeyword_map(
        0, "bool_ignore"
    )  # if one wants to use the --mol_mass=KILODALTON
    keyword_dict["--second_mask_shape"] = SXkeyword_map(
        0, "string"
    )  # if one wants to use the --mol_mass=KILODALTON
    keyword_dict["--s_fill_mask"] = SXkeyword_map(
        0, "bool"
    )  # if one wants to use the --mol_mass=KILODALTON
    keyword_dict["--mask_threshold"] = SXkeyword_map(
        0, "float"
    )  # --mask_threshold=MASK_THRESHOLD (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--chunk_by"] = SXkeyword_map(
        0, "string"
    )  # --mask_threshold=MASK_THRESHOLD (contains keyworkd 'mask' but this should be bool type)
    keyword_dict["--outlier_tracker"] = SXkeyword_map(
        0, "params_any_json"
    )  # --import=INPUT_PARAMS_PATH
    # Use priority 1 for output
    keyword_dict["output"] = SXkeyword_map(
        1, "output"
    )  # output_hdf, output_directory, outputfile, outputfile, --output=OUTPUT, output_stack, output_file
    ...
    keyword_dict["locres_volume"] = SXkeyword_map(
        1, "output"
    )  # locres_volume (this contained keyword "volume" also... This is another reason why priority is introduced...)
    keyword_dict["directory"] = SXkeyword_map(1, "output")  # directory
    keyword_dict["rotpw"] = SXkeyword_map(1, "output")  # rotpw
    keyword_dict["--makevstack"] = SXkeyword_map(
        1, "output_bdb2d_stack"
    )  # --makevstack
    keyword_dict["input_micrograph_list"] = SXkeyword_map(
        1, "mic_one_list"
    )  # input_micrograph_list (contains keyword 'input_micrograph' but this should be image_list type)
    keyword_dict["--ctref_orgstack"] = SXkeyword_map(
        1, "bdb2d_stack"
    )  # --ctref_orgstack=stack_for_continuation
    keyword_dict["input_bdb_stack_path"] = SXkeyword_map(
        1, "bdb2d_stack"
    )  # input_bdb_stack_path (contains keyword 'stack' but this should be bdb type)
    keyword_dict["--substack_basename"] = SXkeyword_map(
        1, "string"
    )  # --substack_basename=SUBSTACK_BASENAME (contains keyword 'volume' but this should be string type)
    keyword_dict["--importctf"] = SXkeyword_map(
        1, "params_cter_txt"
    )  # --importctf=IMPORTCTF (contains keyword '--import' but this should be params_cter_txt type) # NOTE: Toshio Moriya 2018/01/27 This is used by sxprocess.py but this function might be obsoleted at this point
    # Use priority 2 for the others
    keyword_dict["input_data_list"] = SXkeyword_map(
        2, "displayable_list"
    )  # input_data_list
    ...
    keyword_dict["source_micrograph"] = SXkeyword_map(
        2, "mic_both"
    )  # source_micrograph_pattern
    keyword_dict["input_micrograph"] = SXkeyword_map(
        2, "mic_stack"
    )  # input_micrograph_pattern
    keyword_dict["isac_class_avgs_path"] = SXkeyword_map(
        2, "data2d_one"
    )  # isac_class_avgs_path
    keyword_dict["stack"] = SXkeyword_map(
        2, "bdb2d_stack"
    )  # stack, prj_stack, input_stack, --instack=input_stack_file
    keyword_dict["volume"] = SXkeyword_map(
        2, "data3d_one"
    )  # initial_volume, firstvolume, secondvolume, input_volume
    keyword_dict["mask"] = SXkeyword_map(
        2, "data3d_one"
    )  # --mask3D=mask3D, maskfile, mask, --mask=MASK, --mask3D=mask3d_file
    keyword_dict["--focus"] = SXkeyword_map(
        2, "data3d_one"
    )  # --focus=3D_focus_mask, --focus=focus3d_file
    ###	keyword_dict["isac_averages"]                 = SXkeyword_map(2, "data2d_one")          # isac_averages
    keyword_dict["--ctref_initvol"] = SXkeyword_map(
        2, "data3d_one"
    )  # --ctref_initvol=restarting_initial_volume
    keyword_dict["selection_list"] = SXkeyword_map(
        2, "select_mic_stack"
    )  # selection_list
    keyword_dict["--chunk"] = SXkeyword_map(
        2, "select_data2d_stack"
    )  # --chunk0=CHUNK0_FILE_NAME, --chunk1=CHUNK1_FILE_NAME
    keyword_dict["--ctref_subset"] = SXkeyword_map(
        2, "select_data2d_stack"
    )  # --ctref_subset=selection_file_path
    ...
    keyword_dict["input_shift_list_file"] = SXkeyword_map(
        2, "select_drift_params"
    )  # input_shift_list_file
    ###	keyword_dict["--resample_ratio_source"]       = SXkeyword_map(2, "params_any_txt")      # --resample_ratio_source
    keyword_dict["params_file"] = SXkeyword_map(
        2, "params_any_txt"
    )  # --import=INPUT_PARAMS_PATH
    keyword_dict["--import"] = SXkeyword_map(
        2, "params_any_txt"
    )  # --import=INPUT_PARAMS_PATH
    keyword_dict["--export"] = SXkeyword_map(
        2, "params_any_txt"
    )  # --export==OUTPUT_PARAMS_FILE
    ...
    keyword_dict["input_coordinates_pattern"] = SXkeyword_map(
        2, "params_coords_any"
    )  # input_coordinates_pattern
    keyword_dict["input_rebox_pattern"] = SXkeyword_map(
        2, "params_rebox_rbx"
    )  # input_rebox_pattern
    keyword_dict["input_ctf_params_source"] = SXkeyword_map(
        2, "params_cter_txt"
    )  # input_ctf_params_source
    keyword_dict["cter_ctf_file"] = SXkeyword_map(2, "params_cter_txt")  # cter_ctf_file
    keyword_dict["swap_ctf_params"] = SXkeyword_map(
        2, "params_cter_txt"
    )  # swap_ctf_params
    keyword_dict["inputfile"] = SXkeyword_map(2, "params_drift_txt")  # inputfile
    keyword_dict["input_shift_pattern"] = SXkeyword_map(
        2, "params_drift_txt"
    )  # input_shift_pattern
    keyword_dict["input_star_file"] = SXkeyword_map(
        2, "params_relion_star"
    )  # input_star_file
    keyword_dict["--modelpw"] = SXkeyword_map(
        2, "spectrum1d"
    )  # --modelpw=pw2_model_txt
    keyword_dict["--pwreference"] = SXkeyword_map(
        2, "spectrum1d"
    )  # --pwreference=pwreference
    keyword_dict["--PWadjustment"] = SXkeyword_map(
        2, "spectrum1d"
    )  # --PWadjustment=PWadjustment, --PWadjustment=ref_pwspectrum1d_file
    ...
    keyword_dict["--ctref_oldrefdir"] = SXkeyword_map(
        2, "dir"
    )  # --ctref_oldrefdir=refine_dir_path
    keyword_dict["--refinement_dir"] = SXkeyword_map(
        2, "dir"
    )  # --refinement_dir=refinemen_out_dir
    keyword_dict["--previous_run"] = SXkeyword_map(
        2, "dir"
    )  # --previous_run1=sort3d_run1_directory, --previous_run2=sort3d_run2_directory (need for sort3d.txt)
    keyword_dict["--relion_project_dir"] = SXkeyword_map(
        2, "dir"
    )  # --relion_project_dir=DIR_PATH
    keyword_dict["--function"] = SXkeyword_map(
        2, "user_func"
    )  # --function=user_function
    keyword_dict["--function_ai"] = SXkeyword_map(
        2, "user_func"
    )  # --function=user_function
    ...
    keyword_dict["--fl"] = SXkeyword_map(
        2, "abs_freq"
    )  # --fl=cutoff_frequency, (--fl=fl), (--fl=fl), (--fl=lpf_cutoff_freq), --fl=LPF_CUTOFF_FREQ
    ...
    keyword_dict["--res_overall"] = SXkeyword_map(
        2, "abs_freq"
    )  # --res_overall=overall_resolution
    ...
    keyword_dict["--apix"] = SXkeyword_map(
        2, "apix"
    )  # --apix=pixel_size, --apix, --apix=PIXEL_SIZE
    ...
    keyword_dict["--box"] = SXkeyword_map(
        2, "box"
    )  # --box=box_size, --box_size=box_size, --boxsize=BOX_SIZE
    keyword_dict["--rb_box_size"] = SXkeyword_map(2, "box")  # --rb_box_size=BOX_SIZE
    keyword_dict["--radius"] = SXkeyword_map(
        2, "radius"
    )  # --radius=particle_radius, --radius=outer_radius, --radius=outer_radius, --radius=particle_radius, --radius=outer_radius, --radius=outer_radius
    keyword_dict["--sym"] = SXkeyword_map(
        2, "sym"
    )  # --sym=c1, --sym=symmetry, --sym=c4
    keyword_dict["--mol_mass"] = SXkeyword_map(2, "mass")  # --mol_mass=KILODALTON
    keyword_dict["--filament_width"] = SXkeyword_map(
        2, "filament_width"
    )  # --mol_mass=KILODALTON
    ...
    keyword_dict["config_path"] = SXkeyword_map(
        2, "params_any_json"
    )  # --import=INPUT_PARAMS_PATH
    ...
    keyword_dict["_partres"] = SXkeyword_map(
        2, "params_cter_txt"
    )  # input_ctf_params_source
    keyword_dict["_box_pattern"] = SXkeyword_map(
        2, "params_coords_any"
    )  # input_ctf_params_source
...
def handle_exceptional_cases(sxcmd):
    ...
    if sxcmd.name == "sp_window":
        assert (
            sxcmd.token_dict["input_micrograph_pattern"].key_base
            == "input_micrograph_pattern"
        )
...
def remove_MoinMoinWiki_makeup(target_text):
    ...
    while item_head != -1:
        ...
        if item_tail == -1:
            sp_global_def.ERROR(
                'Wiki Format Warning: The string "%s" contains "%s" but not "%s". Removing "%s", but please check the format in Wiki document.'
                % (target_text, makeup_begin, makeup_end, makeup_begin),
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
            ...
            print(
                '### Found a wiki makeup token "%s". Changed to "%s"'
                % (makeup_token, display_item)
            )
...
def construct_token_list_from_MoinMoinWiki(sxcmd_config):
    ...
    print(
        "Start parsing MoinMoinWiki document (%s as %s %s command) "
        % (sxcmd_config.wiki, sxcmd_config.category, sxcmd_config.role)
    )

    if sxcmd_config.format != "MoinMoinWiki":
        sp_global_def.ERROR(
            "Logical Error: Incorrect Wiki format %s! Check the sxcmd_config setting in this script."
            % (sxcmd_config.format),
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )

    # Allocate memory for new SXcmd instance
    sxcmd = sxgui_template.SXcmd(
        sxcmd_config.category, sxcmd_config.role, sxcmd_config.is_submittable
    )
    ...
    section_lists = []
    section_lists.append("= Name =")
    section_name = len(section_lists) - 1
    section_lists.append("= Usage =")
    section_usage = len(section_lists) - 1
    section_lists.append("=== Typical usage ===")
    section_typical = len(section_lists) - 1
    section_lists.append("== Input ==")
    section_input = len(section_lists) - 1
    section_lists.append("== Output ==")
    section_output = len(section_lists) - 1
    ...
    if os.path.exists(sxcmd_config.wiki) == False:
        sp_global_def.ERROR(
            "Rutime Error: Wiki document is not found.",
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )
    ...
    for line_wiki in file_wiki:
        ...
        if current_state == state_searching:
            ...
            if current_state != state_processing:
                sp_global_def.ERROR(
                    "Logical Error: This condition should not happen! State setting must be incorrect.",
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            if line_wiki[0] == "=":  # Assuming the section always starts with "="
                ...
                group = (
                    group_main
                )  # reset group (subsection) for '== Input ==' and '== Output ==' sections
                ...
                if line_wiki.find(section_lists[current_section]) != -1:
                    ...
                    if current_section >= len(section_lists):
                        sp_global_def.ERROR(
                            "Logical Error: This condition should not happen! Section setting must be incorrect.",
                            "%s in %s" % (__name__, os.path.basename(__file__)),
                        )
                    current_state = state_processing
                else:
                ...
                if current_section == section_name:
                    ...
                    if item_tail == -1:
                        sp_global_def.ERROR(
                            "Wiki Format Error: '= Name =' section should contain only one valid line, and the line should starts from 'sx* - ' or 'e2* - ': %s"
                            % line_wiki,
                            "%s in %s" % (__name__, os.path.basename(__file__)),
                        )
                    ...
                    if n_sxcmd_string_token_list == 2:
                        ...
                        sp_global_def.ERROR(
                            "Wiki Format Error: Command string should be only script file name or script file name plus subcommand",
                            "%s in %s" % (__name__, os.path.basename(__file__)),
                        )
                    line_buffer = line_buffer[
                        item_tail + len(target_operator) :
                    ].strip()  # Get the rest of line
                    # Extract the label of this sxscript
                    target_operator = ":"
                    item_tail = line_buffer.find(target_operator)
                    if item_tail == -1:
                        sp_global_def.ERROR(
                            "Wiki Format Error: '= Name =' section should contain a label ended with ':' after 'sx* - ' or 'e2* - ': %s"
                            % line_wiki,
                            "%s in %s" % (__name__, os.path.basename(__file__)),
                        )
                    sxcmd.label = line_buffer[0:item_tail].strip()
                    # Extract the short info about this sxscript (can be empty)
                    sxcmd.short_info = remove_MoinMoinWiki_makeup(
                        line_buffer[item_tail + len(target_operator) :].strip()
                    ...
                    if (
                        line_wiki[0 : len("sx_")] == "sx_"
                        or line_wiki[0 : len("e2")] == "e2"
                    ):
                        usage_token_list = line_wiki.split()
                        head_token_idx = 1
                        if usage_token_list[0] != sxcmd.name + ".py":
                            sp_global_def.ERROR(
                                "Wiki Format Error: First token should be script name with .py (sx*.py or e2*.py)",
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        if sxcmd.subname != "":
                            head_token_idx = 2
                            if usage_token_list[1] != sxcmd.subname:
                                sp_global_def.ERROR(
                                    "Wiki Format Error: Second token of this command should be subname",
                                    "%s in %s" % (__name__, os.path.basename(__file__)),
                                )
                        ...
                        for usage_token in usage_token_list[head_token_idx:]:
                            ...
                            key = usage_token.split("=")[
                                0
                            ]  # Remove characters after '=' if token contains it (i.e. some options)
                            token.key_base = key.strip(
                                "-"
                            )  # Get key base name by removing prefix ('--' or '-' for option)
                            ...
                            token.type = (
                                best_keyword_map.token_type
                            )  # If command token does not contains any keywords, its type stays with ""
                    ...
                    if (
                        ...
                        and line_wiki.find(target_operator) > -1
                        and line_wiki.find(sxcmd.name + ".py") > -1
                    ):
                        sxcmd.mpi_support = True
                    # else: Ignore this line
                elif (
                    current_section == section_input
                    or current_section == section_output
                ):
                    if line_wiki[0] == "*" and line_wiki.find("optional"):
                        ...
                        if key_base == "MPI":
                            ...
                            if (
                                sxcmd.mpi_support == False
                                or sxcmd.mpi_add_flag == False
                            ):
                                sp_global_def.ERROR(
                                    "Logical Error: Since MPI flag is found, the command should support MPI.",
                                    "%s in %s" % (__name__, os.path.basename(__file__)),
                                )
                            continue
                        line_buffer = line_buffer[
                        ...
                        if key_base not in list(sxcmd.token_dict.keys()):
                            sp_global_def.ERROR(
                                "Wiki Format Error: Key base (%s) is missing from 'usage in command line' in '= Usage ='."
                                % key_base,
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        # Get the reference to the command token object associated with this key base name
                        token = sxcmd.token_dict[key_base]
                        if token.key_base != key_base:
                            sp_global_def.ERROR(
                                "Logical Error: Registered command token with wrong key base name into the dictionary.",
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        token.is_in_io = (
                            True
                        )  # Set flag to tell this command token is find in input or output section
                        token.group = (
                            group
                        )  # Set group of command token according to the current subsection
                        # Extract label of command token
                        target_operator = ":"
                        item_tail = line_buffer.find(target_operator)
                        if item_tail == -1:
                            sp_global_def.ERROR(
                                "Wiki Format Error: This line (%s) is missing label. Please check the format in Wiki document."
                                % line_wiki,
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        ...
                        if item_tail == -1:
                            sp_global_def.ERROR(
                                "Wiki Format Error: This line (%s) is missing default setting. Please check the format in Wiki document."
                                % line_wiki,
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        token.help = remove_MoinMoinWiki_makeup(
                            line_buffer[0:item_tail]
                        )
                        line_buffer = line_buffer[
                            item_tail + len(target_operator) :
                        ].strip()  # Get the rest of line
                        ...
                        if item_tail == -1:
                            sp_global_def.ERROR(
                                "Wiki Format Error: This line (%s) is missing ')' for default setting. Please check the format in Wiki document."
                                % line_wiki,
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        default_value = line_buffer[
                            0:item_tail
                        ].strip()  # make sure spaces & new line are not included at head and tail
                        if default_value.find("required") != -1:
                            ...
                            if not token.type:
                                ...
                                token.type = default_value.replace(
                                    "required", ""
                                ).strip()
                            ...
                            token.default = default_value.replace(
                                "question reversed in GUI", ""
                            ).strip()
                            token.type = "bool"
                            if token.default == "True":
                                token.default = (
                                    False
                                )  # convert the default value to opposite boolean
                    ...
                    sp_global_def.ERROR(
                        "Logical Error: This section is invalid. Did you assign an invalid section?",
                        "%s in %s" % (__name__, os.path.basename(__file__)),
                    )

    if current_state != state_done:
        sp_global_def.ERROR(
            "Wiki Format Error: parser could not extract all information necessary (current state %d). Please check if the Wiki format has all required sections."
            % (current_state),
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )
    ...
    for token in sxcmd.token_list:
        if token.is_in_io == False:
            sp_global_def.ERROR(
                "Wiki Format Error: An extra argument or option (%s) is found in 'usage in command line' of '= Usage ='."
                % token.key_base,
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
    ...
    print(
        "Succeeded to parse MoinMoinWiki document (%s as %s %s command)"
        % (sxcmd_config.wiki, sxcmd_config.category, sxcmd_config.role)
    )
...
def remove_DokuWiki_makeup(target_text):
    ...
    target_text = target_text.replace(
        "''", ""
    )  # Might not be necessary for DokuWiki (only for MoinMoinWiki?)
    target_text = target_text.replace(
        "'''", ""
    )  # Might not be necessary for DokuWiki (only for MoinMoinWiki?)
...
def construct_token_list_from_DokuWiki(sxcmd_config):
    ...
    print(
        "Start parsing DokuWiki document (%s as %s %s command) "
        % (sxcmd_config.wiki, sxcmd_config.category, sxcmd_config.role)
    )

    if sxcmd_config.format != "DokuWiki":
        sp_global_def.ERROR(
            "Logical Error: Incorrect Wiki format %s! Check the sxcmd_config setting in this script."
            % (sxcmd_config.format),
            "%s in %s" % (__name__, os.path.basename(__file__)),
    ...
    # Define list of subsections of input section and set current
    group_lists = []
    group_lists.append("=== Main Parameters ===")
    group_main = len(group_lists) - 1
    group_lists.append("=== Advanced Parameters ===")
    group_advanced = len(group_lists) - 1
    current_group_name = (
        group_lists[group_main]
        .replace("===", "")
        .replace("Parameters", "")
        .strip()
        .lower()
    ...
    )  # Assuming the first line starts from ====== COMMAND_NAME =======

    # NOTE: 2015/11/11 Toshio Moriya
    # This should be exception. Need to decide if this should be skipped or exit system.
    if os.path.exists(sxcmd_config.wiki) == False:
        sp_global_def.ERROR(
            "Runtime Error: Wiki document inot found.",
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )
    ...
    for line_wiki in file_wiki:
        ...
        if current_state == state_searching_header:
            ...
            target_operator = "====="
            if (
                line_wiki[: len(target_operator)] != target_operator
                or line_wiki[-len(target_operator) :] != target_operator
            ):
                ...
                # 				print("MRK_DEBUG: line_wiki[-len(target_operator):] := %s" % (line_wiki[-len(target_operator):]))
                sp_global_def.ERROR(
                    "Wiki Format Error: The Wiki document must start from header section title defined by ===== COMMAND_NAME =====.",
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            ...
            if n_sxcmd_string_token_list == 2:
                ...
                sp_global_def.ERROR(
                    "Wiki Format Error: Command string should be only a script file name or a script file name plus a subcommand",
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            ...
            if item_tail == -1:
                sp_global_def.ERROR(
                    "Wiki Format Error: Header section body should contain a label ending with ':': %s"
                    % line_wiki,
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            sxcmd.label = line_buffer[0:item_tail].strip()
            # Extract the short info about this sxscript (can be empty)
            sxcmd.short_info = remove_DokuWiki_makeup(
                line_buffer[item_tail + len(target_operator) :].strip()
            )  # Get the rest of line
            current_state = state_searching
            ...
            if (
                line_wiki[: len(target_operator)] == target_operator
            ):  # Assuming the section always starts with "======"
                # 				print("MRK_DEBUG: line_wiki                          := %s" % (line_wiki))
                ...
                if current_section == section_usage:
                    ...
                    head_token_idx = 1
                    if (
                        line_wiki[0 : len("sp")] == "sp"
                        or line_wiki[0 : len("e2")] == "e2"
                    ):
                        usage_token_list_line = line_wiki.split()
                        if usage_token_list_line[0] != sxcmd.name + ".py":
                            sp_global_def.ERROR(
                                "Wiki Format Error: First token should be script name with .py (sx*.py or e2*.py)",
                                "%s in %s" % (__name__, os.path.basename(__file__)),
                            )
                        if sxcmd.subname != "":
                            head_token_idx = 2
                            if usage_token_list_line[1] != sxcmd.subname:
                                sp_global_def.ERROR(
                                    "Wiki Format Error: Second token of this command should be subname",
                                    "%s in %s" % (__name__, os.path.basename(__file__)),
                                )
                        ...
                        for usage_token in usage_token_list_line[head_token_idx:]:
                            ...
                            if key == "--MPI":
                                print(
                                    '### Found a --MPI flag "%s". Ignoring this key...'
                                    % (key)
                                )
                                continue
                            token.key_base = key.strip(
                    ...
                    if line_wiki.find(group_lists[group_advanced]) > -1:
                        ...
                        if item_tail != -1:
                            ...
                            if item_tail == -1:
                                ...
                                entry = (
                                    line_buffer[:item_tail].replace("%%", "").strip()
                                )
                            ...
                            for dependency in entry.split():
                                try:
                                    ...
                                    inverse = False
                                token.dependency_group.append(
                                    [group_key.strip("-"), state, inverse]
                                )
                                try:
                                    sxcmd.dependency_dict[group_key].append(
                                        [token.key_base, state, inverse]
                                    )
                                except KeyError:
                                    sxcmd.dependency_dict[group_key] = [
                                        [token.key_base, state, inverse]
                                    ]
                        target_operator = ":"
                        item_tail = line_buffer.find(target_operator)
                    ...
                    sp_global_def.ERROR(
                        "Logical Error: This section is invalid. Did you assigne an invalid section?",
                        "%s in %s" % (__name__, os.path.basename(__file__)),
                    )
    ...
    for sxcmd_token_key_base in list(sxcmd.token_dict.keys()):
        ...
        if not is_found:
            sp_global_def.ERROR(
                "Logical Error: Registered key base name in the command token dictionary is not found in the command token list.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        ...
        if not is_found:
            sp_global_def.ERROR(
                "Wiki Format Error: An extra argument or option (%s) is found in the command token dictionary extracted from '===== Input =====' compared with 'usage in command line' of '====== Usage ======'."
    ...
    for sxcmd_token in sxcmd.token_list:
        ...
        if sxcmd_token.key_base not in list(sxcmd.token_dict.keys()):
            sp_global_def.ERROR(
                "Logical Error: Registered key base name in the command token list is not registered as key base name in the command token dictionary.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        # Make sure there are no extra arguments or options in command token list compared with usage token list extracted from "usage in command line" of "====== Usage ======".
        is_found = False
        ...
        if not is_found:
            sp_global_def.ERROR(
                "Wiki Format Error: An extra argument or option (%s) is found in the command token list extracted from '===== Input =====' compared with 'usage in command line' of '====== Usage ======'."
                % sxcmd_token.key_base,
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
    ...
    for usage_token in usage_token_list:
        ...
        if usage_token.key_base not in list(sxcmd.token_dict.keys()):
            sp_global_def.ERROR(
                "Wiki Format Error: An extra argument or option (%s) is found in 'usage in command line' of '====== Usage ======' compared with the command token dictionary extracted from '===== Input =====' ."
                % usage_token.key_base,
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        ...
        if not is_found:
            sp_global_def.ERROR(
                "Wiki Format Error: An additional argument or option (%s) is found in 'usage in command line' of '====== Usage ======' compared with the command token list extracted from '===== Input =====' ."
                % usage_token.key_base,
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
    ...
    print(
        "Succeed to parse DokuWiki document (%s as %s %s command)"
        % (sxcmd_config.wiki, sxcmd_config.category, sxcmd_config.role)
    )
...
def apply_sxsubcmd_config(sxsubcmd_config, sxcmd):
    ...
    fullset_token_dict = copy.copy.deepcopy(sxcmd.token_dict)
    sxcmd.token_list = []
    sxcmd.token_dict = {}
    ...
    if mode_token_edit.key_base not in list(fullset_token_dict.keys()):
        sp_global_def.ERROR(
            "Logical Error: This condition should not happen! Subset command configuration must be incorrect. Key (%s) should exists."
            % (mode_token_edit.key_base),
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )
    ...
    if not sxsubcmd_config.is_modeless:
        ...
        assert (
            sxcmd.mode == ""
        ), "MRK_DEBUG: This subcommand has no mode! sxcmd.mode must be empty string!"
    ...
    for token_edit in sxsubcmd_config.token_edit_list:
        ...
        if token_edit.key_base is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. Invalid None Key (%s)."
                % (token_edit.key_base),
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token_edit.key_base == "":
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. Invalid empty string Key (%s)."
                % (token_edit.key_base),
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        ...
        if token_edit.key_base not in list(fullset_token_dict.keys()):
            ...
            if token_edit.key_prefix is None:
                sp_global_def.ERROR(
                    "Logical Error: This condition should not happen! Subset command configuration must be incorrect. Prefix (%s) for Key (%s) should NOT be None."
                    % (token_edit.key_prefix, token_edit.key_base),
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            if token_edit.key_prefix != "":
                sp_global_def.ERROR(
                    "Logical Error: This condition should not happen! Subset command configuration must be incorrect. Key (%s) should be argument (Prefix (%s) should be empty string)."
                    % (token_edit.key_base, token_edit.key_prefix),
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
            # token = token_edit
            token = sxgui_template.SXcmd_token()
        ...
        if token.key_base is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.key_base should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.key_prefix is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.key_prefix should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.label is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.label should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.help is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.help should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.group is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.group should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.is_required is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_required should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.is_locked is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_locked should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.is_reversed is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_reversed should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.default is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.default should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.restore is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.restore should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
        if token.type is None:
            sp_global_def.ERROR(
                "Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.type should NOT be None.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
...
def insert_sxcmd_category_list_to_file(sxcmd_category_list, output_file):
    ...
    for sxcmd_category in sxcmd_category_list:
        ...
        output_file.write(
            '; %s.name = "%s"' % (sxcmd_category_variable_name, sxcmd_category.name)
        )
        output_file.write(
            '; %s.label = "%s"' % (sxcmd_category_variable_name, sxcmd_category.label)
        )
        output_file.write(
            '; %s.short_info = "%s"'
            % (
                sxcmd_category_variable_name,
                sxcmd_category.short_info.replace('"', "'"),
            )
        )
        output_file.write("\n")
        output_file.write("\t\t")
        output_file.write(
            "%s_list.append(%s)"
            % (sxcmd_category_variable_name, sxcmd_category_variable_name)
        )
        output_file.write("\n")
...
def insert_sxcmd_to_file(sxcmd, output_file, sxcmd_variable_name):
    ...
    output_file.write(
        '; %s.subset_config = "%s"' % (sxcmd_variable_name, sxcmd.subset_config)
    )
    output_file.write('; %s.label = "%s"' % (sxcmd_variable_name, sxcmd.label))
    output_file.write(
        '; %s.short_info = "%s"'
        % (sxcmd_variable_name, sxcmd.short_info.replace('"', "'"))
    )
    output_file.write(
        "; %s.mpi_support = %s" % (sxcmd_variable_name, sxcmd.mpi_support)
    )
    output_file.write(
        "; %s.mpi_add_flag = %s" % (sxcmd_variable_name, sxcmd.mpi_add_flag)
    )
    ...
    output_file.write(
        "; %s.is_submittable = %s" % (sxcmd_variable_name, sxcmd.is_submittable)
    )
    output_file.write("\n")
    ...
    for token in sxcmd.token_list:
        ...
        output_file.write(
            "; token.dependency_group = %s"
            % str(
                [
                    [str(entry) for entry in group_dep]
                    for group_dep in token.dependency_group
                ]
            )
        )
        ...
        output_file.write(
            "; %s.token_dict[token.key_base] = token" % sxcmd_variable_name
        )
        for entry in token.dependency_group:
            if entry[0]:
                output_file.write(
                    "; %s.dependency_dict.setdefault('%s', []).append([token.key_base, '%s', '%s'])"
                    % (sxcmd_variable_name, entry[0], entry[1], entry[2])
                )
        output_file.write("\n")
...
def create_sxcmd_subconfig_window_makevstack():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("makevstack")
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit.default = "none"
    token_edit.restore = "none"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("input_bdb_stack_pattern")
    token_edit.key_prefix = ""
    token_edit.label = "Input BDB image stack pattern"
    token_edit.help = "Specify file path pattern of stack subsets created in particle extraction using a wild card /'*/' (e.g. /'//sp_window_output_dir//*/'). The stack subsets are located in the sp_window output directory."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "dir_list"
    token_edit_list.append(token_edit)

    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Particle Stack",
        None,
        token_edit_list,
        sxsubcmd_mpi_support,
        subset_config="fullset",
    )
...
def create_sxcmd_subconfig_adaptive_mask3d():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("adaptive_mask")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("input_volume")
    token_edit.key_prefix = ""
    token_edit.label = "Input volume"
    token_edit.help = "Input reference volume"
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "data3d_one"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_mask3D")
    token_edit.key_prefix = ""
    token_edit.label = "Output mask"
    token_edit.help = "Output 3D mask"
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "output"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("use_mol_mass")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("mol_mass")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("threshold")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("nsigma")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ndilation")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("edge_width")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("edge_type")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Adaptive 3D Mask", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def create_sxcmd_subconfig_balance_angles():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("balance_angular_distribution")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("params_any_txt")
    token_edit.key_prefix = ""
    token_edit.label = "Projection parameters"
    token_edit.help = "Typically from MERIDIEN with a filename in the form of Refine3D/final_params_0##.txt"
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "params_any_txt"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("selection_list")
    token_edit.key_prefix = ""
    token_edit.label = "Output selection list"
    token_edit.help = "Text file with a list of retained particle images"
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "output"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("max_occupy")
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("angstep")
    token_edit.default = 15
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("symmetry")
    token_edit.default = "c1"
    token_edit_list.append(token_edit)
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Balance Angular Distribution", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def create_sxcmd_subconfig_binary_mask3d():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("binary_mask")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("nerosion")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ndilation")
    token_edit_list.append(token_edit)
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Binary 3D Mask", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def create_sxcmd_subconfig_postrefiner_halfset_vol():
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("combinemaps")
    token_edit.label = "Post-refine halfset volumes"
    token_edit.help = help_string
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("firstvolume")
    token_edit.key_prefix = ""
    token_edit.label = "First unfiltered halfset map"
    token_edit.help = "As generated by sxmeridien."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "data3d_one"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("secondvolume")
    token_edit.key_prefix = ""
    token_edit.label = "Second unfiltered halfset map"
    token_edit.help = "As generated by sxmeridien."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_dir")
    token_edit_list.append(token_edit)

    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("pixel_size")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("mask")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("do_adaptive_mask")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("use_mol_mass")
    token_edit.group = "advanced"
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("do_approx")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ndilation")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("mtf")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("fsc_adj")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("B_enhance")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("B_start")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("B_stop")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    # 	token_edit = SXcmd_token(); token_edit.initialize_edit("randomphasesafter"); token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("fl")
    token_edit.type = "float"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("aa")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output")
    token_edit.group = "advanced"
    token_edit_list.append(token_edit)
...
def create_sxcmd_subconfig_postrefiner_single_vols():
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("combinemaps")
    token_edit.label = "Post-refine single map"
    token_edit.help = help_string
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("input_volume_pattern")
    token_edit.key_prefix = ""
    token_edit.label = "Input volume pattern"
    token_edit.help = "Specify path pattern of input single volumes with a wild card '*' or path to single volume file (without wild card '*'). Use the wild card to indicate the place of variable part of the file names. The path pattern must be enclosed by single quotes (') or double quotes (\") (Note: sxgui.py automatically adds single quotes ('))."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "data3d_one"
    ...
    token_edit.initialize_edit("B_enhance")
    token_edit.is_required = True
    token_edit.help = "Non-zero positive value: program use provided B-factor [A^2] to enhance the map; -1.0: B-factor is not applied."
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("fl")
    token_edit.is_required = True
    token_edit.help = "A value larger than 0.5: low-pass filter to the resolution in Angstrom; -1.0: no low-pass filter."
    token_edit.type = "float"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("aa")
    ...
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "PostRefiner (Single Map)",
        help_string,
        token_edit_list,
        sxsubcmd_mpi_support,
        subset_config="single map",
    )
...
def create_sxcmd_subconfig_variability_preprocess():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("symmetrize")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("prj_stack")
    token_edit.key_prefix = ""
    token_edit.label = "Input image stack"
    token_edit.help = "The images must contain the 3D orientation parameters in headers and optionally also CTF information. The output image stack is bdb:sdata. Please use it as an input image stack of sx3dvariability."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "bdb2d_stack"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_dir")
    token_edit_list.append(token_edit)
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("sym")
    token_edit_list.append(token_edit)
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "3D Variability Preprocess", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def add_sxcmd_subconfig_meridien_shared(token_edit_list, beta=False):
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("radius")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("mask3D")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("symmetry")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("xr")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ts")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("memory_per_node")
    token_edit_list.append(token_edit)

    if beta:
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("theta_min")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("theta_max")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("angle_method")
        token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("an")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("shake")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("small_memory")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ccfpercentage")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("nonorm")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("function")
    token_edit_list.append(token_edit)
    if beta:
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("chunk_by")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("outlier_by")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("filament_width")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("helical_rise")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("plot_ang_dist")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("outlier_tracker")
        token_edit_list.append(token_edit)
        token_edit = sxgui_template.SXcmd_token()
        token_edit.initialize_edit("howmany")
        token_edit_list.append(token_edit)


def add_sxcmd_subconfig_meridien_standard_shared(token_edit_list, beta=False):
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("inires")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("delta")
    token_edit_list.append(token_edit)
...
def add_sxcmd_subconfig_meridien_local_shared_refine(token_edit_list, beta=False):
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("inires")
    token_edit.help = "Resolution of the initial volume. For local refinement, the program automatically calculates the initial resolution using provided orientation parameters."
    token_edit.default = -1.0
    token_edit.restore = -1.0
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("delta")
    token_edit.help = "Initial angular sampling step. For local refinement, the value has to be less than or equal to 3.75."
    token_edit.default = 3.75
    token_edit.restore = [
        str(entry) for entry in [3.75, 1.875, 0.9375, 0.46875, 0.234375]
    ]
    token_edit_list.append(token_edit)
    add_sxcmd_subconfig_meridien_shared(token_edit_list, beta=beta)
...
def create_sxcmd_subconfig_meridien_standard_fresh(beta=False):
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("stack")
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_directory")
    token_edit.type = "output"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("initial_volume")
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("initialshifts")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("skip_prealignment")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("center_method")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("target_radius")
    token_edit_list.append(token_edit)
    ...
    if beta:
        sxcmd_subconfig = SXsubcmd_config(
            "3D Refinement (Alpha)",
            "Performs 3D structure refinement for helical specimen using a quasi-Maximum Likelihood approach.",
            token_edit_list,
            sxsubcmd_mpi_support,
            is_modeless=True,
            subset_config="beta",
        ...
        sxcmd_subconfig = SXsubcmd_config(
            "3D Refinement",
            "Performs 3D structure refinement using a quasi-Maximum Likelihood approach.",
            token_edit_list,
            sxsubcmd_mpi_support,
            is_modeless=True,
        )
...
def create_sxcmd_subconfig_meridien_standard_continuation(beta=False):
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_directory")
    token_edit.label = "Meridien Output Directory"
    token_edit.help = "This directory must exist. The results will be written here."
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    ...
    if beta:
        sxcmd_subconfig = SXsubcmd_config(
            "3D Refinement Restart (Beta)",
            "Restart 3D refinement after the last fully finished iteration of meridien run or local refinement run. One can change some parameters, but MPI settings have to be the same.",
            token_edit_list,
            sxsubcmd_mpi_support,
            is_modeless=True,
            subset_config="restart_beta",
        )
    else:
        sxcmd_subconfig = SXsubcmd_config(
            "3D Refinement Restart",
            "Restart 3D refinement after the last fully finished iteration of meridien run or local refinement run. One can change some parameters, but MPI settings have to be the same.",
            token_edit_list,
            sxsubcmd_mpi_support,
            is_modeless=True,
            subset_config="restart",
        )
...
def create_sxcmd_subconfig_meridien_header_import_xform_projection():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("import")
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("params")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = "xform.projection"
    token_edit.restore = "xform.projection"
    token_edit_list.append(token_edit)

    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Import Projection Parameters",
        "Import projection orientation parameters to the header of the input stack. (Five columns: phi, theta, pshi, sx, sy). These parameters are required by 'Local Refinement from Stack' mode.",
        token_edit_list,
        sxsubcmd_mpi_support,
    )
...
def create_sxcmd_subconfig_meridien_local_stack(beta=False):
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("local_refinement")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("stack")
    token_edit.help = "The stack must have 3D orientation parameters (xform.projection) stored in image headers. They can be imported using sp_header.py."
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_directory")
    ...
    if beta:
        sxcmd_subconfig = SXsubcmd_config(
            "Local Refinement from Stack (Beta)",
            "Perform local refinement in which the restricted search begins from the user-provided orientation parameters stored in image headers. Note delta has to be less than or equal to 3.75[A].",
            token_edit_list,
            sxsubcmd_mpi_support,
            subset_config="stack_beta",
        )
    else:
        sxcmd_subconfig = SXsubcmd_config(
...
def create_sxcmd_subconfig_meridien_local_iteration(beta=False):
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_directory")
    token_edit.label = "Meridien Directory"
    token_edit.help = "This directory must exist. The results will be written there."
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    ...
    if beta:
        sxcmd_subconfig = SXsubcmd_config(
            "Restart Local Refinement (Beta)",
            "Restart local refinement after the last fully finished iteration of meridien run or local refinement run. One can change some parameters, but MPI settings have to be the same as in the original run.",
            token_edit_list,
            sxsubcmd_mpi_support,
            subset_config="iteration_beta",
        )
    else:
        sxcmd_subconfig = SXsubcmd_config(
...
def create_sxcmd_subconfig_meridien_final(beta=False):
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("do_final")
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit.default = -1
    token_edit.restore = -1
    token_edit_list.append(token_edit)

    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_directory")
    token_edit.label = "Meridien Output Directory"
    token_edit.help = "This directory must exist. In this mode information is read from files in this directory and the results will be written there."
    token_edit.is_required = True
    token_edit_list.append(token_edit)
    ...
    if beta:
        sxcmd_subconfig = SXsubcmd_config(
            "Final 3D Reconstruction Only (Beta)",
            "Compute a final 3D reconstruction using either select or best resolution iteration of meridien.",
            token_edit_list,
            sxsubcmd_mpi_support,
        )
    else:
...
def create_sxcmd_subconfig_sort3d_header_import_xform_projection():
    ...
    sxcmd_subconfig = SXsubcmd_config(
        "Import Projection Parameters",
        "Import projection orientation parameters from a file (for example created by sxmeridien) to header of the input stack; they are required by 3DVARIABILITY.py and SORT3D_DEPTH Stack Mode.",
        token_edit_list,
        sxsubcmd_mpi_support,
    )
...
def add_sxcmd_subconfig_sort3d_depth_shared_sorting(token_edit_list):
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("focus")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("radius")
    token_edit_list.append(token_edit)
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("img_per_grp")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("img_per_grp_split_rate")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("minimum_grp_size")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("do_swap_au")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("swap_ratio")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("memory_per_node")
    token_edit_list.append(token_edit)
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("depth_order")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("stop_mgskmeans_percentage")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("nsmear")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("orientation_groups")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("not_include_unaccounted")
    token_edit_list.append(token_edit)
    ###	token_edit = sxgui_template.SXcmd_token(); token_edit.initialize_edit("shake"); token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("notapplybckgnoise")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("random_group_elimination_threshold")
    token_edit_list.append(token_edit)
...
def create_sxcmd_subconfig_sort3d_depth_iteration():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("refinement_dir")
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit.default = "none"
    token_edit.restore = "none"
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("niter_for_sorting")
    token_edit_list.append(token_edit)
    ...
    sxcmd_subconfig = SXsubcmd_config(
        ...
        "Initialize from a given iteration of meridien run using the associated parameters, i.e., full set of orientation parameters per image, including orientation probabilities, normalizations and so on.",
        token_edit_list,
        sxsubcmd_mpi_support,
    )
...
def create_sxcmd_subconfig_sort3d_depth_stack():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("instack")
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit.default = "none"
    token_edit.restore = "none"
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("nxinit")
    token_edit_list.append(token_edit)
    add_sxcmd_subconfig_sort3d_depth_shared_sorting(token_edit_list)
    ###	# NOTE: 2018/01/08 Toshio Moriya
    ###	# post-refiner embedded sort3d_depth is removed recently.
    ...
    sxcmd_subconfig = SXsubcmd_config(
        ...
        "Run from user-provided orientation parameters stored in stack header.  This mode uses only highest probability orientation parameters per image, ths it should be avoided.  This program is used only for testing.",
        token_edit_list,
        sxsubcmd_mpi_support,
    )
...
def create_sxcmd_subconfig_sort3d_makevstack():
    ...
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("makevstack")
    token_edit.label = "Output stack subset"
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit.default = "none"
    token_edit.restore = "none"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("input_bdb_stack_file")
    token_edit.key_prefix = ""
    token_edit.label = "Original image stack"
    token_edit.help = (
        "Specify path to input BDB stack file used for the associated SORT3D_DEPTH run."
    )
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("list")
    token_edit.label = "Image selection file"
    token_edit.help = "Input selection text file containing a list of selected image IDs (or indexes of the data subset) to create a new virtual BDB image stack from an existed stack or virtual stack. Typically, Cluster#.txt created by sxsort3d_depth (e.g. Cluster1.txt)."
    token_edit_list.append(token_edit)
...
def create_sxcmd_subconfig_utility_makevstack():
    ...
    token_edit.initialize_edit("input_bdb_stack_file")
    token_edit.key_prefix = ""
    token_edit.label = "Input BDB image stack"
    token_edit.help = "Specify path to input BDB stack file. "
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "bdb2d_stack"
    ...
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("exlist")
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("step")
    token_edit_list.append(token_edit)

    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Create Virtual Stack", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def create_sxcmd_subconfig_utility_changesize():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("changesize")
    token_edit.is_required = True
    token_edit.is_locked = True
    token_edit.default = True
    token_edit.restore = True
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("input_stack")
    token_edit.key_prefix = ""
    token_edit.label = "Input image or volume"
    token_edit.help = "Input image or volume."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "data2d3d_both"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_stack")
    token_edit.key_prefix = ""
    token_edit.label = "Output resized image or volume"
    token_edit.help = "Resized (decimated or interpolated up) image or volume."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = ""
    token_edit.type = "output"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("ratio")
    token_edit_list.append(token_edit)

    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Change Size of Image or Volume", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def create_sxcmd_subconfig_utility_window():
    token_edit_list = []
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("clip")
    token_edit.label = "Window to specified size [Pixels]"
    token_edit.is_required = True
    token_edit.is_locked = False
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    ...
    token_edit.key_prefix = ""
    token_edit.label = "Input volume"
    token_edit.help = "Path to input volume file."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = "none"
    token_edit.type = "data3d_one"
    token_edit_list.append(token_edit)
    token_edit = sxgui_template.SXcmd_token()
    token_edit.initialize_edit("output_file")
    token_edit.key_prefix = ""
    token_edit.label = "Output windowed volume"
    token_edit.help = "Path to output windowed (clipped/padded) volume file."
    token_edit.group = "main"
    token_edit.is_required = True
    token_edit.default = "none"
    token_edit.type = "output"
    ...
    sxsubcmd_mpi_support = False
    sxcmd_subconfig = SXsubcmd_config(
        "Window Volume", None, token_edit_list, sxsubcmd_mpi_support
    )
...
def build_config_list_MoinMoinWiki():
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/cter.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            exclude_list=["stack_mode"],
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/gui_cter.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/e2display.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            exclude_list=create_exclude_list_display(),
            is_submittable=False,
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/pipe_organize_micrographs.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/e2boxer_old.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            exclude_list=create_exclude_list_boxer_old(),
            is_submittable=False,
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/window.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/e2bdb.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_window_makevstack(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/e2boxer.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            exclude_list=create_exclude_list_boxer(),
            is_submittable=False,
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/isac2.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            exclude_list=create_exclude_list_isac2(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/compute_isac_avg.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/pipe_isac_substack.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/rviper.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/viper.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/pdb2em.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/process.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_adaptive_mask3d(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/pipe_angular_distribution.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/meridien.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_standard_fresh(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/gui_meridien.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            is_submittable=False,
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_postrefiner_halfset_vol(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_local_iteration(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_standard_continuation(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_final(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/header.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_sort3d_header_import_xform_projection(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/3dvariability.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_variability_preprocess(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/sort3d_depth.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_sort3d_depth_iteration(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_sort3d_makevstack(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_sort3d_depth_stack(),
        )
    )
    ### # NOTE: 2018/01/08 Toshio Moriya
    ### # post-refiner embedded sort3d_depth is removed recently.
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_binary_mask3d(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_postrefiner_single_vols(),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/locres.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/filterlocal.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/unblur.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/gui_unblur.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            is_submittable=False,
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/relion2sphire.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/sphire2relion.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_utility_changesize(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/e2proc3d.txt",
            "MoinMoinWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_utility_window(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/MoinMoinWiki/header.txt", "MoinMoinWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_utility_makevstack(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
...
def build_config_list_DokuWiki(is_dev_mode=False):
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_resample_micrographs.txt",
            "DokuWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_organize_micrographs.txt",
            "DokuWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/cryolo_predict.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/window.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/e2bdb.txt",
    ...
    sxcmd_role = "sxr_alt"
    sxcmd_config_list.append(
        SXcmd_config("../doc/cryolo_train.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_restacking.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/rewindow.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/e2boxer_old.txt",
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/compute_isac_avg.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_isac_substack.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/separate_class.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/automatic_2d_class_selection.txt",
            "DokuWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_role = "sxr_pipe"
    sxcmd_config_list.append(
        SXcmd_config("../doc/rviper.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/proj_compare.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_moon_eliminator.txt", "DokuWiki", sxcmd_category, sxcmd_role
        )
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/mask.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    ...
    sxcmd_role = "sxr_alt"
    sxcmd_config_list.append(
        SXcmd_config("../doc/viper.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/pdb2em.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_angular_distribution.txt",
            "DokuWiki",
            sxcmd_category,
            sxcmd_role,
        )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_header_import_xform_projection(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/meridien_alpha.txt",
            "DokuWiki",
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_standard_fresh(beta=True),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_local_stack(beta=True),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_local_iteration(beta=True),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_standard_continuation(beta=True),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_meridien_final(beta=True),
        )
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config(
            ...
            sxcmd_category,
            sxcmd_role,
            subconfig=create_sxcmd_subconfig_balance_angles(),
        )
    )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    if is_dev_mode:
        sxcmd_config_list.append(
            SXcmd_config(
                ...
                sxcmd_category,
                sxcmd_role,
                exclude_list=create_exclude_list_sort3d(),
            )
        )
    sxcmd_config_list.append(
        SXcmd_config(
    ...
    sxcmd_config_list.append(
        SXcmd_config("../doc/recons3d_n.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/batch.txt",
    ...
    sxcmd_role = "sxr_pipe"
    sxcmd_config_list.append(
        SXcmd_config("../doc/locres.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/filterlocal.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    ...
    sxcmd_role = "sxr_pipe"
    sxcmd_config_list.append(
        SXcmd_config("../doc/unblur.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/gui_unblur.txt",
    ...
    sxcmd_role = "sxr_alt"
    sxcmd_config_list.append(
        SXcmd_config("../doc/unblur_old.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    ...
    sxcmd_config_list.append(
        SXcmd_config("../doc/summovie.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/pipe_organize_micrographs.txt",
    ...
    sxcmd_config_list.append(
        SXcmd_config("../doc/relion2sphire.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config("../doc/sphire2relion.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/separate_class.txt", "DokuWiki", sxcmd_category, sxcmd_role
    ...
    sxcmd_config_list.append(
        SXcmd_config("../doc/header.txt", "DokuWiki", sxcmd_category, sxcmd_role)
    )
    sxcmd_config_list.append(
        SXcmd_config(
            "../doc/e2bdb.txt",
...
def main(is_dev_mode=False, is_MoinMoinWiki_mode=False):
    ...
    # --------------------------------------------------------------------------------
    sxcmd_category_list = []
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_cter", "CTF", "CTF Estimation and CTF Assessment"
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_window",
            "Particle Stack",
            "Particle Coordinates and Particle Extraction",
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_isac", "2D Clustering", "ISAC2 2D Clustering and Beautifier"
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_viper", "Initial 3D Modeling", "Initial 3D modeling with VIPER/RVIPER"
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_meridien", "3D Refinement", "MERIDIEN 3d Refinement and PostRefiner"
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_sort3d",
            "3D Clustering",
            "3D Variability and SROT3D_DEPTH 3D Clustering",
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_localres", "Local Resolution", "Local Resolution, and Local Filtering"
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_movie",
            "Movie Micrograph",
            "Micrograph Movie Alignemnt and Drift Assessment",
        )
    )
    sxcmd_category_list.append(
        sxgui_template.SXcmd_category(
            "sxc_utilities", "Utilities", "Miscellaneous Utlitity Commands"
        )
    )
    ...
    for sxcmd_config in sxcmd_config_list:
        if not sxcmd_config.category in sxcmd_category_names:
            sp_global_def.ERROR(
                "Logical Error: sxcmd_config for %s is using invalid category %s."
                % (sxcmd_config.wiki, sxcmd_config.category),
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
    ...
    for line in sxgui_template_file:
        ...
        if current_state == state_template:
            if line.find("# @@@@@ START_INSERTION @@@@@") != -1:
                ...
                for sxcmd_config in sxcmd_config_list:
                    ...
                    if sxcmd_config.format == "MoinMoinWiki":
                        ...
                        sp_global_def.ERROR(
                            "Logical Error: Invalid Wiki format %s! Check the sxcmd_config setting in this script."
                            % (sxcmd_config.format),
                            "%s in %s" % (__name__, os.path.basename(__file__)),
                        )
            ...
            if current_state != state_insertion:
                sp_global_def.ERROR(
                    "Logical Error: This condition should not happen! State setting must be incorrect.",
                    "%s in %s" % (__name__, os.path.basename(__file__)),
                )
    ...
    if current_state == state_insertion:
        sp_global_def.ERROR(
            "Script Template Format Error: START_INSERTION and END_INSERTION must be paired.",
            "%s in %s" % (__name__, os.path.basename(__file__)),
        )
