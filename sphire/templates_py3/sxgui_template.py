...
def translate_to_bdb_path(std_path):
    ...
    if len(path_tokens) < 2:
        raise ValueError(
            "Invalid file path. The path pass must contain at least 'EMAN2DB' directory and '.bdb' file name (e.g './EMAN2DB/data.bdb'). "
        )

    if path_tokens[-2] != "EMAN2DB":
        raise ValueError(
            "Invalid file path. The path pass must contain 'EMAN2DB' directory (e.g './EMAN2DB/data.bdb')."
        )

    if os.path.splitext(path_tokens[-1])[1] != ".bdb":
        raise ValueError(
            "Path is invalid. The path pass must contain '.bdb' file name (e.g './EMAN2DB/data.bdb')."
        )
...
class SXmenu_item(object):
    def __init__(self, name="", label="", short_info=""):
        ...
        self.btn = (
            None
        )  # <Used only in sxgui.py> QPushButton button instance associating with this menu item
        self.widget = (
            None
        )  # <Used only in sxgui.py> SXCmdWidget instance associating with this menu item
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXcmd_token(object):
    def __init__(self):
        ...
        self.key_base = (
            ""
        )  # key base name of command token (argument or option) in command line
        self.key_prefix = (
            ""
        )  # key prefix of of command token. None for argument, "--" or "-" for option
        self.label = ""  # User friendly name of argument or option
        self.help = ""  # Help info
        self.group = ""  # Tab group: main or advanced
        self.dependency_group = [
            ["", "", ""]
        ]  # Depencency group: Disables or enables widgets
        self.is_required = (
            False
        )  # Required argument or options. No default values are available
        self.is_locked = False  # The restore value will be used as the locked value.
        self.is_reversed = (
            False
        )  # Reversed default value of bool. The flag will be added if the value is same as default
        ...
        self.is_in_io = (
            False
        )  # <Used only in wikiparser.py> To check consistency between "usage in command line" and list in "== Input ==" and "== Output ==" sections
        self.restore_widget = (
            None
        )  # <Used only in sxgui.py> Restore widget instance associating with this command token
        self.widget = (
            None
        )  # <Used only in sxgui.py> Main widget instance associating with this command token
        self.subwidget_left = (
            None
        )  # <Used only in sxgui.py> Subwidget instance at the left associating with the helper utility of this command token (e.g. conversion calculator)
        self.subwidget_right = (
            None
        )  # <Used only in sxgui.py> SubWidget instance at the right associating with the helper utility of this command token (e.g. conversion calculator)
        self.calculator_dialog = (
            None
        )  # <Used only in sxgui.py> Calculator dialog instance associating with the helper utility of this command token (e.g. conversion calculator)
        self.other_dialog_list = (
            []
        )  # <Used only in sxgui.py> List of the other calculator dialog instances associating with this command (e.g. conversion calculator)
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXcmd(object):
    def __init__(self, category="", role="", is_submittable=True):
        ...
        self.name = (
            ""
        )  # Name of this command (i.e. name of sx*.py script but without .py extension), used for generating command line
        self.subname = (
            ""
        )  # Subname of this command (i.e. sub-command name), used for generating command line. For fullset command, use empty string
        self.mode = (
            ""
        )  # Key base name of a command option token, defining mode/subset of this command (i.e. option mode name). For fullset command, use empty string
        self.subset_config = (
            ""
        ...
        self.mpi_add_flag = (
            False
        )  # DESIGN_NOTE: 2015/11/12 Toshio Moriya. This can be removed when --MPI flag is removed from all sx*.py scripts
        self.category = (
            category
        ...
        self.is_submittable = (
            is_submittable
        )  # External GUI Application (e.g. sxgui_cter.py) should not be submitted to job queue
        self.token_list = (
            []
        )  # List of command tokens. Need this to keep the order of command tokens
        self.token_dict = (
            {}
        )  # Dictionary of command tokens, organised by key base name of command token. Easy to access a command token but looses their order
        self.dependency_dict = (
            collections.OrderedDict()
        )  # Dictionary of command tokens, containing the dependencies
        self.btn = (
            None
        )  # <Used only in sxgui.py> QPushButton button instance associating with this command
        self.widget = (
            None
        )  # <Used only in sxgui.py> SXCmdWidget instance associating with this command
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
    ...
    def get_mode_name_for(self, target_name):
        ...
        if self.mode != "":
            if target_name in ["file_path"]:
                ...
                mode_name = "%s %s%s" % (
                    mode_name,
                    self.token_dict[self.mode].key_prefix,
                    self.mode,
                )
...
class SXcmd_category(SXmenu_item):
    def __init__(self, name="", label="", short_info=""):
        ...
        self.cmd_list = (
            []
        )  # <Used only in sxgui.py> list of commands in this category. Need this to keep the order of commands
...
class SXconst(object):
    def __init__(self):
        ...
        self.label = (
            ""
        )  # <Used only in sxgui.py> User friendly name of constant parameter
        ...
        self.widget = (
            None
        )  # <Used only in sxgui.py> Widget instance associating with this constant parameter
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXconst_set(SXmenu_item):
    def __init__(self):
        ...
        self.list = (
            []
        )  # <Used only in sxgui.py> list of constant parameters. Need this to keep the order of constant parameters
        self.dict = (
            {}
        )  # <Used only in sxgui.py> dictionary of constant parameters, organised by keys of constant parameters. Easy to access each constant parameter but looses their order
...
class SXoperand(object):
    def __init__(self):
        ...
        self.label = (
            ""
        )  # <Used only in sxgui.py> User friendly name of calculator operand
        self.help = ""  # <Used only in sxgui.py> Help info
        self.register = ""  # <Used only in sxgui.py> Default value
        self.type = ""  # <Used only in sxgui.py> Type of value
        self.is_input = (
            True
        )  # <Used only in sxgui.py> Flag to indicate if this operand is input or output
        self.validated_register = (
            None
        )  # <Used only in sxgui.py> Contain validated numerical value of register only if the value is valid. If not, it is None. This is used as a flag to indicate if the registered value is valid or not
        self.validated = (
            None
        )  # <Used only in sxgui.py> Contain Validated numerical value of command widget only if the value is valid. If not, it is None. This is used as a flag to indicate if the registered value is valid or not
        self.register_widget = (
            None
        )  # <Used only in sxgui.py> Restore widget instance associating with this calculator operand
        self.widget = (
            None
        )  # <Used only in sxgui.py> Widget instance associating with this calculator operand
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXoperand_set(SXmenu_item):
    def __init__(self):
        ...
        self.list = (
            []
        )  # <Used only in sxgui.py> list of calculator operands. Need this to keep the order of calculator operands
        self.dict = (
            {}
        )  # <Used only in sxgui.py> dictionary of calculator operands, organised by keys of calculator operands. Easy to access each calculator operand but looses their order
        # self.btn = None       # <Inherit from SXmenu_item> QPushButton button instance associating with this set
        # self.widget = None    # <Inherit from SXmenu_item> Widget instance associating with this set
        # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
...
class SXLookFeelConst(object):
    ...
    # ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
    default_bg_color = PyQt5.QtGui.QColor(
        229, 229, 229, 192
    )  # default_bg_color = QColor(229, 229, 229, 242) # Greyish-White Transparent
    default_bg_color_string = (
        "rgba(229, 229, 229, 192)"
    )  # default_bg_color = QColor(229, 229, 229, 242) # Greyish-White Transparent
    default_bg_color_calculator = PyQt5.QtGui.QColor(214, 214, 214)  # Greyish-White
    sxinfo_widget_bg_color = PyQt5.QtGui.QColor(
        0, 0, 0, 10
    ...
    def initialise(sxapp, version):
        ...
        for file_name in os.listdir(SXLookFeelConst.file_dialog_dir):
            if (
                SXLookFeelConst.project_dir_raw in file_name
                and file_name != SXLookFeelConst.project_dir
            ):
                print(
                    "WARNING: Directory %s/ detected that belongs to another version of SPHIRE."
                    % file_name
                )
        ...
        SXLookFeelConst.screen_width = (
            sxapp.desktop().screenGeometry(monitor_index).width()
        )
        ...
        if (
            SXLookFeelConst.sxmain_window_height
            <= SXLookFeelConst.sxmain_window_min_height
        ):
            SXLookFeelConst.sxmain_window_height = (
                SXLookFeelConst.sxmain_window_min_height
            )
        ...
        SXLookFeelConst.sxmenu_btn_area_min_width = (
            ...
            + SXLookFeelConst.grid_distance
            + 18
        )
        SXLookFeelConst.sxcmd_btn_area_min_width = 240
        SXLookFeelConst.sxcmd_widget_area_min_width = (
            SXLookFeelConst.sxmain_window_width
            - SXLookFeelConst.sxmenu_btn_area_min_width
            - SXLookFeelConst.sxcmd_btn_area_min_width
        )
    ...
    def generate_sxcmd_wiki_url(sxcmd, wiki_type="SPHIRE"):
        if wiki_type == "SPHIRE":
            ...
            elif sxcmd.name in ["sxpipe"] and sxcmd.subname in [
            ...
            sxcmd_wiki_url = "http://sphire.mpg.de/wiki/doku.php?id=pipeline:%s:%s" % (
                sxcmd_category_name,
                sxcmd.name,
            )
            if sxcmd.subname != "":
            ...
            sxcmd_wiki_url = "%s%s" % (
                sp_global_def.SPARX_DOCUMENTATION_WEBSITE,
                sxcmd.name,
            )
            if sxcmd.subname != "":
                sxcmd_wiki_url = "%s_%s" % (sxcmd_wiki_url, sxcmd.subname)
    ...
    def generate_sxmenu_item_wiki_url(sxmenu_item):
        ...
        if sxmenu_item.name in ["sxc_gui_info"]:
            ...
            sxmenu_item_wiki_url = (
                "http://sphire.mpg.de/wiki/doku.php?id=pipeline:%s:start"
                % (sxmenu_item.name.replace("sxc_", ""))
            )
...
class SXLogoButton(QPushButton):
    def __init__(self, logo_file_path, parent=None):
        ...
        # Width of logo image
        logo_width = (
            SXLookFeelConst.sxmenu_item_btn_width * 2 + SXLookFeelConst.grid_distance
        )
...
class SXPictogramButton(QPushButton):
    def __init__(self, pictogram_name, pictogram_file_path, parent=None):
        ...
        self.customButtonStyle = """
			...
			""".format(
            pictogram_file_path, old_div(pictogram_width, 6)
        )
        self.customButtonStyleClicked = """
			SXPictogramButton:pressed {{background-color: rgba(0, 0, 0, 0); border: 2px solid rgb(153, 153, 153); border-radius: {1}px; image: url("{0}");}}
...
class SXMenuItemBtnAreaWidget(QWidget):
    def __init__(self, sxconst_set, sxcmd_category_list, sxinfo, helical, parent=None):
        ...
        for sxcmd_category in sxcmd_category_list:
            if (
                sxcmd_category.name != "sxc_utilities"
                and sxcmd_category.name != "sxc_movie"
            ):
                self.add_sxmenu_item_btn_widget(
                    sxcmd_category, sxcmd_category_btn_subarea_widget
                )
            else:  # assert(sxcmd_category.name == "sxc_utilities")
                self.add_sxmenu_item_btn_widget(
                    sxcmd_category, misc_func_btn_subarea_widget
                )
        ...
        sxmenu_item_btn_area_widget.setStyleSheet(
            "QWidget#SXMenuItemBtnAreaWidget {background-color: rgba(0, 0, 0, 153);}"
        )
        sxmenu_item_btn_area_widget.setFixedWidth(
            SXLookFeelConst.sxmenu_btn_area_min_width
        )
        ...
        if helical:
            sxmenu_item_btn_pictograph_file_path = "{0}sxgui_logo_sphire_helix.png".format(
                EMAN2.get_image_directory()
            )
        else:
            sxmenu_item_btn_pictograph_file_path = "{0}sxgui_logo_sphire.png".format(
                EMAN2.get_image_directory()
            )
    ...
    def add_sxmenu_item_btn_widget(self, sxmenu_item, sxmenu_item_btn_subarea_widget):
        assert (
            isinstance(sxmenu_item, SXmenu_item) == True
        )  # Assuming the sxmenu_item is an instance of class SXmenu_item

        sxmenu_item_btn_pictograph_file_path = "{0}sxgui_pictograph_{1}.png".format(
            EMAN2.get_image_directory(), sxmenu_item.name.replace("sxc_", "")
        )
        sxmenu_item.btn = SXPictogramButton(
            sxmenu_item.name.replace("sxc_", ""),
            sxmenu_item_btn_pictograph_file_path,
            self,
        )
        cur_widget_counts = sxmenu_item_btn_subarea_widget.layout().count()
        sxmenu_item_btn_subarea_widget.layout().addWidget(
            sxmenu_item.btn, old_div(cur_widget_counts, 2), cur_widget_counts % 2
        )
...
class SXCmdWidget(QWidget):
    ...
    def __init__(self, sxconst_set, sxcmd, helical, parent=None):
        ...
        self.gui_settings_file_path = "%s/gui_settings_%s_%d.txt" % (
            self.sxcmd.get_category_dir_path(SXLookFeelConst.project_dir),
            self.sxcmd.get_mode_name_for("file_path"),
            int(helical),
        )
        ...
        palette.setBrush(
            PyQt5.QtGui.QPalette.Background,
            PyQt5.QtGui.QBrush(SXLookFeelConst.sxcmd_widget_bg_color),
        )
        ...
        for key in self.sxcmd.dependency_dict:
            if key not in self.sxcmd.token_dict:
                continue
            self.sxcmd.token_dict[key].widget.my_changed_state.connect(
                self.change_state
            )
    ...
    def change_state(
        self, dependency, state, old_dependency=None, sender=None, prev_sender=None
    ):
        """
        ...
        final_state = True
        for name, exp_state, inverse in parent.sxcmd.token_dict[
            dependency
        ].dependency_group:
            if not name:
                continue
            ...
            if (
                len(parent.sxcmd.token_dict[dependency].dependency_group) == 2
                and is_enabled
            ):
        ...
        for sxcmd_token in self.sxcmd.token_list:
            ...
            if do_continue:
                ...
                external_file_path = str(
                    sxcmd_token.widget[external_file_path_index].text()
                )

                # This is not default value
                if external_file_path not in [
                    "",
                    sxcmd_token.default[external_file_path_index],
                ]:
                    ...
                    if os.path.splitext(external_file_path)[1] != ".py":
                        QMessageBox.warning(
                            ...
                            "Exteranl File Path (%s) should include the python script extension (.py)."
                            % (external_file_path),
                        )
                        return ""
                    dir_path, file_basename = os.path.split(external_file_path)
                    file_basename = file_basename.replace(".py", "")
                    sxcmd_line += " %s%s=[%s,%s,%s]" % (
                        sxcmd_token.key_prefix,
            ...
            # Then, handle the other cases//
                ...
                if sxcmd_token.type == "bool_ignore":
                    ...
                    if sxcmd_token.is_required:
                        ...
                        if (
                            not sxcmd_token.is_reversed
                            and (
                                sxcmd_token.widget.checkState()
                    ...
                    if sxcmd_token.widget.text() == sxcmd_token.default:
                        ...
                        if sxcmd_token.is_required == True:
                            QMessageBox.warning(
                                self,
                                "Invalid parameter value",
                                "Token (%s) of command (%s) is required. Please set the value for this."
                                % (
                                    sxcmd_token.label,
                                    self.sxcmd.get_mode_name_for("human"),
                                ),
                            )
                            return ""
                        ...
                        if sxcmd_token.type not in [
                            ...
                            widget_text = widget_text.strip(
                                "'"
                            )  # make sure the string is not enclosed by (')
                            widget_text = widget_text.strip(
                                '"'
                            )  # make sure the string is not enclosed by (")
                            widget_text = "'%s'" % (
                                widget_text
                            )  # then, enclose the string value with single quotes (')
                        ...
                        if sxcmd_token.key_prefix == "":
                            ...
                            sp_global_def.ERROR(
                                "Logical Error: Encountered unexpected prefix for token (%s) of command (%s). Consult with the developer."
                                % (
                                    sxcmd_token.key_base,
                                    self.sxcmd.get_mode_name_for("human"),
                                ),
    ...
    def generate_cmd_line(self):
        ...
        if sxcmd_line:
            ...
            if self.sxcmd.mpi_support:
                ...
                if required_key_base != None:
                    required_divisor = int(
                        str(self.sxcmd.token_dict[required_key_base].widget.text())
                    )
                    ...
                    if required_divisor == 0:
                        QMessageBox.warning(
                            self,
                            "Invalid parameter value",
                            '"%s" must be larger than 0. Please check the setting'
                            % (required_label),
                        )
                        return ""
                    ...
                    if valid_np % required_divisor != 0:
                        ...
                        QMessageBox.warning(
                            self,
                            "Invalid parameter value",
                            'The number of "MPI processes" (%d) is invalid. It MUST BE multiplicity of "%s" (%d). Please check the setting. A close valid number is %d.'
                            % (np, required_label, required_divisor, valid_np),
                        )
                        return ""
            ...
            cmd_line = ""
            if (
                self.sxcmd_tab_main.qsub_enable_checkbox.checkState()
                == PyQt5.QtCore.Qt.Checked
            ):
                ...
                if os.path.exists(self.sxcmd_tab_main.qsub_script_edit.text()) != True:
                    QMessageBox.warning(
                        self,
                        "Invalid parameter value",
                        "Invalid file path for qsub script template (%s)."
                        % (self.sxcmd_tab_main.qsub_script_edit.text()),
                    )
                    return ""
                ...
                for line in file_template:
                    if line.find("XXX_SXCMD_LINE_XXX") != -1:
                        if np > 1:
                            ...
                            cmd_line = (
                                "XXX_SXCMD_LINE_XXX "
                                + line.split("XXX_SXCMD_LINE_XXX")[-1]
                            )
                        ...
                        if cmd_line.find("XXX_SXMPI_JOB_NAME_XXX") != -1:
                            cmd_line = cmd_line.replace(
                                ...
                                str(self.sxcmd_tab_main.qsub_job_name_edit.text()),
                            )
                file_template.close()
                ...
                if (
                    ...
                    sp_global_def.ERROR(
                        "Logical Error: Encountered unexpected condition for sxcmd_tab_main.qsub_enable_checkbox.checkState. Consult with the developer.",
                        "%s in %s" % (__name__, os.path.basename(__file__)),
                    )
                ...
                if cmd_line == "":
                    if np > 1:
                        ...
                        cmd_line = (
                            "XXX_SXCMD_LINE_XXX "
                            + cmd_line.split("XXX_SXCMD_LINE_XXX")[-1]
                        )
                if cmd_line.find("XXX_SXMPI_NPROC_XXX") != -1:
                    cmd_line = cmd_line.replace("XXX_SXMPI_NPROC_XXX", str(np))
    ...
    def execute_cmd_line(self, execute=True, output_dir=".", number=None):
        ...
        if cmd_line:
            ...
            for sxcmd_token in self.sxcmd.token_list:
                if (
                    sxcmd_token.type == "output"
                    or sxcmd_token.type == "output_continue"
                    or sxcmd_token.type == "output_bdb2d_stack"
                ):
                    if os.path.exists(
                        sxcmd_token.widget.text()
                    ) or EMAN2db.db_check_dict(str(sxcmd_token.widget.text())):
                        ...
                        if sxcmd_token.type == "output_continue":
                            reply = QMessageBox.question(
                                ...
                                "Output Directory/File (%s) already exists. Do you really want to run the program with continue mode?"
                                % (sxcmd_token.widget.text()),
                            ...
                            QMessageBox.warning(
                                self,
                                "Output Directory/File",
                                "Output Directory/File (%s) already exists. Please change the name and try it again. Aborting execution ..."
                                % (sxcmd_token.widget.text()),
                            )
                            return
            ...
            if (
                ...
                if os.path.exists(template_file_path) == False:
                    QMessageBox.warning(
                        self,
                        "Invalid parameter value",
                        "Invalid file path for qsub script template (%s). Aborting execution ..."
                        % (template_file_path),
                    )
                    return
                file_template = open(self.sxcmd_tab_main.qsub_script_edit.text(), "r")
                if number is not None:
                    file_name_qsub_script = os.path.join(
                        output_dir,
                        "{0:04d}_".format(number)
                        + "qsub_"
                        + str(self.sxcmd_tab_main.qsub_job_name_edit.text())
                        + ".sh",
                    )
                ...
                cmd_line = (
                    str(self.sxcmd_tab_main.qsub_cmd_edit.text())
                    + " "
                    + file_name_qsub_script
                )
                print(
                    "Wrote the following command line in the queue submission script: "
                )
                ...
                QMessageBox.warning(
                    self,
                    "Qsub template needs to be specified for pipeline option",
                    "Qsub template needs to be specified for pipeline option",
                )
                return
            else:
    ...
    def add_to_pipeline(self):
        name = PyQt5.QtGui.QFileDialog.getExistingDirectory(
            self,
            "Pipeline output directory",
            SXLookFeelConst.file_dialog_dir,
        ...
        if dir_name:
            ...
            for file_name in sorted(os.listdir(dir_name)):
                try:
                    ...
                    QMessageBox.warning(
                        self,
                        "Invalid file in pipeline directory",
                        "There is a file in the pipeline directory not starting with four digits. Abort...",
                    )
                    return

            self.execute_cmd_line(
                execute=False, output_dir=dir_name, number=int(number) + 1
            )
    ...
    def print_cmd_line(self):
        ...
        if cmd_line:
            ...
            print(message_line)
            print(cmd_line)
            PyQt5.QtGui.QMessageBox.information(
    ...
    def write_params(self, file_path_out):
        ...
        file_out.write(
            "@@@@@ %s gui settings - " % (self.sxcmd.get_mode_name_for("human"))
        )
                    ...
                    if cmd_token.type == "user_func":
                        ...
                        for widget_index in range(n_widgets):
                            ...
                            file_out.write(
                                ...
                                % (
                                    cmd_token.key_base,
                                    cmd_token.label[widget_index],
                                    cmd_token.default[widget_index],
                                    val_str,
                                )
                            )
                    # Then, handle the other cases
        ...
        file_out.write(
            "%s == %s \n"
            % (
                "MPI Command Line Template",
                str(self.sxcmd_tab_main.mpi_cmd_line_edit.text()),
            )
        )
        ...
        file_out.write(
            ...
            % (
                ...
                str(self.sxcmd_tab_main.qsub_script_edit.text()),
            )
        )
        ...
        if (
            ...
            for line_in in file_in:
                ...
                if label_in == "MPI processors":
                    ...
                    if val_str_in == "YES":
                        self.sxcmd_tab_main.qsub_enable_checkbox.setChecked(
                            PyQt5.QtCore.Qt.Checked
                        )
                    ...
                    if item_tail != 0:
                        QMessageBox.warning(
                            self,
                            "Invalid Parameter File Format",
                            'Command token entry should start from "%s" for key base name in line (%s) of file (%s). The format of this file might be corrupted. Please save the parameter file again.'
                            % (target_operator, line_in, file_path_in),
                        )
                    label_in = label_in[
                    ...
                    if item_tail == -1:
                        QMessageBox.warning(
                            self,
                            "Invalid Parameter File Format",
                            'Command token entry should have "%s" closing key base name in line (%s) of file (%s). The format of this file might be corrupted. Please save the parameter file again.'
                            % (target_operator, line_in, file_path_in),
                        )
                    ...
                    if key_base not in list(self.sxcmd.token_dict.keys()):
                        QMessageBox.warning(
                            self,
                            "Invalid Parameter File Format",
                            'Invalid base name of command token "%s" is found in line (%s) of file (%s). This parameter file might be incompatible with the current version. Please save the parameter file again.'
                            % (key_base, line_in, file_path_in),
                        )
                    cmd_token = self.sxcmd.token_dict[key_base]
                    if not cmd_token.is_locked:
                        # First, handle very special cases
                        if cmd_token.type == "user_func":
                            cmd_token.widget[function_type_line_counter].setText(
                                val_str_in
                            )
                            function_type_line_counter += 1
                            function_type_line_counter %= (
                                n_function_type_lines
                            )  # function have two line edit boxes
                        else:
                            if cmd_token.type in ("bool", "bool_ignore"):
            ...
            QMessageBox.warning(
                ...
                "The specified file parameter file for %s could not be read."
                % self.sxcmd.get_mode_name_for("human"),
            )
    ...
    def select_file(self, target_widget, file_format=""):
        ...
        if file_format == "displayable_list":
            ...
            name = QFileDialog.getOpenFileNames(
                self,
                "Select any displayable files",
                SXLookFeelConst.file_dialog_dir,
                "Typical displayable files (*.hdf *.bdb *.mrc *.mrcs *.spi *.img *.tif *.tiff *.png *.txt);; HDF (*.hdf);; BDB (*.bdb);; MRC (*.mrc);; MRCS (*.mrcs);; Spider (*.spi);; Imagic (*.img *hed);; TIFF (*.tif *.tiff);; PNG (*.png);; Text (*.txt);; Gatan (*.dm2 *.dm3 *.dm4);; FEI (*.ser);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PGM (*.pgm);; PIF (*.pif);; SAL (*.hdr *.img);; SITUS (*.situs);; V4L (*.v4l);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
            ...
            for a_file_path in file_path_list:
                ...
                try:  # Check if the path is bdb
                    a_file_path = translate_to_bdb_path(
                        a_file_path
                    )  # Convert the standard path to bdb key if possible.
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any image/volume file",
                SXLookFeelConst.file_dialog_dir,
                "Typical image & volume files (*.hdf *.bdb *.mrc *.mrcs *.spi *.img *.tif *.tiff *.png);; HDF (*.hdf);; BDB (*.bdb);; MRC (*.mrc);; MRCS (*.mrcs);; Spider (*.spi);; Imagic (*.img *hed);; TIFF (*.tif *.tiff);; PNG (*.png);; Gatan (*.dm2 *.dm3 *.dm4);; FEI (*.ser);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PGM (*.pgm);; PIF (*.pif);; SAL (*.hdr *.img);; SITUS (*.situs);; V4L (*.v4l);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            if file_path:
                ...
                try:  # Check if the path is bdb
                    file_path = translate_to_bdb_path(
                        file_path
                    )  # Convert the standard path to bdb key if possible.
                except ValueError:  # If the path is not bdb, we will receive this exception
                    pass  # This is not bdb path. Then, use standard path
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select MRC micrograph/movie file",
                SXLookFeelConst.file_dialog_dir,
                "MRC micrograph & movie files (*.mrc *.mrcs);; MRC (*.mrc);; MRCS (*.mrcs)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any micrograph/movie file",
                SXLookFeelConst.file_dialog_dir,
                "Typical micrograph & movie files (*.mrc *.mrcs *.tif *.tiff *.hdf *.bdb *.spi *.img);; MRC (*.mrc);; MRCS (*.mrcs);; TIFF (*.tif *.tiff);; HDF (*.hdf);; BDB (*.bdb);; Spider (*.spi);; Imagic (*.img);; PNG (*.png);; Gatan (*.dm2 *.dm3 *.dm4);; FEI (*.ser);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PGM (*.pgm);; PIF (*.pif);; SAL (*.img );; SITUS (*.situs);; V4L (*.v4l);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select MRC micrograph file",
                SXLookFeelConst.file_dialog_dir,
                "MRC micrograph files (*.mrc);; MRCS (*.mrcs)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any micrograph file",
                SXLookFeelConst.file_dialog_dir,
                "Typical micrograph files (*.mrc *.mrcs *.tif *.tiff *.hdf *.bdb *.spi *.img);; MRC (*.mrc);; MRCS (*.mrcs);; TIFF (*.tif *.tiff);; HDF (*.hdf);; BDB (*.bdb);; Spider (*.spi);; Imagic (*.img);; PNG (*.png);; Gatan (*.dm2 *.dm3 *.dm4);; FEI (*.ser);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PGM (*.pgm);; PIF (*.pif);; SAL (*.img );; SITUS (*.situs);; V4L (*.v4l);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileNames(
                self,
                "Select MRC micrograph files",
                SXLookFeelConst.file_dialog_dir,
                "MRC files (*.mrc);; MRCS (*.mrcs)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path_list = name[0]
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select MRC movie file",
                SXLookFeelConst.file_dialog_dir,
                "MRC movie files (*.mrcs);; MRC (*.mrc)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any movie file",
                SXLookFeelConst.file_dialog_dir,
                "Typical movie files (*.mrcs *.mrc *.bdb *.hdf *.spi *.img );; MRCS (*.mrcs);; MRC (*.mrc);; BDB (*.bdb);; HDF (*.hdf);; Spider (*.spi);; Imagic (*.img *hed);; Gatan (*.dm4);; FEI (*.ser);; LST (*.lst);; LSTFAST (*.lsx *.lst);; PIF (*.pif);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                ...
                SXLookFeelConst.file_dialog_dir,
                "Typical image files (*.hdf *.bdb *.mrc *.spi *.img *.tif *.tiff *.png);; HDF (*.hdf);; BDB (*.bdb);; MRC (*.mrc);; Spider (*.spi);; Imagic (*.img);; TIFF (*.tif *.tiff);; PNG (*.png);; Gatan (*.dm2 *.dm3 *.dm4);; FEI (*.ser);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PGM (*.pgm);; PIF (*.pif);; SAL (*.img );; SITUS (*.situs);; V4L (*.v4l);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any image stack file",
                SXLookFeelConst.file_dialog_dir,
                "Typical image stack files (*.bdb *.hdf *.mrcs *.spi *.img );; BDB (*.bdb);; HDF (*.hdf);; MRCS (*.mrcs);; Spider (*.spi);; Imagic (*.img *hed);; Gatan (*.dm4);; FEI (*.ser);; LST (*.lst);; LSTFAST (*.lsx *.lst);; PIF (*.pif);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select HDF volume file",
                SXLookFeelConst.file_dialog_dir,
                "HDF volume files (*.hdf)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any volume file",
                SXLookFeelConst.file_dialog_dir,
                "Typical volume files (*.hdf *.bdb *.mrc *.spi *.img);; HDF (*.hdf);; BDB (*.bdb);; MRC (*.mrc);; Spider (*.spi);; Imagic (*.img);; Gatan (*.dm4);; EM (*.em);; ICOS (*.icos);; Amira (*.am);; DF3 (*.d3);; FITS (*.fts);; LST (*.lst);; LSTFAST (*.lsx *.lst);; OMAP (*.omap);; PIF (*.pif);; SITUS (*.situs);; VTK (*.vtk);; XPLOR (*.xplor);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select HDF volume stack file",
                SXLookFeelConst.file_dialog_dir,
                "HDF volume stack files (*.hdf)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any volume stack file",
                SXLookFeelConst.file_dialog_dir,
                "Typical volume stack files (*.hdf *.bdb *.mrcs *.img);; HDF (*.hdf);; BDB (*.bdb);; MRCS (*.mrcs);; Imagic (*.img *hed);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select micrograph/movie selection file",
                SXLookFeelConst.file_dialog_dir,
                "Micrograph/Movie selection files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select micrograph selection file",
                SXLookFeelConst.file_dialog_dir,
                "Micrograph selection files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select image selection file",
                SXLookFeelConst.file_dialog_dir,
                "Image selection files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select drift shift params selection file",
                SXLookFeelConst.file_dialog_dir,
                "Drift shift params selection files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                ...
                SXLookFeelConst.file_dialog_dir,
                "Parameters text files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select projection parameters file",
                SXLookFeelConst.file_dialog_dir,
                "Projection parameters files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select EMAN BOX coordinates file",
                SXLookFeelConst.file_dialog_dir,
                "EMAN BOX coordinates files (*.box)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select any coordinates file",
                SXLookFeelConst.file_dialog_dir,
                "Typical coordinates files (*.box *.json *.dat *.txt);; EMAN BOX (*.box);; EMAN2 JSON (*.json);; SPIDER DAT (*.dat);; SPHIRE TXT (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select CTER partres parameters file",
                SXLookFeelConst.file_dialog_dir,
                "CTER partres parameters files (*.txt)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select SPHIRE rebox file",
                SXLookFeelConst.file_dialog_dir,
                "SPHIRE rebox files (*.rbx)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select drift shift parameters file",
                SXLookFeelConst.file_dialog_dir,
                "MotionCor2|cisTEM Unblur (*.log);;Drift shift parameters files (*.txt);;All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select rotational matrix file",
                SXLookFeelConst.file_dialog_dir,
                "Rotational matrix files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select RELION STAR file",
                SXLookFeelConst.file_dialog_dir,
                "RELION STAR files (*.star);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select 1D power spectrum file",
                SXLookFeelConst.file_dialog_dir,
                "1D power spectrum files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select MTF data file",
                SXLookFeelConst.file_dialog_dir,
                "MTF data files (*.txt);; All files (*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                self,
                "Select PDB data file",
                SXLookFeelConst.file_dialog_dir,
                "PDB data files (*.pdb *.pdb*)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
            ...
            name = QFileDialog.getOpenFileName(
                ...
                SXLookFeelConst.file_dialog_dir,
                "All files (*);; Python (*.py) ;; Executable files (*.exe)",
                options=QFileDialog.DontUseNativeDialog,
            )
            if isinstance(name, tuple):
                file_path = str(name[0])
        ...
        ###				file_path += SXLookFeelConst.format_path(str(a_file_path)) + " "
            ...
            if file_format:
                name = QFileDialog.getOpenFileName(
                    self,
                    "Select %s file" % (file_format.upper()),
                    SXLookFeelConst.file_dialog_dir,
                    "%s files (*.%s)" % (file_format.upper(), file_format),
                    options=QFileDialog.DontUseNativeDialog,
                )
            else:
                name = QFileDialog.getOpenFileName(
...
class SXLineEdit(PyQt5.QtGui.QLineEdit):
    ...
    def __init__(self, dependency, parent=None):
        ...
        self.textChanged.connect(
            lambda x: self.my_changed_state.emit(self.dependency, x)
        )
...
class SXCmdTab(QWidget):
    def __init__(self, name, helical, parent=None):
        ...
        const_cmd_token_restore_tooltip = (
            "Retrieve the registed constant value for this parameter"
        )
        ...
        title_row_span = 1
        title_col_span = 2
        short_info_row_span = 1
        short_info_col_span = 5
        func_btn_row_span = 1
        func_btn_col_span = 2
        token_label_row_span = 1
        token_label_col_span = 4
        token_widget_row_span = 1
        token_widget_col_span = 1.5
        cmd_frame_row_span = 32
        cmd_frame_col_span = 7
        ...
        palette.setBrush(
            PyQt5.QtGui.QPalette.Background,
            PyQt5.QtGui.QBrush(SXLookFeelConst.sxcmd_tab_bg_color),
        )
        ...
        grid_layout.setColumnMinimumWidth(
            grid_col_origin + token_label_col_span, token_widget_min_width
        )
        grid_layout.setColumnMinimumWidth(
            grid_col_origin + token_label_col_span + token_widget_col_span,
            token_widget_min_width,
        )
        grid_layout.setColumnMinimumWidth(
        ...
        submit_layout.setContentsMargins(
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
        ...
        submit_layout.setColumnMinimumWidth(
            grid_col_origin + token_label_col_span, token_widget_min_width
        )
        submit_layout.setColumnMinimumWidth(
            grid_col_origin + token_label_col_span + token_widget_col_span,
        ...
        title_layout.setColumnStretch(
            grid_row_origin + token_label_col_span,
            title_layout.columnStretch(grid_row_origin + token_label_col_span) + 1,
        )
        ...
        if tab_group == "main":
            ...
            temp_label = QLabel(
                "<b>%s</b>" % (self.sxcmdwidget.sxcmd.get_mode_name_for("human"))
            )
            ...
            title_layout.addWidget(
                temp_label, grid_row, grid_col_origin, title_row_span, title_col_span
            )
            ...
            title_layout.addWidget(
                temp_label,
                grid_row,
                grid_col_origin + title_col_span,
                short_info_row_span,
                short_info_col_span,
            )
            ...
            if curr_group == tab_group:
                ...
                if cmd_token.type == "user_func":
                    ...
                    grid_layout.addWidget(
                        ...
                        grid_row,
                        grid_col_origin,
                        token_label_row_span,
                        token_label_col_span,
                    )

                    assert cmd_token.is_required == False
                    restores = cmd_token.restore[widget_index][helical]
                    if len(restores) == 1:
                        cmd_token_restore_widget[widget_index] = QPushButton(
                            "%s" % restores[0]
                        )
                        cmd_token_restore_widget[widget_index].setStyleSheet(
                            custom_style
                        )
                        cmd_token_restore_widget[widget_index].setToolTip(
                            "<FONT>" + default_cmd_token_restore_tooltip + "</FONT>"
                        )
                        grid_layout.addWidget(
                            cmd_token_restore_widget[widget_index],
                            grid_row,
                        ...
                        cmd_token_restore_widget[widget_index].clicked.connect(
                            functools.partial(
                                self.handle_restore_widget_event,
                                cmd_token,
                                widget_index,
                            )
                        )
                        ...
                        cmd_token_restore_widget[widget_index].lineEdit().setReadOnly(
                            True
                        )
                        cmd_token_restore_widget[widget_index].lineEdit().setAlignment(
                        ...
                        cmd_token_restore_widget[widget_index].activated.connect(
                            functools.partial(
                                self.handle_restore_widget_event,
                                cmd_token,
                                widget_index,
                    ...
                    cmd_token_widget[widget_index].setText(
                        cmd_token.restore[widget_index][helical][0]
                    )
                    cmd_token_widget[widget_index].setToolTip(
                        "<FONT>" + cmd_token.help[widget_index] + "</FONT>"
                    )
                    grid_layout.addWidget(
                        cmd_token_widget[widget_index],
                        grid_row,
                        grid_col_origin + token_label_col_span + token_widget_col_span,
                        token_widget_row_span,
                        token_widget_col_span,
                    ...
                    assert cmd_token.is_required == False
                    cmd_token_restore_widget[widget_index] = QPushButton(
                        "%s" % cmd_token.restore[widget_index][helical][0]
                    )
                    ...
                    cmd_token_widget[widget_index].setText(
                        cmd_token.restore[widget_index][helical][0]
                    )  # Because default user functions is internal
                    cmd_token_widget[widget_index].setToolTip(
                        "<FONT>" + cmd_token.help[widget_index] + "</FONT>"
                    )
                    grid_layout.addWidget(
                    ...
                    cmd_token_restore_widget[widget_index].clicked.connect(
                        functools.partial(
                            self.handle_restore_widget_event, cmd_token, widget_index
                        )
                    )
                    ...
                    temp_btn.setToolTip(
                        "<FONT>"
                        + "Display open file dialog to select .%s python script file</FONT>"
                        % file_format
                    )
                    grid_layout.addWidget(
                        temp_btn,
                        ...
                        grid_col_origin
                        + token_label_col_span
                        + token_widget_col_span * 2,
                        token_widget_row_span,
                        token_widget_col_span,
                    )
                    temp_btn.clicked.connect(
                        functools.partial(
                            self.sxcmdwidget.select_file,
                            cmd_token_widget[widget_index],
                            file_format,
                        )
                    )
                ...
                # Then, handle the other cases
                    ...
                    if cmd_token.type in ("bool", "bool_ignore"):
                        if len(restores) == 1:
                            ...
                            if cmd_token.type in list(parent.sxconst_set.dict.keys()):
                                ...
                                if cmd_token.is_locked:
                                    ...
                                    is_btn_enable = False
                                    cmd_token_restore_tooltip = (
                                        locked_cmd_token_restore_tooltip
                                    )
                                else:
                                    btn_name = "required"
                                    ...
                                    is_btn_enable = False
                                    cmd_token_restore_tooltip = (
                                        required_cmd_token_restore_tooltip
                                    )
                            ...
                            cmd_token_restore_widget.clicked.connect(
                                functools.partial(
                                    self.handle_restore_widget_event, cmd_token
                                )
                            )
                        else:
                            ...
                            cmd_token_restore_widget.lineEdit().setAlignment(
                                PyQt5.QtCore.Qt.AlignCenter
                            )
                            ...
                            cmd_token_restore_widget.activated.connect(
                                functools.partial(
                                    self.handle_restore_widget_event, cmd_token
                                )
                            )
                        ...
                        if cmd_token.type == "displayable_list":
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select displayable data files of any supported formats</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>" + "This is %s button</FONT>" % file_format
                            )
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select an image/volume file of any supported format</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a MRC format micrograph or movie file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a micrograph or movie file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a MRC format micrograph file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a micrograph file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select MRC format micrograph files</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select micrograph files of any supported formats</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a MRC format movie file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a movie file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a HDF format image file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a image file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            cmd_token.type == "bdb2d_stack"
                            or cmd_token.type == "output_bdb2d_stack"
                        ):
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a BDB format image stack file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a image stack file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a HDF format volume file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a volume file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a HDF format volume stack file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a volume stack file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a micrograph/movie selection file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a micrograph selection file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a image selection file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a drift shift parameters selection file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a parameters text file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a projection parameters file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a EMAN BOX coordinates file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a coordinates file of any supported format</FONT>"
                            )
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            grid_layout.addWidget(
                                temp_btn,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a CTER partres parameters file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a SPHIRE rebox file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a drift shift parameters file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a rotational matrix file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a RELION STAR file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dailog to select a JSON file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dailog to select a h5 file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a 1D power spectrum file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a MTF data file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select a PDB data file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.setToolTip(
                                "<FONT>"
                                + "Display open file dialog to select an executable file</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            cmd_token.type == "dir"
                            or cmd_token.type == "dir_list"
                            or cmd_token.type == "output_continue"
                        ):
                            temp_btn = QPushButton("Select directory")
                            temp_btn.setMinimumWidth(func_btn_min_width)
                            temp_btn.setToolTip(
                                "<FONT>" + "Display select directory dialog" + "</FONT>"
                            )
                            grid_layout.addWidget(
                                temp_btn,
                                grid_row,
                            ...
                            temp_btn.clicked.connect(
                                functools.partial(
                                    self.sxcmdwidget.select_dir, cmd_token_widget
                                )
                            )
                            file_format = "INVISIBLE"
                            temp_btn = QPushButton("%s" % file_format)
                            ...
                            cmd_token_subwidget_left = QPushButton(
                                "Label for Resolution [A]"
                            )
                            cmd_token_subwidget_left.setEnabled(False)
                            cmd_token_subwidget_left.setMinimumWidth(func_btn_min_width)
                            cmd_token_subwidget_left.setToolTip(
                                "<FONT>"
                                + "Resolution [A] corresponding to absolute frequency [1/Pixel]"
                                + "</FONT>"
                            )
                            grid_layout.addWidget(
                                cmd_token_subwidget_left,
                                grid_row,
                                grid_col_origin
                                + token_label_col_span
                                + token_widget_col_span * 2,
                            ...
                            cmd_token_subwidget_right = QPushButton(
                                "Use resolution [A]"
                            )
                            cmd_token_subwidget_right.setToolTip(
                                "<FONT>"
                                + "Display calculator dialog to use the resolution [A] instead of absolute frequency [1/Pixel]. It calculates absolute frequency [1/Pixel] (abs_freq) From resolution [A] (ares) using a give pixel size [A/Pixel] (apix), where abs_freq = apix/ares. </FONT>"
                            )
                            cmd_token_subwidget_right.setMinimumWidth(
                                func_btn_min_width
                            )
                            grid_layout.addWidget(
                                cmd_token_subwidget_right,
                                grid_row,
                                grid_col_origin
                                + token_label_col_span
                                + token_widget_col_span * 3,
                            ...
                            const_register_widget_apix = self.sxcmdwidget.sxconst_set.dict[
                                "apix"
                            ].register_widget
                            cmd_token_calculator_dialog = SXDialogCalculator(
                                const_register_widget_apix,
                                cmd_token_widget,
                                cmd_token_subwidget_left,
                                self,
                            )
                            ...
                            cmd_token_widget.editingFinished.connect(
                                cmd_token_calculator_dialog.reflect_external_local_update_abs_freq
                            )
                            # Connect the right subwidget "clicked" event to the calculator dialog
                            cmd_token_subwidget_right.clicked.connect(
                                cmd_token_calculator_dialog.show
                            )
                        ...
                        ###							grid_layout.addWidget(temp_btn, grid_row, grid_col_origin + token_label_col_span + token_widget_col_span * 3, token_widget_row_span, token_widget_col_span)
                            ...
                            if cmd_token.type not in [
                                ...
                                sp_global_def.ERROR(
                                    "Logical Error: Encountered unsupported type (%s). Consult with the developer."
                                    % cmd_token.type,
                                    "%s in %s" % (__name__, os.path.basename(__file__)),
                                )
                    ...
                    cmd_token_restore_widget.setToolTip(
                        "<FONT>" + cmd_token_restore_tooltip + "</FONT>"
                    )
        ...
        if is_necessary_to_connet_now:
            ...
            for cmd_token_abs_freq in cmd_token_abs_freq_list:
                ...
                for other_cmd_token_abs_freq in cmd_token_abs_freq_list:
                    ...
                    if cmd_token_abs_freq.key_base != other_cmd_token_abs_freq.key_base:
                        cmd_token_abs_freq.other_dialog_list.append(
                            other_cmd_token_abs_freq.calculator_dialog
                        )
                        cmd_token_abs_freq.calculator_dialog.sxcmd_token_other_dialog_list_abs_freq.append(
                            other_cmd_token_abs_freq.calculator_dialog
                        )
                ...
                cmd_token_abs_freq.calculator_dialog.sxcmd_token_widget_apix = (
                    cmd_token_apix.widget
                )
                # Register this absolut frequency calculator dialog to pixel size token
                cmd_token_apix.other_dialog_list.append(
                    cmd_token_abs_freq.calculator_dialog
                )
            ...
            cmd_token_apix.widget.editingFinished.connect(
                functools.partial(
                    self.handle_apix_token_editing_finished_event, cmd_token_apix
                )
            )
        ...
        if tab_group == "main":
            ...
            self.mpi_nproc_edit.setToolTip(
                "<FONT>"
                + "Number of processors to use. default is single processor mode"
                + "</FONT>"
            )
            submit_layout.addWidget(
                self.mpi_nproc_edit,
            ...
            self.save_params_btn.setToolTip(
                "<FONT>" + "Save gui parameter settings" + "</FONT>"
            )
            ...
            if "SPHIRE_MPI_COMMAND_LINE_TEMPLATE" in os.environ:
                mpi_command_line_template = os.environ[
                    "SPHIRE_MPI_COMMAND_LINE_TEMPLATE"
                ]
            else:
                mpi_command_line_template = ""
            self.mpi_cmd_line_edit.setText(mpi_command_line_template)
            self.mpi_cmd_line_edit.setToolTip(
                "<FONT>"
                + 'Template of MPI command line (e.g. "mpirun -np XXX_SXMPI_NPROC_XXX --host n0,n1,n2 XXX_SXCMD_LINE_XXX"). if empty, use "mpirun -np XXX_SXMPI_NPROC_XXX XXX_SXCMD_LINE_XXX"</FONT>'
            )
            submit_layout.addWidget(
                self.mpi_cmd_line_edit,
                grid_row,
                grid_col_origin + token_label_col_span,
                token_widget_row_span,
                token_widget_col_span,
            ...
            self.load_params_btn.setToolTip(
                "<FONT>"
                + "Load gui parameter settings to retrieve a previously-saved one"
                + "</FONT>"
            )
            ...
            self.set_text_entry_widget_enable_state(
                self.mpi_nproc_edit, self.sxcmdwidget.sxcmd.mpi_support
            )
            if not self.mpi_nproc_edit.isEnabled():
                ...
                self.mpi_nproc_edit.setText("1")
            self.set_text_entry_widget_enable_state(
                self.mpi_cmd_line_edit, self.sxcmdwidget.sxcmd.mpi_support
            )
            ...
            if is_qsub_enabled == True:
                ...
                self.qsub_enable_checkbox.setCheckState(PyQt5.QtCore.Qt.Unchecked)
            self.qsub_enable_checkbox.setToolTip(
                "<FONT>" + "Submit job to queue" + "</FONT>"
            )
            self.qsub_enable_checkbox.stateChanged.connect(
                self.set_qsub_enable_state
            )  # To control enable state of the following qsub related widgets
            self.qsub_enable_checkbox.setEnabled(self.sxcmdwidget.sxcmd.is_submittable)
            submit_layout.addWidget(
                self.qsub_enable_checkbox,
                grid_row,
                grid_col_origin + token_label_col_span,
                token_widget_row_span,
                token_widget_col_span,
            ...
            if self.sxcmdwidget.sxcmd.is_submittable == True:
                self.qsub_job_name_edit.setText(
                    self.sxcmdwidget.sxcmd.get_mode_name_for("file_path")
                )
            ...
            self.qsub_job_name_edit.setToolTip(
                "<FONT>" + "Name of this job" + "</FONT>"
            )
            submit_layout.addWidget(
                self.qsub_job_name_edit,
                grid_row,
                grid_col_origin + token_label_col_span,
                token_widget_row_span,
                token_widget_col_span,
            ...
            self.qsub_cmd_edit.setToolTip(
                "<FONT>" + "Name of submission command to queue job" + "</FONT>"
            )
            submit_layout.addWidget(
                self.qsub_cmd_edit,
                grid_row,
            ...
            self.cmd_line_btn.setToolTip(
                "<FONT>"
                + "Generate command line from gui parameter settings and automatically save settings"
                + "</FONT>"
            )
            ...
            self.pipe_line_btn.setToolTip(
                "<FONT>"
                + "Generate executable files that and add them to the queue folder."
                + "</FONT>"
            )
            ...
            if self.sxcmdwidget.sxcmd.is_submittable == True:
                if "SPHIRE_SUBMISSION_SCRIPT_TEMPLATE" in os.environ:
                    submission_script_template = os.environ[
                        "SPHIRE_SUBMISSION_SCRIPT_TEMPLATE"
                    ]
                else:
            ...
            self.qsub_script_edit.setToolTip(
                "<FONT>"
                + "File name of submission script template (e.g. $PROJECT_DIR/msgui_qsub.sh)"
                + "</FONT>"
            )
            submit_layout.addWidget(
            ...
            self.qsub_script_open_btn.setToolTip(
                "<FONT>"
                + "Display open file dialog to select job submission script template file"
                + "</FONT>"
            )
            self.qsub_script_open_btn.clicked.connect(
                functools.partial(
                    self.sxcmdwidget.select_file,
                    self.qsub_script_edit,
                    "submission_template",
            ...
            self.execute_btn.setToolTip(
                "<FONT>"
                + "Run %s and automatically save gui parameter settings</FONT>"
                % self.sxcmdwidget.sxcmd.get_mode_name_for("human")
            )
    ...
    def set_text_entry_widget_enable_state(widget, is_enabled):
        ...
        bg_color = default_palette.color(
            PyQt5.QtGui.QPalette.Inactive, PyQt5.QtGui.QPalette.Base
        )
        if is_enabled == False:
            bg_color = default_palette.color(
                PyQt5.QtGui.QPalette.Disabled, PyQt5.QtGui.QPalette.Base
            )
    ...
    def set_qsub_enable_state(self):
        ...
        if self.sxcmdwidget.sxcmd.mpi_support:
            self.set_text_entry_widget_enable_state(
                self.mpi_cmd_line_edit, not is_enabled
            )
    ...
    def handle_restore_widget_event(self, sxcmd_token, widget_index=0, restore_idx=0):
        ...
        if sxcmd_token.type == "user_func":
            assert (
                len(sxcmd_token.widget) == 2
                and len(sxcmd_token.restore) == 2
                and widget_index < 2
            )
            try:
...
class SXCmdCategoryWidget(QWidget):
    def __init__(self, sxconst_set, sxcmd_category, helical=False, parent=None):
        ...
        self.sxcmd_btn_area_row_span = (
            self.sxcmd_btn_row_span * SXLookFeelConst.expected_cmd_counts
        )
    ...
    def setup_layout(self, background_brush):
        ...
        self.grid_layout.setColumnMinimumWidth(
            0, SXLookFeelConst.sxcmd_btn_area_min_width
        )
    ...
    def add_sxcmd_widgets(self, helical):
        ...
        for sxcmd in self.sxcmd_category.cmd_list:
            if sxcmd.role != current_role:
                ...
                title = QLabel(
                    "<span style='font-size:18pt; font-weight:600; color:#000000;'><b>%s </b></span><span style='font-size:12pt; font-weight:60; color:#000000;'>(shift-click for wiki)</span>"
                    % label_text
                )
                self.grid_layout.addWidget(
                    title,
                    self.grid_row,
                    self.grid_col_origin,
                    self.sxcmd_btn_row_span,
                    self.sxcmd_btn_col_span,
                )
            ...
            sxcmd.btn.clicked.connect(
                functools.partial(self.handle_sxcmd_btn_event, sxcmd)
            )
    ...
    def handle_sxcmd_btn_event(self, sxcmd):
        ...
        if self.cur_sxcmd == sxcmd:
            return

        if self.cur_sxcmd != None:
            custom_style = (
                "QPushButton {font: normal; color:black; }"
        ...
        if self.cur_sxcmd != None:
            ...
            custom_style = (
                "QPushButton {font: bold; color:blue; }"
...
class SXConstSetWidget(QWidget):
    def __init__(self, sxconst_set, sxcmd_category_list, helical=False, parent=None):
        ...
        self.gui_settings_file_path = "%s/gui_settings_project_%d.txt" % (
            SXLookFeelConst.project_dir,
            int(helical),
        )
        ...
        title_col_span = 1
        short_info_row_span = 1
        short_info_col_span = 1
        title_min_width = 300
        short_info_min_width = 300
        short_info_min_height = 80

        const_set_row_origin = 0
        const_set_col_origin = 0
        const_label_row_span = 1
        const_label_col_span = 1
        const_register_widget_row_span = 1
        const_register_widget_col_span = 1
        const_widget_row_span = 1
        const_widget_col_span = 1
        ...
        func_btn_row_span = 1
        func_btn_col_span = 1
        register_btn_row_span = 1
        register_btn_col_span = 2
        ...
        palette.setBrush(
            PyQt5.QtGui.QPalette.Background,
            PyQt5.QtGui.QBrush(SXLookFeelConst.default_bg_color),
        )
        self.setPalette(palette)
        ...
        global_layout.setRowStretch(
            global_row_span - 1, global_layout.rowStretch(global_row_origin) + 1
        )
        ...
        const_set_layout.setContentsMargins(
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
        ...
        for sxconst in self.sxconst_set.list:
            ...
            const_set_layout.addWidget(
                temp_label,
                const_set_grid_row,
                const_set_col_origin,
                const_label_row_span,
                const_label_col_span,
            )
            ...
            const_set_layout.addWidget(
                sxconst_register_widget,
                const_set_grid_row,
                const_set_row_origin + const_label_col_span,
                const_register_widget_row_span,
                const_register_widget_col_span,
            )
            sxconst_register_widget.setToolTip(
                "<FONT>" + "Retrieve this registered value to edit box" + "</FONT>"
            )
            sxconst_register_widget.clicked.connect(
                functools.partial(self.handle_regster_widget_event, sxconst)
            )
            ...
            const_set_layout.addWidget(
                sxconst_widget,
                const_set_grid_row,
                const_set_row_origin
                + const_label_col_span
                + const_register_widget_col_span,
                const_widget_row_span,
                const_widget_col_span,
            )
        ...
        self.execute_btn.setToolTip(
            "<FONT>"
            + "Register project constant parameter settings to automatically set values to command arguments and options"
            + "</FONT>"
        )
        ...
        btn_layout.addWidget(
            ...
            btn_grid_row,
            btn_col_origin,
            register_btn_row_span,
            register_btn_col_span,
        )
        ...
        self.save_consts_btn.setToolTip(
            "<FONT>" + "Save project constant parameter settings" + "</FONT>"
        )
        self.save_consts_btn.clicked.connect(self.save_consts)
        btn_layout.addWidget(
            self.save_consts_btn,
            btn_grid_row,
            btn_col_origin,
            func_btn_row_span,
            func_btn_col_span,
        ...
        self.load_consts_btn.setToolTip(
            "<FONT>"
            + "Load project constant parameter settings to retrieve the previously-saved one"
            + "</FONT>"
        )
        self.load_consts_btn.clicked.connect(self.load_consts)
        btn_layout.addWidget(
            self.load_consts_btn,
            btn_grid_row,
            btn_col_origin + func_btn_col_span,
            func_btn_row_span,
            func_btn_col_span,
        )
    ...
    def switch_gui(self):
        ...
        self.parent.sxmenu_item_widget_stacked_layout_global.setCurrentIndex(
            self.parent.helical
        )
        self.parent.sxmenu_item_widget_stacked_layout[
            self.parent.helical
        ].setCurrentWidget(self.parent.sxconst_set[self.parent.helical].widget)
        ...
        for sxcmd_category in self.sxcmd_category_list:
            ...
            for sxcmd in sxcmd_category.cmd_list:
                ...
                for cmd_token in sxcmd.token_list:
                    if not cmd_token.is_locked and cmd_token.type in list(
                        self.sxconst_set.dict.keys()
                    ):
        ...
        if line_in.find("@@@@@ project settings gui settings") != -1:
            ...
            for line_in in file_in:
                ...
                if item_tail != 0:
                    QMessageBox.warning(
                        self,
                        "Invalid Project Settings File Format",
                        'Project settings entry should start from "%s" for entry key in line (%s). The format of this file might be corrupted. Please save the project settings file again.'
                        % (target_operator, line_in),
                    )
                label_in = label_in[
                    item_tail + len(target_operator) :
                ].strip()  # Get the rest of line
                ...
                if item_tail == -1:
                    QMessageBox.warning(
                        self,
                        "Invalid Project Settings File Format",
                        'Project settings entry should have "%s" closing entry key in line (%s) The format of this file might be corrupted. Please save the project settings file again.'
                        % (target_operator, line_in),
                    )
                ...
                if key not in list(self.sxconst_set.dict.keys()):
                    QMessageBox.warning(
                        self,
                        "Invalid Project Settings File Format",
                        'Invalid entry key for project settings "%s" is found in line (%s). This project settings file might be incompatible with the current version. Please save the project settings file again.'
                        % (key, line_in),
                    )
            ...
            QMessageBox.warning(
                self,
                "Fail to load project settings",
                "The specified file is not project settings file.",
            )
...
class SXInfoWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            "background-color: {0}".format(SXLookFeelConst.default_bg_color_string)
        )
        widget = QWidget(self)
        ...
        self.label.setStyleSheet(
            'border-image: url("{0}"); background-color: transparent'.format(pic_name)
        )
        ...
        scroll_area.setStyleSheet(
            "background-color: {0}".format(SXLookFeelConst.sxcmd_tab_bg_color_string)
        )
...
class SXDialogCalculator(QDialog):
    ...
    def __init__(
        ...
        self.sxconst_register_widget_apix = (
            sxconst_register_widget_apix
        )  # SXconst.register_widget of apix type
        ...
        self.sxcmd_token_widget_apix = (
            None
        )  # SXcmd_token.widget of apix type if this command has one
        self.sxcmd_token_other_dialog_list_abs_freq = (
            []
        )  # SXcmd_token.other_dialog_list of the associated abs_freq type token if this command has more than one abs_freq type tokens
        ...
        operand_set_row_origin = 0
        operand_set_col_origin = 0
        operand_label_row_span = 1
        operand_label_col_span = 1
        operand_register_widget_row_span = 1
        operand_register_widget_col_span = 1
        operand_widget_row_span = 1
        operand_widget_col_span = 1
        ...
        palette.setBrush(
            PyQt5.QtGui.QPalette.Background,
            PyQt5.QtGui.QBrush(SXLookFeelConst.default_bg_color_calculator),
        )
        self.setPalette(palette)
        ...
        operand_layout.setContentsMargins(
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
            SXLookFeelConst.grid_margin,
        ...
        for sxoperand in self.sxoperand_set.list:
            ...
            operand_layout.addWidget(
                temp_label,
                operand_grid_row,
                operand_set_col_origin,
                operand_label_row_span,
                operand_label_col_span,
            )
            ...
            if sxoperand.is_input:
                ...
                sxoperand_register_widget.setToolTip(
                    "<FONT>" + "Retrieve this default value to edit box" + "</FONT>"
                )
                sxoperand_register_widget.clicked.connect(
                    functools.partial(self.handle_regster_widget_event, sxoperand)
                )
            else:
            ...
            operand_layout.addWidget(
                sxoperand_register_widget,
                operand_grid_row,
                operand_set_row_origin + operand_label_col_span,
                operand_register_widget_row_span,
                operand_register_widget_col_span,
            )
            ...
            operand_layout.addWidget(
                sxoperand_widget,
                operand_grid_row,
                operand_set_row_origin
                + operand_label_col_span
                + operand_register_widget_col_span,
                operand_widget_row_span,
                operand_widget_col_span,
            )
        ...
        self.cancel_btn.setToolTip(
            "<FONT>"
            + "Cancel the unit convertion. Close the calculator dialog without applying the converted value to the associated edit box"
            + "</FONT>"
        )
        ...
        self.apply_btn.setToolTip(
            "<FONT>"
            + "Apply converted value to the corresponding command arguments and options"
            + "</FONT>"
        )
    ...
    def construct_sxoperand_set(self):
        sxoperand_set = SXoperand_set()
        sxoperand_set.name = "sxc_calculator"
        sxoperand_set.label = "Absolute Frequency Calculator"
        sxoperand_set.short_info = "From resolution [A] (ares), calculate absolute frequency [1/Pixel] (abs_freq) using a give pixel size [A/Pixel] (apix), where abs_freq = apix/ares."
        sxoperand = SXoperand()
        sxoperand.key = "ares"
        sxoperand.label = "Resolution [A]"
        sxoperand.help = "Resolution [A] to calculate the corresponding absolute frequency [1/Pixel] using a given pixel size [A/Pixel]."
        sxoperand.register = " -9999.9"
        sxoperand.type = "float"
        sxoperand_set.list.append(sxoperand)
        sxoperand_set.dict[sxoperand.key] = sxoperand
        sxoperand = SXoperand()
        sxoperand.key = "apix"
        sxoperand.label = "Pixel size [A/Pixel]"
        sxoperand.help = "Pixel size of dataset [A/Pixel]."
        sxoperand.register = " -9999.9"
        sxoperand.type = "apix"
        sxoperand_set.list.append(sxoperand)
        sxoperand_set.dict[sxoperand.key] = sxoperand
        sxoperand = SXoperand()
        sxoperand.key = "abs_freq"
        sxoperand.label = "Absolute Frequency [1/Pixel]"
        sxoperand.help = "Absolute frquency [1/Pixel] corresponding to the given resolution [A/Pixel]."
        sxoperand.register = " -9999.9"
        sxoperand.type = "abs_freq"
        sxoperand.is_input = False
        sxoperand_set.list.append(sxoperand)
        sxoperand_set.dict[sxoperand.key] = sxoperand
    ...
    def handle_convert_units(self, is_enable_message=True):
        ...
        if apix is None:
            ...
            if is_enable_message:
                QMessageBox.warning(
                    self,
                    "Invalid Pixel Size [A/Pixel]",
                    "Invalid Value {} for Pixel Size [A/Pixel] is provided. It must be a non-zero positive float value...".format(
                        apix_str
                    ),
                )
        else:
            ...
            if ares is None:
                sxoperand_ares.validated = None
                sxoperand_abs_freq.validated = None
                sxoperand_abs_freq.widget.setText(
                    "Invalid Resolution {}".format(ares_str)
                )
                self.apply_btn.setEnabled(False)
                if is_enable_message:
                    QMessageBox.warning(
                        self,
                        "Invalid Resolution [A]",
                        "Invalid Value {} for Resolution [A] is provided. It must be a float value, and larger than or equal to Nyquist resolution {} [A] (2.0 * Pixel Size [A/Pixel])...".format(
                            ares_str, nyquist_res_str
                        ),
                    )
            else:
    ...
    def handle_apply_unit_conversion(self):
        ...
        if abs_freq is None:
            sp_global_def.ERROR(
                "Logical Error: Encountered unexpected condition in SXDialogCalculator.handle_apply_unit_conversion(). Consult with the developer.",
                "%s in %s" % (__name__, os.path.basename(__file__)),
            )
            ...
            sxoperand_abs_freq.register = "{}".format(
                sxoperand_abs_freq.validated_register
            )
            ...
            for (
                sxcmd_token_other_dialog_abs_freq
            ) in self.sxcmd_token_other_dialog_list_abs_freq:
    ...
    def synchronize_external_update_to_ares(self):
        ...
        if sxoperand_abs_freq.validated is None:
            ...
            sxoperand_abs_freq.widget.setText(
                "Mode {}".format(sxoperand_abs_freq.register)
            )
            ...
            if sxoperand_apix.validated is None:
                ...
                sxoperand_ares.validated_register = None
                sxoperand_ares.register = "Invalid Pixel Size {}".format(
                    sxoperand_apix.widget.text()
                )
        ...
        if sxoperand_ares.validated_register is not None:
            ...
            self.sxcmd_token_widget_abs_freq.setText(
                "{}".format(sxoperand_abs_freq.register)
            )
            # Update left subwidget of this command token for absolute frequency [1/Pixel] associating to this dialog
            self.sxcmd_token_subwidget_left_ares.setText(
                "{}[A]@{}[A/Pix]".format(
                    sxoperand_ares.validated, sxoperand_apix.validated
                )
            )
        else:
            ...
            if sxoperand_abs_freq.validated is None:
                ...
                assert sxoperand_abs_freq.validated is not None
                self.sxcmd_token_subwidget_left_ares.setText(
                    "{}".format(sxoperand_ares.register)
                )
...
class SXMainWindow(QMainWindow):  # class SXMainWindow(QWidget):
    def __init__(self, helical, parent=None):
        ...
        self.sxinfo.append(self.construct_sxinfo())  # Construct application information
        self.sxconst_set.append(
            self.construct_sxconst_set(0)
        )  # Construct project constant set for project settings
        self.sxconst_set.append(
            self.construct_sxconst_set(1)
        )  # Construct project constant set for project settings
        self.sxcmd_category_list.append(
            self.construct_sxcmd_category_list()
        )  # Construct list of categorised sxscript objects (extracted from associated wiki documents)
        self.sxcmd_category_list.append(
            self.construct_sxcmd_category_list()
        )  # Construct list of categorised sxscript objects (extracted from associated wiki documents)
        ...
        central_widget_global.setStyleSheet(
            'QWidget#central {{background-image: url("{0}")}}'.format(
        ...
        for idx in range(2):
            ...
            # --------------------------------------------------------------------------------
            sxmenu_item_btn_area_widget = SXMenuItemBtnAreaWidget(
                self.sxconst_set[idx],
                self.sxcmd_category_list[idx],
                self.sxinfo[idx],
                idx,
                central_widget,
            )
            ...
            central_layout.addLayout(
                self.sxmenu_item_widget_stacked_layout[idx], stretch=1
            )

            # Construct and add a widget for project constants settings
            self.sxconst_set[idx].widget = SXConstSetWidget(
                self.sxconst_set[idx],
                self.sxcmd_category_list[idx],
                helical=idx,
                parent=self,
            )
            self.sxmenu_item_widget_stacked_layout[idx].addWidget(
                self.sxconst_set[idx].widget
            )
            ...
            for sxcmd_category in self.sxcmd_category_list[idx]:
                ...
                sxcmd_category.widget = SXCmdCategoryWidget(
                    self.sxconst_set[idx], sxcmd_category, idx
                )
                self.sxmenu_item_widget_stacked_layout[idx].addWidget(
                    sxcmd_category.widget
                )

            # Construct and add a widget for GUI application information
            self.sxinfo[idx].widget = SXInfoWidget()
            self.sxmenu_item_widget_stacked_layout[idx].addWidget(
                self.sxinfo[idx].widget
            )
            ...
            for i, sxcmd_category in enumerate(self.sxcmd_category_list[idx]):
                sxcmd_category.btn.clicked.connect(
                    functools.partial(
                        self.handle_sxmenu_item_btn_event, sxcmd_category, i
                    )
                )
            self.sxconst_set[idx].btn.clicked.connect(
                functools.partial(
                    self.handle_sxmenu_item_btn_event, self.sxconst_set[idx]
                )
            )
            self.sxinfo[idx].btn.clicked.connect(
                functools.partial(self.handle_sxmenu_item_btn_event, self.sxinfo[idx])
            )
            ...
            logo_container.setStyleSheet(
                'border-image: url("{0}sxgui_pictograph_info.png")'.format(
                    EMAN2.get_image_directory()
                )
            )
    ...
    def construct_sxinfo(self):
        ...
        sxinfo = SXmenu_item()
        sxinfo.name = "sxc_gui_info"
        sxinfo.label = "GUI Appliation Information"
        sxinfo.short_info = "DUMMY STRING"

        # Store GUI application information as a class data member
        return sxinfo

    def construct_sxconst_set(self, helical):
        sxconst_set = SXconst_set()
        sxconst_set.name = "sxc_project"
        sxconst_set.label = "Project Settings"
        sxconst_set.short_info = "Set constant parameter values for this project. These constants will be used as default values of associated arguments and options in command settings. However, the project settings here are not required to run commands."
        sxconst = SXconst()
        sxconst.key = "protein"
        sxconst.label = "Protein name"
        sxconst.help = "a valid string for file names on your OS."
        sxconst.register = "MY_PROTEIN"
        sxconst.type = "string"
        sxconst_set.list.append(sxconst)
        sxconst_set.dict[sxconst.key] = sxconst
        sxconst = SXconst()
        sxconst.key = "apix"
        sxconst.label = "Micrograph pixel size [A]"
        sxconst.help = ""
        sxconst.register = "1.0"
        sxconst.type = "float"
        sxconst_set.list.append(sxconst)
        sxconst_set.dict[sxconst.key] = sxconst
        sxconst = SXconst()
        sxconst.key = "ctfwin"
        sxconst.label = "CTF window size [pixels]"
        sxconst.help = "it should be slightly larger than particle box size"
        sxconst.register = "512"
        sxconst.type = "int"
        sxconst_set.list.append(sxconst)
        sxconst_set.dict[sxconst.key] = sxconst
        sxconst = SXconst()
        sxconst.key = "box"
        sxconst.label = "Particle box size [pixels]"
        sxconst.help = ""
        sxconst.register = "-1"
        sxconst.type = "int"
        sxconst_set.list.append(sxconst)
        ...
        sxconst = SXconst()
        sxconst.key = "radius"
        sxconst.label = "Protein particle radius [pixels]"
        sxconst.help = ""
        sxconst.register = "-1"
        sxconst.type = "int"
        sxconst_set.list.append(sxconst)
        ...
        sxconst = SXconst()
        sxconst.key = "sym"
        sxconst.label = "Point-group symmetry"
        sxconst.help = "e.g. c1, c4, d5"
        sxconst.register = "c1"
        sxconst.type = "string"
        sxconst_set.list.append(sxconst)
        sxconst_set.dict[sxconst.key] = sxconst
        sxconst = SXconst()
        sxconst.key = "mass"
        sxconst.label = "Protein molecular mass [kDa]"
        sxconst.help = ""
        sxconst.register = "-1.0"
        sxconst.type = "float"
        sxconst_set.list.append(sxconst)
        ...
        if helical:
            sxconst = SXconst()
            sxconst.key = "filament_width"
            sxconst.label = "Filament width [pixels]"
            sxconst.help = "Width of the filament in case of filamentous processing."
            sxconst.register = "-1"
            sxconst.type = "int"
            sxconst_set.list.append(sxconst)
            sxconst_set.dict[sxconst.key] = sxconst
        sxconst = SXconst()
        sxconst.key = "config"
        sxconst.label = "Imaging configurations"
        sxconst.help = "a free-style string for your record. please use it to describe the set of imaging configurations used in this project (e.g. types of microscope, detector, enegy filter, abbration corrector, phase plate, and etc."
        sxconst.register = "MY_MICROSCOPE"
        sxconst.type = "int"
        sxconst_set.list.append(sxconst)
        sxconst_set.dict[sxconst.key] = sxconst
        ...
        for sxcmd in sxcmd_list:
            for sxcmd_token in sxcmd.token_list:
                ...
                if sxcmd_token.type == "user_func":
                    ...
                    sxcmd_token.label = [
                        sxcmd_token.label,
                        "Python script for user function",
                    ]
                    sxcmd_token.help = [
                        sxcmd_token.help,
                        "Please leave it none/blank if file is not external to SPHIRE",
                    ]
            ...
            assert sxcmd.category in sxcmd_category_dict, "sxcmd.category %s" % (
                sxcmd.category
            )
    ...
    def handle_sxmenu_item_btn_event(self, sxmenu_item, i=0):
        ...
        modifiers = QApplication.keyboardModifiers()
        if modifiers == PyQt5.QtCore.Qt.ShiftModifier:
            sxmenu_item_wiki_url = SXLookFeelConst.generate_sxmenu_item_wiki_url(
                sxmenu_item
            )
            print("Opening Wiki Page ...")
        ...
        if self.cur_sxmenu_item == sxmenu_item:
            return

        if self.cur_sxmenu_item != None:
            self.cur_sxmenu_item.btn.setStyleSheet(
                self.cur_sxmenu_item.btn.customButtonStyle
            )
        ...
        if self.cur_sxmenu_item != None:
            self.cur_sxmenu_item.btn.setStyleSheet(
                self.cur_sxmenu_item.btn.customButtonStyleClicked
            )
            self.sxmenu_item_widget_stacked_layout[self.helical].setCurrentWidget(
                self.cur_sxmenu_item.widget
            )
...
def main():
    ...
    sxapp.setWindowIcon(
        PyQt5.QtGui.QIcon(EMAN2.get_image_directory() + "sxgui_icon_sphire.png")
    )
    ...
    sxmain_window.resize(
        SXLookFeelConst.sxmain_window_width, SXLookFeelConst.sxmain_window_height
    )
    sxmain_window.move(
        PyQt5.QtCore.QPoint(
            SXLookFeelConst.sxmain_window_left, SXLookFeelConst.sxmain_window_top
        )
    )
