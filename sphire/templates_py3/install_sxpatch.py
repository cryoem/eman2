...
def install_patch_src_files(
    patch_src_root_dir_path,
    patch_src_subdir_rpath,
    backup_root_dir_path,
    install_dir_path,
    install_subdir_rpath,
    python_script_directive=None,
):

    print_progress("Checking '%s' subdirectory..." % patch_src_subdir_rpath)
    patch_src_subdir_rpath = os.path.join(
        patch_src_root_dir_path, patch_src_subdir_rpath
    )

    if os.path.exists(patch_src_subdir_rpath):
        print_progress(
            "Making the list of patch files in '%s'..." % patch_src_subdir_rpath
        )
        ...
        print_progress(
            "Found %d Python script files in '%s'."
            % (len(patch_src_file_path_list), patch_src_subdir_rpath)
        )

        if len(patch_src_file_path_list) > 0:
            backup_subdir_path = os.path.join(
                backup_root_dir_path, patch_src_subdir_rpath
            )
            # print_progress("MRK_DEBUG: backup_subdir_path := %s" % backup_subdir_path)
            if not os.path.exists(backup_subdir_path):
                print_progress(
                    "Making backup subdirectory '%s'..." % backup_subdir_path
                )
        ...
        for patch_src_file_path in patch_src_file_path_list:
            ...
            print_progress(
                "Replacing '%s' with '%s'..." % (install_file_path, patch_src_file_path)
            )
            ...
            if os.path.exists(install_file_path):
                ...
                if install_1st_line != python_script_directive:
                    print_progress(
                        "WARNING!!! Found unexpected Python script directive '%s' in '%s'. However, this should not cause any problems..."
                        % (install_1st_line, install_file_path)
                    )
                install_file.close()
            else:
                print_progress(
                    "%s does not exist in '%s'. The program will not make the backup..."
                    % (patch_file_name, install_subdir_path)
                )
            ...
            if python_script_directive is not None:
                ...
                if patch_src_1st_line != "#!/usr/bin/env python":
                    print_progress(
                        "WARNING!!! Found unexpected Python script directive '%s' in '%s'. However, this should not cause any problems..."
                        % (patch_src_1st_line, patch_src_file_path)
                    )
            ...
            os.chmod(
                install_file_path, install_file_stat.st_mode | 0o111
            )  # Make all executable, Use octal system representation for bits 0o111 = 0b001001001 = Linux 111 = 0d73
    else:
        print_progress("Nothing to do...")
        return
...
def install_sxpatch(args):
    ...
    if install_dir_path is None:
        ...
        if sphire_gui_abs_path is None:
            print_progress(
                "Program can not find the SPHIRE installation directory. Please manually specify the directory using --install_dir_path options."
            )
            return
        ...
        if not os.path.exists(install_dir_path):
            print_progress(
                "Provided SPHIRE installation directory does not exist. Please check the directory path provided with --install_dir_path options."
            )
            return
    ...
    install_patch_src_files(
        patch_src_root_dir_path,
        "eman2/sparx/bin",
        backup_root_dir_path,
        install_dir_path,
        "bin",
        python_script_directive,
    )
    print(" ")
    install_patch_src_files(
        patch_src_root_dir_path,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Python script to install patch for a SPHIRE release."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Specify path to SPHIRE installation directory. By default, the program try to detect it automatically by extracting the path from 'which sphire'. (default none)",
    )
