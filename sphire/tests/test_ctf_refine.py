...
from bin_py3 import sp_ctf_refine as oldfu
from sphire.bin import sp_ctf_refine as fu

from os import path
from tests.test_module import ABSOLUTE_PATH_TO_RESOURCES,ABSOLUTE_OLDBIN_PATH,ABSOLUTE_BIN_PATH,remove_dir,IMAGE_2D,get_arg_from_pickle_file,ABSOLUTE_PATH, give_ali_vol_data, give_ormq_data
import unittest
...
class Test_helperFunctions(unittest.TestCase):
    ...
    @unittest.skip("nov_21 COMPATIBILITY TEST FAIL!!!")
    def test_calc_2d_projection(self):
        (volft, params, interpolation_method, return_real) = get_arg_from_pickle_file(path.join(ABSOLUTE_PATH_TO_RESOURCES, "projection.prgl.pkl"))[0]
    ...
    def test_calc_similarity_real(self):
        (img1, img2, mask) = get_arg_from_pickle_file(path.join(ABSOLUTE_PATH_TO_RESOURCES, "statistics.ccc.pkl"))[0]
...
@unittest.skip("nov_21 IT FAILS because there is not '06_SUBSTACK_ANO'' folder in Adnan files ")
