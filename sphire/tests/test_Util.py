...
class Test_Util_diff_between_matrix_of_3D_parameters_angles(unittest.TestCase):
    ...
    def internal_calculate_avg_diff(self, projs, matrix_rot):
        from sp_utilities import angle_between_projections_directions
                ...
                for k in range(n):
                    ...
                    diff.append(
                        angle_between_projections_directions(
                            self.mult_transform(projs[i][k], matrix_rot[i][j]),
                            projs[j][k],
                        )
                    )
    ...
    def internal_calculate_avg_diff_Util(self, projs, matrix_rot):
        ...
        avg_diff_per_image = Util.diff_between_matrix_of_3D_parameters_angles(
            trans_projs, trans_matrix
        )
        return avg_diff_per_image
    ...
    def mult_transform(self, v1, v2):
        ...
        return [
            ...
            T.get_params("spider")["tx"],
            T.get_params("spider")["ty"],
        ]
    ...
    def calculate_matrix_rot(self, projs):
        ...
        for i in range(sc):
            for j in range(i):
                matrix_rot[i][j] = self.wrap_rotation_between_anglesets(
                    projs[i], projs[j]
                )
        return matrix_rot
