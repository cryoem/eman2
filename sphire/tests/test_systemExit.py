...
class Test_MORPHOLOGY(unittest.TestCase):
    ...
    def test_cter_vpp_invalid_pixelSize_SYSEXIT(self):
        ...
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            oldfu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
    ...
    def test_cter_vpp_invalid_windowSize_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=0, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            oldfu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=0, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
        self.assertEqual(cm_new.exception.code, None)
        self.assertEqual(cm_new.exception.code, cm_old.exception.code)
    ...
    def test_cter_vpp_no_image_input_in_path_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_vpp(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_vpp(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        remove_dir(self.output_directory)
    ...
    def test_cter_vpp_output_dir_already_exist_SYSEXIT(self):
        ...
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_vpp(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop, vpp_options=self.vpp_options)
        self.assertEqual(cm_new.exception.code, None)
        self.assertEqual(cm_new.exception.code, cm_old.exception.code)
    ...
    def test_cter_pap_invalid_pixelSize_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_pap_invalid_windowSize_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=0,pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start,f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=0,pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start,f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_pap_no_image_input_in_path_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_pap(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_pap(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_pap_output_dir_already_exist_SYSEXIT(self):
        ...
        mkdir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        self.assertEqual(cm_new.exception.code, None)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_pap(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        self.assertEqual(cm_new.exception.code, None)
    ...
    def test_cter_mrk_invalid_pixelSize_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_mrk(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0,  Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_mrk(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn, pixel_size=0,  Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_mrk_invalid_windowSize_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_mrk(self.input_image_path, self.output_directory, selection_list=None, wn=0, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_mrk(self.input_image_path, self.output_directory, selection_list=None, wn=0, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_mrk_no_image_input_in_path_SYSEXIT(self):
        ...
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_mrk(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_old:
            fu.cter_mrk(".", self.output_directory, selection_list=None, wn=self.wn, pixel_size=self.pixel_size, Cs=self.cs, voltage=self.voltage, f_start=self.i_start, f_stop=self.i_stop)
        remove_dir(self.output_directory)
    ...
    def test_cter_mrk_output_dir_already_exist_SYSEXIT(self):
        ...
        mkdir(self.output_directory)
        with self.assertRaises(SystemExit) as cm_new:
            fu.cter_mrk(self.input_image_path, self.output_directory, selection_list=None, wn=self.wn,
