...
def get_real_data(dim=2):
    ...
    if dim == 2:
        ...
        argum = get_arg_from_pickle_file(
            path.join(ABSOLUTE_PATH_TO_RESOURCES, "alignment.align2d_scf_new")
        )
    ...
    if dim == 3:
        argum = get_arg_from_pickle_file(
            path.join(ABSOLUTE_PATH_TO_RESOURCES, "alignment.ali_vol_func_new")
        )
...
######Comment by adnan  ### ## Still need to figure out what is tavg in the pickle file
def give_ali2d_single_iter_data():
    ...
    argum = get_arg_from_pickle_file(ABSOLUTE_PATH_TO_RESOURCES + "alignment.ali2d_single_iter")
    (data,numr, wr,_,tavg,_, _,_,_,_) = argum[0]

    return data, numr, wr, cs, tavg, cnx, cny, xrng, yrng, step


def give_ornq_data():
    image_values_name =  "_image_values.pkl"
    image_head_name =  "_image_head.pkl"

    crefim_values_name = "_crefim_values.pkl"
    crefim_head_name =  "_crefim_head.pkl"

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , image_values_name), "rb") as f3:
        image_data = pickle.load(f3, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , image_head_name), "rb") as f4:
        image_head = pickle.load(f4, encoding='latin1')


    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , crefim_values_name), "rb") as f1:
        crefim_data = pickle.load(f1, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , crefim_head_name), "rb") as f2:
        crefim_head = pickle.load(f2, encoding='latin1')

    image = sp_utilities.numpy2em_python(image_data)
    for keys in image_head:
        image.set_attr(keys, image_head[keys])

    crefim = sp_utilities.numpy2em_python(crefim_data)
    for keys in crefim_head:
        crefim.set_attr(keys, crefim_head[keys])

    return image , crefim


def give_ormq_data(all=False):

    image_values_name =  "ormq_image_values.pkl"
    image_head_name =  "ormq_image_head.pkl"

    crefim_values_name = "ormq_crefim_values.pkl"
    crefim_head_name =  "ormq_crefim_head.pkl"
    ...
    if all == False:
        ...
        cny = 36.0
        delta = 90.0
        return image,crefim, xrng, yrng, step, mode, numr, cnx, cny, delta




def give_ali_vol_data():
    old_pkl = "resources/applications.ali_vol"

    vol_values_name = "vol_values.pkl"
    vol_head_name =  "vol_head.pkl"

    refv_values_name =  "refv_values.pkl"
    refv_head_name =  "refv_head.pkl"

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , vol_values_name), "rb") as f3:
        vol_data = pickle.load(f3, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , vol_head_name), "rb") as f4:
        vol_head = pickle.load(f4, encoding='latin1')


    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , refv_values_name), "rb") as f1:
        refv_data = pickle.load(f1, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , refv_head_name), "rb") as f2:
        refv_head = pickle.load(f2, encoding='latin1')

    vol = sp_utilities.numpy2em_python(vol_data)
    for keys in vol_head:
        vol.set_attr(keys, vol_head[keys])

    refv = sp_utilities.numpy2em_python(refv_data)
    for keys in refv_head:
        refv.set_attr(keys, refv_head[keys])

    return vol , refv


def give_ali2d_base_data():
    stack_values_name =  "stack_values.pkl"
    stack_head_name =  "stack_head.pkl"
    dill._dill._reverse_typemap["ObjectType"] = object

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , stack_values_name), "rb") as f1:
        data_arras = pickle.load(f1, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , stack_head_name), "rb") as f2:
    ...
    for i in range(len(data_arras)):
        ...
        for key in data_heads[i]:
            stack[i].set_attr(key, data_heads[i][key])
...
def give_rotate_3D_shift_data():
    ...
    data_values_name =  "data_values.pkl"
...
def give_projection_prgl_data():
    ...
    volft_values_name =  "volft_values.pkl"
    volft_head_name =  "volft_head.pkl"

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , volft_values_name), "rb") as f3:
        volft_data = pickle.load(f3, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , volft_head_name), "rb") as f4:
        volft_head = pickle.load(f4, encoding='latin1')

    volft = sp_utilities.numpy2em_python(volft_data)
    for keys in volft_head:
        volft.set_attr(keys, volft_head[keys])

    return volft


def give_alignment_shc_data(all=False):
    xfomimg_values_name =  "xfomimg_values.pkl"
    xfomimg_head_name =  "xfomimg_head.pkl"

    refrings_values_name =  "refrings_data.pkl"
    refrings_head_name =  "refrings_head.pkl"

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , xfomimg_values_name), "rb") as f3:
        xfomimg_data = pickle.load(f3, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , xfomimg_head_name), "rb") as f4:
        xfomimg_head = pickle.load(f4, encoding='latin1')


    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , refrings_values_name), "rb") as f3:
        refrings_data = pickle.load(f3, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , refrings_head_name), "rb") as f4:
        refrings_head = pickle.load(f4, encoding='latin1')

    xformimg = sp_utilities.numpy2em_python(xfomimg_data)
    for keys in xfomimg_head:
        xformimg.set_attr(keys, xfomimg_head[keys])
    ...
    if all == False:
        ...
        yrng = 0
        step = 1.0
        list_of_ref_ang = [[1.0, 1.0]]

        refrings = []
        for i in range(len(refrings_data)):
            refrings.append(sp_utilities.numpy2em_python(refrings_data[i]))
            for key in refrings_head[i]:
                refrings[i].set_attr(key, refrings_head[i][key])

        return xformimg, refrings, list_of_ref_ang, numr, xrng, yrng, step



def give_ave_series_data():
    data_ave_values_name = "data_ave_values.pkl"
    data_ave_head_name =  "data_ave_head.pkl"
    dill._dill._reverse_typemap["ObjectType"] = object

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , data_ave_values_name), "rb") as f1:
        data_arras = pickle.load(f1, encoding='latin1')

    with io.open(os.path.join(ABSOLUTE_PATH_TO_RESOURCES , data_ave_head_name), "rb") as f2:
        data_heads = pickle.load(f2, encoding='latin1')
