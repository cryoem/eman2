...
from tests.test_module import ABSOLUTE_OLDBIN_PATH,ABSOLUTE_PATH,ABSOLUTE_BIN_PATH,remove_list_of_file,remove_dir
import unittest
...
ABSOLUTE_PATH_TO_RESOURCES_TO_IMG =path.join(ABSOLUTE_PATH,"resources_tests/03_PARTICLES_BDB/mpi_proc_007")
...
class Test_Error_cases(unittest.TestCase):
    ...
    @unittest.skip("nov_21 IT FAILS because MPI FINALIZE")
    def test_lowest_resolution_error(self):
        sp_global_def.BATCH = True
        self.remove_folders()
        testargs_new =  [path.join(ABSOLUTE_BIN_PATH, "sp_cter.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES_TO_IMG,"TcdA1-0187_frames_ptcls.mrcs"),self.new_output_folder, '--apix=1.0', '--f_start=0.3' ]
        testargs_old = [path.join(ABSOLUTE_OLDBIN_PATH, "sp_cter.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES_TO_IMG, "TcdA1-0187_frames_ptcls.mrcs"), self.old_output_folder, '--apix=1.0','--f_start=0.3']
    ...
    def test_highest_resolution_error(self):
        sp_global_def.BATCH = True
        self.remove_folders()
        testargs_new =  [path.join(ABSOLUTE_BIN_PATH, "sp_cter.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES_TO_IMG, "TcdA1-0187_frames_ptcls.mrcs"), self.new_output_folder, '--apix=1.0', '--f_stop=0.3']
        testargs_old = [path.join(ABSOLUTE_OLDBIN_PATH, "sp_cter.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES_TO_IMG, "TcdA1-0187_frames_ptcls.mrcs"), self.old_output_folder, '--apix=1.0', '--f_stop=0.3']
        with patch.object(sys, 'argv', testargs_new):
            with self.assertRaises(SystemExit):
                old_stdout = sys.stdout
                print_new = StringIO()
    ...
    @unittest.skip("nov_21 IT FAILS because there is not 'CorrectedSums'' folder in Adnan files ")
