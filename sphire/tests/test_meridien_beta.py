...
class Test_run(unittest.TestCase):
    out_dir_old = "oldmeridien_beta"
    out_dir_new = "newmeridien_beta"
    ...
    def test_new(self):
        ...
        testcommand_old = (MPI_PATH
            + " -np "
            +str(NUM_PROC)
            +" "+path.join(ABSOLUTE_OLDBIN_PATH,"sp_meridien_beta.py")+" "
        ...
        testcommand_new = (
            ...
            + " -np "
            +str(NUM_PROC)
            +" "+path.join(ABSOLUTE_BIN_PATH,"sp_meridien_beta.py")
            + " 'bdb:"+path.join(ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER_NEW,"06_SUBSTACK#isac_substack'")
            +" "+self.out_dir_new
            + " " + path.join(ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER_NEW, "09_ADJUSTMENT","vol3d_ref_moon_eliminated_mask.hdf")
            + " --initialshifts"
