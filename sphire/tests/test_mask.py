...
from bin_py3 import sp_mask as oldfu
from sphire.bin import sp_mask as fu

from os import path
from tests.test_module import ABSOLUTE_OLDBIN_PATH,ABSOLUTE_BIN_PATH,remove_dir,ABSOLUTE_PATH_TO_RESOURCES
import unittest
...
class Test_Error_cases(unittest.TestCase):
    @unittest.skip("nov_21 IT FAILS because MPI_INIT")
    ...
    def test_invalid_shape(self):
        testargs_new = [path.join(ABSOLUTE_BIN_PATH, "sp_mask.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES, "17_LOCAL_RES","localres.hdf") ,"output_folderMASK","--mol_mass=1.0",
                        "--second_mask="+path.join(ABSOLUTE_PATH_TO_RESOURCES, "17_LOCAL_RES","localres_ang.hdf"),"--second_mask_shape='sphere'"]
        testargs_old = [path.join(ABSOLUTE_OLDBIN_PATH, "sp_mask.py"),path.join(ABSOLUTE_PATH_TO_RESOURCES, "17_LOCAL_RES","localres.hdf") ,"output_folderMASK","--mol_mass=1.0",
                        "--second_mask="+path.join(ABSOLUTE_PATH_TO_RESOURCES, "17_LOCAL_RES","localres_ang.hdf"),"--second_mask_shape='sphere'"]
...
class Test_run(unittest.TestCase):
    ...
    def test_(self):
        testargs_new =  [path.join(ABSOLUTE_BIN_PATH, "sp_mask.py"),
                         path.join(ABSOLUTE_PATH_TO_RESOURCES,"09_ADJUSTMENT","vol3d_ref_moon_eliminated.hdf"),
                         "--pixel_size=1.14",
                         self.new_output_folder,
                         "--mol_mass=1400",
                         "--edge_width=10"]
        testargs_old = [path.join(ABSOLUTE_OLDBIN_PATH, "sp_mask.py"),
                        path.join(ABSOLUTE_PATH_TO_RESOURCES, "09_ADJUSTMENT",
