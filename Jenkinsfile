...
def runCronJob() {
    sh 'echo ${HOME_DIR}/workspace/build-scripts-cron/cronjob.sh $STAGE_NAME v2.21a'
    if(true) {
        if(SLAVE_OS != 'win')
            sh "rsync -n -avzh --stats ${INSTALLERS_DIR}/eman2.${SLAVE_OS}.sh ${DEPLOY_DEST}/eman2.${STAGE_NAME}.unstable.sh"
        else
