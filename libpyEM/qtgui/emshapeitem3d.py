...
class EMStructure(EMShapeBase):
	...
	def getNodeDialogWidget(attribdict):
		...
		grid = QtGui.QGridLayout()
		node_name_data_label = QtGui.QLabel("Structure Label")
		attribdict["node_name"] = QtGui.QLineEdit()
		data_path_label = QtGui.QLabel("Structure Path")
		...
		EMStructure.attribdict = attribdict
		QtCore.QObject.connect(browse_button, QtCore.SIGNAL('clicked()'), EMStructure._on_browse)
	...
	def _on_browse():
		...
		if filename:
			EMStructure.attribdict["data_path"].setText(filename)
			EMStructure.fName = filename
			name = os.path.basename(str(filename))
			EMStructure.attribdict["node_name"].setText(str(name))

	@staticmethod
	def getNodeForDialog(attribdict):
		"""Create a new node using a attribdict"""
		return EMStructure(str(attribdict["data_path"].text()), transform=EMItem3D.getTransformFromDict(attribdict))
	
	def __init__(self, pdb_file=None, parent=None,transform=None):
		EMShapeBase.__init__(self, parent=parent, children=set(), transform=transform)
