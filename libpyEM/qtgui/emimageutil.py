...
##		self.orig_target  = target
	...
	#def mousePressEvent(self, event):
		#if self.target != None:
			#if self.target().isinwin(event.x(),self.main_widget().viewport_height()-event.y()):
	...
	#def keyPressEvent(self,event):
		#if self.target != None:
			#pos =self.main_widget().mapFromGlobal(QtGui.QCursor.pos())
			#if self.target().isinwin(pos.x(),self.main_widget().viewport_height()-pos.y()):
	...
	#def isinwin(self,x,y):
		#if self.target != None:
			#if self.target().child.isinwin(x,y) : return True
