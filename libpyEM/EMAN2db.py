...
class DBDict(object):
	...
	def realopen(self, ro=False):
		...
		for old_key in self.bdb.keys():
			self.key_translation_dict[dumps(loads(old_key), -1)] = old_key
	...
	def get_data_path(self, key):condsds
	...
	def load_item(self, key, fkey=b'', txn=None):
		...
		try:
			r = loads(self.bdb.get(fkey + dumps(key, -1), txn=txn))
		except:
			try:
				r = loads(self.bdb.get(fkey + dumps(key, -1).replace('\x00', '\x01'), txn=txn))
			except:
				try:
					r = loads(self.bdb.get(fkey + dumps(key, -1).replace('\x01', '\x00'), txn=txn))
				except:
					try:
						r = loads(self.bdb.get(self.key_translation_dict[fkey + dumps(key, 2)], txn=txn))
					except:
						raise
		return r
