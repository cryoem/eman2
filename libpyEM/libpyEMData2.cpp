...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_calc_ccf_overloads_1_3, EMAN::EMData::calc_ccf, 1, 3)
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_read_data_2_6, EMAN::EMData::read_data, 2, 6)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_write_data_2_6, EMAN::EMData::write_data, 2, 6)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_rot_scale_trans2D_background_1_4, EMAN::EMData::rot_scale_trans2D_background, 1, 4)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_rot_scale_conv_new_background_4_5, EMAN::EMData::rot_scale_conv_new_background, 4, 5)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_delete_disconnected_regions_0_3, EMAN::EMData::delete_disconnected_regions, 0, 3)

BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(EMAN_EMData_helicise_3_5, EMAN::EMData::helicise, 3, 5)
}// namespace
...
//    class_< EMAN::EMData, std::auto_ptr<EMAN::EMData> >("EMData", init<  >())
	...
	.def("calc_center_of_mass", &EMAN::EMData::calc_center_of_mass, "Calculate the center of mass using an algorithm written by S Ludtke")
	...
	.def("set_attr", &EMAN::EMData::set_attr_python, args("key", "val"), "Set a header attribute's value from Python.\n \nkey - The header attribute name.\nval - The attribute value.")
//	.def("set_attr", &EMAN::EMData::set_attr)
	...
	.def("write_data",&EMAN::EMData::write_data,EMAN_EMData_write_data_2_6(args("fsp", "loc", "area", "file_nx", "file_ny", "file_nz"), "Dump the image pixel data in native byte order to a disk file.\n \nfsp - The filename to read the image data from\nloc - Location to seek to in the file before writing (size_t)\narea - The image region you want to read, default 0 means read the whole image(default=Null)\nfile_nx - Image x size.(default=0)\nfile_ny - Image y size.(default=0)\nfile_nz Image z size.(default=0)"))
	.def("read_data",&EMAN::EMData::read_data,EMAN_EMData_read_data_2_6(args("fsp", "loc", "area", "file_nx", "file_ny", "file_nz"), "Read the image pixel data in native byte order from a disk file.\nThe image should already have the correct dimensions.\n \nfsp - The filename to read the image data from\nloc - Location to seek to in the file before writing (size_t)\narea - The image region you want to read, default 0 means read the whole image(default=Null)\nfile_nx - Image x size.(default=0)\nfile_ny - Image y size.(default=0)\nfile_nz Image z size.(default=0)"))
	...
	.def("do_ift_cuda", &EMAN::EMData::do_ift_cuda, EMAN_EMData_do_ift_cuda_overloads_0_1(args("preserve_input"), "return the inverse fourier transform (IFT) image of the current\nimage. The result exists only on the GPU.\n \npreserve_input - whether or not this EMData object should be preserved. If this is unecessary than we can avoid a copy and run faster(default=True).\n \nreturn The FFT of the current image in real/imaginary format, existing on the GPU.")[ return_value_policy< manage_new_object >() ])
	.def("calc_ccf_cuda", &EMAN::EMData::calc_ccf_cuda, return_value_policy< manage_new_object >())
	.def("cut_slice_cuda", &EMAN::EMData::cut_slice_cuda, return_value_policy< manage_new_object >(), args("tr"), "cuda equivalent of get_cut_slice\n \ntr - orientation of the slice as encapsulated in a Transform object\n \nexception - ImageDimensionException If this image is not 3D.\nexception ImageFormatException If this image is complex.")
	.def("set_gpu_rw_current", &EMAN::EMData::set_gpu_rw_current)
	.def("column_sum_cuda",&EMAN::EMData::column_sum_cuda,return_value_policy< manage_new_object >(), "Get the column sum as an EMData object using CUDA\n \nReturn object exists solely on the GPU\nexception - ImageDimensionException if this image is not 2D")
	.def("make_rotational_footprint_cuda", &EMAN::EMData::make_rotational_footprint_cuda, EMAN_EMData_make_rotational_footprint_cuda_overloads_0_1(args("unwrap"), "unwrap - (default=true)")[ return_value_policy< manage_new_object >() ])
	// These ones are currently meant mainly for testing purposes
	.def("_copy_gpu_rw_to_cpu", &EMAN::EMData::copy_gpu_rw_to_cpu, "Explicitly force a copy from the cuda device pointer to the CPU\nOriginally added to facilitate testing only.")
...
//	.def("symplane0_ctf", &EMAN::EMData::symplane0_ctf)
	...
	.def("rot_scale_conv_new_background", &EMAN::EMData::rot_scale_conv_new_background, EMAN_EMData_rot_scale_conv_new_background_4_5(args("ang", "delx", "dely", "kb", "scale"), "")[return_value_policy< manage_new_object >()])
