...
# Copied from e2simmxxplor.py (Authored by David Woolford)
...
def main():
	...
	usage = """e2classmxxplor.py <project directory>

	This program allows you to look at per-particle classification based on a
	pre-computed classification matrix. If a particle seems to be mis-classified,
	you can use this program to help figure out why. It will display a single
	asymmetric triangle on the unit sphere, with cylinders representing the
	similarity value for the selected particle vs. each possible reference
	projection. Use the normal middle-click on the asymmetric unit viewer for
	more options.
	"""

	parser = EMArgumentParser(usage=usage,version=EMANVERSION)
	parser.add_argument("--classmx",'-cmx',type=str,help="Specify the classmx\
		file to be explored.",required=False)
	parser.add_argument("--projection",'-prj',type=str,help="Specify the \
		projection file from which euler angles will be parsed",required=False)
	parser.add_argument("--particle",'-ptcl',type=str,help="Specify the particles\
		to be used in this visualization",required=False)
	...
	explorer = EMClassmxExplorer(window)
	window.model = explorer

	if options.particle:
		explorer.set_particle_file(options.particle)
	if options.projection:
		explorer.set_projection_file(options.projection)
	if options.classmx:
		explorer.set_classmx_file(options.classmx)
...
class EMClassmxExplorer(EM3DSymModel):

	def __init__(self, gl_widget, projection_file=None,classmx_file=None,particle_file=None):
		self.init_lock = True # a lock indicated that we are still in the __init__ function
		EM3DSymModel.__init__(self, gl_widget)
		self.window_title = "ClassmXplor"
		self.projection_file = projection_file # a projection file produced by e2project3d
		self.classmx_file = classmx_file
		self.particle_file = particle_file
		self.num_particles = None # keep track of the total number of particles ( the y dimension of the classmx file)
	...
	def set_data(self,classmx_file):
		"""Can only set data using the name of a classmx file"""
		if "EMAN2DB" in classmx_file: classmx_file = db_convert_path(classmx_file)
		self.classmx_file = classmx_file
		e = EMData()
		e.read_image(classmx_file,0,True)
	...
	def set_classmx_file(self,classmx_file):
		self.classmx_file = classmx_file

	def set_particle_file(self,particle_file):
		self.particle_file = particle_file

	def get_num_particles(self):
		if self.classmx_file == None:
			return None
		nx, ny =  gimme_image_dimensions2D(self.classmx_file)
		return ny

	def init_vitals(self):
		""""Vital information"""
		if not file_exists(self.projection_file):
			raise RuntimeError("%s does not exist, this is vital" %self.projection_file)
		if not file_exists(self.classmx_file):
			raise RuntimeError("%s does not exist, this is vital" %self.classmx_file)
		if self.particle_file != None and not file_exists(self.particle_file):
			raise RuntimeError("%s does not exist, this is vital" %self.particle_file)
	...
	def __update_cmp_attribute(self,update=True):
		...
		for i in range(len(self.projections)):
			...
			e.read_image(self.classmx_file,0,False,r)
			allcmp.append(e.get(0))
		if min(allcmp)>0 :
			for i in range(len(self.projections)):
				# We plot reciprocal values so taller peaks are better...
				self.projections[i].set_attr("cmp",1.0/allcmp[i])
		else :
			for i in range(len(self.projections)):
				# We plot -1* values so taller peaks are better...
				self.projections[i].set_attr("cmp",-allcmp[i])
		self.regen_dl()
	...
	def object_picked(self,object_number):
		if object_number == self.current_projection:
			return
	...
	def update_display(self,update=True):
		...
		if self.particle_file != None:
			...
			# Image 0 - which 'class' each particle (y axis) is associated with.
			e.read_image(self.classmx_file,0,False,r)
			classval=e.get(0)
			# Image 1 - a 'weight' associated with each classification.
			e.read_image(self.classmx_file,1,False,r)
			self.weight = e.get(0)
			# Image 2 - dx, translation to bring particle in alignment with this reference
			e.read_image(self.classmx_file,2,False,r)
			d["tx"] = e.get(0)
			# Image 3 - dy, same
			e.read_image(self.classmx_file,3,False,r)
			d["ty"] =  e.get(0)
			# Image 4 - dalpha, rotation
			e.read_image(self.classmx_file,4,False,r)
			d["alpha"] = e.get(0)
			# Image 5 - 0 or 1, flip required
			e.read_image(self.classmx_file,5,False,r)
			d["mirror"] = bool(e.get(0))
			# Image 6 - scale factor (1.0 no scaling)
			e.read_image(self.classmx_file,6,False,r)
			self.scale = e.get(0)
	...
	def get_inspector(self):
		if not self.inspector :
			self.inspector=EMClassmxXplorInspector(self)
...
class EMClassmxXplorInspector(EMSymInspector):
	...
	def __init__(self,target,enable_trace=False,enable_og=False) :
		...
		if self.target().projection_file == None:
			self.add_classmx_dir_data_widgets()
		else:
			self.add_classmx_options()

	def add_classmx_options(self):
		self.classmx_tab= QtGui.QWidget()
		vbl = QtGui.QVBoxLayout(self.classmx_tab)

		self.__init_ptcl_slider(vbl)
		self.tabwidget.insertTab(0,self.classmx_tab,"Classmx")
		self.tabwidget.setCurrentIndex(0)

	def __init_ptcl_slider(self,layout):
		if self.combo == None:
			n = self.target().get_num_particles()
		else:
	...
	def add_classmx_dir_data_widgets(self):
		self.data = classmx_xplore_dir_data()
		if len(self.data) == 0:
			raise RuntimeError("There is no classmx refinement data in the current directory")

		self.classmx_dir_tab= QtGui.QWidget()
		vbl = QtGui.QVBoxLayout(self.classmx_dir_tab)
		...
		self.update_classmx_list(True)
		vbl.addWidget(self.list_widget)
		self.tabwidget.insertTab(0,self.classmx_dir_tab,"Classmx")
		self.tabwidget.setCurrentIndex(0)
	...
	def update_target(self,update=False):
		classmx_file = self.classmx_selection()
		if classmx_file == None:
			return
		...
		try:
			idx = ( i for i in range(len(data[3])) if data[3][i] == classmx_file ).next()
		except:
			return
		projection_file = data[2][idx]
		
		self.target().set_classmx_file(classmx_file)
	...
	def update_classmx_list(self,first_time=False):
		selected_items = self.list_widget.selectedItems() # need to preserve the selection
...
def classmx_xplore_dir_data():
	"""
	Returns a list containing the names of files that can be used by e2classmxxplor
	Format - [refine_dir, particle_file,[projection_00,projection_01..],[classmx_00, classmx_01..]]
	"""
	...
	for dir in dirs:
			...
			prjs_even=[pfx+i for i in dl if "projections_" in i and i[-4:]==".hdf" and "_even" in i]
			classmx_even=[pfx+i for i in dl if "classmx_" in i and "_even" in i]
			...
			if len(prjs_odd)==len(classmx_odd):
				ret.append([dir,str(ptcl[1]),prjs_odd,classmx_odd])
			else:
				print "Mismatch in :",zip(prjs_odd,classmx_odd)
