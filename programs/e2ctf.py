...
def init_sfcurve(opt):
	...
	if opt :
		...
		try:
			db_misc=db_open_dict("e2bdb:e2ctf.misc",True)
			m=db_misc["strucfac"]
			...
			print "No  structure factor found, using default internal structure factor. If fitting results are poor, consider rerunning autofix once structure factor has been computed."
...
#		envelope=[i for i in envelope if i[1]>0]	# filter out all negative peak values
		...
		sc=j[1]/sfact(j[0])
		print "Transitioning to internal structure factor at %1.1f A with scale factor %1.2f"%(1/j[0],sc)
...
#	print er,(parms[0]-dforig)**2,parms[1]
	...
	er*=(1.0+400.0*(parms[0]-dforig)**4)		# This is a weight which biases the defocus towards the initial value
