...
#
# Author: Jesus G. Galaz 9/1/2010 - Last Update 11/10/2012 
...
def main():
	...
	parser.add_argument("--boxsize",type=int,help="If NOT specipied, the reference's box size will match that of the data. If specified, both the refernece and the data will be resized.",default=None)
	parser.add_argument("--refsym", trype=str, default='', help='Will apply the specified symmetry to the edited reference.')
	parser.add_argument("--apix",type=float,default=None,help="If specified, the program will assume this is the correct apix for the DATA, which will be written to the data's header, and will also be used to scale the reference.")
	...
	if options.apix:
		if options.input:
			...
			if '.hdf' in options.input:
				nptcls = EMUtil.get_imag_count(data)
	...
	'''	
	if options.input:
		target_boxsize = EMData(data,0,True)['nx']
	...
	if options.boxsize:
		if options.input:
			...
			for i in range(nptcls):
				a =EMData(data,i)
				a['nx'] = options.boxsize
				a['ny'] = options.boxsize
				a['nz'] = options.boxsize
				a.write_image(data,i)
	...
	scalefactor_int = round(scalefactor)
	ref = ref.process("math.meanshrink",{"n":scalefactor_int})
	
	ref_apix = ref['apix_x']
	scalefactor = ref_apix/target_apix
	ref = ref.process("xform.scale",{"scale":scalefactor,"clip":target_boxsize})
