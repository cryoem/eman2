...
#			#print orienttracedata,classtracedata
	...
	detect_convergence(options.convergence,options.dtheta,orienttracedata,classtracedata,good,good_data,dd)
	good.close()
	good_data.close()
	...
	if options.sym != "c1":
		...
		if options.eman1data != None:
			eman1data2 = Eman1Data(filenames)
			orienttracedata = eman1data2.orienttracedata
			classtracedata = eman1data2.classtracedata
			eman1data3 = Eman1Data(filenames)
			od = eman1data3.orienttracedata
			cd = eman1data3.classtracedata
			dd = eman1data2
		else:
			jwdata3 = JiangWenLstFile(filenames)
			orienttracedata = jwdata3.euler_data
			classtracedata = jwdata3.class_dummy_data
			jwdata2 = JiangWenLstFile(filenames)
			od = jwdata2.euler_data
			cd = jwdata2.class_dummy_data
			dd = jwdata3
		...
		good_data.write("#e2ptcltrace.py data file\n")
		detect_convergence(options.convergence,options.dtheta,orienttracedata,cd,good,good_data,dd)
		good.close()
		good_data.close()
	...
	write_data(orienttracedata,classtracedata,bad,bad_data,dd)
	bad.close()
	bad_data.close()
...
def write_data(data,classdata,good,good_data,dd):
	soln_idx = []
	...
	for number,d in enumerate(data):
		...
		dd.write_lst_idx_entry(number,good)
...
def detect_convergence(converge,dtheta,data,classdata,good,good_data,dd,optional=None):
	...
	for number,d in enumerate(data):
		...
		for i in range(0,limit):
			...
			angle_sum /= (m-1)
			if angle_sum <= dtheta:
				dd. write_output(i,idx,soln_idx,number,good,good_data,angle_solns,optional)
#				good.write(str(number) + "\tstart.hed\n")
#				
#				good_data.write(str(number) + '. ********\n')
#				if optional == None:
...
class Eman1Data:
	...
	def write_lst_idx_entry(self,number,file):
		file.write(str(number) + "\tstart.hed\n")
		
	def write_output(self,i,idx,soln_idx,number,good,good_data,angle_solns,optional=None):
...
class JiangWenLstFile:
	...
	def write_output(self,i,idx,soln_idx,number,good,good_data,angle_solns,optional=None):
		...
		good.write(self.key_list[number]+"\n")
