...
"""
	...
	if options.eman1data != None:
		...
		print "writing out converged data"
		...
		if options.sym != "c1":
			...
			print "writing out converged, reduced data"
...
def detect_convergence(converge,dtheta,data,classdata,good,good_data):
	m = converge
	n = len(data[0])
	limit = (n-m)+1
	for number,d in enumerate(data):
		...
		for i in range(0,limit):
			...
			if angle_sum <= dtheta:
				...
				good_data.write(str(classdata[number][i][1]) +"\t"+str(classdata[number][i][0])+ "\t(" + str(d[i][0])+","+str(d[i][1])+","+str(d[i][2])+")\n")
				for k,idx in enumerate(soln_idx):
					good_data.write(str(classdata[number][idx][1]) + "\t"+str(classdata[number][idx][0])+ "\t(" + str(d[idx][0])+","+str(d[idx][1])+","+str(d[idx][2])+")\t"+str(angle_solns[k])+"\n")
