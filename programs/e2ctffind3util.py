...
"""
	...
	parser.add_argument("--apix", default=0.0, type=float,help="The angstrom per pixel of the micrographs", guitype='floatbox', row=1, col=0, rowspan=1, colspan=1)
	parser.add_argument("--cs", default=0.0, type=float,help="The spherical aberration of the microscope", guitype='floatbox', row=1, col=1, rowspan=1, colspan=1)
	parser.add_argument("--voltage", default=0.0, type=float,help="The voltage (in kV) of the microscope", guitype='floatbox', row=2, col=0, rowspan=1, colspan=1)
	parser.add_argument("--ac", default=0.0, type=float,help="The amplitude contrast of the micrographs", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--maxres", default=0.0, type=float,help="The highest resolution to be fitted (Angstroms)", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--minres", default=0.0, type=float,help="The lowest resolution to be fitted (Angstroms)", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--defocusmin", default=0.0, type=float,help="The starting defocus value for grid search (microns)", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--defocusmax", default=0.0, type=float,help="The end defocus value for grid search (microns)", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--defocusstep", default=0.0, type=float,help="The step width for grid search (microns)", guitype='floatbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--runctffind3", default=False, action="store_true",help="Run ctffind3 on the selected micrographs?", guitype='boolbox', row=2, col=1, rowspan=1, colspan=1)
	parser.add_argument("--allmicrographs", default=False, action="store_true",help="Run Ctffind3 on all micrographs in the micrographs directory?", guitype='boolbox', row=5, col=1, rowspan=1, colspan=1)
	parser.add_argument("--windowsize", default=0, type=int,help="The amplitude contrast of the micrographs", guitype='intbox', row=4, col=1, rowspan=1, colspan=1)
	parser.add_argument("--verbose", "-v", dest="verbose", action="store", metavar="n", type=int, default=0, help="verbose level [0-9], higner number means higher level of verboseness", guitype='intbox', row=3, col=0, rowspan=1, colspan=1)
...
def import_ctf(voltage, cs, ac, apix, verbose):
	...
	for filename in os.listdir("micrographs"):
		if os.path.exists("ctffind3/" + base_name(filename)+"_ctffind3.log"):
			print "ctffind3/" + base_name(filename)+"_ctffind3.log"
				...
				if len(line) > 1:
					if len(line.split()) == 6:
						if line.split()[5] == "Values":
							...
							if "ctf" in jdb.keys():
								print "ctf already exists:" + filename
								ctf = jdb['ctf']
								ctf0 = ctf[0]
								ctf0dict = ctf[0].to_dict()
								ctf0dict['dfdiff'] = e2dfdiff
								ctf0dict['dfang'] = e2dfang
								ctf0dict['defocus'] = e2defocus
								ctf0.from_dict(ctf0dict)
								ctf[0] = ctf0
								jdb.setval("ctf",ctf)
							else:
								jdb = js_open_dict(os.getcwd() + "/info/" + base_name(filename) + "_info.json")
	...
	launch_childprocess("e2ctf.py --voltage {} --cs {} --ac {} --apix {} --allparticles --autofit --curdefocusfix --verbose {}".format(voltage,cs,ac,apix,verbose-1))
