...
def main():
	...
	parser.add_argument("--refinentry",type=int,help="Number of local perturbations to initialize local alignment with (default=8)",default=8)
	...
	for itr in range(startitr,options.niter+startitr):
		...
		if options.refine:
			gd+=" --refine --maxang {:.1f} --refinentry {:d} ".format(options.maxang,options.refinentry)
			if options.randphi:
				gd+=" --randphi"
			if options.rand180:
				gd+=" --rand180"
