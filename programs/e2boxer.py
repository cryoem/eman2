...
class boxerTopaz(QtCore.QObject):
	...
	def do_train(args = None) :
		...
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz convert -s {downsample} -o topaz/processed/particles/particles.txt boxfiles/*.box ;echo")
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz preprocess -s {downsample} -o topaz/processed/micrographs/ topaz/mrc_micro/*.mrc ;echo")
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz train --train-images topaz/processed/micrographs/ --train-targets topaz/processed/particles/particles.txt --radius 3 --model {model} --image-ext .mrc --method GE-binomial --autoencoder 0 --num-particles {nexpected} --epoch-size 1000 --num-epochs 10 --num-workers {threads} --device {gpu} --save-prefix topaz/model --output topaz/results.txt ;echo")
	...
	def do_autobox(micrograph,goodrefs,badrefs,bgrefs,apix,nthreads,params,prog=None):
		...
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz convert -x {downsample} -o topaz/processed/predicted_particles/{selected_micrograph}predicted_full.txt topaz/processed/predicted_particles/{selected_micrograph}predicted.txt ;echo")
	...
	def do_autobox_all(filenames,goodrefs,badrefs,bgrefs,apix,nthreads,params,prog=None):
		...
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz extract topaz/processed/micrographs/*.mrc --model topaz/model_epoch10.sav --radius {pixradius} --threshold {threshold} --num-workers {threads} --output topaz/processed/predicted_particles/predicted.txt ;echo")
		launch_childprocess(f". {boxerTopaz.conda_init_path} && conda activate topaz && topaz convert -x {downsample} -o topaz/processed/predicted_particles/all_predicted_full.txt topaz/processed/predicted_particles/predicted.txt ;echo")
