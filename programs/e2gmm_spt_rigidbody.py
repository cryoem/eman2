...
def main():
	...
	parser.add_argument("--xfin", type=str,help="tranform input", default=None)
	parser.add_argument("--xfout", type=str,help="tranform output", default=None)
	...
	if options.angle_range!=None:
		angrng=[float(s) for s in options.angle_range.split(',')]
		angrng=angrng[-3:]
		print("search additoinal angle", angrng)
		angrng=np.arange(angrng[0], angrng[1], angrng[2]).astype(floattype)
		angrng=np.deg2rad(angrng)/np.pi
		print(angrng)
	else:
		angrng=[0.]
