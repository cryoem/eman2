...
#
# Author: Steve Ludtke 02/01/2011 (sludtke@bcm.edu)
...
def main():
	...
	iterative multi-model single particle reconstruction in EMAN2. It has MANY options, many
...
"""
	...
	parser.add_argument("--nomirror", dest="nomirror", default=False, action="store_true",help="Turn projection over the mirror portion of the asymmetric unit off")
	parser.add_argument("--input", dest="input", default=None,type=str, help="The name of the image file containing the particle data", guitype='filebox', browser='EMSetsTable(withmodal=True,multiselect=False)', row=0, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--models", dest="models", type=str,default="threed.0a.mrc", help="A comma separated list of initial 3-D models for refinement", guitype='filebox', browser='EMModelsTable(withmodal=True,multiselect=True)', row=5, col=0, rowspan=1, colspan=3, mode="refinement")
	...
	parser.add_argument("--path", default=None, type=str,help="The name of a directory where results are placed. If unspecified will generate one automatically of type multi_??.")
	...
	parser.add_argument("--sym", dest = "sym", default="c1",help = "Specify symmetry - choices are: c<n>, d<n>, tet, oct, icos. Optionally specify a comma separated list to specify per-model symmetry.", guitype='strbox', row=10, col=0, rowspan=1, colspan=3, mode="refinement")
	...
	parser.add_argument("--simalign",type=str,help="The name of an 'aligner' to use prior to comparing the images", default="rotate_translate_flip", guitype='comboparambox', choicelist='re_filter_list(dump_aligners_list(),\'refine|3d\', 1)', row=18, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--simaligncmp",type=str,help="Name of the aligner along with its construction arguments",default="dot", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=19, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--simralign",type=str,help="The name and parameters of the second stage aligner which refines the results of the first alignment", default=None, guitype='comboparambox', choicelist='re_filter_list(dump_aligners_list(),\'refine|3d\', 1)', row=20, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--simraligncmp",type=str,help="The name and parameters of the comparitor used by the second stage aligner. Default is dot.",default="dot", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=21, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--simcmp",type=str,help="The name of a 'cmp' to be used in comparing the aligned images", default="dot:normalize=1", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=17, col=0, rowspan=1, colspan=3, mode="refinement")
	...
	parser.add_argument("--shrink", dest="shrink", type = int, default=0, help="Optionally shrink the input particles by an integer amount prior to computing similarity scores. For speed purposes. Default=0, no shrinking", guitype='shrinkbox', row=16, col=0, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--twostage", dest="twostage", type = int, help="Optionally run a faster 2-stage similarity matrix, ~5-10x faster, generally same accuracy. Value specifies shrink factor for first stage, typ 1-3",default=0, guitype='intbox', row=16, col=1, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--prefilt",action="store_true",help="Filter each reference (c) to match the power spectrum of each particle (r) before alignment and comparison",default=False, guitype='boolbox', row=16, col=2, rowspan=1, colspan=1, mode="refinement")
	
	# options associated with e2classify.py
	parser.add_argument("--sep", type=int, help="The number of classes a particle can contribute towards (default is 1)", default=1, guitype='intbox', row=23, col=2, rowspan=1, colspan=1, mode="refinement")
	...
	parser.add_argument("--classkeep",type=float,help="The fraction of particles to keep in each class, based on the similarity score generated by the --cmp argument.", default=0.8, guitype='floatbox', row=24, col=0, rowspan=1, colspan=2, mode="refinement")
	parser.add_argument("--classkeepsig", default=False, action="store_true", help="Change the keep (\'--keep\') criterion from fraction-based to sigma-based.", guitype='boolbox', row=24, col=2, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--classiter", type=int, help="The number of iterations to perform. Default is 1.", default=3, guitype='intbox', row=23, col=0, rowspan=1, colspan=2, mode="refinement")
	parser.add_argument("--classalign",type=str,help="If doing more than one iteration, this is the name and parameters of the 'aligner' used to align particles to the previous class average.", default="rotate_translate_flip", guitype='comboparambox', choicelist='re_filter_list(dump_aligners_list(),\'refine|3d\', 1)', row=28, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classaligncmp",type=str,help="This is the name and parameters of the comparitor used by the fist stage aligner  Default is dot.",default="phase", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=29, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classralign",type=str,help="The second stage aligner which refines the results of the first alignment in class averaging. Default is None.", default=None, guitype='comboparambox', choicelist='re_filter_list(dump_aligners_list(),\'refine|3d\', 1)', row=30, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classraligncmp",type=str,help="The comparitor used by the second stage aligner in class averageing. Default is dot:normalize=1.",default="dot:normalize=1", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=31, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classaverager",type=str,help="The averager used to generate the class averages. Default is \'mean\'.",default="mean", guitype='combobox', choicelist='dump_averagers_list()', row=26, col=0, rowspan=1, colspan=2, mode="refinement")
	parser.add_argument("--classcmp",type=str,help="The name and parameters of the comparitor used to generate similarity scores, when class averaging. Default is \'dot:normalize=1\'", default="dot:normalize=1", guitype='comboparambox', choicelist='re_filter_list(dump_cmps_list(),\'tomo\', True)', row=27, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classnormproc",type=str,default="normalize.edgemean",help="Normalization applied during class averaging", guitype='combobox', choicelist='re_filter_list(dump_processors_list(),\'normalize\')', row=25, col=0, rowspan=1, colspan=3, mode="refinement")
	parser.add_argument("--classrefsf",default=False, action="store_true", help="Use the setsfref option in class averaging to produce better filtered averages.", guitype='boolbox', row=26, col=2, rowspan=1, colspan=1, mode="refinement")
	...
	parser.add_argument("--pad", type=int, dest="pad", help="To reduce Fourier artifacts, the model is typically padded by ~25 percent - only applies to Fourier reconstruction", default=0, guitype='intbox', row=34, col=2, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--recon", dest="recon", default="fourier", help="Reconstructor to use see e2help.py reconstructors -v", guitype='combobox', choicelist='dump_reconstructors_list()', row=33, col=0, rowspan=1, colspan=2, mode="refinement")
	parser.add_argument("--m3dkeep", type=float, help="The percentage of slices to keep in e2make3d.py", default=0.8, guitype='floatbox', row=35, col=0, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--m3dkeepsig", default=False, action="store_true", help="The standard deviation alternative to the --m3dkeep argument", guitype='boolbox', row=35, col=1, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--m3dsetsf", default=False, action="store_true", help="The standard deviation alternative to the --m3dkeep argument", guitype='boolbox', row=35, col=2, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--m3diter", type=int, default=2, help="The number of times the 3D reconstruction should be iterated", guitype='intbox', row=33, col=2, rowspan=1, colspan=1, mode="refinement")
	parser.add_argument("--m3dpreprocess", type=str, default="normalize.edgemean", help="Normalization processor applied before 3D reconstruction", guitype='combobox', choicelist='re_filter_list(dump_processors_list(),\'normalize\')', row=34, col=0, rowspan=1, colspan=2, mode="refinement")
	parser.add_argument("--m3dpostprocess", type=str, default=None, help="Post processor to be applied to the 3D volume once the reconstruction is completed", guitype='comboparambox', choicelist='re_filter_list(dump_processors_list(),\'filter.lowpass|filter.highpass\')', row=36, col=0, rowspan=1, colspan=3, mode="refinement")
	...
	parser.add_argument("--parallel","-P",type=str,help="Run in parallel, specify type:<option>=<value>:<option>:<value>",default=None, guitype='strbox', row=3, col=0, rowspan=1, colspan=2, mode="refinement")
...
#		error = True
	...
	for i in range(0,options.iter) :
		...
		if ( run(get_projection_cmd(options)) != 0 ):
		...
		if ( run(get_simmx_cmd(options)) != 0 ):
		...
		if ( run(get_classify_cmd(options)) != 0 ):
		...
		if ( run(get_classaverage_cmd(options)) != 0 ):
		...
		for ii,p in enumerate(previous_models):
			n=new_models[ii]
			msk=options.mask_images[ii]
			flt=options.filtered_models[ii]
			...
			if options.mass:
				...
				if options.automask3d:
					...
					mask.write_image(msk,0)
#					db_close_dict(options.mask_image)	# What is this for ? 
					b.mult(mask)
...
def check(options,verbose=0):
	...
	for i in options.models:
		if not file_exists(i):
			print "Error: 3D image %s does not exist" %options.models
			error = True
	...
	if ( file_exists(options.models[0]) and options.input != None and file_exists(options.input)):
		(xsize, ysize ) = gimme_image_dimensions2D(options.input);
		(xsize3d,ysize3d,zsize3d) = gimme_image_dimensions3D(options.models[0])
