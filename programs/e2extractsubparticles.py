...
#	parser.add_argument("--output",type=str,default=None,help="Specify output filename, which will contain nptcl*sym particles. Default=sets/subptcl_<refine>_<iter>.hdf")
	...
	outbispec=output.split("__")[0]+"__ctf_flip_bispec.hdf"
	...
	print("Extraction complete, creating bispectra")
	
	if not DEBUG:
		launch_childprocess("e2proc2dpar.py {output} {outbispec} --process filter.highpass.gauss:cutoff_freq=0.01 --process normalize.edgemean --process math.bispectrum.slice:size={bssz}:fp={bsfp} --threads {threads}".format(
			output=output,outbispec=outbispec,bssz=bispec_invar_parm[0],bsfp=bispec_invar_parm[1],threads=options.threads+1))
