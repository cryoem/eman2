...
def main():
	...
	if (options.iter == 1 and options.alignr != None ):
		print "README - If you iterate only once you do not need to specify the --alignr argument. It has no effect."
	...
	# note the parsing of the options.alignr parameters is left for later
	options.alircmp=parsemodopt(options.alignrcmp)
		...
		if (it == 1):
			...
			for i in classification.items():
				...
				for j in i[1]:
					...
					if ( options.alignr != None ):
						refineparms=parsemodopt(options.alignr)
						
						refineparms[1]["az"] = tmp1.get_attr_default("align.az",0)
						refineparms[1]["dx"] = tmp1.get_attr_default("align.dx",0)
						refineparms[1]["dy"] = tmp1.get_attr_default("align.dy",0)
					...
					j[3] = tmp1.get_attr_default("align.dx",0)
					j[4] = tmp1.get_attr_default("align.dy",0)
					j[5] = tmp1.get_attr_default("align.az",0)
					
			if (options.debug):
				if ( options.alignr != "" ): print "Performed refinement alignment"
