...
def main():
	...
	if pad_dims != None:
		...
		#FIXME - this naive approach will not work when users employ sophisticated 3D DFT sampling techniques, such as those employed
		# the Fourier reconstruction routine
		output.clip_inplace(Region((xpad-options.xsize)/2,(ypad-options.ysize)/2, (ypad-options.ysize)/2, options.xsize, options.ysize, options.ysize))
...
def bw_reconstruction(options):
	...
	for i in xrange(0,total_images):
		...
		d = read_and_process_image(options,i)
...
def read_and_process_image(options,i):
	d = EMData()
...
def fourier_reconstruction(options):
	...
	for j in xrange(0,options.iter): #4):     #change back when the thr issue solved
		...
		if ( j > 0 ):
			...
			for i in xrange(0,total_images):
				image = read_and_process_image(options,i)
