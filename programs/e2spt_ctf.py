...
def main():
	...
	parser.add_argument("--skipstripping", type=str, default='', help="""Default=None. Comma-separated list of image indexes to exclude from strip-based fitting (in this case, only global defocus tiling the entire image will be measured).""")
	...
	parser.add_argument("--useglobaldefocus", action='store_true', default=False,help="""Skips stripping (strip-based
		defocus fitting) for all images.""")
	...
	options = sptmakepath (options, 'spt_ctf')
	...
	if options.subtiltsdir:
		...
		if not options.radius:
			print "ERROR: --radius required with --subtiltsdir"
			sys.exit(1)
		...
		if options.autoicethickness:
			options.icethickness = autoIcethickness + options.radius
		else:
			if options.icethickness:
				if options.icethickness < options.radius:
					options.icethickness == options.radius
			else:
				if options.radius:
					options.icethickness == options.radius
				else:
					print "ERROR: if --autoicethickness was not on, then --icethickness or --radius are required"
		...
		if not options.icethickness:
			if options.radius:
				...
				print "\nERROR: provide a value for --icethickness or --radius if processing --imagestem."
				sys.exit(1)
		...
		if not options.icethickness:
			if options.radius:
				...
				print "\nERROR: provide a value for --icethickness or --radius if processing --tiltseries."
				sys.exit(1)
...
def sptctffit( options, apix, imagefilenames, angles, icethickness ):
	...
	for label in imagefilenames:
		...
		if math.fabs( angle ) > 0.5:
			...
			stripwidth = int( math.fabs( allowabledzpixels * (numpy.cos( math.radians (angle) ) / numpy.sin( math.radians (angle) ) ) - icethickness / numpy.sin( math.radians( angle )) ) )#this is in pixels
