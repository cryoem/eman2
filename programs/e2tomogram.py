...
#			if float(clip2.get_attr("mean"))!=float(clip2.get_attr("mean_nonzero")) # continue
			...
			mask.to_one()
			mask.process("eman1.mask.sharp",{"outer_radius":box/2})
			clip1*=mask
			clip1-=float(clip1.get_attr("mean_nonzero"))
			clip1*=mask
			
			clip2.process("eman1.normalize")
			clip2s=clip2.copy(0)
			clip2s.process("eman1.math.squared")
			...
			if bigpad*2>padbox/2 : ccf.process("eman1.mask.sharp",{"outer_radius":padbox/2-1})
			else : ccf.process("eman1.mask.sharp",{"outer_radius":bigpad*2})		# max translation
...
#	Log.logger().set_level(Log.LogLevel.VARIABLE_LOG)
	...
	while ii< len(cmplist)-1:
		...
		for img in iml:
			img.process("eman1.normalize.edgemean")
		im1=iml[0].copy(0)
		for img in iml[1:]:
			im1+=img
		iml=None
		im1.process("eman1.normalize.edgemean")
		if options.highpass>0 :im1.process("eman1.filter.highpass.gaussian",{"highpass":options.highpass})
		if (options.lowpass>0) : im1.process("eman1.filter.lowpass.gaussian",{"lowpass":options.lowpass})
		if options.localavg>1: im1.write_image("aliref.hed",i[0])
		
		im2=EMData()
		im2.read_image(args[inn],i[1])
		im2.process("eman1.normalize.edgemean")
		if options.highpass>0 : im2.process("eman1.filter.highpass.gaussian",{"highpass":options.highpass})
		if (options.lowpass>0) : im2.process("eman1.filter.lowpass.gaussian",{"lowpass":options.lowpass})
	...
	#		for x in vec4: print x
			...
			mask=ref.copy_head()
			mask.to_one()
			mask.process("eman1.mask.sharp",{"outer_radius":rgnp[3]/2,"dx":cen[0],"dy":cen[1]})
			ref*=mask
			ref-=float(ref.get_attr("mean_nonzero"))
			ref*=mask
			
			im2.process("eman1.normalize")
			im2s=im2.copy(0)
			im2s.process("eman1.math.squared")
...
#					print "%d,%d\t%5.2f %5.2f\t%5.2f"%(pairs[-1][0],pairs[-1][1],pairs[-1][2],pairs[-1][3],pairs[-1][4])
	...
	if options.tiltaxis!=400.0 :
		...
		for i in range(nimg):
			...
			a.process("eman1.mask.dampedzeroedgefill")
			a.process("eman1.normalize")
			a.process("eman1.mask.gaussian",{"outer_radius":a.get_xsize()/4})
			b=a.do_fft()
			b.process("eman1.complex.normpixels")
			sum+=b
