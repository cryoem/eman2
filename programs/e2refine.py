...
def main():
	...
	parser.add_option("--check", dest="check", default=False, action="store_true",help="Checks the contents of the current directory to verify that e2refine.py command will work - checks for the existence of the necessary starting files and checks their dimensions. Performs no work ")
	...
	parser.add_option("--classkeep",type="float",help="The fraction of particles to keep in each class, based on the similarity score generated by the --cmp argument. Default is 1.0.",default=1.0)
	parser.add_option("--classkeepsig", action="store_true", default=False, help="Change the keep (\'--keep\') criterion from fraction-based to sigma-based.")
	parser.add_option("--classiter", type="int", help="The number of iterations to perform. Default is 3.", default=3)
	...
	parser.add_option("--classaligncmp",type="string",help="This is the name and parameters of the comparitor used by the fist stage aligner  Default is dot.",default="dot:normalize=1")
	...
	parser.add_option("--make3diter", type=int, default=4, help="The number of times the 3D reconstruction should be iterated")
...
def get_make3d_cmd(options):
	e2make3dcmd = "e2make3d.py %s --sym=%s --hard=%f --iter=%d -f" %(options.cafile,options.sym,options.hard,options.make3diter)
...
def check_make3d_args(options, it=-1, verbose=False):
	...
	if ( options.make3diter < 1 ):
		print "Error, --make3diter must be greater than or equal to 1"
		error = True
...
def get_classaverage_cmd(options):
	...
	e2cacmd += " --ref=%s --iter=%d --keep=%f -f" %(options.projfile,options.classiter,options.classkeep)
...
def check_classaverage_args(options,it=-1,verbose=False):
	...
	if ( options.classkeep < 0 and options.classkeepsig == False ):
		print "Error: can not specify a negative value (%f) of classkeep unless classkeepsig is also specified" %options.classkeep
		error = True
...
def get_projection_cmd(options):
	...
	if ( options.numproj ):
		...
		print "Error: you must specify one of either --prop or --num_proj for e2project3d.py to work"
		exit(1)
...
def check_projection_args(options, verbose=False):
	...
	if ( not options.prop and not options.numproj ):
		print "Error: you must specify one of either --prop or --num_proj"
		error = True
	...
	try:
		p = parsemodopt(options.projector)
		Projectors.get(p[0], p[1])
	except RuntimeError, inst:
		print "ERROR: the specified projector (%s) does not exist or cannot be constructed" %p[0]
		print inst
		error = True
