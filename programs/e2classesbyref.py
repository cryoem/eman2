...
#		sys.exit(1)
			...
			com="e2proc2dpar.py {inp} {out} --process filter.highpass.gauss:cutoff_freq=0.01 --process normalize.edgemean --process mask.soft:outer_radius={maskrad}:width={maskw} --process math.harmonicpow:fp=4 --threads {threads}".format(
...
#			print "New DF {} -> {}   ( {} -> {} )".format(lastdf,ctf.defocus,lastdfn,i)
			...
			if ptclbs.has_attr("is_harmonic_fp"):
				dfmod=ctfim.process("math.harmonicpow",{"fp":4})
			else:
