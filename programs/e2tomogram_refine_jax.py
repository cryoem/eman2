#!/usr/bin/env python
#
# e2tomogram_refine_jax.py - JAX-optimized version of e2tomogram_refine.py
#
# Key changes from original:
#   - Python for-loops inside @jit replaced with jax.lax.scan
#     (prevents XLA from unrolling 9x38=342 loop iterations into the graph)
#   - Persistent compilation cache for instant startup on repeat runs
#   - Matmul precision set to float32 for numerical stability
#   - All numerical results should be identical to the original
...
# --- JAX configuration for fast compilation and caching ---
...
def main():
	...
	usage="e2tomogram_refine_jax.py <tilt series> "
...
def insert_slice(volume, weight, data, ang, mask=1):
	...
	# 8 interpolation offsets - small enough to unroll safely
...
def insert_slice_multi(volume, weight, data, ang, mask=np.ones(100)):
	"""Insert multiple slices using jax.lax.scan instead of Python for-loop.
	
	With scan, XLA traces the loop body ONCE and reuses it for all tilts.
	Original Python for-loop caused XLA to unroll 38 copies into the graph.
	"""
	def body(carry, inputs):
		volume, weight = carry
		d, a, m = inputs
		volume, weight = insert_slice(volume, weight, d, a, m)
		return (volume, weight), None
	
	(volume, weight), _ = jax.lax.scan(body, (volume, weight), (data, ang, mask))
	return volume, weight
