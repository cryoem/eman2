\section{Miscellaneous} \index{Misc.}
...
result.write_image("reconstructor.mrc")\end{verbatim}
	...
	set_policy(YOUR_FUNCTION, return_value_policy(reference_existing_object))
      \item For other cases, do: \\set_policy(YOUR_FUNCTION, return_internal_reference())
      \end{enumerate}
    \item
...
public:
    ...
    \begin{enumerate}
      \item 
	\textbf{portable_fseek()} should be used for fseek.
      \item
	\textbf{portable_ftell()} should be used for ftell.
    \end{enumerate}
...
float omega = r.mrc_omega();\end{verbatim}
    ...
    \subsection{Using Numeric Python} \index{Numeric Python!Usage}
    \begin{itemize}
      \item
