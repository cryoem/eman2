...
def main():
	...
	parser.add_option("--ML",              action="store_true",   default=False,               help="use maximum likelyhood method")	
	...
	if len(args) < 3 or len(args) > 4:
		...
		from applications import localhelicon_MPI, localhelicon_MPInew, localhelicon_MPIming, localhelicon_MPIML
		...
		elif options.ML:	localhelicon_MPIML(args[0], args[1], args[2], options.seg_ny, mask, irp, oup, options.rs, xrp, options.ynumber, \
			txsp, options.delta, options.initial_theta, options.delta_theta, options.an, options.maxit, options.CTF, options.snr, \
				options.dp, options.dphi, options.psi_max, \
			rminp, rmaxp, options.fract, options.npad,options.sym, options.function,\
			options.apix, options.debug, y_restrict2, options.searchit, options.snakeknots, options.slowIO)	
