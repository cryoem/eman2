...
def localhelicon_MPIML(stack, ref_vol, outdir, seg_ny, maskfile, ir, ou, rs, xr, ynumber,\
						txs, delta, initial_theta, delta_theta, an, maxit, CTF, snr, dp, dphi, psi_max,\
						rmin, rmax, fract,  npad, sym, user_func_name, \
						pixel_size, debug, y_restrict, search_iter, slowIO):
...
#  					setfilori_SP(data[seg_start: seg_end], pixel_size, dp, dphi)
			...
			xod1 = [[] for i in xrange(seg_start, seg_end)]	
			for refang in ref_angles:
				n1 = sin(radians(refang[1]))*cos(radians(refang[0]))
				n2 = sin(radians(refang[1]))*sin(radians(refang[0]))
				n3 = cos(radians(refang[1]))
					...
					for im in xrange( seg_start, seg_end ):
						...
						if( (n1*imn1 + n2*imn2 + n3*imn3)>=ant ):
							...
							if(refrings[0] == None):
								...
								startref = time()
								nr = int(2*psi_max/psistep)+1
								nc = nr//2
								ref = [None]*nr
								for i in xrange(0,nr):
									ref[i] = prgl(volft,[ refang[i][0],refang[i][1],refang[i][2]+(i-nc)*psistep, 0.0,0.0], 1, False)
									ref[i].set_attr("is_complex",0)
							...
							startsearch = time()
								...
								if direction == "up" :  
									reduced_psiref = psiref -  90.0
									ima = dataft[im]
								else:               	
									reduced_psiref = psiref - 270.0
									ima = dataft[im].conjg()
							...
							# 	enr = nr + bnr
								...
								if enr > bnr: 
									N = ima.get_ysize()  # assumed image is square, but because it is FT take y.
									rnx   = int(round(xrng[N_step]/stepx[N_step]))
									rny   = int(round(tyrng/stepy))
									...
									for i in xrange(bnr, enr):
										for iy in xrange(-rny, rny + 1):
											for ix in xrange(-rnx, rnx + 1):
												dss = fshift(ima, ix, iy)
												peak = -Util.sqed(dss, refrings[nc+i], model_blank(N,N,1,1), model_blank(N,N,1,1))
												xod1[im].append(peak)
									
							
							# angb, tx, ty, pik, = directaligriddingconstrainedML(dataft[im], kb, refrings, \
# 								psi_max, psistep, xrng[N_step], tyrng, stepx[N_step], stepy, psi, tx, ty, direction)
							endsearch = time()	
							#print "image id, searching time is ", im, endsearch-startsearch, iddi
							searchtime = searchtime +endsearch-startsearch 
...
# 									neworient[im][:4] = [angb, tx, ty, refang]
			...
			#compute all the probabilities for every segment.
			for kl in xrange(seg_start, seg_end):
