...
#
"""
I removed this function.  It appears to be a copy of iter_isac and was confusing me.  PAP 09/07/2015
iter_isac_with_program_state_stack_restart
"""
...
def mpi_sort(data_m, total_number_of_images, nref, comm):
	"""
	...
	total_number_of_processes = mpi_comm_size(comm)
	...
	# print "index_of_sorted_data_m:", len(index_of_sorted_data_m), index_of_sorted_data_m, 
	...
	while global_index_for_positioning_merged_data_distributively < total_number_of_images*nref:
		...
		recv_array  = mpi_gather(data_to_send, 1, MPI_FLOAT, 1, MPI_FLOAT, 0, comm)
		if (my_id == 0):
			recv_array_argmax = recv_array.argmax()
		else:
			recv_array_argmax = 0
			
		recv_array_argmax = mpi_bcast(recv_array_argmax, 1, MPI_INT, 0, comm)
		...
		if process_that_needs_to_receive_data == my_id and recv_array_argmax == my_id:
			...
			global_index_for_data = mpi_recv(1,MPI_INT, recv_array_argmax, MPI_TAG_UB, comm).tolist()[0]
			global_sorted_indices.append(global_index_for_data)
		elif recv_array_argmax == my_id:
			global_index_for_data = index_of_data + this_process_image_start*nref
			mpi_send(global_index_for_data, 1, MPI_INT, process_that_needs_to_receive_data, MPI_TAG_UB, comm)
			local_index += 1
...
def mpi_assign_groups(data_m, nref, nima, comm):
	from mpi          import mpi_comm_size, mpi_comm_rank, MPI_COMM_WORLD, MPI_FLOAT, MPI_INT
	from mpi          import mpi_bcast, mpi_barrier, mpi_send, mpi_recv, mpi_comm_split
	...
	parallel_index_of_sorted_data_m = mpi_sort(data_m, nima, nref, comm)
	...
	for proc_id in xrange(total_number_of_processes):
		...
		del_row = mpi_bcast(del_row, nref, MPI_INT, proc_id, comm)
		del_col = mpi_bcast(del_col, nima, MPI_INT, proc_id, comm)
		id_list_size = mpi_bcast(id_list_size, nima, MPI_INT, proc_id, comm)
		kt = mpi_bcast(kt, 1, MPI_INT, proc_id, comm)[0]
		last_group = mpi_bcast(group, 1, MPI_INT, proc_id, comm)[0]
		# print "kt, last_group", kt, last_group
		if last_group >= 0:
			group = last_group
	...
	for i in xrange(1, total_number_of_processes):
		if my_id == 0:
			id_list_receive[i] = wrap_mpi_recv(i, comm)
			id_list_index_receive[i] = wrap_mpi_recv(i, comm)
		elif my_id == i:
			wrap_mpi_send(id_list, 0, comm)
			wrap_mpi_send(id_list_index, 0, comm)
