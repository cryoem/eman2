...
#  NEW MPI WORK 03/17/2017

def iter_isac_pap(stack, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, \
	...
	if  not candidatesexist:
		...
		while Iter <= max_round and couldnt_find_stable < 3 and K > 3:
			...
			refi = isac_MPI_pap(data, refi, maskfile=None, outname=None, ir=ir, ou=ou, rs=rs, xrng=xr, yrng=yr, step=ts, 
			...
			for mloop in xrange(1, match_first+1):
				if myid == main_node:
					...
					print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
				refi = isac_MPI_pap(data, current_refim, maskfile=None, outname=None, ir=ir, ou=ou, rs=rs, xrng=xr, yrng=yr, step=ts,
	...
	for mloop in xrange(1, match_second+1):
		...
		if K_left > 0:
			...
			refim_left = isac_MPI_pap(data_left, refim_left, maskfile=None, outname=None, ir=ir, ou=ou, rs=rs, xrng=xr, yrng=yr, step=ts, 
...
#					refim[i].write_image("init_group%d_2nd_phase_round%d.hdf"%(color, mloop), i)
		...
		refim = isac_MPI_pap(alldata, refim, maskfile=None, outname=None, ir=ir, ou=ou, rs=rs, xrng=xr, yrng=yr, step=ts, 
				maxit=maxit, isac_iter=main_iter, CTF=CTF, snr=snr, rand_seed=-1, color=color, comm=group_comm, 
				stability=True, stab_ali=stab_ali, iter_reali=iter_reali, thld_err=thld_err, FL=FL, FH=FH, FF=FF, dst=dst, method = alimethod)
...
def isac_MPI_pap(stack, refim, maskfile = None, outname = "avim", ir=1, ou=-1, rs=1, xrng=0, yrng=0, step=1, 
