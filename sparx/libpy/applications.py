...
def mapcoords(x, y, r, nx, ny):
	...
	r: ratio by which old image is resampled by
	nx, ny: dimensions of old image
	
	Output:
	
	x'',y'':	Let (x',y') be the closest point to (x,y) 
				where (x',y') = (Util.round(x''/r), Util.round(y''/r)) for some (x'', y'') 
				in new image. 
	'''	
	# Neighborhood of (x,y) in old image which contains a point (x',y') such that
	# (x',y') = (Util.round(x''/r), Util.round(y''/r)) for some (x'', y'') 
	if r > 1:
		nbrhd = 1
	else:
	...
	for i in xrange(-nbrhd, nbrhd+1):
		...
		for j in xrange(-nbrhd, nbrhd+1):
			...
			if xnew >0 and ynew > 0:
				...
				if dst > mindist or mindist < 0:
					mindist = dst
					minxnew = xnew
					minynew = ynew
					
	if minxnew < 0 or minynew < 0:
...
def windowmic(outdir, micname, hcoordsname, stkformat, pixel_size, boxsize, ptcl_overlap, inv_contrast, new_pixel_size, rmax):
	'''
	...
	INPUT
			...
			stkformat: File format under which windowed segments will be stored - handles 'bdb' and 'hdf'
			
			pixel_size: The pixel size of the micrographs in which the helices were boxed.
			
			boxsize: Integer. Box size of segments to be windowed.
			...
			rmax: Float. Radius of filament in Angstroms. If rmax < 0, then it's set to boxsize/2 - 2 in pixels using pixel size new_pixel_size.
	
	OUTPUT
	
	       A stack of windowed segments (in file format stkformat) corresponding to EACH boxed helix in the input micrograph.
	...
	filt_gaussh(get_im(micname), 1./boxsize).write_image(tmpfile)  # remove frequencies too low for the box size
	
	# Set box coordinates in e2helixboxer database
	cmd ="e2helixboxer.py %s"%(tmpfile) + helix_coords+" --helix-width=%d"%(boxsize)
	call(cmd, shell=True)
	
	# Window the segments
	cmd ="e2helixboxer.py %s"%tmpfile+ptcl_images+ptcl_coords+" --helix-width=%d  --ptcl-overlap=%d  --ptcl-length=%d --ptcl-width=%d  --gridding --ptcl-norm-edge-mean"%(boxsize, ptcl_overlap, boxsize, boxsize)
	call(cmd, shell=True)
	...
	for h in xrange(nhelices):
		...
		if stkformat == 'bdb':
			...
			otcl_images  = os.path.join(outdir, "QT" + ptcl_images)
...
def windowallmic(dirid, micid, micsuffix, outdir, rise, pixel_size, boxsize, stkformat = 'bdb', hcoords_suffix = "_boxes.txt", ptcl_overlap=-1, inv_contrast=False, new_pixel_size=-1, rmax = -1.0):
	'''
	Input
	
		dirid: A string. Any directory containing dirid as a contiguous string will be searched
		       for micrographs to be windowed. These micrographs are assumed to be those which were used to box the helices, and which has 
		       pixel size pixel_size.
		       
		micid: A string. Any file with suffix micsuffix containing micid as contiguous string will be
		       windowed. 
		       
		micsuffix: A string denoting micrograph type, e.g. 'hdf' or '.hdf' - assumed to be a suffix. 
		
		outdir: A string. A directory called outdir will be created in each micrograph directory and the windowed segments and possibly resampled micrographs (if pixel size changed) will be put there.
		
		rise:  helical symmetry parameter rise in Angstroms.
		
		pixel_size: The pixel size of the micrographs in which the helices were boxed.
		
		boxsize: Integer. Final box size in pixels of segments to be windowed.
		
		stkformat: File format under which windowed segments will be stored - handles 'bdb' and 'hdf'
		
		hcoords_suffix: String identifier which when concatenated with the name of a micrograph name gives the name of the text file 
						containing coordinates of ALL helices boxed from the micrograph - NAME_OF_MICROGRAPH + hcoords_suffix
	...
	Output
		...
		Segments windowed from each micrograph will be stored in the output 
		directory (in the directory where micrograph is) under the specified format.
		
	Example of use: 
		
		In directory mic0, there is a micrograph mic0.hdf.
		...
		output (in a directory called outdir under directory mic0) a stack
		of segments windowed from the boxed helix in mic0.hdf: QTresampled_mic0_abox_0.hdf
		
		If two helices had been boxed in mic0.hdf, then there would be two output stacks: 
		QTresampled_mic0_abox_0.hdf and QTresampled_mic0_abox_1.hdf 
		...
		pixel_size=1.2
		new_pixel_size=1.84
		rmax=60 # radius of filament in Angstroms
		
		windowallmic(dirid, micid, micsuffix, outdir,  rise, pixel_size, boxsize, stkformat =stkformat, hcoords_suffix = "_boxes.txt", ptcl_overlap=-1, inv_contrast=False, new_pixel_size=new_pixel_size, rmax = rmax)
	'''
	import os
	from utilities      import print_begin_msg, print_end_msg, print_msg
	...
	# this is assuming rise is integer number of pixels using pixel size new_pixel_size
	if ptcl_overlap < 0:
		ptcl_overlap = boxsize - int(rise/new_pixel_size)
		...
		for i2, v2 in enumerate(flist2):
			...
			if filename.find(micid)>-1:
				...
				if( os.path.exists(hcoordsname) ):
					...
					print_msg("\n\nPreparing to window helices in micrograph %s with box coordinate file %s\n\n"%(micname, hcoordsname))
					coutdir = os.path.join(os.path.join(topdir, v1), outdir)
					windowmic(coutdir, micname, hcoordsname, stkformat, pixel_size, boxsize, ptcl_overlap, inv_contrast, new_pixel_size, rmax)
