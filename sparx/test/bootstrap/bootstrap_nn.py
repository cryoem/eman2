...
while iarg < len(argv) :
    tag = argv[iarg]
    if( tag == "-h" ) :
        print "usage: bootstrap_nn [-h] -i proj_file -vp volume_prefix -sp status_prefix -n niter -media memory|filename -npad npad"
        exit(-1)
    elif( tag == "-i" ) :
        iarg=iarg+1
...
recons3d_bootstrap( proj_stack, myvolume_file, list_proj, myiter, mymedia, npad, "c1", mystatus )
