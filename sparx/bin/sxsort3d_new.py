...
def get_sorting_image_size(original_data, partids, sparamstructure, snorm_per_particle, log):
	...
	rdata = downsize_data_for_rec3D_smearing(original_data, Tracker["nxinit_refinement"], return_real = False, npad = 1)
...
def do_EQKmeans_nways_clustering_stable_seeds(workdir, initial_partids, params, sort_res, log_main):
	...
	if Tracker["number_of_groups"]>1: # In case the number of the input particles is small
		while Tracker["number_of_groups"] >1:
			...
			parameterstructure = read_paramstructure_for_sorting(partids, Tracker["paramstructure_dict"], Tracker["paramstructure_dir"])	
			...
			for indep_run_iter in xrange(0, Tracker["constants"]["indep_runs"]): # N independent runs
				...
				if Blockdata["myid"] == Blockdata["main_node"]:
					...
					cmdexecute(cmd)
				Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], MPI_COMM_WORLD)
				#memory_check( "before 1 EQKmeans_by_dmatrix_orien_groups %d"%Blockdata["myid"])
				...
				#memory_check( "after 1 EQKmeans_by_dmatrix_orien_groups  %d"%Blockdata["myid"])
...
def compare_two_images_eucd(data, ref_vol):
	...
	for im in xrange(len(data)):
		...
		if Tracker["applybckgnoise"]:
			#print(Blockdata["unrolldata"][data[im].get_attr("particle_group")])
...
def check_mpi_settings(log):
	...
	try:
		...
		if( Blockdata["myid"] == Blockdata["main_node"]):print(line, "system mem info: %5.1f"%mem_gib)
	except:
		mem_gib = None
...
def get_input_from_datastack(log_main):# Case three
	...
	Tracker["an"] = 0.0 #Tracker["constants"]["an"]
...
def do_final_maps(number_of_groups, minimum_size, selected_iter, refinement_dir, masterdir, rec3d_image_size, log_main):
	...
	if( Blockdata["myid"] == Blockdata["main_node"]):
		...
		log_main.add(msg)
		print(line, msg)
		#msg = "nproc_do_final_per_node from data: %d"%int(nproc_do_final_per_node)
		#log_main.add(msg)
		#print(line, msg)
	...
	else: Blockdata["no_of_groups"] = nproc_do_final_per_node
	if minimum_size//5>nproc_do_final_per_node*Blockdata["no_of_groups"]: Blockdata["no_of_processes_per_group"] = nproc_do_final_per_node//2
	else:  Blockdata["no_of_processes_per_group"] = max(minimum_size//5//Blockdata["no_of_groups"]//2, 2)
	if( Blockdata["myid"] == Blockdata["main_node"]):
		msg = "determined CPU used per node: %d"%Blockdata["no_of_processes_per_group"]
		log_main.add(msg)
		print(line, msg)		
	...
	Blockdata["no_of_processes_per_group"] = Blockdata["nproc"]//Blockdata["no_of_groups"]
	Blockdata["ncpuspernode"] = Blockdata["no_of_processes_per_group"]
	Blockdata["nsubset"]  = Blockdata["ncpuspernode"]*Blockdata["no_of_groups"]
	create_subgroup()
...
def do3d_sorting_groups_nofsc_smearing_iter(data, paramstructure, norm_per_particle, iteration):
	global Tracker, Blockdata
	keepgoing = 1
	if(Blockdata["myid"] == Blockdata["nodes"][0]):
		cmd = "{} {}".format("mkdir", os.path.join(Tracker["directory"], "tempdir"))
	...
	mpi_barrier(MPI_COMM_WORLD)
	do3d_sorting_group_insertion_smearing(data, paramstructure, norm_per_particle, randomset=1)
	mpi_barrier(MPI_COMM_WORLD)
	fsc143                    = 0
	fsc05                     = 0
	Tracker["fsc143"]         = 0
...
def do3d_sorting_group_insertion_smearing(sdata, sparamstructure, snorm_per_particle, randomset=2):
	...
	if randomset ==1:
		for index_of_groups in xrange(Tracker["number_of_groups"]):
			for procid in xrange(2, 3):
				tvol, tweight, trol = recons3d_trl_struct_group_MPI(myid = Blockdata["myid"], main_node = Blockdata["nodes"][procid],\
				  prjlist = sdata,  random_subset = procid, group_ID = index_of_groups, paramstructure = sparamstructure, \
				  norm_per_particle = snorm_per_particle, CTF = (not Tracker["constants"]["noctf"]),\
...
def recons3d_trl_struct_group_MPI(myid, main_node, prjlist, random_subset, group_ID, paramstructure, norm_per_particle = None, \
      upweighted = True, mpi_comm= None, CTF = True, target_size=-1):
	"""
		recons3d_4nn_ctf - calculate CTF-corrected 3-D reconstruction from a set of projections using three Eulerian angles, two shifts, and CTF settings for each projeciton image
		Input
			list_of_prjlist: list of lists of projections to be included in the reconstruction
			...
			if random_subset == 2:
				...
				if	prjlist[im].get_attr("chunk_id") == random_subset:	
					...
					for ii in xrange(len(tdir)):
						...
						r.insert_slice( recdata, Transform({"type":"spider","phi":refang[iang][0],"theta":refang[iang][1],"psi":refang[iang][2]+ipsi*delta}), toprab*avgnorm/norm_per_particle[im])
	#  clean stuff
	#del recdata, tdir, ipsiandiang, allshifts, probs
	reduce_EMData_to_root(fftvol, myid, main_node, comm=mpi_comm)
	reduce_EMData_to_root(weight, myid, main_node, comm=mpi_comm)
	del rshifts_shrank
	if myid == main_node:dummy = r.finish(True)
...
def downsize_data_for_sorting(original_data, return_real = False, preshift = True, npad = 1):
	...
	for im in xrange(nima):
		...
		cdata[im].set_attr("is_complex",0)	
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if( Blockdata["myid"] == Blockdata["main_node"]):print(line,"finishing downsizing data for sorting")
	return cdata, rdata
...
def EQKmeans_by_dmatrix_orien_groups(original_data, partids, ptls_in_orien_groups, paramstructure, norm_per_particle, clean_volumes = False):
	...
	for iproc in xrange(Blockdata["nproc"]):
		...
		proc_list[iproc] = [iproc_image_start, iproc_image_end]
	compute_noise(Tracker["nxinit"])
	cdata, rdata = downsize_data_for_sorting(original_data, preshift = True, npad = 1)# Attentions to shifts!
	while total_iter<Tracker["total_number_of_iterations"]:
		...
		while iter <iter_max:
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				...
				print(msg)
			mpi_barrier(MPI_COMM_WORLD)
			#memory_check("before do3d_sorting  myid  %d"%(Blockdata["myid"]))		
			do3d_sorting_groups_nofsc_smearing_iter(rdata, paramstructure, norm_per_particle, iteration = total_iter)
			#memory_check("after do3d_sorting   myid  %d"%(Blockdata["myid"]))
			mpi_barrier(MPI_COMM_WORLD)	
			if(Blockdata["myid"] == Blockdata["main_node"]):
				msg = "rec3d ends"
				log.add(msg)
...
def downsize_data_for_rec3D_smearing(original_data, particle_size, return_real = False, npad = 1):
	# The function will read from stack a subset of images specified in partids
	#   and assign to them parameters from partstack with optional CTF application and shifting of the data.
	# So, the lengths of partids and partstack are the same.	
	global Tracker, Blockdata
	...
	#         2. rdata: data for reconstruction, 4nn return real image
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if( Blockdata["myid"] == Blockdata["main_node"]): print(line, "reading data starts ")		
	nima       = len(original_data)
	rdata      = [None]*nima
	mask2D     = model_circle(Tracker["constants"]["radius"],Tracker["constants"]["nnxo"],Tracker["constants"]["nnxo"])
		...
		for procid in xrange(2):
			for ifsc in xrange(2):
				tvol, tweight, trol = recons3d_4nnsorting_group_fsc_MPI(myid = Blockdata["myid"], main_node = Blockdata["nodes"][procid],  \
				  ...
				  CTF = (not Tracker["constants"]["noctf"]), upweighted = False, target_size = (2*Tracker["nxinit"]+3))
				if(Blockdata["myid"] == Blockdata["nodes"][procid]):
					tvol.set_attr("is_complex",0)
...
def recons3d_4nnsorting_group_fsc_MPI(myid, main_node, prjlist, fsc_half, random_subset, group_ID, paramstructure, norm_per_particle, \
	...
	import copy
	rshifts_shrank  = copy.deepcopy(Tracker["rshifts"])	
	if Tracker["nxinit"] != Tracker["nxinit_refinement"]:
		for im in xrange(len(rshifts_shrank)):
			rshifts_shrank[im][0] *= float(Tracker["nxinit"])/float(Tracker["constants"]["nnxo"])
			rshifts_shrank[im][1] *= float(Tracker["nxinit"])/float(Tracker["constants"]["nnxo"])
	nnx = prjlist[0].get_xsize()
...
def merge_two_unfiltered_maps(map1_file, map2_file, cluster_ID):
	...
	if Tracker["constants"]["B_enhance"] !=-1: #3 One specifies and then apply B-factor sharpen
		if Tracker["constants"]["B_enhance"] == 0.0: # auto mode
			...
			if Tracker["constants"]["B_stop"]!=0.0: freq_max = 1./Tracker["constants"]["B_stop"]
			if freq_min>= freq_max:ERROR("Your B_start is too high! Decrease it and re-run the program!", "--postprocess option", 1, Blockdata["myid"])
...
def main():
	...
	parser.add_option("--postlowpassfilter",               type   ="float",         default=0.0,                       help="=0.0, low_pass filter to resolution limit; =some value, low_pass filter to some valume; =-1, not low_pass filter applied")
	...
	if Blockdata["no_of_groups"]<2: ERROR("Insuffcient number of computing nodes, the smallest number of computing nodes is two", "sort3d", 1, Blockdata["myid"])		
	...
	Constants["nofinal_sharpen"]            = options.nofinal_sharpen
	...
	Constants["postlowpassfilter"]           = options.postlowpassfilter
	...
	else: ERROR("Incorrect refinement_dir or instack", "sxsort3d_new.py", 1, Blockdata["myid"])
...
### Check mpi setting
	...
	if options.post_sorting_sharpen: # post sorting option!
		...
		if(Blockdata["myid"] == Blockdata["main_node"]): print(" reconstruct maps for sharpening ")
		number_of_groups = 0
		...
		if(Blockdata["myid"] == Blockdata["main_node"]): print(" % clusters are found  "%number_of_groups)
		...
		for iproc in xrange(number_of_groups):
			if iproc == Blockdata["myid"]:
				print("merge volume %d"%iproc)
	...
	for indep_sort3d in xrange(Tracker["total_sort3d_indepent_run"]):		
		...
		if Blockdata["myid"] == Blockdata["main_node"]:
			...
			if import_from_sparx_refinement:
				...
				print(line, msg)
				log_main.add(msg)
			else: ERROR("Not a meridien refinement", "sort3d", 1, Blockdata["myid"])
		...
		Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], MPI_COMM_WORLD)
		#memory_check( "before sort3d do_EQKmeans_nways_clustering_stable_seeds  %d"%Blockdata["myid"])	
		...
		#memory_check( "after sort3d do_EQKmeans_nways_clustering_stable_seeds  %d"%Blockdata["myid"])
		...
		parameterstructure = read_paramstructure_for_sorting(os.path.join(Tracker["indep_sort3d_dir"], "sort3d_partition.txt"),\
	...
	while iter_rsort< Tracker["total_iter_rsort"]:
		...
		if Tracker["number_of_groups"]>1: #continue N-ways_clustering on unaccounted particles
			...
			final_sort             = []
			#memory_check( "before rsort do_EQKmeans_nways_clustering_stable_seeds  %d"%Blockdata["myid"])
			...
			os.path.join(Tracker["constants"]["masterdir"],"refinement_parameters.txt"), sorting["Accounted"], log_main)
			#memory_check( "after rsort do_EQKmeans_nways_clustering_stable_seeds  %d"%Blockdata["myid"])			
	...
	if not Tracker["constants"]["nofinal_sharpen"]:
		if (import_from_relion_refinement ==1 or import_from_data_stack ==1):ERROR("Only applicable to sphire refinement", "option post_sorting_sharpen ", 1, Blockdata["myid"])
		Tracker["constants"]["orgres"]				= 0.0
		Tracker["constants"]["refinement_delta"]	= 0.0
		Tracker["constants"]["refinement_ts"]		= 0.0
		Tracker["constants"]["refinement_xr"]		= 0.0
