...
###		sxinfo = SXmenu_item(); sxinfo.name = "GUI Information"; sxinfo.label = "GUI Appliation Information"; sxinfo.short_info = "DUMMY STRING"
	...
	def construct_sxcmd_category_list(self):
		...
		sxcmd = SXcmd(); sxcmd.name = "sxpipe"; sxcmd.subname = "isac_substack"; sxcmd.mode = ""; sxcmd.subset_config = ""; sxcmd.label = "ISAC2 Stack Subset"; sxcmd.short_info = "Create a virtual subset stack consisting of particles accounted for by ISAC2 by retrieving particle numbers associated with the ISAC2 or Beautifier class averages. The command also saves a selection file containing the retrieved original image numbers and 2D alignment parameters. In addition, it stores the 2D alignment parameters to the stack header."; sxcmd.mpi_support = False; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_isac"; sxcmd.role = "sxr_pipe"; sxcmd.is_submittable = True
		...
		token = SXcmd_token(); token.key_base = "memory_per_node"; token.key_prefix = "--"; token.label = "Memory per node"; token.help = "User specifies the actual memory per node. By default, program will find the memory per node from system. In case of failure in finding it from system, program puts 2 GB per CPU. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = "-1.0"; token.restore = "-1.0"; token.type = "float"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		...
		token = SXcmd_token(); token.key_base = "img_per_grp"; token.key_prefix = "--"; token.label = "Number of images per group"; token.help = "User expected group size in integer. By default sorting is set on search mode. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = "-1"; token.restore = "-1"; token.type = "int"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		...
		token = SXcmd_token(); token.key_base = "overhead"; token.key_prefix = "--"; token.label = "Overhead memory per node [GB]"; token.help = "User provided information about overhead memory usage per node in GB (NOT per CPU). By default, it is estimated as 5.0 GB per node. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = "5."; token.restore = "5."; token.type = "float"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		...
		token = SXcmd_token(); token.key_base = "num_core_set"; token.key_prefix = "--"; token.label = "Number of core set images"; token.help = "Map will not be reconstructed if number of core set images is less than this number. By default, the program will choose the maximum number between minimum group size and 100 as num_core_set. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = "-1"; token.restore = "-1"; token.type = "int"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		token = SXcmd_token(); token.key_base = "nstep"; token.key_prefix = "--"; token.label = "Number of steps to decrease minimum group size"; token.help = "An integer number controls group size constrained Kmeans clustering. By default, it is set 5. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = "5"; token.restore = "5"; token.type = "int"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		token = SXcmd_token(); token.key_base = "use_umat"; token.key_prefix = "--"; token.label = "Use fuzzy membership of particle images in making maps"; token.help = "A boolean flag to apply fuzzy group membership to control sorting stability. By default, it is false. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		token = SXcmd_token(); token.key_base = "not_freeze_groups"; token.key_prefix = "--"; token.label = "Remove small groups during within box comparison"; token.help = "A boolean flag to control the group searching. By default, sorting will not remove small groups. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
