...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		sxcmd = SXcmd(); sxcmd.name = "sxpipe"; sxcmd.subname = "organize_micrographs"; sxcmd.mode = ""; sxcmd.label = "Organize micrographs"; sxcmd.short_info = "Organize micrographs by moving micrographs in a selecting file from input directory (specified by input micrographs pattern) to output directory."; sxcmd.mpi_support = False; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_cter"; sxcmd.role = "sxr_util"; sxcmd.is_submittable = True
		...
		token = SXcmd_token(); token.key_base = "input_selection_list"; token.key_prefix = ""; token.label = "Input micrograph selecting list"; token.help = "Specify a name of text file containing a list of selected micrograph names or paths. The file extension must be '.txt'. The directory path of each entry will be ignored if there is. "; token.group = "main"; token.is_required = True; token.is_locked = False; token.default = ""; token.restore = ""; token.type = "txt"; sxcmd.token_list.append(token)
		token = SXcmd_token(); token.key_base = "output_directory"; token.key_prefix = ""; token.label = "Output directory"; token.help = "The micrographs in selecting list will be moved to this directory. This directory will be created automatically if it does not exist. "; token.group = "main"; token.is_required = True; token.is_locked = False; token.default = ""; token.restore = ""; token.type = "string"; sxcmd.token_list.append(token)
		token = SXcmd_token(); token.key_base = "reverse"; token.key_prefix = "--"; token.label = "Reverse operation"; token.help = "Move back micrographs from output directory to input directory. Please use this option to restore the previously-moved micrographs. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token)
		token = SXcmd_token(); token.key_base = "check_consistency"; token.key_prefix = "--"; token.label = "Check consistency of inputs"; token.help = "Create a text file containing the list of Micrograph ID entries might have inconsitency among inputs (i.e. mic_consistency_check_info.txt). "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token)
