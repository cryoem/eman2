...
###		sxinfo = SXmenu_item(); sxinfo.name = "GUI Information"; sxinfo.label = "GUI Appliation Information"; sxinfo.short_info = "DUMMY STRING"
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "do_approximation"; token.key_prefix = "--"; token.label = "Approximate values"; token.help = "Approximate the values of the soft edge area instead of using the exact values. This will lead to a less smoothened mask, but will mirror the previous behaviour. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = [['', '', ''], ['edge_width', '0', 'True']]; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token; sxcmd.dependency_dict.setdefault('edge_width', []).append([token.key_base, '0', 'True'])
