...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "selection_list"; token.key_prefix = ""; token.label = "Micrograph selecting list"; token.help = "Specify a name of text file containing a list of selected micrograph names or paths. The file extension must be '.txt'. The directory path of each entry will be ignored if there are any. "; token.group = "main"; token.is_required = True; token.is_locked = False; token.default = ""; token.restore = ""; token.type = "txt"; sxcmd.token_list.append(token)
