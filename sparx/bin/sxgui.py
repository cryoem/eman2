...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		sxcmd = SXcmd(); sxcmd.name = "sxwindow"; sxcmd.mode = ""; sxcmd.label = "Particle Extraction"; sxcmd.short_info = "Window particles from micrographs using the particle coordinates."; sxcmd.mpi_support = True; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_window"; sxcmd.role = "sxr_pipe"; sxcmd.is_submittable = True
		...
		sxcmd = SXcmd(); sxcmd.name = "e2proc3d"; sxcmd.mode = "clip"; sxcmd.label = "Pad/Clip VIPER Model"; sxcmd.short_info = "Pad or clip. Pad or clip volume to the specific dimensions. Specify 1, 3 or 6 arguments; '<x>[,<y>,<z>[,<xc>,<yc>,<zc>]]'. "; sxcmd.mpi_support = False; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_viper"; sxcmd.role = "sxr_pipe"; sxcmd.is_submittable = True
		...
		token = SXcmd_token(); token.key_base = "continue_from_iter"; token.key_prefix = "--"; token.label = "Restarting iteration"; token.help = "The iteration from which 3D refinement should be continued. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = "-1"; token.restore = "-1"; token.type = "int"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "wn"; token.key_prefix = "--"; token.label = "Processing window size [Pixels]"; token.help = "If it is necessary to speed up the processing time, set a non-zero positive integer to this option. Then, for data processing, program windows original images with the specified size by clipping the edges. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.default = "0"; token.restore = "0"; token.type = "int"; sxcmd.token_list.append(token)
