...
###		sxinfo = SXmenu_item(); sxinfo.name = "GUI Information"; sxinfo.label = "GUI Appliation Information"; sxinfo.short_info = "DUMMY STRING"
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "overwrite"; token.key_prefix = ""; token.label = ""; token.help = ""; token.group = ""; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = [['', '', '']]; token.default = ""; token.restore = ""; token.type = ""; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
