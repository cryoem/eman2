...
###		sxinfo = SXmenu_item(); sxinfo.name = "GUI Information"; sxinfo.label = "GUI Appliation Information"; sxinfo.short_info = "DUMMY STRING"
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "use_umat"; token.key_prefix = "--"; token.label = "Use fuzzy assignment of particle images to groups. A boolean flag to use fuzzy group membership, which improves program convergence. By default, it is false. (default False"; token.help = "Use fuzzy assignment of particle images to groups. A boolean flag to use fuzzy group membership, which improves program convergence. By default, it is false. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = ['', '', '']; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
