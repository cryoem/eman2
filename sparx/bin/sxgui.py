...
class Popuptwodali(QWidget):
	...
	self.RUN_button.move(230, 520)
        ...
        #Just a label
	title2= QtGui.QLabel('<b>Important</b> parameters', self)
	title2.move(10,40)
	...
	self.gencmdline(saveparams=True)
...
class Popupadvparams_ali2d(QWidget):
	...
	usrfunc= QtGui.QLabel('User Function', self)
	usrfunc.move(10,300)
...
class PopupHelicalRefinement(QWidget):
	...
	self.setWindowTitle('sxihrsr')
        #Here we just set a label and its position in the window
	title1=QtGui.QLabel('<b>sxihrsr</b> - iterative real space helical reconstruction', self)
	title1.move(10,10)
    ...
    def advparams(self):
        print "Opening a new popup window..."
        self.w = Popupadvparams_ihrsr(self.savedparmsdict)
        self.w.resize(500,380)
        self.w.show()
	self.setadv=True
    def setactiveheader(self):
...
class Popupadvparams_ihrsr(QWidget):
    def __init__(self,savedparms):
        QWidget.__init__(self)
        #Here we just set the window title
