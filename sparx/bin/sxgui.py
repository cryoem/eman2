...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "input_image_path"; token.key_prefix = ""; token.label = "Input micrographs"; token.help = "For any of Micrograph Modes, specify input micrographs path pattern with a wild card (*). Images of bdb format can not be used as as input micrographs. A particle stack can also be supplied with --stack_mode, but this mode is not supported by sxgui. "; token.group = "main"; token.is_required = True; token.default = ""; token.restore = ""; token.type = "any_micrograph"; sxcmd.token_list.append(token)
