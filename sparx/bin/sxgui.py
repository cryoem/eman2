...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		sxcmd = SXcmd(); sxcmd.name = "sxprocess"; sxcmd.subname = ""; sxcmd.mode = "combinemaps"; sxcmd.label = "Sharpening"; sxcmd.short_info = "Combine a pair of unfiltered odd & even 3D density maps, then enhance the power spectrum of high frequencies. B-factor can be estimated from the two unfiltered maps. "; sxcmd.mpi_support = False; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_meridien"; sxcmd.role = "sxr_pipe"; sxcmd.is_submittable = True
		token = SXcmd_token(); token.key_base = "combinemaps"; token.key_prefix = "--"; token.label = "Combine unfiltered maps"; token.help = "Combine a pair of unfiltered odd & even 3D density maps, then enhance the power spectrum of high frequencies. B-factor can be estimated from the two unfiltered maps. "; token.group = "main"; token.is_required = True; token.is_locked = True; token.default = True; token.restore = True; token.type = "bool"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "output_dir"; token.key_prefix = "--"; token.label = "Output directory"; token.help = "Specify path to the output directory of this combine unfiltered maps process. By default, the program uses the current directory. However, GUI requires the path other than the current directory. "; token.group = "main"; token.is_required = True; token.is_locked = False; token.default = ""; token.restore = ""; token.type = "output"; sxcmd.token_list.append(token)
		token = SXcmd_token(); token.key_base = "pixel_size"; token.key_prefix = "--"; token.label = "Pixel size [A]"; token.help = "Pixel size in A. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.default = "1.0"; token.restore = "1.0"; token.type = "apix"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "aa"; token.key_prefix = "--"; token.label = "Low-pass filter fall-off [1/Pixels]"; token.help = "Low-pass filter fall-off. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.default = "0.01"; token.restore = "0.01"; token.type = "float"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "swap_ratio"; token.key_prefix = "--"; token.label = "Swap ratio"; token.help = "Specify a float number between 0.0 and 50. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = "1.0"; token.restore = "1.0"; token.type = "float"; sxcmd.token_list.append(token)
		...
		sxcmd = SXcmd(); sxcmd.name = "e2bdb"; sxcmd.subname = ""; sxcmd.mode = "makevstack"; sxcmd.label = "Create Cluster Stack Subset"; sxcmd.short_info = "Make a 'virtual' BDB image stack with the specified name from one or more other stacks. "; sxcmd.mpi_support = False; sxcmd.mpi_add_flag = False; sxcmd.category = "sxc_sort3d"; sxcmd.role = "sxr_pipe"; sxcmd.is_submittable = True
