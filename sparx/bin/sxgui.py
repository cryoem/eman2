...
class SXMainWindow(QMainWindow): # class SXMainWindow(QWidget):
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "noctf"; token.key_prefix = "--"; token.label = "No CTF correction"; token.help = "Use this option if full CTF correction should not be applied. Useful for negative stained data. Always use the CTF correction for cryo data. By default, the program will do full CTF correction. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token)
		token = SXcmd_token(); token.key_base = "nopwadj"; token.key_prefix = "--"; token.label = "Do not adjust PW spectrum"; token.help = "Do not adjust PW spectrum to analytical model or given 1-D rotationally averaged power spectrum. By default, the program applies PW spectrum adjustment to model. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "decimate"; token.key_prefix = "--"; token.label = "Image decimate factor"; token.help = "Decimate images by this factor and change the pixel size. Specify a value large than 1.0 to expand images, and less than 1.0 to shrink them. By default, it does not change size of images"; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = "1.0"; token.restore = "1.0"; token.type = "float"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "output_dir"; token.key_prefix = "--"; token.label = "Output directory"; token.help = "The master output directory for sorting. The results will be written here. This directory will be created automatically if it does not exist. Here, you can find a log.txt that describes the sequences of computations in the program. "; token.group = "main"; token.is_required = True; token.is_locked = False; token.default = ""; token.restore = ""; token.type = "output"; sxcmd.token_list.append(token)
		...
		token = SXcmd_token(); token.key_base = "nofinal_sharpen"; token.key_prefix = "--"; token.label = "Do not reconstruct final maps"; token.help = "Do not reconstruct unfiltered final maps for post refinement process. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.default = False; token.restore = False; token.type = "bool"; sxcmd.token_list.append(token)
