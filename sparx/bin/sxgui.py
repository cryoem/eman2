...
###		sxinfo = SXmenu_item(); sxinfo.name = "GUI Information"; sxinfo.label = "GUI Appliation Information"; sxinfo.short_info = "DUMMY STRING"
	...
	def construct_sxcmd_category_list(self):
		...
		token = SXcmd_token(); token.key_base = "nerosion"; token.key_prefix = "--"; token.label = "Erosion value"; token.help = "Number of times to erode binarized volume. One circle of erosion will remove about 2 pixels from the mask. "; token.group = "main"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = [['', '', '']]; token.default = "0"; token.restore = "0"; token.type = "int"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token
		...
		token = SXcmd_token(); token.key_base = "s_edge_type"; token.key_prefix = "--"; token.label = "Second - Soft-edge type"; token.help = "The type of soft-edge for moon-eliminator 3D mask and a moon-eliminated soft-edged 3D mask. Available methods are (1) \'cosine\' for cosine soft-edged (used in PostRefiner) and (2) \'gaussian\' for gaussian soft-edge. "; token.group = "advanced"; token.is_required = False; token.is_locked = False; token.is_reversed = False; token.dependency_group = [['', '', ''], ['use_second_mask', 'True', 'False']]; token.default = "cosine"; token.restore = "cosine"; token.type = "string"; sxcmd.token_list.append(token); sxcmd.token_dict[token.key_base] = token; sxcmd.dependency_dict.setdefault('use_second_mask', []).append([token.key_base, 'True', 'False'])
