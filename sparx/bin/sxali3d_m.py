...
def main():
	...
	parser.add_option("--nassign",  type="int",  default= 3,                    help="  number of reassignment iterations performed for each angular step (set to 3) ")
	...
	if len(args) < 3 or len(args) > 4:
		...
		ali3d_m(args[0], args[1], args[2], maskfile, options.maxit, options.ir, options.ou, options.rs, options.xr, options.yr, options.ts, options.delta, options.an, options.center, options.nassign, options.nrefine, options.CTF, options.snr, options.ref_a, options.sym, options.function, options.MPI, options.debug, options.fourvar)
