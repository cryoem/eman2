...
def get_input_from_sparx_ref3d(log_main):# case one
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		...
		if Tracker_refinement["constants"]["stack"][0:4]=="bdb:": 
			...
			refinement_stack = "bdb:"+Tracker_refinement["constants"]["stack"][4:]
		else:
			#refinement_stack = os.path.join(refinement_dir_path, Tracker_refinement["constants"]["stack"]) # very rare case
			refinement_stack = Tracker_refinement["constants"]["stack"]  # very rare case
		# Particles are not in the refinement directory, and the path relative to the project directory is in the tracker JSON file.
