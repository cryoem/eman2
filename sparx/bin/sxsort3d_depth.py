...
def Kmeans_minimum_group_size_orien_groups(original_data, partids, params, paramstructure, norm_per_particle, minimum_group_size, clean_volumes = False):
	...
	if( Blockdata["myid"] == Blockdata["main_node"]):
		...
		msg = "Total_stack:  %d K = : %d  nxinit: %d  CTF:  %s  Symmetry:  %s  stop percentage: %f  3-D mask: %s focus mask: %s  Comparison method: %s  minimum_group_size: %d orien  %d"% \
		   ...
		   stopercnt, Tracker["constants"]["mask3D"], Tracker["constants"]["focus3D"], Tracker["constants"]["comparison_method"], minimum_group_size, len(ptls_in_orien_groups))
		log_main.add(msg)
	...
	while total_iter < max_iter:
		...
		if Blockdata["myid"] == Blockdata["main_node"]:
			...
			best_score, changed_nptls, keepgoing, best_assignmen, iter_assignment = \
			     AI_MGSKmeans(iter_assignment, last_iter_assignment, best_assignment, keepgoing, best_score, stopercnt, minimum_group_size, log_main)
	...
	if require_check_setting:
		if(Blockdata["myid"] == Blockdata["main_node"]): print("Too large changed images, and the sorting settings, such as img_per_grp requires a check")
...
def Kmeans_minimum_group_size_relaxing_orien_groups(original_data, partids, params, paramstructure, norm_per_particle, minimum_group_size, clean_volumes = False):
	...
	###=====<------------
	if Tracker["constants"]["mask3D"]: # prepare mask
		mask3D = get_im(Tracker["constants"]["mask3D"])
...
def do_boxes_two_way_comparison_mpi(nbox, input_box_parti1, input_box_parti2, depth, log_main):
	...
	if Blockdata["myid"]==Blockdata["main_node"]:
		...
		log_main.add('{:>5} {:>8}  {:^8}   {:>15} {:>22}  {:>5}'.format('Group', '    size',  ' status ',   'reproducibility', 'random reproducibility', ' std '))
		from math import sqrt
		for index_of_any in xrange(len(list_stable)):
			...
			if( len(any) >= Tracker["constants"]["minimum_grp_size"] ):
				...
				new_list.append(any)
				nclass +=1
				log_main.add('{:>5} {:>8d}   {:^8}      {:>7.1f}      {:>15.1f}           {:>5.1f}'.format(index_of_any, len(any),'accepted', score3, \
					    gave[index_of_any], gvar[index_of_any]))
				stat_list.append([score3,  gave[index_of_any], gvar[index_of_any]])
				tmp_list.append(len(any)*(-1))
			else:
				log_main.add('{:>5} {:>8d}   {:^8}      {:>7.1f}      {:>15.1f}           {:>5.1f}'.format(index_of_any, len(any), 'rejected', score3, \
					   gave[index_of_any], gvar[index_of_any]))
		###
		if len(tmp_list)>1:
			tmp_new_list = []
...
def do_withinbox_two_way_comparison(partition_dir, nbox, nrun, niter):
	...
	for im in xrange(len(ptp2)): msg +='{:8d} '.format(len(ptp2[im]))
	log_list.append(msg)
	if(len(core1) != len(core2)): ERROR("The two partitions have different lengths", "do_withinbox_two_way_comparison", 1, 0)
	full_list  = []
	for a in core1: full_list.append(a[1])
	full_list.sort()
	total_data = len(full_list)
...
def prep_ptp_single(all_lists, full_list):
	...
	c = set(full_list)
	if list(a.difference(c)) !=[]: ERROR("Accounted and unaccounted in total do not match the total number of images", "prep_ptp_single", 1, Blockdata["myid"])
	else:
		pdict = {}
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		...
		for ig, value in Tracker["generation"].items():
			for ic in xrange(value):
				cluster_file = os.path.join(Tracker["constants"]["masterdir"], "generation_%03d"%atoi(ig), "Cluster_%03d.txt"%ic)
				...
				copyfile(os.path.join(Tracker["constants"]["masterdir"], "generation_%03d"%atoi(ig), "vol_grp%03d_iter000.hdf"%ic), os.path.join(Tracker["constants"]["masterdir"], "vol_cluster%03d.hdf"%nclusters))
				cluster = read_text_file(os.path.join(Tracker["constants"]["masterdir"], "generation_%03d"%atoi(ig), "Cluster_%03d.txt"%ic))
				cluster_file = "Cluster_%03d.txt"%nclusters
				vol_file     = "vol_cluster%03d.hdf"%nclusters
				msg = '{:>8} {:>8}   {:^24}        {:^6}          {:^6}          {:>5}  {:^15} {:^20} '.format(nclusters, len(cluster), ig, round(all_gen_stat_list[atoi(ig)][ic][0],1), round(all_gen_stat_list[atoi(ig)][ic][1],1), round(all_gen_stat_list[atoi(ig)][ic][2],1), cluster_file,  vol_file)
				nclusters +=1
				NACC +=len(cluster)
				log_file.add(msg)
		Unaccounted_file = os.path.join(Tracker["constants"]["masterdir"], "generation_%03d"%atoi(ig), "Unaccounted.txt")
