...
"""
	...
	if len(args) < 1 or len(args) > 5:
		...
		if options.searchxshift >0:
			...
			gendisks_MPI(args[0], mask3d, options.ref_nx, options.ref_nx, options.ref_nz, options.apix, options.dp, options.dphi, fract=options.fract, rmax=rmaxp, rmin=rminp, CTF=options.CTF, options.function, sym=options.sym, dskfilename=options.gendisk)
			global_def.BATCH = False
		else:
