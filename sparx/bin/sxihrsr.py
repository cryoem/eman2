...
def main():
	...
	parser.add_option("--dpixel_size",        type="float",  default= -1.0,               help="pixel size in Angstroms")   
	...
	if len(args) < 3 or len(args) > 5:
		...
		if options.searchxshift >0:
			...
			volalixshift_MPI(args[0], args[1], args[2], options.search_rng, options.pixel_size, options.dp, options.dphi, options.fract, options.rmax, options.rmin, mask,options.maxit, options.CTF, options.snr, options.sym,  options.function, options.fourvar, options.npad, options.debug)
