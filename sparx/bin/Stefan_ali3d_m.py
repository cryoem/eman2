...
def main():
	...
	usage = progname + " stack ref_vols outdir <mask> --ir=inner_radius --ou=outer_radius --rs=ring_step --xr=x_range --yr=y_range  --ts=translational_searching_step  --delta=angular_step --center=1  --maxit=max_iter --CTF --snr=1.0 --ref_a=S --sym=c1 "
	...
	if len(args) < 3 or len(args) > 4:
		...
		from stefan import ali3d_m_MPI
		global_def.BATCH = True
		ali3d_m_MPI(args[0], args[1], args[2], mask, options.ir, options.ou, options.rs, options.xr, options.yr, options.ts, options.delta, options.an, options.center, options.maxit, options.CTF, options.snr, options.ref_a, options.sym)
