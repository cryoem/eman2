...
from applications import ali2d_w
from optparse import OptionParser
import sys
def main():
	progname = os.path.basename(sys.argv[0])
	usage = progname + " stack outdir --ir=inner_radius --ou=outer_radius --rs=ring_step --xr=x_range --yr=y_range  --ts=translation_step --c=center --maxit=max_iter"
	...
	if len(args) != 2:
		...
		ali2d_w(args[0], args[1], options.ir, options.ou, options.rs, options.xr, options.yr, options.ts, options.c, options.maxit)				
