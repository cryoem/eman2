...
###	usage = progname + """  input_micrograph_list_file  input_micrograph_pattern  input_coordinates_pattern  output_directory  --coordinates_format  --box_size=box_size  --skip_invert  --import_ctf=ctf_file  --limit_ctf  --resample_ratio=resample_ratio  --defocus_error=defocus_error  --astigmatism_error=astigmatism_error
	usage = progname + """  input_micrograph_list_file  input_micrograph_pattern  input_coordinates_pattern  output_directory  --coordinates_format  --box_size=box_size  --skip_invert  --import_ctf=ctf_file  --limit_ctf  --astigmatism_error=astigmatism_error  --resample_ratio=resample_ratio
...
###			# NOTE: 2016/10/20 Toshio Moriya
###			# Code before removing options.defocus_error
###			#
###			ctf_dict={}
...
###			# NOTE: 2016/10/20 Toshio Moriya
###			# Code after removing options.defocus_error
###			#
			ctf_dict={}
			for ctf_params in ctf_list:
				assert(len(ctf_params) == n_idx_cter)
				# mic_baseroot is name of micrograph minus the path and extension
				mic_baseroot = os.path.splitext(os.path.basename(ctf_params[idx_cter_mic_name]))[0]
				if(ctf_params[idx_cter_sd_astig_ang] > options.astigmatism_error):
					print("The astigmatism angular SD of %s (%f degree) exceeds the specified limit (%f degree). Resetting astigmatism parameters to zeros..." % (mic_baseroot, ctf_params[idx_cter_sd_astig_ang], options.astigmatism_error))
					ctf_params[idx_cter_astig_amp] = 0.0
					ctf_params[idx_cter_astig_ang] = 0.0
				ctf_dict[mic_baseroot] = ctf_params
