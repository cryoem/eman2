...
def main():
	...
	usage = progname + " stack outdir <maskfile> --K=number_of_classes --trials=num_trials --opt_method=optimization_method --maxit=max_iter --CTF --rand_seed=1000 --crit=criterion_name --F=factor_temperature --T0=init_temperature --MPI --SA2"
	...
	(options, args) = parser.parse_args()
		...
		k_means_main(args[0], args[1], mask, options.opt_method, options.K, options.rand_seed, options.maxit, options.trials, options.crit, options.CTF, options.F, options.T0, options.MPI, options.SA2, options.debug)
