		...
		if((Tracker["no_improvement"]>=Tracker["constants"]["limit_improvement"]) and (Tracker["no_params_changes"]>=Tracker["constants"]["limit_changes"]) and (not Tracker["large_at_Nyquist"])):
			if( Tracker["delta"] < 0.75*Tracker["acc_rot"] ):#<<<----it might cause converge issues when shake is 0.0
				...
				if( Tracker["delta"] <= old_div(3.75,2.0) ):  #  MOVE DOWN TO RESTRICTED
					...
					if( Tracker["delta"] <= degrees(atan(old_div(0.25,Tracker["constants"]["radius"]))) ): Tracker["state"] = "FINAL"
...
def params_changes( params, oldparams ):
	...
	return round(old_div(anger,n),5), round(sqrt(shifter/2/n),5)
...
def compute_search_params(acc_trans, shifter, old_range):
	...
	if range > 8.0*step :   step   = old_div(range,8.0) # change 4 to 8
...
def compute_sigma(projdata, params, first_procid, dryrun = False, myid = -1, mpi_comm = -1):
	...
	if  dryrun:
		...
		if( myid == Blockdata["main_node"]):
			Util.mul_scalar(tavg, old_div(1.0,float(sum(Tracker["nima_per_chunk"]))))
			...
			for i in range(ngroups):
				for k in range(1,nv):
					...
					if( qt > 0.0 ):	tmp1[k] = old_div(2.0,qt)
				#  smooth
				tmp1[0] = tmp1[1]
				tmp1[-1] = tmp1[-2]
				for ism in range(0):  #2
					for k in range(1,nv-1):  tmp2[k] = old_div((tmp1[k-1]+tmp1[k]+tmp1[k+1]),3.0)
...
def get_shrink_data(nxinit, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		#  Apply varadj
		if not nonorm:
			Util.mul_scalar(data[im], old_div(Tracker["avgvaradj"][procid],wnorm))
...
def get_refangs_and_shifts():
	...
	k = int(ceil(old_div(Tracker["xr"],Tracker["ts"])))
	...
	k = int(ceil(old_div(Tracker["xr"],ts_coarse)))
...
def Xali3D_direct_ccc(data, refang, shifts, ctfs = None, bckgnoise = None, kb3D = None):
	...
	npsi = int(old_div(360.,Tracker["delta"]))
	nang = len(refang)
	ndat = len(data)
	...
	for i in range(nang):
		if( ( Blockdata["myid"] == Blockdata["main_node"])  and  (i%(max(1,old_div(nang,5))) == 0) and (i>0)):
			print( "  Angle :%7d   %5d  %5.1f"%(i,ndat,float(i)/float(nang)*100.) + "%" +"   %10.1fmin"%(old_div((time()-at),60.)))
			...
			for itemp in range(i, min(i+Blockdata["no_of_processes_per_group"], nang)):
				if( itemp-i == Blockdata["myid_on_node"]):
					for j in range(npsi):
						...
						Util.mul_scalar(temp, old_div(1.0,nrmref))
...
def XXali3D_direct_ccc(data, refang, shifts, coarse_angles, coarse_shifts, ctfs = None, bckgnoise = None, kb3D = None):
	...
	n_coarse_psi = int(old_div(360.,coarse_delta))
	...
	lenbigbuf = nang*npsi
	orgsize = lenbigbuf*size_of_one_image #  This is number of projections to be computed simultaneously times their size
	if( Blockdata["myid"] == 0 ):  print("  BIGBUFFER  ",old_div(float(orgsize),1.e9),"GB")
	if( Blockdata["myid_on_node"] == 0 ): size = orgsize
	else:  size = 0
	...
	for i in range(n_coarse_ang):
		if( ( Blockdata["myid"] == Blockdata["main_node"])  and  (i%(max(1,old_div(n_coarse_ang,5))) == 0) and (i>0)):
			print( "  Angle :%7d   %5d  %5.1f"%(i,ndat,float(i)/float(n_coarse_ang)*100.) + "%" +"   %10.1fmin"%(old_div((time()-at),60.)))
	...
	#print  " >>>  %4d   %12.3e       %12.5f     %12.5f     %12.5f     %12.5f     %12.5f"%(best,simis[0],newpar[0][0],newpar[0][1],newpar[0][2],newpar[0][3],newpar[0][4])
	###if Blockdata["myid"] == Blockdata["main_node"]:  print "  Finished :",time()-at
	if Blockdata["myid"] == Blockdata["main_node"]:   print("  COARSE SEARCHES DONE  ","   %10.1fmin"%(old_div((time()-at),60.)))
	at = time()
	...
	mpi_barrier(Blockdata["shared_comm"])
	if( Blockdata["myid"] == Blockdata["main_node"] ):   print("  REFPROJ DONE  ","   %10.1fmin"%(old_div((time()-at),60.)))
	at = time()
	...
	for kl,emimage in enumerate(data):
		...
		for i2 in range(4):
			...
			for i3 in range(2):  # psi
				itpsi = int(old_div((coarse_angles[oldiang][2] + ipsi*coarse_delta - refang[iang][2]+360.0),Tracker["delta"]))
				itpsi = (itpsi + i3)%npsi
	...
	mpi_barrier(Blockdata["shared_comm"])
	if Blockdata["myid"] == Blockdata["main_node"]:   print("  FINE SEARCH DONE  ","   %10.1fmin"%(old_div((time()-at),60.)))
...
def ali3D_polar_ccc(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	mpi_barrier(Blockdata["shared_comm"])
	if(Blockdata["myid"] == Blockdata["main_node"]):
		print( "  Reference projections generated : %10.1fmin"%(old_div((time()-at),60.)))
	...
	for im in range(nima):
		...
		if Tracker["constants"]["CTF"] :
			...
			ctf_params.apix = old_div(ctf_params.apix,(old_div(float(Tracker["nxinit"]),float(Tracker["constants"]["nnxo"]))))
		...
		if( ( Blockdata["myid"] == Blockdata["main_node"])  and  (im%(max(1,old_div(nima,5))) == 0) and (im>0)):
			print( "  Number of images :%7d   %5d  %5.1f"%(im,nima,float(im)/float(nima)*100.) + "%" +"   %10.1fmin"%(old_div((time()-at),60.)))

		if( im < min(nima, 1) and procid == 0):
			#   Search all and compare with direct to figure what keepfirst might be
			keepfirst = old_div((n_coarse_ang *  n_coarse_psi),10)#keepfirst = (n_coarse_ang *  n_coarse_psi * n_coarse_shifts)/10
			...
			for iln in range(keepfirst):
				...
				xod1[iln] = old_div(Util.innerproduct(data[ishift], temp, None),nrmref)  # peak
				##xod2[iln] = hashparams
			...
			for iln in range(Tracker["keepfirst"]):
				...
				if(ipsiandiang != pre_ipsiandiang):
					...
					temp.set_attr("is_complex",0)
					nrmref = sqrt(Util.innerproduct(temp, temp, None))
				peak = old_div(Util.innerproduct(data[ishift], temp, None),nrmref)
				xod1[iln] = peak
				##xod2[iln] = hashparams
		...
		while(iln<lit):
			...
			while( ipsiandiang == old_div(cod2[iln],1000) ):
				...
				cod1[iln] = old_div(Util.innerproduct(data[ishift], temp, None),nrmref)
				iln += 1
				if(iln == lit  ):  break
	...
	mpi_barrier(MPI_COMM_WORLD)
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		print( "  Finished projection matching   %10.1fmin"%(old_div((time()-at),60.)))
	#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
def ali3D_primary_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		#  Apply varadj
		if not nonorm:
			Util.mul_scalar(dataimage, old_div(Tracker["avgvaradj"][procid],wnorm))
	...
	Tracker["avgvaradj"][procid] = mpi_reduce( Tracker["avgvaradj"][procid], 1, MPI_FLOAT, MPI_SUM, Blockdata["main_node"], MPI_COMM_WORLD )
	if(Blockdata["myid"] == Blockdata["main_node"]):
		Tracker["avgvaradj"][procid] = old_div(float(Tracker["avgvaradj"][procid]),Tracker["nima_per_chunk"][procid])
	else:  Tracker["avgvaradj"][procid] = 0.0
	Tracker["avgvaradj"][procid] = bcast_number_to_all(Tracker["avgvaradj"][procid], Blockdata["main_node"])
	mpi_barrier(MPI_COMM_WORLD)
	...
	if( Blockdata["myid"] == 0 ):
		...
		svar = sqrt(max(0.0,old_div((float(svar) - snum*sava**2),(snum -1))))
		line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		print(line, "Smear stat  (number of images, ave, sumsq, min, max)):  %7d    %12.3g   %12.3g  %7d  %7d"%(snum,sava,svar,smin,smax))
	...
	mpi_barrier(MPI_COMM_WORLD)
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		print( "  Finished sigma2   %10.1fmin"%(old_div((time()-at),60.)))
...
def ali3D_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		#write_text_row([[newpar[0][2][j][0],newpar[0][2][j][1]] for j in xrange(len(newpar[0][2]))],os.path.join(Tracker["directory"], "polar%1d.txt"%procid))
		print( "  Statistics finished : %10.1fmin"%(old_div((time()-at),60.)))
	return newpar, norm_per_particle
...
def ali3D_primary_local_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	numberofrefs_inmem = int(Tracker["constants"]["memory_per_node"]/4/(old_div((size_of_one_image*disp_unit),1.0e9)))
	...
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		print( "  " )
		line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
		print(  line, "Processing data for polar onx: %3d, nx: %3d, nxpolar: %3d, CTF: %s, preshift: %s,  MEM: %6.2fGB."%(Tracker["constants"]["nnxo"], Tracker["nxinit"], Tracker["nxpolar"], Tracker["constants"]["CTF"], preshift,old_div(orgsize,1.0e9)) )
	...
	for icone in range(max_number_of_cones):
		...
		if( icone < number_of_cones ):  #  This is executed for individual number of cones, some nodes may have fewer.
			...
			for icnm in range(max(1,lenass)):  # I have to enter the loop even it there is no assignment
				...
				if( ( Blockdata["myid"] == Blockdata["main_node"])  and  (lima%(max(1,old_div(nima,5))) == 0) and (lima>0)):
					print( "  Number of images :%7d   %5d  %5.1f"%(lima,nima,float(lima)/float(nima)*100.) + "%" +"   %10.1fmin"%(old_div((time()-at),60.)))
				...
				if( lima == 1 and procid == 0):
					...
					if( lenass > 0):
						...
						for iln in range(keepfirst):
							...
							##junk = time()
							#xod1[iln] = -Util.sqed(data[ishift], temp, ctfa, bckgnoise)
							xod1[iln] = old_div(Util.innerproduct(data[ishift], temp, None),nrmref)
							##eat += time()-junk
							##xod2[iln] = hashparams
...
def do_final_rec3d(partids, partstack, original_data, oldparams, oldparamstructure, projdata, final_iter=-1, comm = -1 ):
	...
	if( Blockdata["subgroup_myid"] > -1 ):
		...
		if carryon == 0: ERROR("Failed to read refang and rshifts: %s %s "%(os.path.join(final_dir, "refang.txt"), os.path.join(final_dir, "rshifts.txt")), "do_final_rec3d", 1, data["subgroup_myid"])
		refang  = wrap_mpi_bcast(refang, Blockdata["main_node"], comm)
		rshifts = wrap_mpi_bcast(rshifts, Blockdata["main_node"], comm)

		partids =[None, None]
		if(Blockdata["subgroup_myid"] == Blockdata["main_node"]):
			if not os.path.exists(os.path.join(Tracker["constants"]["masterdir"], "tempdir")):os.mkdir(os.path.join(Tracker["constants"]["masterdir"], "tempdir"))
			l = 0
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		shutil.copyfile(os.path.join(Tracker["constants"]["masterdir"], "main%03d"%Tracker["mainiteration"], "params_%03d.txt"%Tracker["mainiteration"]), os.path.join(Tracker["constants"]["masterdir"], "final_params_%03d.txt"%Tracker["mainiteration"]))
...
def recons3d_final(masterdir, do_final_iter_init, memory_per_node, orgstack = None):
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		volume_size = old_div((1.5*4*(2.0*Tracker["constants"]["nnxo"]+3.0)**3),1.e9)
		#  Estimated data size
		data_size = max(Tracker["nima_per_chunk"])*4*float(Tracker["constants"]["nnxo"]**2)/float(Blockdata["no_of_groups"])/1.0e9
		nnprocs = min( Blockdata["no_of_processes_per_group"], int((old_div((memory_per_node - data_size*1.2), volume_size) ) ) )
...
###<<<<-----ctref_rec3d
def do3d_nosmearing(procid, data, params, mpi_comm = -1):
	global Tracker, Blockdata
	#  Without filtration
	from reconstruction import recons3d_trl_struct_MPI
	if( mpi_comm < -1 ): mpi_comm = MPI_COMM_WORLD
	...
	shrinkage = old_div(float(Tracker["nxinit"]),float(Tracker["constants"]["nnxo"]))
	tvol, tweight, trol = recons3d_trl_struct_MPI_nosmearing(myid = Blockdata["subgroup_myid"], main_node = Blockdata["nodes"][procid], prjlist = data, \
...
def rec3d_continuation_nosmearing(original_data, mpi_comm):
	...
	#	print_dict(Tracker, "Current state of variables")
	...
	continuation_do_maps_mpi(Tracker["directory"])
	if(Blockdata["myid"] == Blockdata["nodes"][0]):  shutil.rmtree(os.path.join(Tracker["directory"], "tempdir"))
	Tracker["directory"] = temp
	return

def rec3d_tmp(mainiteration, original_data):
	global Tracker, Blockdata
	while mainiteration<= 2:
		Tracker["mainiteration"]     = mainiteration
		Tracker["directory"]         = os.path.join(Tracker["constants"]["masterdir"], "main%03d"%Tracker["mainiteration"])
		Tracker["previousoutputdir"] = os.path.join(Tracker["constants"]["masterdir"], "main%03d"%(Tracker["mainiteration"]-1))
		oldparams        = [None, None]
		partids          = [None, None]
		partstack        = [None, None]
		Tracker["keepfirst"] = -1	
		...
		for procid in range(2):
			if(Blockdata["myid"] == Blockdata["main_node"]):
				shutil.copyfile(os.path.join(Tracker["previousoutputdir"], "particle_groups_%d.txt"%procid), os.path.join(Tracker["directory"], "particle_groups_%d.txt"%procid))
				shutil.copyfile(os.path.join(Tracker["previousoutputdir"], "chunk_%d_%03d.txt"%(procid, mainiteration-1)), os.path.join(Tracker["directory"], "chunk_%d_%03d.txt"%(procid, mainiteration)))
				shutil.copyfile(os.path.join(Tracker["previousoutputdir"], "groupids.txt"), os.path.join(Tracker["directory"], "groupids.txt"))
		...
		compute_sigma(original_data[0]+original_data[1],oldparams[0]+oldparams[1], len(oldparams[0]), False, Blockdata["myid"], mpi_comm = MPI_COMM_WORLD)
		Tracker["directory"] = temp
		newparams            = [None, None]
		...
		for procid in range(2):
			...
			del params
			new_oldparamstructure_dir = os.path.join(Tracker["directory"], "oldparamstructure")
			if procid ==0 and Blockdata["myid"]== Blockdata["main_node"]:
				if not os.path.exists(new_oldparamstructure_dir): os.mkdir(new_oldparamstructure_dir)
			projdata[procid] = get_shrink_data(Tracker["nxinit"], procid, original_data[procid], oldparams[procid],\
			  return_real = False, preshift = True, apply_mask = False,  nonorm = True, nosmearing = False)
			mpi_barrier(MPI_COMM_WORLD)
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			if(Blockdata["subgroup_myid"] == Blockdata["nodes"][procid]): print(line, "3-D reconstruction of group %d"%procid)
			#original_data[procid] = None
		
			do3d(procid, projdata[procid], newparamstructure[procid], refang, rshifts, norm_per_particle[procid], Blockdata["myid"], MPI_COMM_WORLD)
			projdata[procid]  = []
			if( Blockdata["myid_on_node"] == 0 ):
				for kproc in range(Blockdata["no_of_processes_per_group"]):
					if( kproc == 0 ):
						...
						fout.close()
						del dummy
			else: wrap_mpi_send(newparamstructure[procid], 0, Blockdata["shared_comm"])
		...
		if(Blockdata["myid"] == Blockdata["main_node"]):
			...
			write_text_row(params, os.path.join(Tracker["directory"],"params_%03d.txt"%(Tracker["mainiteration"])))
			Tracker["anger"], Tracker["shifter"]= params_changes(params, read_text_row(os.path.join(Tracker["previousoutputdir"],"params_%03d.txt"%(Tracker["mainiteration"]-1))))
			write_text_row( [ [Tracker["anger"], Tracker["shifter"]]], os.path.join(Tracker["directory"] ,"error_thresholds_%03d.txt"%(Tracker["mainiteration"])))
		else: Tracker = 0
		Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], MPI_COMM_WORLD)
		
		if(Blockdata["myid"] == Blockdata["main_node"]):
			cfsc = read_text_file(os.path.join(Tracker["directory"], "driver_%03d.txt"%Tracker["mainiteration"]))
			nfsc_143  = 1
			nfsc_half = 1
			for ifreq in range(1, len(cfsc)):
				if cfsc[ifreq]<0.143:
					nfsc_143 = ifreq + 1
					break
				elif cfsc[ifreq]==0.143:
					nfsc_143 = ifreq
					break
			for ifreq in range(1, len(cfsc)):
				if cfsc[ifreq]<0.5:
...
def continuation_recompute_groups(masterdir, option_orgstack, option_old_refinement_dir, option_subset, option_initvol, option_selected_iter, shell_line_command, mpi_comm=-1):
	global Tracker, Blockdata
	if mpi_comm == -1: mpi_comm = MPI_COMM_WORLD
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if Blockdata["myid"] == Blockdata["nodes"][0]:
		print(line, "continuation_recompute_groups")
	...
	if option_subset =='':   option_subset = None
	
	if not from_orgstack: # import state variables from old refinement
		if Blockdata["myid"] == Blockdata["main_node"]:
			if option_selected_iter ==-1:
				selected_iter = 3
				while os.path.exists(os.path.join(option_old_refinement_dir, "main%03d"%selected_iter)):selected_iter +=1
				selected_iter -=1
				...
				Tracker = convert_json_fromunicode(json.load(fout))
				fout.close()
				if Tracker["constants"]["best"] !=selected_iter:
					fout     = open(os.path.join(old_refinement_iter_dir, "Tracker_%03d.json"%Tracker["constants"]["best"]),"r")
					Tracker  = convert_json_fromunicode(json.load(fout))
					fout.close()
			else:
			...
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			print(line, "continuation: continuation starts from %d of %s"%(selected_iter, option_old_refinement_dir))
		else: Tracker = 0
		Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], mpi_comm) # balance processors
	else:
		if(Blockdata["myid"] == Blockdata["main_node"]):
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			print(line,  "continuation: continuation starts from  %s "%option_orgstack)
	...
	for iproc in range(Blockdata["nproc"]):
		if(Blockdata["myid"] == iproc):
			while not os.path.exists(Tracker["directory"]):  sleep(5)
	mpi_barrier(MPI_COMM_WORLD)
	
	partids = [None]*2
	for procid in range(2): partids[procid] = os.path.join(Tracker["directory"],"chunk_%01d_000.txt"%procid)
	partstack = [None]*2
	for procid in range(2): partstack[procid] = os.path.join(Tracker["directory"],"params-chunk_%01d_000.txt"%procid)
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		if option_orgstack:
			tmp_params_list = EMUtil.get_all_attributes(Tracker["constants"]["stack"],"xform.projection")
			params_list =[None]*len(tmp_params_list)
			for iptl in range(len(params_list)):
				dp = tmp_params_list[iptl].get_params("spider")
				params_list[iptl] = [dp["phi"], dp["theta"], dp["psi"], -dp["tx"], -dp["ty"], 0.0, 0.0, 1.0]
			del tmp_params_list
		else:
			params_list = read_text_row(os.path.join(option_old_refinement_dir, "main%03d"%selected_iter, "params_%03d.txt"%selected_iter))
			for iptl in range(len(params_list)):
				params_list[iptl][5] = 0.0
				params_list[iptl][6] = 0.0
				params_list[iptl][7] = 1.0
		l =len(params_list)
		partids[0]  = l1
		partids[1]  = l2
		write_text_row(params_list, os.path.join(Tracker["directory"], "params_000.txt"))
		for procid in range(2):
			tmp_params = []
			for iptl in range(len(partids[procid])): tmp_params.append(params_list[partids[procid][iptl]])
			write_text_row(tmp_params, partstack[procid])
			partids[procid] = os.path.join(Tracker["directory"],"chunk_%01d_000.txt"%procid)
	...
	if(Blockdata["myid"] == Blockdata["nodes"][0]):
		...
		cfsc = read_text_file(os.path.join(Tracker["directory"], "driver_000.txt"))
		nfsc_143  = 1
		nfsc_half = 1
		for ifreq in range(1, len(cfsc)):
			if cfsc[ifreq]<0.143:
	...
	if option_initvol:# continue refinement using the given reference
		if(Blockdata["myid"] == Blockdata["nodes"][0]):
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			print(line, "continuation: continuation uses user provided %s as the intitial references"%option_initvol)
			shutil.copyfile(option_initvol, os.path.join(Tracker["directory"], "vol_0_000.hdf"))
			shutil.copyfile(option_initvol, os.path.join(Tracker["directory"], "vol_1_000.hdf"))
		mpi_barrier(MPI_COMM_WORLD)
...
def continuation_read_subset_data(masterdir, option_old_refinement_dir, option_selected_cluster, option_selected_iter, shell_line_command):
	global Tracker, Blockdata
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if Blockdata["myid"] == Blockdata["nodes"][0]:
		print(line, "continuation_keep_groups")
	#case 1
	selected_iter = option_selected_iter	
	old_refinement_iter_dir    = os.path.join(option_old_refinement_dir, "main%03d"%selected_iter)
	...
	if nproc_old_ref3d < Blockdata["nproc"]:
		ERROR( "ERROR!! continuation uses more Cpus than the initial run", "continuation_read_subset_data", 1, Blockdata["myid"])
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		if option_selected_cluster !='': cluster = sorted(read_text_file(option_selected_cluster))
		else: cluster= list(range(len(chunk_one)+len(chunk_two)))
	else: cluster = 0
	...
	except:ERROR( "ERROR!! symmetry info is absent", "continuation_read_subset_data",1, Blockdata["myid"])
	...
	if proc_start<proc_end:
		for myproc in range(proc_start, proc_end):
			for ichunk in range(2):
				oldparams = []
...
def continuation_do_maps_mpi(continuation_from_sort3d_iter_dir):
	global Tracker, Blockdata
	
	Tracker["directory"] = continuation_from_sort3d_iter_dir
	Tracker["maxfrad"]   = Tracker["nxinit"]//2
	# steptwo of final reconstruction
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if Blockdata["myid"] == Blockdata["main_node"]: print(line, "do3d_continuation_get_maps_mpi")
	if(Blockdata["no_of_groups"] >1):
...
def continuation_rec3d_faked_iter(masterdir, selected_iter=-1, comm = -1):
	global Tracker, Blockdata
	...
	Blockdata["accumulatepw"]       = [[],[]]
	if selected_iter ==-1: ERROR("Iteration number has to be determined in advance.","ctref_rec3d_faked_iter",1, Blockdata["myid"])  
	carryon = 1
	line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
	if(Blockdata["myid"] == Blockdata["main_node"]):
	...
	if carryon == 0: 
		...
		"rshifts.txt")), "ctref_rec3d_faked_iter", 1, Blockdata["myid"])
	...
	for procid in range(2):
		...
		nproc_previous = bcast_number_to_all(nproc_previous, source_node = Blockdata["main_node"], mpi_comm = comm)
		if Blockdata["myid"] == Blockdata["main_node"]:
			for iproc in range(nproc_previous):
				...
				fout.close()
		else: oldparamstructure[procid] = [0]
...
def update_memory_estimation():
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		...
		volume_size  = old_div((1.5*4*(2.0*image_size+3.0)**3),1.e9)
		#nnprocs = Blockdata["no_of_processes_per_group"]
		nnprocs = min(Blockdata["no_of_processes_per_group"], int((old_div((Tracker["constants"]["memory_per_node"] - data_size*1.2),volume_size))))
...
def update_tracker(shell_line_command):
	...
	if options_no_default_value.ccfpercentage != None:
		Tracker["constants"]["ccfpercentage"] 				= old_div(options_no_default_value.ccfpercentage,100.0)
...
5.  mpirun -np 64 --hostfile four_nodes.txt  sxmeridien.py vton3 --do_final=21
	...
	if( (not do_final_mode) and (not do_continuation_mode) ):
		...
		if not restart_mode: #<<<-------Fresh run
			...
			Constants["ccfpercentage"]     			= old_div(options.ccfpercentage,100.)
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				if( Tracker["constants"]["mask3D"] and (not os.path.exists(Tracker["constants"]["mask3D"]))): checking_flag = 0
			...
			if( old_div(options.xr,options.ts)<1.0 ): ERROR("Incorrect translational searching settings, search range cannot be smaller than translation step ","meridien",1,Blockdata["myid"])
		...
		if not restart_mode: #<<<-------Fresh run
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				fout = open(os.path.join(initdir,"Tracker_%03d.json"%Tracker["mainiteration"]),'w')
				json.dump(Tracker, fout)
				fout.close()
		...
		if( options.do_final < 0):  ERROR("Incorrect iteration number in do_final  %d"%options.do_final,"meridien",1,Blockdata["myid"])
