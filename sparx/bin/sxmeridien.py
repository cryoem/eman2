	...
	#    Now the program has to start making decisions.
		...
		if move_up_phase:
			...
			#  Exhaustive searches
				...
				Tracker["nxinit"]      = min(Tracker["newnx"] + Tracker["nxstep"], Tracker["constants"]["nnxo"])
				Tracker["icurrentres"] = Tracker["ireachedres"]
				Tracker["nsoft"]       = 0
				Tracker["local"]       = False
				Tracker["zoom"]        = True
	...
	while(keepgoing):
		...
		if doit:
			...
			if( myid == main_node ):
				...
				if( Tracker["nxinit"] == Tracker["constants"]["nnxo"] ):
					if(Tracker["constants"]["mask3D"] is None):
						mask = cosinemask(model_blank(Tracker["constants"]["nnxo"],Tracker["constants"]["nnxo"],Tracker["constants"]["nnxo"],1.0),int(Tracker["constants"]["radius"]*float(Tracker["nxinit"])/float(Tracker["constants"]["nnxo"])+0.5)-3,5)
					else:
						mask = get_im(Tracker["constants"]["mask3D"])
