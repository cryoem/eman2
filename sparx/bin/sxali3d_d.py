...
def main():
	...
	parser.add_option("--center",   type="float",  default= -1,                 help="  -1. average center method 0. if you do not want the volume to be centered, 1. center the volume using cog (default=1)")
	...
	if len(args) < 3 or len(args) > 4:
		...
		ali3d_d(args[0], args[1], args[2], mask, options.ir, options.ou, options.rs, options.xr, options.yr, options.ts, options.delta, options.an, options.center, options.maxit, options.CTF, options.snr, options.ref_a, options.sym, options.function, options.debug, options.MPI)
