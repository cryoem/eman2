...
class SXcmd:
	...
	def construct_token_list_from_wiki(self, wiki_file_path):
				...
				if line_wiki[0] == "=": # Assuming the section always starts with "="
					...
					if current_section == section_name:
						...
						if line_wiki[0:len("sx")] == "sx":
							...
							if usage_token_list[0] != self.name + ".py": ERROR("Wiki Format Error: First token should be script name with .py (sx*.py)", "%s in %s" % (__name__, os.path.basename(__file__)))
						...
						if line_wiki[0] == "*" and line_wiki.find("optional"):
							...
							if key_base == "MPI":
								...
								if self.mpi_support == False or self.mpi_add_flag == False: ERROR("Logical Error: Since MPI flag is found, the command should support MPI.", "%s in %s" % (__name__, os.path.basename(__file__)))
								continue
							...
							if key_base not in self.token_dict.keys(): ERROR("Wiki Format Error: Key base (%s) is missing from 'usage in command line' in '= Usage ='." % key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	def insert_to_file(self, output_file, sxcmd_variable_name):
		output_file.write("\t")
		output_file.write("%s = SXcmd()" % sxcmd_variable_name)
		output_file.write("; %s.name = \"%s\"" % (sxcmd_variable_name, self.name)) 
		output_file.write("; %s.short_info = \"%s\"" % (sxcmd_variable_name, self.short_info.replace("\"", "'")))
		output_file.write("; %s.mpi_support = %s" % (sxcmd_variable_name, self.mpi_support))
		output_file.write("; %s.mpi_add_flag = %s" % (sxcmd_variable_name, self.mpi_add_flag))
		output_file.write("\n")
		...
		for line in sxgui_template_file:
			...
			if current_state == state_template:
				if line.find("# >>>>> START_INSERTION >>>>>") != -1:
					...
					# Insert information about sx coommand (sx*.py)
					output_file.write("")
					output_file.write("# ========================================================================================\n")
					output_file.write("class SXcmd_token:\n")
					output_file.write("\tdef __init__(self):\n")
					output_file.write("\t\t# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><\n")
					output_file.write("\t\t# class variables\n")
					output_file.write("\t\tself.key_base = \"\"          # key base name of command token (argument or option) in command line\n")
					output_file.write("\t\tself.key_prefix = \"\"        # key prefix of of command token. None for argument, '--' or '-' for option\n")
					output_file.write("\t\tself.label = \"\"             # User friendly name of argument or option\n")
					output_file.write("\t\tself.help = \"\"              # Help info\n")
					output_file.write("\t\tself.group = \"\"             # Tab group: main or advanced\n")
					output_file.write("\t\tself.is_required = False    # Required argument or options. No default value are available\n")
					output_file.write("\t\tself.default = \"\"           # Default value\n")
					output_file.write("\t\tself.type = \"\"              # Type of value\n")
					output_file.write("\t\tself.is_in_io = False       # <Used only here> To check consistency between 'usage in command line' and list in '== Input ==' and '== Output ==' sections\n")
					output_file.write("\t\tself.widget = None          # <Used only in sxgui.py> Widget instances Associating with this command token\n")
					output_file.write("\t\t# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><")
					output_file.write("\n")
					output_file.write("# ========================================================================================\n")
					output_file.write("class SXcmd:\n")
					output_file.write("\tdef __init__(self):\n")
					output_file.write("\t\t# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><\n")
					output_file.write("\t\t# class variables\n")
					output_file.write("\t\tself.name = \"\"              # Name of this command (i.e. name of sx*.py script but without .py extension)\n")
					output_file.write("\t\tself.short_info = \"\"        # Short description of this command\n")
					output_file.write("\t\tself.mpi_support = False    # Flag to indicate if this command suppors MPI version\n")
					output_file.write("\t\tself.mpi_support = False    # Flag to indicate if this command suppors MPI version\n")
					output_file.write("\t\tself.mpi_add_flag = False   # NOTE: 2015/11/12 Toshio Moriya. This can be removed when --MPI flag is removed from all sx*.py scripts\n")
					output_file.write("\t\tself.token_list = []        # list of command tokens. Need this to keep the order of command tokens\n")
					output_file.write("\t\tself.token_dict = {}        # dictionary of command tokens, organised by key base name of command token. Easy to access a command token but looses their order\n")
					output_file.write("\t\t# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><\n")
					output_file.write("\n")
		
					output_file.write("# ========================================================================================\n")
					output_file.write("def construct_sxcmd_list():\n")
					output_file.write("\tsxcmd_list = []\n")
					output_file.write("\n")
					...
					for wiki_file_path in wiki_file_path_list:
						...
						sxcmd.insert_to_file(output_file, sxcmd_variable_name)
						output_file.write("\n")
						output_file.write("\tsxcmd_list.append(%s)\n" % sxcmd_variable_name)
						output_file.write("\n")
			
					output_file.write("\t# Create command token dictionary for each SXcmd instance\n")
					output_file.write("\tfor sxcmd in sxcmd_list:\n")
					output_file.write("\t\tfor token in sxcmd.token_list:\n")
					output_file.write("\t\t\t\tsxcmd.token_dict[token.key_base] = token\n")
					output_file.write("\n")
					output_file.write("\treturn sxcmd_list\n")				# else: do nothing
				# else: do nothing
			else:
