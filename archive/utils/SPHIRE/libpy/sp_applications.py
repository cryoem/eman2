    ...
    for N_step in range(len(xrng)):
        ...
        if CUDA:
            R.setup(
                ...
                last_ring,
                step[N_step],
                int(xrng[N_step] / step[N_step] + 0.5),
                int(yrng[N_step] / step[N_step] + 0.5),
                CTF,
            )
            for im in range(len(data)):
        ...
        for Iter in range(max_iter):
            ...
            if total_iter != max_iter * len(xrng):
                ...
                if myid == main_node:
                    log.add(
                        "Mirror consistency rate = %8.4f%%"
                        % (float(mirror_consistent) / total_nima * 100)
                    )
                    if mirror_consistent != 0:
                        log.add(
                            "Among the mirror-consistent images, average of pixel errors is %0.4f, and their distribution is:"
            ...
            for j in range(numref):
                ...
                if refi[j][2] < 4:
                    ...
                    for INter in range(max_inter + 1):
                        ...
                        if CTF:
                            for i in range(lctf):
                                ctm[i] = 1.0 / (ctf2[j][0][i] + 1.0 / snr)
                            av1 = sp_filter.filt_table(refi[j][0], ctm)
                            for i in range(lctf):
                                ctm[i] = 1.0 / (ctf2[j][1][i] + 1.0 / snr)
            ...
            for j in range(numref):
                if refi[j][2] < 4:
                    ...
                    if CTF:
                        ...
                        # Now the total average
                        for i in range(lctf):
                            ctm[i] = 1.0 / (ctf2[j][0][i] + ctf2[j][1][i] + 1.0 / snr)
                        refi[j][0] = sp_filter.filt_table(
                            EMAN2_cppwrap.Util.addn_img(refi[j][0], refi[j][1]), ctm
                        )
                    else:
...
def within_group_refinement(
    ...
    if method == "SHC":
        ...
        rings = sp_alignment.prepref(
            data,
            sp_utilities.model_circle(nx // 2 - 1, nx, nx),
            cnx,
            cnx,
            numr,
            mode,
    ...
    if mask is None:
        if radius != None:
            ...
            mask = sp_utilities.model_circle(
                float(min(nx, ny, nz) // 2 - 2), nx, ny, nz
            )
