...
# James Michael Bell, 2014 (jmbell@bcm.edu)
...
def get_usage():
	...
	e2tvrecon.py reconstructs an image from its tomographic projections with an incomplete set of projections. For a correct reconstruction without a-priori information, one would usually require l or more angles). In addition, noise may be added to the projections. In order to reconstruct the original image, we minimize a function that is the sum of a L2 data fit term and the total variation of the image. Proximal iterations using the FISTA scheme are used.
...
def filter_projections(proj_set, reg=False):
	...
	labels: ndarray of type np.uint32, of shape image.shape
		New array where each pixel has the rank-order value of the corresponding pixel in `image`. Pixel values are between 0 and n - 1, where n is the number of distinct unique values in
        `image`.
