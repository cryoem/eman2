...
def main():
	...
	parser.add_argument("--niters", default=50, type=int, help="Specify the number of iterative reconstructions to complete before returning the final reconstructed volume. The default number is 50")
	parser.add_argument("--beta", default=0.2, type=float, help="Specify the total-variation regularization weight parameter 'beta'. The default is 0.2.")
	...
	for i, sinogram in enumerate(sinograms):
		...
		toc = time.time()
		if options.verbose > 7: print "Reconstruction %i of %i completed in %s s"%(i+1, ysize,str(toc-tic))
