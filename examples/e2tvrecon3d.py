...
def main():
	...
	parser.add_argument("--noisiness", default=0.0, type=float, help="Specify the total-variation penalization/regularization weight parameter 'beta'. The default is 5.0.")
	...
	for fname in os.listdir(options.path):
		if "sinogram" in fname:
			inputpath = fname
			twodpath = "twod.hdf"
			noisiness = str(options.noisiness)
			verbosity = str(options.verbose)
			tlt = options.tlt
			inputbeta = str(beta)
			if i >= 1000:
				reconpath = "2Drecon_" + str(i)
			if i >= 100:
				reconpath = "2Drecon_0" + str(i)
			if i >= 10:
				reconpath = "2Drecon_00" + str(i)
			else:
				reconpath = "2Drecon_000" + str(i)
			## PARALLELIZED
			if options.parallel:
			...
			## UNPARALLELIZED
				...
				os.popen("e2tvrecon2d.py --tiltseries %s --tlt %s --path %s --output %s --beta %s -v %s --noise --noisiness %s --subpix %s"%(inputpath, tlt, reconpath, twodpath, inputbeta, verbosity, noisiness, str(subpix)))
				if options.verbose > 3:
					print "2-D Reconstruction %i Complete"%(i)
			i += 1
	if options.parallel:
		p_status = p.wait()
