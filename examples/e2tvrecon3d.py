...
def make_tiltstacks( options, xpix, ypix ):
	...
	stackname = it.imap(options.path + "/tiltstack_%04i.hdf".__mod__, it.count(0))
	files=[]
	for y in range(ypix):
		next(stackname)
...
def get_best_betas( imgpath, nslices=None, niters=400 ):
	...
	results = Parallel(n_jobs=-1)(delayed(rec_error)(b, niters) for b in betas)
	...
	best_betas = [ks[np.argmin(errors)], ks[np.argmin(segmentation_error)]]
	return best_betas
