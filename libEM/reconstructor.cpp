...
const string rel_nn4_ctfwReconstructor::NAME = "rel_nn4_ctfw";
...
//	force_add<XYZReconstructor>();
    ...
    force_add<rel_nn4_ctfwReconstructor>();
}
...
//####################################################################################
//** rel_nn4_ctfw reconstructor

rel_nn4_ctfwReconstructor::rel_nn4_ctfwReconstructor()
{
	m_volume  = NULL;
	m_wptr    = NULL;
}

rel_nn4_ctfwReconstructor::rel_nn4_ctfwReconstructor( const string& symmetry, int size, int npad, float snr, int sign )
{
	setup( symmetry, size, npad, snr, sign );
}
...
void rel_nn4_ctfwReconstructor::setup()
{
	if( ! params.has_key("size") ) throw std::logic_error("Error: image size is not given");
...
void rel_nn4_ctfwReconstructor::setup( const string& symmetry, int size, int npad, float snr, int sign )
{
	m_weighting = ESTIMATE;
	if( params.has_key("weighting") ) {
		if( int( params["weighting"])==0 ) m_weighting = NONE;
...
void rel_nn4_ctfwReconstructor::buildFFTVolume() {
	int offset = 2 - m_vnxp%2;
...
void rel_nn4_ctfwReconstructor::buildNormVolume()
{
	m_wptr = params["weight"];
...
int rel_nn4_ctfwReconstructor::insert_slice(const EMData* const slice, const Transform& t, const float weight)
{
	// sanity checks
	if (!slice) {
		LOGERR("try to insert NULL slice");
...
int rel_nn4_ctfwReconstructor::insert_buffed_slice( const EMData* buffed, float weight )
{
	const float* bufdata = buffed->get_data();
	float* cdata = m_volume->get_data();
	float* wdata = m_wptr->get_data();
...
int rel_nn4_ctfwReconstructor::insert_padfft_slice_weighted( EMData* padfft, EMData* ctf2d2, EMData* bckgnoise, const Transform& t, float weight )
{
	Assert( padfft != NULL );
...
#ifdef False
EMData* rel_nn4_ctfwReconstructor::finish(bool)
{
	m_volume->set_array_offsets(0, 1, 1);
	m_wptr->set_array_offsets(0, 1, 1);
	m_volume->symplane0_ctf(m_wptr);
...
nn4_ctfwReconstructor::nn4_ctfwReconstructor( const string& symmetry, int size, int npad, float snr, int sign )
{
	setup( symmetry, size, npad, snr, sign );
}
...
void nn4_ctfwReconstructor::setup( const string& symmetry, int size, int npad, float snr, int sign )
{
	m_weighting = ESTIMATE;
	if( params.has_key("weighting") ) {
		if( int( params["weighting"])==0 ) m_weighting = NONE;
