...
void* EMUtil::em_malloc(const size_t size) {
#ifdef 	EMAN2_USING_CUDA_MALLOC
	return cuda_malloc(size);
#else // EMAN2_USING_CUDA_MALLOC
	return malloc(size);
#endif // EMAN2_USING_CUDA_MALLOC
}

void* EMUtil::em_calloc(const size_t nmemb,const size_t size) {
#ifdef 	EMAN2_USING_CUDA_MALLOC
	size_t real_size = nmemb*size;
	void* d = cuda_malloc(real_size);
	EMUtil::em_memset(d,0,real_size);
	return d;
#else // EMAN2_USING_CUDA_MALLOC
...
void* EMUtil::em_realloc(void* data,const size_t new_size,const size_t old_size) {
#ifdef 	EMAN2_USING_CUDA_MALLOC
	void* dst = cuda_malloc(new_size);
	if (old_size < new_size ) {
		em_memcpy(dst,data,old_size);
	} else {
		em_memcpy(dst,data,new_size);
	}
	return dst;
#else // EMAN2_USING_CUDA_MALLOC
...
void EMUtil::em_memset(void* data, const int value, const size_t size) {
//#ifdef 	EMAN2_USING_CUDA_MALLOC
//	cuda_memset(data,0,size);
//#else // EMAN2_USIN_CUDA_MALLOC
//	
//#endif // EMAN2_USING_CUDA_MALLOC
	memset(data, value, size);
}
void EMUtil::em_free(void*data) {
#ifdef 	EMAN2_USING_CUDA_MALLOC
	cuda_free(data);
#else // EMAN2_USING_CUDA_MALLOC
	free(data);
...
void EMUtil::em_memcpy(void*dst,const void* const src,const size_t size) {
#ifdef 	EMAN2_USING_CUDA_MALLOC
