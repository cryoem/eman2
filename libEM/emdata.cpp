...
EMData *EMData::get_clip(const Region & area)
{
	if (get_ndim() != area.get_ndim()) {
		Log::logger()->error("cannot get %dD clip out of %dD image", get_ndim(), area.get_ndim());
		return 0;
	}
...
void EMData::insert_clip(EMData * block, const Point < int >&origin)
{
	...
	if (area.inside_region(Size(nx, ny, nz))) {
		Log::logger()->error("outside of destination image not supported.");
		return;
	}
#endif
	int x0 = origin.x;
...
{
	if (!is_complex()) {
		Log::logger()->error("cannot run IFT on real-space image");
		return 0;
	}
...
Point < float >EMData::normalize_slice(EMData * slice, float alt, float az, float phi)
{
	if (!is_complex() || !slice->is_complex() || !parent) {
		Log::logger()->error("normalize slice only works on complex images");
		return Point < float >();
	}
	...
	for (int y = 0; y < ny; y++) {
		for (int x = 0; x < nx / 2; x++) {
			...
			if (rad < ny / 2 - 1) {
				...
				int x0 = 2 * (int) floor(xx + 0.5);
				int y0 = (int) floor(yy + 0.5);
				int z0 = (int) floor(zz + 0.5);
...
EMData *EMData::little_big_dot(EMData * with, bool do_sigma)
{
	...
	float sum2 = (Util::square(with->get_sigma()) + Util::square(with->get_mean()));
	...
	if (do_sigma) {
		for (int j = ny2 / 2; j < ny - ny2 / 2; j++) {
			for (int i = nx2 / 2; i < nx - nx2 / 2; i++) {
				...
				float tmp_f1 = (sum1 / 2.0 - sum) / (nx2 * ny2);
				float tmp_f2 = Util::square(with->get_mean() - summ / (nx2 * ny2));
		...
		for (int j = ny2 / 2; j < ny - ny2 / 2; j++) {
			for (int i = nx2 / 2; i < nx - nx2 / 2; i++) {
				...
				if (dot2 == 0) {
					...
					ret_data[i + j * nx] = dot / (nx2 * ny2 * dot2 * with->get_sigma());
				}
			}
		}
	}
...
vector < float >EMData::calc_fourier_shell_correlation(EMData * with)
{
	...
	if (!EMUtil::is_same_size(this, with)) {
		Log::logger()->error("size mismatch in calc_fourier_shell_correlation");
		return vector < float >();
	}
...
void EMData::set_attr_dict(string key, EMObject val)
{
	attr_dict[key] = val;
}

float EMData::get_skewness()
{
	float mean = get_mean();
	...
	int size = nx * ny * nz;
	for (int i = 0; i < size; i++) {
		k += pow((double) (rdata[i] - mean) / sigma, (double) 3.0);
	}
	return (k / size);
}

float EMData::get_kurtosis()
{
	float mean = get_mean();
	float sigma = get_sigma();
	...
	int size = nx * ny * nz;
	for (int i = 0; i < size; i++) {
		k += pow((double) (rdata[i] - mean) / sigma, (double) 4.0);
	}
	return (k / size - 3.0);
}
...
void EMData::dump_data(string filename)
{
	...
	if (!hfile) {
		Log::logger()->error("cannot open dump header file: %s", headerfile.c_str());
		return;
	}
	...
	if (!dfile) {
		Log::logger()->error("cannot open dump data file: %s", datafile.c_str());
		return;
	}
...
{
	if (n == 0) {
		Log::logger()->error("divided by zero");
		return *this;
	}
	*this *= (1.0 / n);
	return *this;
...
{
	if (!EMUtil::is_same_size(this, with)) {
		Log::logger()->error("dot_rotate_translate requires same size image");
		return 0;
	}

	if (nz != 1) {
		Log::logger()->error("dot_rotate_translate can only handle 2D images");
		return 0;
	}
	...
	float mx1 = sin(da);
	float y = -ny / 2.0;
	float my0 = mx0 * (-nx / 2.0 - 1.0) + nx / 2.0 - dx;
	float my1 = -mx1 * (-nx / 2.0 - 1.0) + ny / 2.0 - dy;
	double result = 0;
...
void EMData::rotate_translate(const Rotation & rotation, const Vec3 < float >&translation)
{
	...
	if (nz == 1) {
		...
		int k = 0;
		float x = -mx[1] * ny / 2.0 + nx / 2.0 - translation[0];
		float y = -mx[0] * ny / 2.0 + ny / 2.0 - translation[1];

		for (int j = 0; j < ny; j++) {
			float x2 = x - mx[0] * nx / 2.0;
			float y2 = y + mx[1] * nx / 2.0;

			for (int i = 0; i < nx; i++) {
				if (x2 < 0 || x2 > nx - 2.0 || y2 < 0 || y2 > ny - 2.0) {
					des_data[k] = 0;
				}
				else {
					int ii = (int) floor(x2 + 0.5);
					int jj = (int) floor(y2 + 0.5);
		...
		float x4 = (mx[0][0] * (-nx / 2.0) + mx[0][1] * (-ny / 2.0) +
					mx[0][2] * (-nz / 2.0)) + nx / 2.0 - translation[0];
		float y4 = (mx[1][0] * (-nx / 2.0) + mx[1][1] * (-ny / 2.0) +
					mx[1][2] * (-nz / 2.0)) + ny / 2.0 - translation[1];
		float z4 = (mx[2][0] * (-nx / 2.0) + mx[2][1] * (-ny / 2.0) +
					mx[2][2] * (-nz / 2.0)) + nz / 2.0 - translation[2];
			...
			for (int j = -ny / 2; j < ny / 2; j++) {
				...
				for (int i = -nx / 2; i < nx / 2; i++) {
					if (i * i + j * j + k * k >= mr) {
						...
						float x = floor(x2 + 0.5);
						float y = floor(y2 + 0.5);
						float z = floor(z2 + 0.5);
...
{
	if (nx != ny || nz != 1) {
		Log::logger()->error("Radon transform only works on square 2D images");
		return 0;
	}
...
Point < int >EMData::get_max_location() const
{
	int di = 1;
	if (is_complex() && !is_ri()) {
		di = 2;
...
int EMData::get_min_index() const
{
	Point < int >min_location = get_min_location();
	int i = min_location.x + min_location.y * nx + min_location.z * nx * ny;
	return i;
}


int EMData::get_max_index() const
{
	Point < int >max_location = get_max_location();
...
{
	if (!with) {
		Log::logger()->error("Empty 'with' image. calc_ccfx requires 'with'.");
		return 0;
	}
	...
	if (nz > 1) {
		Log::logger()->error("calc_ccfx only works on 2D images");
		return 0;
	}
...
void EMData::calc_rcf(EMData * with, vector < float >&sum_array)
{
	...
	for (int i = 8; i < nx2; i += 6) {
		...
		float norm = array_size * array_size * (4.0 * sigma) * (4.0 * with_sigma);
...
EMData *EMData::calc_ccf(EMData * with, bool tocorner, EMData * filter)
{
	...
	if (filter) {
		if (!EMUtil::is_same_size(filter, cf)) {
			Log::logger()->error("calc_ccf: cannot apply improperly sized filter!");
			return 0;
		}
	...
	if (with && !EMUtil::is_same_size(f1, cf)) {
		Log::logger()->error("calc_ccf: Images must be the same size.");
		return 0;
	}
...
EMData *EMData::calc_mutual_correlation(EMData * with, bool tocorner, EMData * filter)
{
	if (with && !EMUtil::is_same_size(this, with)) {
		Log::logger()->error("to calculate mutual correlation, Images must be the same size.");
		return 0;
	}
	...
	if (filter) {
		if (!EMUtil::is_same_size(filter, cf)) {
			Log::logger()->error("cannot apply improperly sized filter");
			return 0;
		}
	...
	for (int z = 0; z < nz; z++) {
		for (int y = 0; y < ny; y++) {
			for (int x = 0; x < nx; x += step, c += step) {
				float r = 0;
				if (is_complex()) {
					r = (Util::hypot3(x / 2.0, y - ny / 2.0, z - half_nz) - x0) / dx;
				}
				else {
					r = (Util::hypot3(x - nx / 2.0, y - ny / 2.0, z - half_nz) - x0) / dx;
				}
...
{
	if (nz > 1) {
		Log::logger()->error("calc radial dist: can only handle 2D images.");
		return vector < float >();
	}
	...
	for (int y = 0; y < ny; y++) {
		for (int x = 0; x < nx; x += step, c += step) {
			...
			if (y != ny / 2.0 || x != 0) {
				if (is_complex()) {
					a = atan2(y - ny / 2.0, x / 2.0);
				}
				else {
					a = atan2(y - ny / 2.0, x - nx / 2.0);
				}
			}
			...
			if (fabs(Util::angle_sub_pi(a, acen)) <= awid) {
				float r = 0;
				if (is_complex()) {
					r = (hypot(x / 2.0, y - ny / 2.0) - x0) / dx;
				}
				else {
					r = (hypot(x - (nx - 1) / 2.0, y - (ny - 1) / 2.0) - x0) / dx;
				}
	...
	const float scale1 = 0.57;
	const float scale2 = 0.105;
...
{
	if (!with) {
		Log::logger()->error("input image is NULL");
		return 0;
	}
...
EMData *EMData::convolute(EMData * with)
{
	...
	if (with && !EMUtil::is_same_size(f1, cf)) {
		Log::logger()->error("Convolution: Images must be the same size.");
		return 0;
	}
...
int EMData::common_lines(EMData * image1, EMData * image2, int mode, int steps, bool horizontal)
{
	...
	for (int i = 0; i < steps * 2; i += 2, a += da) {
		...
		for (float r = 3.0; r < rmax - 3.0; j += 2, r += 1.0) {
			float x = r * cos(a);
			float y = r * sin(a);
			...
			if (x < 0) {
				x = -x;
				y = -y;
				Log::logger()->error("CCL ERROR %d, %f !\n", i, -x);
			}
...
float EMData::get_density_center()
{
	float center = get_mean();
	float sigma = get_sigma();
	float ds = sigma / 2;
	int size = nx * ny * nz;
	float *d = get_data();
...
float EMData::get_sigma_diff()
{
	...
	float sig_diff = fabs(sigup - sigdown) / get_sigma();
	...
	if (ndims == 2) {
		...
		for (int j = 0; j < ny; j++) {
			for (int i = 0; i < nx; i += 2, k += 2) {
				float r = hypot((float) i / 2.0, (float) (j - ny / 2.0));
				r = (r - x0) / step;
		...
		int k = 0;
		for (int m = 0; m < nz; m++) {
			float mnz = (m - nz / 2.0) * (m - nz / 2.0);
			for (int j = 0; j < ny; j++) {
				float jny = (j - ny / 2.0) * (j - ny / 2.0);
				for (int i = 0; i < nx; i += 2, k += 2) {
					float r = sqrt((i * i / 4.0) + jny + mnz);
					r = (r - x0) / step;
