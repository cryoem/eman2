...
{                //   if real image was odd and square
	if (!is_complex()) {
		...
		throw ImageFormatException("2D odd square complex image"
			" expected. Input image is real image.");
	}
	...
	if (!is_fftodd()) {
		LOGERR("odd-sided image expected. Input image is even-sided");
		throw ImageFormatException("2D odd square complex image "
		               "expected. Input image is not odd.");
	}
//      calculate the nx, ny of the original images
        int onx= get_xsize();
        int ony= get_ysize();
...
//	if (flags & ~EMDATA_SHUFFLE) {
	if (!is_shuffle()) {
	     printf("dimensions of original image nx=%d ny=%d \n",(onx-1),ony);
             printf("entered Shuffle \n");
	     if ((onx-1)!=ony) {
		LOGERR("square image expected. Original image is not square");
		throw ImageFormatException("2D odd square complex image "
				" expected. Original image is not square.");
	     } // end square size check
	     int out_nx=2*(onx-1);
	     int out_ny=ony;
...
//	     float *d = dat->get_data(); 
	     ...
	     //  if onx=6, nx_orig=5, out_nx=10, ony=5, ny_orig=5, out_ny=5
	     // center is at nx=5,6, ny=3
	     d[onx-2][(ony-1)/2] = rnewdata[0][0]; 
	     d[onx-1][(ony-1)/2] = rnewdata[1][0]; // A block
            for (int iy = 1; iy < (ony+1)/2; iy++) { // B and C blocks
		d[onx-2][ iy+(ony-1)/2] =  rnewdata[0][iy];
		d[onx-1][ iy+(ony-1)/2] =  rnewdata[1][iy];
		d[onx-2][-iy+(ony-1)/2] =  rnewdata[0][iy];
		d[onx-1][-iy+(ony-1)/2] = -rnewdata[1][iy];
	     }
            for (int ix = 2; ix < onx; ix=ix+2) { // D blocks
		d[ ix+onx-2][(ony-1)/2]=  rnewdata[ix  ][0];
		d[ ix+onx-1][(ony-1)/2]=  rnewdata[ix+1][0];
		d[-ix+onx-2][(ony-1)/2]=  rnewdata[ix  ][0];
		d[-ix+onx-1][(ony-1)/2]= -rnewdata[ix+1][0];
	     }
            for (int ix = 2; ix < onx; ix=ix+2) { // E blocks
		for (int iy= 1 ; iy <(ony+1)/2 ; iy++){ 
		    d[ ix+onx-2][iy+(ony-1)/2]  =  rnewdata[ix  ][iy];  // E
		    d[ ix+onx-1][iy+(ony-1)/2]  =  rnewdata[ix+1][iy];  // E
		    d[-ix+onx-2][-iy+(ony-1)/2] =  rnewdata[ix  ][iy];  // E*
		    d[-ix+onx-1][-iy+(ony-1)/2] = -rnewdata[ix+1][iy];  // E*
		 }
	     }
            for (int ix = 2; ix < onx; ix=ix+2) { // E blocks
		for (int iy= (ony+1)/2 ; iy <ony ; iy++){ 
		    d[ ix+onx-2][ iy-(ony+1)/2]  =  rnewdata[ix  ][iy];  // F
		    d[ ix+onx-1][ iy-(ony+1)/2]  =  rnewdata[ix+1][iy];  // F
		    d[-ix+onx-2][-iy+ony+2]  =  rnewdata[ix  ][iy];  // F*
 		    d[-ix+onx-1][-iy+ony+2]  = -rnewdata[ix+1][iy];  // F*
		 }
	     }
	     dat->done_data();
...
//	     float *d = dat->get_data(); 
	     ...
	     int onyv = ony;
	     // center is at nx=5,6, ny=3
	     d[0][0] = rnewdata[onxv-2][(onyv-1)/2]; 
	     d[1][0] = rnewdata[onxv-1][(onyv-1)/2]; // A block
            for (int iy = 1; iy < (onyv+1)/2; iy++) { // B and C blocks
		d[0][iy] =  rnewdata[onxv-2][ iy+(onyv-1)/2];
		d[1][iy] =  rnewdata[onxv-1][ iy+(onyv-1)/2];
		d[0][onyv-iy] =  rnewdata[onxv-2][-iy+(onyv-1)/2];
		d[1][onyv-iy] = -rnewdata[onxv-1][-iy+(onyv-1)/2];
	     }
            for (int ix = 2; ix < onxv; ix=ix+2) { // D blocks
		d[ix  ][0]=  rnewdata[ ix+onxv-2][(onyv-1)/2];
		d[ix+1][0]=  rnewdata[ ix+onxv-1][(onyv-1)/2];
	     }
            for (int ix = 2; ix < onxv; ix=ix+2) { // E blocks
		for (int iy= 1 ; iy <(onyv+1)/2 ; iy++){ 
		    d[ix  ][iy] =  rnewdata[ ix+onxv-2][iy+(onyv-1)/2];  // E
		    d[ix+1][iy] =  rnewdata[ ix+onxv-1][iy+(onyv-1)/2];  // E
		 }
	     }
            for (int ix = 2; ix < onxv; ix=ix+2) { // E blocks
		for (int iy= (onyv+1)/2 ; iy <onyv ; iy++){ 
		    d[ix  ][iy]  =  rnewdata[ ix+onxv-2][ iy-(onyv+1)/2];  // F
		    d[ix+1][iy]  =  rnewdata[ ix+onxv-1][ iy-(onyv+1)/2];  // F
		 }
	     }
	     dat->done_data();
	     dat->set_complex(true);
