...
EMData *RotateCHAligner::align(EMData * this_img, string ) const
{
	...
	if (ndot > 0) {
	    float dl = 2.0 * M_PI / max_num;
	    float ep = aa / ndot;
	    ep = (ep - floor(ep / dl) * dl) * 2.0 * M_PI / dl;
	    ca -= ep;
	    ca = ca - floor(ca / (2.0 * M_PI)) * 2.0 * M_PI;
...
EMData *RotateTranslateAligner::align(EMData * this_img, string cmp_name) const
{
    ...
    trans_params["useparent"] = EMObject(0);
    trans_params["intonly"] = EMObject(1);
...
EMData *RotateTranslateBestAligner::align(EMData * this_img, string cmp_name) const
{
    ...
    Dict trans_params;
    trans_params["with"] = params["with"];
    trans_params["intonly"] = EMObject(0);
    trans_params["useparent"] = EMObject(1);
    trans_params["maxshift"] = params["maxshift"];
    this_copy->align("Translational", trans_params);
...
EMData *RotateTranslateRadonAligner::align(EMData * this_img, string ) const
{
	...
	si = co = max = ta = 0;
	for (int x = 0; x < size / 2; x++) {
	    si += sin(x * 2.0 * M_PI / size) * vert[x];
	    co += cos(x * 2 * M_PI / size) * vert[x];
	    if (fabs(vert[x]) > max) {
		max = fabs(vert[x]);
	    }
	}
...
{    
	...
	if (usedot == 2) {
	    ...
	    printf("%f vs %f  (%1.1f, %1.1f  %1.2f) (%1.1f, %1.1f  %1.2f)\n",
		   dot1, dot2, trans[0], trans[1], rot.eman_alt() * 180. / M_PI,
		   trans2[0], trans2[1], rot2.eman_alt() * 180. / M_PI);
	}
    }
    else {
	Dict cmp_params;
	cmp_params["with"] = params["with"];
	cmp_params["keepzero"] = EMObject(1);
...
EMData *RTFSlowAligner::align(EMData * this_img, string cmp_name) const
{
	...
	for (int dy = -half_maxshift; dy <= half_maxshift; dy++) {
		...
		if (hypot(dx, dy) <= half_maxshift) {
		    ...
		    if (cm < bestval) {
			bestval = cm;
			bestang = 2.0 * M_PI * a->get_max_index() / (float) a->get_xsize();
			bestdx = dx;
			bestdy = dy;
			bestflip = dflip;
		    }
...
EMData *RTFRadonAligner::align(EMData * this_img, string ) const
{
	...
	drw = 1;
	radonwith = with->do_radon();
	params["radonwith"] = EMObject(radonwith);
    }
...
EMData *RefineAligner::align(EMData * this_img, string cmp_name) const
{
	...
	int np = 3;
	Dict gsl_params;
	gsl_params["this"] = EMObject(this_img);
	gsl_params["with"] = EMObject(with);
