...
{   
	...
	string::size_type i0 = target.find_first_not_of(delimiter, position);
...
{   
	string::size_type i0=position;
	string::size_type i1=position;	// start and end indices
...
{   
	...
	string::size_type i0=position, i1=position; 
	while ( 1 )  
	{   
		if ( i1 == string::npos )
			return count;
		i0 = target.find_first_not_of(delimiter, i1);
		if ( i0 == string::npos )
			return (! delimToken ? count : count + target.length() - i1);
		count++;
