...
// prototype function
int rnd_asg(unsigned short int*, unsigned short int*, int, int);
void criterion_part(float*, unsigned short int*, unsigned short int*, float*, float*, float*, float*, int, int, int);
...
void print_int(unsigned short int* data, int size) {
...
// Main, k-means core
int cuda_kmeans(float* h_IM, float* h_AVE, unsigned short int* h_ASG, float* h_INFO, int N, int m, int K, int maxite, float F, int rnd) {    
    // host vars
    float* h_DIST;
    float* h_AVE2;
    float* h_IM2;
    unsigned short int* h_NC;
    ...
    if (rnd_asg(h_ASG, h_NC, K, N) != EXIT_SUCCESS) return EXIT_FAILURE;
	...
	for (i = 0; i < K; i++) {
		...
		return 2;  // if anyone is empty quite with error 2
	    }
	}
...
int rnd_asg(unsigned short int* ASG, unsigned short int* NC, int K, int N) {
    int ret = 20;
    int flag = 0;
    int n, k;
 ...
 */
void criterion_part(float* DIST, unsigned short int* ASG, unsigned short int* NC, \
