...
EMBytes GLUtil::render_annotated24(EMData *emdata, EMData *intmap, int x0, int y0, int ixsize,
						   int iysize, int bpl, float scale, int min_gray, int max_gray,
						   float render_min, float render_max, int flags , const vector<float> & rgblist)
{
   ENTERFUNC;
   ...
   // }
	...
	if (rgblist.size() > 0){
	unsigned int rgbSize = rgblist.size();



   for (unsigned int i=0; i<rgbSize; i+=3) {
	   rtable.push_back((int)rgblist[i]);
   }
   for (unsigned int i=1; i<rgbSize; i+=3) {
	   gtable.push_back((int)rgblist[i]);
   }
   for (unsigned int i=2; i<rgbSize; i+=3) {
	   btable.push_back((int)rgblist[i]);
   }
   rtable[0] =0;
   gtable[0] =0;
   btable[0] =0;
   ...
   //int index = i/3;
   // r=(unsigned char) ctable[i];
   // g=(unsigned char) ctable[i+1];
   // b=(unsigned char) ctable[i+2];
